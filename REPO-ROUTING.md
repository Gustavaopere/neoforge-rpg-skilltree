...
| RPG-specific provider integration behavior and evidence | `Gustavaopere/neoforge-rpg-skilltree` | Exact provider versions still come from physical evidence; shared/cross-mod proofs belong in Factory. |
...
3. RPG-specific provider adapters and their runtime evidence belong here; reusable cross-mod compatibility tooling belongs in Factory.
4. Another mod's runtime belongs in that mod's repository.
5. Do not create a runtime dependency on Minecraft Mod Factory by default; shared runtime code requires an explicit versioned library/API boundary.
6. Provider adapters must be isolated and backed by exact-version evidence.
7. Cross-pipeline asset conversion must be explicit, source-preserving and loss-audited.
8. Before writes, inspect current GitHub state and concurrent work; never overwrite another branch's work.
9. The live repository tree wins over historical conceptual paths.
10. Historical shared engineering material may be consulted through Git history for migration/provenance, but must not be extended as a parallel control plane.
...
- RPG runtime operating contract: `AGENT-WORKFLOW.md`.
