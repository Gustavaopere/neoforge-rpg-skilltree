...
python3 "$ROOT/scripts/verify-gradle-wrapper.py"
python3 "$ROOT/scripts/verify-volcanoes-saveddata-nbt-static-access.py"
OUT="$ROOT/build/core-test-classes"; rm -rf "$OUT"; mkdir -p "$OUT"
mapfile -t SOURCES < <(find "$ROOT/src/main/java/dev/gustavopere/rpgskilltree/core" "$ROOT/src/test/java/dev/gustavopere/rpgskilltree/core" -name '*.java' ! -name '*JUnitTest.java' -print | sort)
...
java -cp "$OUT" dev.gustavopere.rpgskilltree.core.A0021A0040CombatPolicyTest
java -cp "$OUT" dev.gustavopere.rpgskilltree.core.A0021A0040MasteryPolicyTest
java -cp "$OUT" dev.gustavopere.rpgskilltree.core.A0021A0040RespecReconcileTest
java -cp "$OUT" dev.gustavopere.rpgskilltree.core.A0041A0060NotionContractTest
...
java -cp "$OUT" dev.gustavopere.rpgskilltree.core.A0081A0090Chat3ValidationTest
