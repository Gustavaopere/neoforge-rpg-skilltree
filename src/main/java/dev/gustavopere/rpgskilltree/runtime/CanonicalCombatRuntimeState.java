...
public final class CanonicalCombatRuntimeState {
    ...
    private static final CanonicalActionCorrelationService CORRELATION = new CanonicalActionCorrelationService(
        CORRELATION_RETENTION_MILLIS, PROJECTILE_RETENTION_MILLIS, MAX_TRACKED_ACTIONS);
    private static final CanonicalCriticalService CRITICAL = new CanonicalCriticalService(
        () -> ThreadLocalRandom.current().nextDouble(), CRITICAL_RETENTION_MILLIS, MAX_TRACKED_ACTIONS);
    ...
    public static synchronized CanonicalActionIdentity newRoot(ServerPlayer player, String sourceId, long nowMillis) {
        Objects.requireNonNull(player);
        return CORRELATION.newRoot(actorId(player), sourceId, nowMillis);
    }

    public static synchronized CanonicalActionIdentity beginMelee(ServerPlayer player, String targetId, long nowMillis) {
    ...
    public static synchronized Optional<CanonicalActionIdentity> claimMeleeForProvider(ServerPlayer player, String targetId, long nowMillis) {
    ...
    public static synchronized void beginAim(ServerPlayer player, long nowMillis) {
        ...
        if (focusGeneration || preparationStarted) AIMS.put(actorId(player), new AimSession(aimAction, preparationStarted));
    }
    ...
    public static synchronized CanonicalFocusService.AimStatus sampleAim(ServerPlayer player, long nowMillis) {
        ...
        CanonicalFocusService.AimStatus result = CombatPerkRuntimeState.state().focusService().sampleAim(
            new CanonicalFocusService.AimSampleRequest(aim.action, true, true, bowInUse, player.isSprinting(), rank,
                player.getYRot(), player.getXRot(), 1.0D, 1.0D),
            CombatPerkRuntimeState.state(), nowMillis);
        if (player.isSprinting() && aim.preparationStarted) {
            CombatPerkRuntimeState.state().focusService().armPreparation(aim.action, false, CombatPerkRuntimeState.state(), nowMillis);
    ...
    public static synchronized void recordUseStop(ServerPlayer player, double drawFraction, long nowMillis) {
        ...
        PENDING_CANCELLED_DRAWS.put(actorId, new PendingCancelledDraw(drawFraction, Math.addExact(nowMillis, 50L)));
    }
    ...
    public static synchronized boolean resolvePendingCancelledDraw(ServerPlayer player, long nowMillis) {
        ...
        return CombatPerkRuntimeState.state().focusService().applyCancelledDrawLoss(
            actorId, true, true, pending.drawFraction, CombatPerkRuntimeState.state(), nowMillis);
    }

    public static synchronized CanonicalActionIdentity recordLoose(ServerPlayer player, boolean fullyDrawn, long cooldownMillis, long nowMillis) {
        String actorId = actorId(player);
        PENDING_CANCELLED_DRAWS.remove(actorId);
        AimSession aim = AIMS.remove(actorId);
        CombatPerkRuntimeState.state().focusService().endAimTracking(actorId);
        if (aim != null && aim.preparationStarted) {
            CombatPerkRuntimeState.state().focusService().armPreparation(
                aim.action, fullyDrawn && !player.isSprinting(), CombatPerkRuntimeState.state(), nowMillis);
        }
        ...
        SHOTS.put(ActionKey.of(shot), new ShotFacts(fullyDrawn, cooldownMillis, player.getX(), player.getY(), player.getZ(),
            Math.addExact(nowMillis, CORRELATION_RETENTION_MILLIS)));
        return shot;
    }

    public static synchronized Optional<ShotCorrelation> correlateProjectile(ServerPlayer owner, String projectileId, long nowMillis) {
        pruneShots(nowMillis);
        Optional<CanonicalActionIdentity> action = CORRELATION.correlateProjectile(actorId(owner), projectileId, nowMillis);
        if (action.isEmpty()) return Optional.empty();
        ShotFacts facts = SHOTS.get(ActionKey.of(action.get()));
        if (facts != null) {
            PROJECTILE_SHOTS.put(projectileId, new ProjectileShotFacts(action.get(), facts.shotX, facts.shotY, facts.shotZ,
                Math.addExact(nowMillis, PROJECTILE_RETENTION_MILLIS)));
        }
        return Optional.of(new ShotCorrelation(action.get(), facts));
    }

    public static synchronized Optional<ProjectileShotFacts> projectileShotFacts(ServerPlayer owner, String projectileId, long nowMillis) {
        pruneShots(nowMillis);
    ...
    public static synchronized CanonicalActionIdentity projectileAction(ServerPlayer owner, String projectileId, long nowMillis) {
        return CORRELATION.projectileAction(projectileId, nowMillis)
            .orElseGet(() -> CanonicalActionIdentity.root(actorId(owner), "projectile/" + projectileId, "neoforge:projectile"));
    }

    public static boolean resolveCritical(CanonicalActionIdentity action, WeaponFamily family, CombatPerkRanks ranks,
                                          boolean providerCritical, long nowMillis) {
        double bonusChance = NotionCombatPerkRules.criticalChanceBonus(family, ranks);
        return CRITICAL.resolve(new CanonicalCriticalRequest(action, true, true, true, providerCritical, bonusChance), nowMillis);
    }

    public static boolean resolveProviderCritical(CanonicalActionIdentity action, boolean providerCritical, long nowMillis) {
        return CRITICAL.resolve(new CanonicalCriticalRequest(action, true, true, true, providerCritical, 0.0D), nowMillis);
    }
    ...
    public static synchronized boolean hasAim(ServerPlayer player) { return AIMS.containsKey(actorId(player)); }

    /**
     * Entity recreation/dimension changes invalidate correlations and entity-bound shot facts,
     * but keep canonical critical decisions so stale callbacks cannot obtain a reroll.
     */
    public static synchronized void clearTransientPreservingGuards(ServerPlayer player) {
        String actorId = actorId(player);
        AIMS.remove(actorId);
        PENDING_CANCELLED_DRAWS.remove(actorId);
        ...
        // CRITICAL intentionally survives until normal retention expiry.
    }
    ...
    private static String actorId(ServerPlayer player) { return player.getUUID().toString(); }

    public record ShotCorrelation(CanonicalActionIdentity action, ShotFacts facts) {}
    public record ShotFacts(boolean fullyDrawn, long cooldownMillis, double shotX, double shotY, double shotZ, long expiresAtMillis) {}
    public record ProjectileShotFacts(CanonicalActionIdentity action, double shotX, double shotY, double shotZ, long expiresAtMillis) {}
