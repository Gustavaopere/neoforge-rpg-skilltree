...
public final class A0061A0080EpicFightHooks {
    ...
    private static void onDamagePre(DealDamageEvent.Pre event) {
        ...
        A0061A0080CombatPolicy.HitFacts facts = new A0061A0080CombatPolicy.HitFacts(
            ...
            player.isSprinting(), A0061A0080RuntimeState.isStationary(player),
            false, true, true, now
        );
        ...
        boolean executionArmCandidate = ranks.rank("A0073") > 0
            ...
            && state.reserveExecutionArmCandidate(actor, targetId, root, now);
        ...
        remember(source, targetId, new PendingHit(
            root, executionReserved, executionArmCandidate, firstBloodReservation, opportunityReserved
        ));
    }
    ...
    private static void onDamagePost(DealDamageEvent.Post event) {
        ...
        if (pending.executionReserved()) {
            ...
            if (ranks.rank("A0073") > 0) state.armExecutionConfirmed(actor, targetId, pending.rootActionId(), now);
            else state.rollbackExecution(actor, targetId, pending.rootActionId());
        }

        if (pending.firstBloodReservation() != FirstBloodReservation.NONE) {
            if (ranks.rank("A0074") > 0) {
                state.commitFirstBlood(actor, targetId, pending.rootActionId(), pending.firstBloodReservation(), now);
            } else {
    ...
    private static void rollbackPending(A0061A0080CombatState state, String actor, String targetId, PendingHit pending) {
        if (pending.executionReserved() || pending.executionArmCandidate()) {
            state.rollbackExecution(actor, targetId, pending.rootActionId());
        }
        if (pending.firstBloodReservation() != FirstBloodReservation.NONE) {
            state.rollbackFirstBlood(actor, targetId, pending.rootActionId());
        }
