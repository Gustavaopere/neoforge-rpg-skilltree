...
public final class EntityScalingLifecycleStateTest {
    ...
    private static void persistedStateRoundTripsWithoutRerolling() {
        ...
        EntityScalingState decoded = EntityScalingStateCodec.decodeState(encoded);
    ...
    private static void malformedPayloadsFailClosed() {
        ...
        expect(IllegalArgumentException.class, () -> EntityScalingStateCodec.decodeState(unsupported));

        byte[] trailing = Arrays.copyOf(encoded, encoded.length + 1);
        expect(IllegalArgumentException.class, () -> EntityScalingStateCodec.decodeState(trailing));

        byte[] truncated = Arrays.copyOf(encoded, encoded.length - 1);
        expect(IllegalArgumentException.class, () -> EntityScalingStateCodec.decodeState(truncated));
    }
