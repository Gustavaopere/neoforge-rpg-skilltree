...
public final class A0001A0020NotionContractTest {
    ...
    private static void expectRootGate(String code,String specialization,String mastery,int amount){var n=CombatPerkTreeModel.node(code).orElseThrow();require(n.startingPoint(),code+" root");require(n.minCharacterLevel()==8,code+" level");require(n.requiredSpecializations().contains(specialization),code+" gateway");require(n.requiredMastery().getOrDefault(mastery,0)==amount,code+" mastery");}
    private static void expectCapstoneGate(String code,String specialization,String mastery,int amount){var n=CombatPerkTreeModel.node(code).orElseThrow();require(n.requiredSpecializations().contains(specialization),code+" gateway");require(n.requiredMastery().getOrDefault(mastery,0)==amount,code+" capstone mastery");}
