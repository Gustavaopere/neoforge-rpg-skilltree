...
- **Mod:** BlockUI
- **Arquivo JAR:** blockui-1.0.212-1.21.1-snapshot.jar
- **Versão 1.21.1:** 1.0.212-1.21.1-snapshot
...
- **Dependências:** Biblioteca estrutural. Pack físico atual confirma MineColonies 1.1.1387, Structurize 1.0.833, Multi-Piston 1.2.58 e Domum Ornamentum 1.0.236-snapshot no ecossistema consumidor.
- **Estado no pack:** Integrado ao Github
- **Estado da pesquisa:** Verificado
- **Compatibilidade/Riscos:** Snapshot/beta 1.0.212: riscos em API/layout changes, resource/XML reload, callback duplication, concurrent mutation durante draw loops, client/server data authority e consumers compilados para snapshot diferente. UI não deve virar authority de gameplay.
- **Fonte:** https://www.curseforge.com/minecraft/mc-mods/blockui
- **Procedência:** modlist física atual + CurseForge oficial BlockUI 1.0.212-1.21.1-snapshot, file ID 8883780, 15/09/2026 + stack físico MineColonies 1.1.1387 / Structurize 1.0.833 / Multi-Piston 1.2.58 / Domum Ornamentum 1.0.236-snapshot.
- **Observações:** Runtime name UI Library Mod. 1.0.212 é snapshot NeoForge 1.21.1. Changelog oficial: fix de `ConcurrentModificationException` nos View draw loops. XML continua definindo estrutura; backing Window trata callbacks/data.
- **Atualização/Status:** READITADO EM 18/09/2026 — BlockUI atualizado de 1.0.211 para 1.0.212 snapshot; fix de ConcurrentModificationException nos View draw loops incorporado. Stack físico atual reconciliado com MineColonies 1.1.1387.
- **Decisão:** Sem decisão
...
> ✅ Versão física confirmada: `blockui-1.0.212-1.21.1-snapshot.jar`, mod id `blockui`, runtime name `UI Library Mod`, versão `1.0.212-1.21.1-snapshot`. É uma build **snapshot/beta** para NeoForge 1.21.1 e uma dependência estrutural do ecossistema MineColonies.
...
- BlockUI `1.0.212-1.21.1-snapshot`.
...
A build instalada é explicitamente `snapshot`. O histórico anterior documentava 1.0.211; o runtime físico atual é 1.0.212 snapshot. Portanto:
...
## 17. Matriz de testes
1. Dedicated server boot com BlockUI 1.0.212 + MineColonies stack atual.
...
## 18. Evidência
- modlist física atual: BlockUI 1.0.212 snapshot, MineColonies 1.1.1387, Structurize 1.0.833, Multi-Piston 1.2.58 e Domum Ornamentum 1.0.236-snapshot;
...
## 19. Atualização instalada — 1.0.212
A build física atual `blockui-1.0.212-1.21.1-snapshot.jar`, publicada em 15/09/2026, traz um fix específico no changelog oficial:
- correção de **ConcurrentModificationException nos loops de draw de View**.
Isso torna mutação de child/view collections durante render/draw um regression gate explícito para telas dos consumers.
Regression gates adicionais:
- [ ] abrir/fechar telas MineColonies/Structurize repetidamente sem ConcurrentModificationException;
- [ ] widgets que adicionam/removem views durante callbacks não corrompem o draw loop;
- [ ] resource reload e reabertura de tela não duplicam handlers/views;
- [ ] UI continua apenas presentation/input; state de gameplay permanece server-authoritative.
