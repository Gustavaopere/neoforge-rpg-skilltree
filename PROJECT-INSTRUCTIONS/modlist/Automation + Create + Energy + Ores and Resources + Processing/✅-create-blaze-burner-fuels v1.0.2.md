...
> **Autoridade física atual — 23/09/2026.** `modlist(1).txt` contém **587 entradas top-level incluindo o modloader**; este item ocupa a ordem física **#135**: JAR `create_blaze_burner_fuels-1.0.2-neoforge-1.21.1.jar`, mod id `create_blaze_burner_fuels`, runtime `1.0.2`, SHA-1 `9f055fedf963a6c352c655cb6d58b720b08256b2`.
...
A release **1.0.2** removeu explicitamente **log recycling** para evitar conflito com recipes de sawing do próprio Create. Esse delta é um regression gate de recipe coexistence: não reintroduzir a rota removida por KubeJS/datapack sem revisar o conflito que motivou a remoção upstream.
...
8. Log recycling removido na 1.0.2 é reintroduzido e volta a conflitar com sawing recipes do Create.
## 9. Matriz de testes
...
A página oficial 1.0.2 sustenta a lista de fuels, burn times e blocos acima e o changelog confirma a remoção de log recycling para evitar conflito com Create sawing recipes. Esta ficha não afirma que materiais homônimos de outros mods sejam automaticamente intercambiáveis; essa equivalência depende de registry/tag runtime.
