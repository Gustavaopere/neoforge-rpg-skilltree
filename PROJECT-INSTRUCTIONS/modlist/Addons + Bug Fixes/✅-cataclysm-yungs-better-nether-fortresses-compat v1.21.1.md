...
- **Atualização/Status:** PADRÃO ALEX'S MOBS REVALIDADO EM 20/09/2026 — registro histórico da reconciliação física #87 na snapshot então vigente; posição física atual #88: `cataclysmfortresses-1.21.1-NeoForge.jar` / `1.21.1` conferidos contra a snapshot então vigente; Better Fortress→`#cataclysm:berserker_spawn`, condição pós-Ignis, server-only/tag lifecycle e providers Cataclysm 3.33 + YUNG's Better Nether Fortresses 3.1.5 preservados.
