# Easy NPC — JAR/build 7.12.1 · runtime metadata não declarada

> **Reauditoria física e de migração — 16/09/2026.** A autoridade física atual é `easy_npc-neoforge-1.21.1-7.12.1.jar`, mod id `easy_npc`, SHA-1 `ed56854e39545cefdde1fe6695d9f52a31393bdc`, em NeoForge 1.21.1. **A coluna de versão runtime da modlist continua vazia.** Portanto `7.12.1` é versão do filename/build/publicação e não será inventada como metadata runtime. Config UI e Bundle também estão fisicamente em 7.12.1. O dossiê migrado do Notion foi preservado e atualizado para os deltas 7.12.0/7.12.1 agora instalados.
...
- **Dependências:** NeoForge 1.21.1. Easy NPC Config UI `7.12.1` e Easy NPC Bundle `7.12.1` estão fisicamente presentes. Easy Model Entities `2.4.0` também está presente e satisfaz o piso opcional EME `2.4.0+` introduzido na linha 7.12.0. Epic Fight `21.17.3.1` permanece presente como integração suportada.
- **Compatibilidade/Riscos:** NPC persistente é state crítico: riscos de duplicate restore, stale index/file state, backup stutter, owner-login restore, follow/look target após logout, action/trade double execution, custom model/pose interference, UI/client divergindo do servidor, preset IDs instáveis e spawn/import stacking. As linhas 7.11.0→7.12.1 contêm correções explícitas para várias dessas superfícies.
...
- **Observações:** Fail-closed preservado: `7.12.1` vem do filename/publicação do artefato instalado, não da metadata runtime. A família Core/Bundle/Config UI está alinhada em 7.12.1. EME 2.4.0 está fisicamente presente.
- **Procedência:** modlist física atual de 16/09/2026 confirma o JAR do Core, mod id `easy_npc`, campo runtime vazio, SHA-1 e versões físicas de Config UI/Bundle/EME. Changelogs oficiais 7.12.0/7.12.1 foram revalidados externamente.
- **Histórico da decisão:** sem histórico adicional registrado.
- **Atualização/Status:** REAUDITADO EM 16/09/2026 — build física atualizada de 7.11.0 para 7.12.1, mantendo runtime metadata explicitamente vazia.
...
## 23. Evidências
- modlist física atual: Core 7.12.1 por filename/build, mod id, **campo de versão runtime vazio**, SHA-1; Config UI/Bundle 7.12.1; Easy Model Entities 2.4.0 por filename/build; Epic Fight presente;
