...
> **AUTORIDADE FÍSICA ATUAL — 15/09/2026.** O pack agora contém `EuphoriaPatcher-1.10.5-r5.9.3-neoforge.jar`, mod id `euphoria_patcher`, versão física/runtime `1.10.5-r5.9.3-neoforge`, SHA-1 `2b3f92878d6867fa0199937da25ba882b9db7902`. Iris permanece fisicamente em `1.8.14-beta.1+mc1.21.1`. A release instalada atualiza o baseline para **Complementary Shaders r5.9.3** e é bug-fix release da linha 1.10.x.
>
> **Regra de leitura:** todas as referências abaixo a `1.10.0-r5.9` como versão “atual/instalada” são preservadas como snapshot histórico do conteúdo migrado do Notion e ficam supersedidas por este bloco. O dossiê funcional permanece válido; riscos relativos a baseline de shader, cache, config e Iris continuam aplicáveis à build nova.

## Delta físico instalado — 1.10.5-r5.9.3
- A antiga nota upstream de 15/09/2026 passa a descrever a build efetivamente instalada.
- O contrato de compatibilidade do patcher agora é `r5.9.3`; não reutilizar base Complementary de revisão diferente por suposição.
- Continuam obrigatórios testes de repatch, shader reload, dimension switching/cache, preservação de settings e compile/link no Iris atual.
- O boundary não muda: Euphoria Patcher é client-side e não recebe gameplay authority.

> Fonte canônica desta importação: Notion — `Auditoria Mestre da Modlist — NeoForge 1.21.1`  
> Estado no momento da reconciliação: `Integrado ao Github`  
> Autoridade física histórica da migração: `modlist.txt`, 595 entradas totais incluindo o modloader  
> Reconciliado em: 2026-09-15

## Propriedades do registro migrado

- **Mod:** Euphoria Patcher
- **Arquivo JAR no snapshot migrado:** `EuphoriaPatcher-1.10.0-r5.9-neoforge.jar`
- **Versão 1.21.1 no snapshot migrado:** `1.10.0-r5.9-neoforge`
- **Categoria:** Visual
- **Decisão:** Sem decisão
...
- **Observações:** Runtime físico histórico 1.10.0-r5.9. Upstream 1.10.1-r5.9.1 era uma bug-fix release compatível com 1.21.1: corrigia reflective horses/banners, adicionava opção de foliage shadows, melhorava integração visual com Spyglass Astronomy; no patcher, melhorava dimension switching com cache por dimensão (com custo de RAM), settings updater para dois shader styles e corrigia wildcard de `dimensions.properties` que quebrava o catch-all `*` do Iris.
- **Procedência:** modlist.txt física histórica confirmava o artefato Euphoria Patcher 1.10.0-r5.9. Revalidação oficial em 12/09/2026 confirmava a existência da release 1.10.1-r5.9.1 de 11/09/2026 compatível com Minecraft 1.21.1.
- **Histórico da decisão:** vazio.
- **Atualização/Status histórico:** PADRÃO ALEX'S MOBS REVALIDADO EM 12/09/2026 — lote físico #268: pack permanecia em Euphoria Patcher 1.10.0-r5.9; upstream publicou 1.10.1-r5.9.1 em 11/09/2026 com suporte explícito a 1.21.1. Update disponível registrado sem alterar a autoridade física instalada daquele snapshot.
- **Data da última decisão:** 2026-08-26

> **Runtime físico do snapshot migrado:** `EuphoriaPatcher-1.10.0-r5.9-neoforge.jar` · mod id `euphoria_patcher` · versão `1.10.0-r5.9-neoforge` · NeoForge 1.21.1 · client-side. O pack possuía **Iris 1.8.14-beta.1**; a presença de um Complementary shaderpack compatível não é inferida da modlist de mods. Para o estado atual, usar a autoridade física no topo.
...
## 9. Iris
Iris 1.8.14-beta.1+mc1.21.1 está instalado. Ele é o loader de shader efetivo. Falha de compile/link GLSL, shader reload ou pipeline incompatível deve ser diagnosticada separando Iris, Complementary base e Euphoria patch.
...
## 12. Riscos
1. base Complementary de revisão incompatível com a revisão instalada;
...
## 14. Evidências
- modlist física atual de 15/09/2026: `EuphoriaPatcher-1.10.5-r5.9.3-neoforge.jar`, mod id/version/hash + Iris atual;
