...
- **Atualização/Status:** PADRÃO ALEX'S MOBS REVALIDADO EM 11/09/2026 — reconciliação final física #90: artefato `cbcatfix-1.21.1-neoforge-1.0.1.jar` reconfirmado; metadata runtime `1.0.0` preservada sem normalização fictícia. Crash/cluster fixes, rocket automation, Rocket Pod, big rockets/rails e shell scope permanecem documentados.
...
- **Procedência:** modlist.txt física atual de 11/09/2026 + metadata runtime/log atual + CurseForge oficial CBCAT Fix build 1.0.1 e changelog da release, além das fontes já auditadas. Reconciliação final: filename físico `cbcatfix-1.21.1-neoforge-1.0.1.jar` e runtime metadata `1.0.0` permanecem deliberadamente distintos; nenhuma divergência foi mascarada.
