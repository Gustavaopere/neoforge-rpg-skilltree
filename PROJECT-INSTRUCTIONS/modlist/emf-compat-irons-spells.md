...
- **Compatibilidade/Riscos:** Riscos de arms pose ser sobrescrita/duplicada, head/body/legs indevidamente capturados, first/third mismatch, outro animation mod competir pelos braços e Core/EMF/ISS version drift. A página oficial ainda mostra file listing 1.0.0, mas o JAR físico 2.0.0 possui SHA-1 exatamente associado ao CurseForge file 8793934 em índice atual.
...
- **Observações:** Projeto é Client-only. Documentação oficial: casting captura `Both arms`; head, body e legs ficam sob EMF. Funciona para outros jogadores e foi testado com Fresh Animations: Player Extension e Detailed Animations. Fail-closed: 2.0.0 vem do JAR físico/hash/file index atual, não da listagem antiga visível na página principal.
- **Procedência:** Modlist física canônica de 08/09/2026 confirma `emf_compat_iron_spells_1.21.1_2.0.0.jar`, mod id `emf_compat_iron_spells`, versão 2.0.0 e SHA-1 515b545870fce128bbf01a0ccacdd19566ed3b22. Índice de arquivos associa o mesmo hash ao CurseForge file 8793934; página oficial fornece escopo/pose boundary Client-only.
