...
- **Dependências:** Runtime físico atual: NeoForge 21.1.250 + Create 6.0.10 + Petrolpark 1.5.11. Source matching 0.4.3 usa Create 6.0.10 e fixa Petrolpark 1.5.0 no build; README do port ainda cita 1.4.31+/1.4.32+ como requirement histórico. Registrate/Flywheel/Ponder seguem infraestrutura do stack; JEI/Curios/CBC/Create Connected/Farmer's Delight são integrações opcionais conforme source.
...
- **Procedência:** modlist.txt física atual de 20/09/2026 — 587 mods incluindo o modloader — confirma `destroy-1.21.1-0.4.3.jar` / runtime `destroy` 0.4.3, Create 6.0.10 e Petrolpark 1.5.11. Source oficial do port `NHblock714/Destroy`, branch `1.21.1-neo`, matching 0.4.3, revalidado em 20/09/2026; upstream Petrolpark 1.20.1 permanece apenas origem histórica.
- **Observações:** Runtime 0.4.3/source matching preservados. O pack está em Petrolpark 1.5.11, acima do pin 1.5.0 do source 0.4.3 e das referências 1.4.31+/1.4.32+ do README; isso é version drift a validar, não incompatibilidade comprovada. Chemistry data-driven continua elements→molecules→reactions com sync S2C/rejoin.
- **Atualização/Status:** REATUALIZADO EM 20/09/2026 — lote físico #217: destroy-1.21.1-0.4.3.jar / runtime 0.4.3 e source matching 1.21.1-neo reconfirmados. Pack físico usa Create 6.0.10 e Petrolpark 1.5.11; chemistry/mixtures/Vat/pollution/data sync permanecem atuais, com drift de library tratado fail-closed.
- **Decisão:** Sem decisão
...
- Flywheel `1.0.5`.
O README ainda menciona petrolpark library `1.4.31+` / `1.4.32+` como requirement histórico do port; para o source 0.4.3, **`gradle.properties`**** é a referência de build mais específica** e fixa 1.5.0. A modlist física atual usa **Petrolpark 1.5.11**, acima desse pin. Essa diferença é version drift a validar com chemistry/mixtures/contraptions, não incompatibilidade comprovada.
...
## 17. Evidências
- modlist física atual de 20/09/2026: `destroy-1.21.1-0.4.3.jar` / runtime `0.4.3`, Create 6.0.10 e Petrolpark 1.5.11;
