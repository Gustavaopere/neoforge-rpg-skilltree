...
- **Dependências:** Required content oficial: KubeJS e Create. Runtime físico: KubeJS 2101.7.2-build.377 + Create 6.0.10. Client & Server.
...
- **Observações:** JAR físico build.18 permanece atual para NeoForge 1.21.1. A linha pública continua Beta. O core KubeJS físico está em build.377.
- **Procedência:** modlist(1).txt física anexada e reconferida em 25/09/2026 + Modrinth oficial KubeJS Create build.18 + KubeJS Wiki Create 1.21.1.
- **Atualização/Status:** PADRÃO ALEX'S MOBS REVALIDADO EM 25/09/2026 — KubeJS Create 2101.3.1-build.18/JAR físico reconfirmado; matriz física reconciliada com KubeJS 2101.7.2-build.377 e Create 6.0.10. Create recipe builders, sequenced assembly, lifecycle/reload, Beta-only boundary e riscos preservados.
- **Data da última decisão:** 2026-09-06

> **Autoridade física atual — 25/09/2026.** `modlist(1).txt` contém 587 entradas top-level incluindo o modloader; este item ocupa a ordem física #362: JAR `kubejs-create-neoforge-2101.3.1-build.18.jar`, mod id `kubejs_create`, runtime `2101.3.1-build.18`, SHA-1 `d4dce8f82f2d37545f085522a782c1bfda222154`.

<callout icon="⚙️" color="orange_bg">
	**ESCOPO CANÔNICO.** Runtime físico: `kubejs-create-neoforge-2101.3.1-build.18.jar`, mod id `kubejs_create`, versão `2101.3.1-build.18`. É a integração KubeJS ↔ Create para NeoForge 1.21.1. A linha pública 2101.3.1 é **Beta-only**; isso é maturidade do artefato, não motivo automático para downgrade.
</callout>
...
## 5. Runtime do pack
Create físico é 6.0.10 e KubeJS é 2101.7.2-build.377. A build KubeJS Create 2101.3.1-build.18 é a publicação NeoForge 1.21–1.21.1 mais recente localizada e declara KubeJS/Create como required content.
...
Modrinth confirma build.18 Beta, NeoForge 1.21–1.21.1, Client & Server e required content KubeJS/Create. KubeJS Wiki documenta os builders 1.21.1. Revalidação externa em 13/09/2026 mantém `2101.3.1-build.18` como a build NeoForge 1.21.1 mais recente localizada. Scripts reais do pack não foram auditados; nenhuma recipe custom é inferida sem localizar os arquivos.
