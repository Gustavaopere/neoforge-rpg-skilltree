...
- **Procedência:** modlist(1).txt física atual de 20/09/2026 + runtime `1.21.1-v1-neoforge` + CurseForge/documentação oficial Cosmetic Armor Reworked já auditadas. Reconciliação final: JAR/runtime permanecem exatamente `cosmeticarmorreworked-1.21.1-v1-neoforge.jar` / `1.21.1-v1-neoforge`; sem divergência física.
...
- **Atualização/Status:** PADRÃO ALEX'S MOBS REVALIDADO EM 20/09/2026 — reconciliação final física #110: `cosmeticarmorreworked-1.21.1-v1-neoforge.jar` / `1.21.1-v1-neoforge` conferidos contra a modlist atual; dual-slot authority, render/stat separation, death/drop, visibility, API e lifecycle preservados.
