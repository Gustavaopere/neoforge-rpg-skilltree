...
- **Arquivo JAR:** bits_n_bobs-2.3.5.jar
- **Versão 1.21.1:** 2.3.5
...
- **Dependências:** Create; Sable Companion 1.4.2 está embarcado em META-INF/jarjar e não é entrada top-level. Pack usa Create 6.0.10. Bits 'n Tracks e Create: Sulfuric Resonance não estão top-level na modlist física atual; Create Tracks+ 1.0.6b6 está presente, mas não foi presumido como consumer/substituto sem evidência.
- **Estado no pack:** Integrado ao Github
- **Estado da pesquisa:** Verificado
- **Compatibilidade/Riscos:** Riscos atuais: kinetic duplication, contraption lifecycle, feature suppression/config, regressão de Chain Drive/cogwheel integrations e API drift. A 2.3.5 adiciona compat de modelo para Create: Sulfuric Resonance, reforma o cogwheel material system para reduzir quebra de modelos de outros mods e limpa a Chain Drive API; isso torna Bits 'n' Tracks explicitamente incompatível até ser atualizado. Bits 'n' Tracks e Sulfuric Resonance não estão top-level na modlist física atual.
- **Fonte:** https://www.curseforge.com/minecraft/mc-mods/create-bits-n-bobs
- **Procedência:** modlist(1).txt física atual de 18/09/2026 + metadata/JAR (`bits_n_bobs` 2.3.5; Sable Companion 1.4.2 jarjar) + CurseForge oficial das releases NeoForge 1.21.1 2.3.4 (12/09/2026) e 2.3.5 (15/09/2026) + decisão curatorial previamente auditada.
- **Observações:** Runtime físico 2.3.5. A 2.3.4 aumentou os bits disponíveis para cores de headlamp. A 2.3.5 adicionou compat de modelo de cogwheel com Create: Sulfuric Resonance, limpou a Chain Drive API (com incompatibilidade explícita de Bits 'n' Tracks até atualização) e reformou o cogwheel material system para reduzir quebra de modelos de outros mods. Os fixes 2.3.2 permanecem baseline histórica.
- **Atualização/Status:** PADRÃO ALEX'S MOBS REVALIDADO EM 18/09/2026 — runtime físico reconciliado de 2.3.2 para 2.3.5. Delta oficial 2.3.4/2.3.5 incorporado: expansão das cores de headlamp; compat de modelos com Sulfuric Resonance; refactor do cogwheel material system; nova Chain Drive API com Bits 'n' Tracks explicitamente incompatível até atualização. Decisão `Manter` e Sable Companion 1.4.2 jarjar preservados.
- **Decisão:** Manter
- **Histórico da decisão:** 2026-09-06 — decisão anterior Tirar por incompatibilidade visual explícita com Create: Sulfuric Resonance. 2026-09-07 — decisão explícita do usuário substitui a anterior: MANTER Bits 'n Bobs e aceitar/monitorar o conflito visual com CSR. 2026-09-09 — runtime atualizado/revalidado em 2.3.2; CSR não está mais presente no snapshot físico, então o conflito permanece apenas como histórico, sem alterar a decisão Manter. 2026-09-18 — runtime físico atualizado para 2.3.5; decisão `Manter` preservada. A nova compat upstream com Sulfuric Resonance reduz o conflito histórico de modelos, mas o provider segue ausente da modlist atual; Bits 'n' Tracks também está ausente e a incompatibilidade declarada da nova Chain Drive API não é ativa nesta instância.
...
> ⚠️ Versão física confirmada: `bits_n_bobs-2.3.5.jar`, mod id `bits_n_bobs`, runtime `2.3.5`, com Create `6.0.10` no pack. O JAR contém **Sable Companion 1.4.2** em `META-INF/jarjar`; ele não é mod top-level. A decisão curatorial explícita **MANTER** permanece. A 2.3.5 adiciona compat de modelo para Create: Sulfuric Resonance, mas esse provider não está presente na modlist física atual; Bits 'n' Tracks também está ausente e é explicitamente incompatível com a nova Chain Drive API até atualização.
...
O projeto também adiciona troca/variação de madeira para cogwheels. Essa superfície de assets/modelos carregava historicamente incompatibilidade com Sulfuric Resonance. Na 2.3.5, o upstream adicionou compat de modelo específica para Create: Sulfuric Resonance e reformou o cogwheel material system para reduzir quebra de modelos de outros mods. O provider Sulfuric Resonance continua ausente da modlist física atual, portanto a compat nova é um regression gate futuro, não integração ativa.
Não tratar wood swap como mudança de ratio/kinetic behavior sem evidência específica.
## 11. Feature suppression e deltas 2.3.2 → 2.3.5
...
A release **2.3.2**, preservada como baseline histórica, acrescentou três correções oficiais:
...
A release **2.3.4** aumentou os bits disponíveis para **headlamp colours**.
A release física **2.3.5** acrescenta três mudanças oficiais materialmente relevantes:
- compat de modelo de cogwheel com **Create: Sulfuric Resonance**;
- limpeza/refactor da **Chain Drive API**, com incompatibilidade explícita de **Bits 'n' Tracks** até esse addon ser atualizado;
- reforma do **cogwheel material system** para reduzir quebra de modelos de cogwheel em outros mods.
Na modlist física atual, Bits 'n Tracks e Sulfuric Resonance não estão top-level. Portanto a incompatibilidade da nova API e a compat Sulfuric não são integrações ativas nesta instância, mas devem permanecer como regression gates se esses providers forem reintroduzidos.
Riscos:
...
- addon externo ainda consumir a Chain Drive API anterior;
- resource/model layer de outro addon divergir do cogwheel material system reformado.
...
Na modlist física atual de 18/09/2026, **Create: Sulfuric Resonance não está presente**. O conflito histórico não é tratado como risco ativo desta instância. Além disso, a 2.3.5 passou a declarar compat de modelo específica com Sulfuric Resonance e reformou o material system dos cogwheels. Se o provider voltar, o render-test específico deve ser reativado para validar a compat upstream na combinação física real; a decisão histórica não é apagada.
## 14. Client/server
...
6. Se Sulfuric Resonance for reintroduzido, validar a compat de modelo adicionada na 2.3.5 contra a combinação física real; não é risco ativo no snapshot atual.
7. Consumer antigo da Chain Drive API — especialmente Bits 'n' Tracks — quebrar até atualizar para o novo contrato; Bits 'n' Tracks não está top-level no snapshot atual.
8. Resource pack sobrescrever cogwheel assets e ampliar incompatibilidade.
9. Sable Companion jarjar confundido com top-level.
## 17. Matriz de testes
...
7. Nixies/headlamps/chairs e outros blocks após resource reload; cobrir a expansão de headlamp colours da 2.3.4.
8. Feature suppression: item suprimido não deixa creative tab/recipe quebrado.
9. Se Create: Sulfuric Resonance voltar ao pack, executar render-test específico dos cogwheel models com a compat adicionada na 2.3.5; no snapshot atual esse provider está ausente.
10. Se Bits 'n' Tracks voltar ao pack, bloquear a combinação até confirmar versão adaptada à nova Chain Drive API.
11. Verificar ausência de entrada top-level Sable Companion no catálogo.
## 18. Evidência
- modlist física atual de 18/09/2026: Bits 'n' Bobs 2.3.5, Create 6.0.10 e Sable Companion 1.4.2 jarjar; Sulfuric Resonance e Bits 'n Tracks não estão top-level neste snapshot;
- [CurseForge oficial — release 2.3.4](https://www.curseforge.com/minecraft/mc-mods/create-bits-n-bobs/files/8863252): expansão dos bits disponíveis para headlamp colours;
- [CurseForge oficial — release 2.3.5](https://www.curseforge.com/minecraft/mc-mods/create-bits-n-bobs/files/8886545): compat de cogwheel model com Sulfuric Resonance, nova Chain Drive API e reforma do cogwheel material system;
...
> ⚙️ Ownership canônico: Create = kinetic/fluid/contraption engine; Bits 'n' Bobs = extensões registradas. O conflito visual com Sulfuric Resonance permanece **preservado no histórico curatorial**, mas a 2.3.5 adiciona compat de modelo upstream; ele não é conflito ativo porque esse provider está ausente do snapshot atual. A nova Chain Drive API é um version gate para qualquer retorno de Bits 'n' Tracks.
