...
- **Observações:** JAR físico `MineColonies_Tweaks-1.21.1-3.33.jar`, mod id `minecolonies_tweaks`, runtime 3.33. O source exato declara MC 1.21.1 e target MineColonies 1.1.1368. O pack roda MineColonies 1.1.1387; diferença aceita pelo range, mas sensível devido aos muitos mixins. Defaults de config documentados não equivalem aos valores efetivamente persistidos no serverconfig.
- **Procedência:** modlist(1).txt física anexada e reconferida em 25/09/2026 + CurseForge oficial da release 3.33 + repositório oficial pinado no commit 33efa6f49e0a3f660726456aa5e403f219177360.
- **Atualização/Status:** PADRÃO ALEX'S MOBS REVALIDADO EM 25/09/2026 — MineColonies Tweaks 3.33/JAR físico reconfirmado; runtime atual MineColonies 1.1.1387 e Compatibility 3.57 reconciliados. O bridge Let’s Do está ausente da modlist física atual. Dossiê e decisão Manter preservados.
...
> **Autoridade física atual — 25/09/2026.** `modlist(1).txt` contém 587 entradas top-level incluindo o modloader; este item ocupa a ordem física #397: JAR `MineColonies_Tweaks-1.21.1-3.33.jar`, mod id `minecolonies_tweaks`, runtime `3.33`, SHA-1 `1b65bef2848d6f13e8d785f34a76e4113284d08e`.
...
O MineColonies físico deste pack é `minecolonies-1.1.1387-1.21.1-snapshot.jar`, enquanto Tweaks 3.33 foi compilado contra 1.1.1368. O contrato permite a versão mais nova, mas há **drift relevante entre o baseline 1.1.1368 e o runtime atual** e o addon possui muitos mixins contra classes internas de MineColonies. Isso é um risco concreto de regressão estrutural; não é prova de incompatibilidade.
...
Essas superfícies são relevantes para qualquer datapack/addon que ensine receitas, altere entregáveis ou reconfigure requests. Interação com #396 MineColonies Compatibility deve ser validada, não presumida. O bridge MineColonies Let’s Do está ausente da modlist física atual e não é tratado como integração ativa.
...
### MineColonies 1.1.1387
É a maior área de risco. 3.33 foi construído com 1.1.1368 e injeta em muitas classes internas. O loader aceitar 1.1.1387 não prova que todos os injection points continuam semanticamente equivalentes.
### #396 MineColonies Compatibility 3.57
...
O bridge foi removido da modlist física atual. Referências históricas não contam como integração ativa deste runtime.
...
- receita/integração de #396; não incluir o bridge Let’s Do ausente.
...
- **Limite:** esta auditoria documenta superfícies concretamente presentes no source; não afirma que cada configuração foi runtime-tested no pack. A diferença MineColonies 1.1.1368 → 1.1.1387 permanece como risco de teste, não como incompatibilidade declarada.
