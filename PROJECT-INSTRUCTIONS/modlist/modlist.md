...
> A tabela principal indexa somente mods top-level que, no estado atual da auditoria, possuem dossiê certificado (`✅-`) **e** versão explicitamente registrada no nome do arquivo. Itens já organizados por categoria que perderam aprovação durante uma reauditoria permanecem visíveis em **Itens organizados sem aprovação**, mas não contam como certificados.
>
> A ordem segue a posição física real em `modlist.txt`. O campo de pasta usa o conjunto completo de categorias temáticas atuais do projeto correspondente no CurseForge, em ordem alfabética, com ` + ` como separador compatível com Windows. Categorias de loader, versão do Minecraft e ambiente não entram na pasta.
>
...
Categorias verificadas em 15/09/2026 no projeto CurseForge correspondente ao artefato instalado. Project IDs e URLs abaixo são evidência de identidade/categorização externa; não substituem `modlist.txt`/JAR físico para presença, filename, versão instalada ou ordem.
...
Estes dossiês continuam nas pastas determinadas pelo conjunto exato de categorias do CurseForge, porém **não possuem `✅-`** porque a reauditoria de 15/09/2026 encontrou informação upstream posterior ao conteúdo canônico do Notion. A autoridade física instalada não foi alterada.

| # | Mod | JAR / versão física | Pasta / categorias | Upstream localizado | Motivo do bloqueio |
|---:|---|---|---|---|---|
| 106 | Cold Sweat | `ColdSweat-2.4.2.jar` · `2.4.2` | `Adventure and RPG + Armor, Tools, and Weapons + Utility & QoL` | `2.4.3` — 13/09/2026 | Notion/GitHub migrado não incorporam a release 2.4.3 e seus deltas; reconciliação da origem pendente. |
| 425 | M.R.U | `mru-1.0.33+1.21.1-neoforge.jar` · `1.0.33+1.21.1` | `API and Library` | `1.0.40` para 1.21.1 NeoForge — 11/09/2026 | O Notion afirma que 1.0.33 era a release mais recente localizada; isso deixou de ser factual. |
| 467 | Petrolpark Library | `petrolpark-1.21.1-1.5.10.jar` · `1.5.10` | `Addons + API and Library + Create` | `1.5.11` Beta — 14/09/2026 | O Notion afirma que 1.5.10 era a build mais recente localizada; existe 1.5.11 posterior. |
| 550 | Somake Spells - Iron's Spells Addon | `somakespells-1.0.8-1.21.1-fix.jar` · runtime `1.0.8` | `Addons + Adventure and RPG + Armor, Tools, and Weapons + Magic` | `1.0.9` — 12/09/2026 | O Notion afirma que 1.0.8 era a release mais recente localizada; 1.0.9 é posterior. A chave vazia `Data da última decisão` também foi restaurada no Markdown para paridade estrutural. |
| 584 | Thirst Was Reclaimed | `ThirstWasReclaimed-1.21.1-3.0.4.jar` · `1.21.1-3.0.4` | `Food` | `3.0.5` — 10/09/2026 | O Notion afirma que 3.0.4 continuava a release 1.21.1 mais recente; 3.0.5 é posterior. |

Evidência externa dos bloqueios: [Cold Sweat](https://www.curseforge.com/minecraft/mc-mods/cold-sweat/files/all), [M.R.U](https://www.curseforge.com/minecraft/mc-mods/mru/files/all), [Petrolpark Library](https://www.curseforge.com/minecraft/mc-mods/petrolpark-library/files/all), [Somake Spells](https://www.curseforge.com/minecraft/mc-mods/somake-spells-irons-spells-addon/files/8867079) e [Thirst Was Reclaimed](https://www.curseforge.com/minecraft/mc-mods/thirst-was-reclaimed/files/8852248).
