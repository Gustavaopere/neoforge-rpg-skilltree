...
- **Procedência:** modlist(1).txt física atual de 20/09/2026 + runtime 3.0.0+1.21.neoforge + CurseForge/Modrinth oficiais Continuity 3.0.0 NeoForge e changelog oficial já auditados. Reconciliação final: JAR/runtime permanecem exatamente `continuity-3.0.0+1.21.neoforge.jar` / `3.0.0+1.21.neoforge`; boundary client-only preservado.
...
- **Atualização/Status:** PADRÃO ALEX'S MOBS REVALIDADO EM 20/09/2026 — reconciliação final física #106: `continuity-3.0.0+1.21.neoforge.jar` / `3.0.0+1.21.neoforge` conferidos contra a modlist atual; CTM/emissive/overlay pipeline, moving blocks, reload/cache, performance e regressões 3.0.0 preservados. Client-only.
