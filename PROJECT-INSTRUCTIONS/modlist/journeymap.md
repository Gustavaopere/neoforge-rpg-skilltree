...
- **Autoridade física:** `journeymap-neoforge-1.21.1-6.0.7.jar`, mod id `journeymap`, runtime `1.21.1-6.0.7`
...
- **Dependências:** Java 21, Minecraft 1.21.1 e NeoForge 21.1.228+ segundo a linha 6.0.x. Pack usa NeoForge 21.1.248. O JAR físico 6.0.7 embarca via JarJar Common Networking 1.0.21-1.21.1, JourneyMap API 2.0.0-1.21.1 e PNGJ 2.1.0; nenhum é entrada top-level separada.
...
- **Observações:** Runtime físico continua 6.0.7. Existe release oficial JourneyMap 6.0.8 NeoForge 1.21.1 de 09/09/2026; atualização ainda não foi aplicada. O changelog específico da 6.0.8 não foi pinado nesta auditoria, portanto nenhum fix novo foi inventado.
- **Procedência:** modlist.txt física anexada e reconferida em 12/09/2026 + CurseForge oficial JourneyMap listando 6.0.8 NeoForge 1.21.1 em 09/09/2026 e 6.0.7 físico + documentação 6.0.x + inventário físico JarJar do host confirmando Common Networking 1.0.21, JourneyMap API 2.0.0 e PNGJ 2.1.0.
- **Fonte:** https://modrinth.com/plugin/journeymap/version/IAVJBO0C
- **Atualização/Status:** PADRÃO ALEX'S MOBS REVALIDADO EM 12/09/2026 — JourneyMap 1.21.1-6.0.7/JAR físico reconfirmado, porém atualização oficial 6.0.8 NeoForge 1.21.1 foi publicada em 09/09/2026. Gap de versão registrado sem alterar a modlist. JarJar físico completo agora documentado: Common Networking 1.0.21, JourneyMap API 2.0.0 e PNGJ 2.1.0.
...
JourneyMap 6.0 usa a API v2. A release 6.0.7 declara **JourneyMap API v2.0.0-1.21.1**. O JAR físico também contém, em `META-INF/jarjar/`, `common-networking-neoforge-1.0.21-1.21.1.jar` e `pngj-2.1.0.jar`. Os três componentes — Common Networking 1.0.21, JourneyMap API 2.0.0 e PNGJ 2.1.0 — são **dependências internas do host**, não três mods top-level adicionais.
...
### Atualização disponível — 6.0.8
A listagem oficial atual contém **JourneyMap 6.0.8 para NeoForge 1.21.1**, publicada em 09/09/2026. O pack físico permanece em 6.0.7, portanto existe um **gap de versão real**. A catalogação não substitui o JAR: qualquer adoção da 6.0.8 deve ser tratada como mudança operacional e smoke-tested com JourneyMap Integration/overlays. Como o changelog específico da 6.0.8 não foi pinado nesta execução, nenhum fix adicional é atribuído à nova build.
...
- **Modlist física:** JAR/mod id/version e JarJars `common-networking-neoforge-1.0.21-1.21.1`, `journeymap-api-neoforge-2.0.0-1.21.1` e `pngj-2.1.0`.
