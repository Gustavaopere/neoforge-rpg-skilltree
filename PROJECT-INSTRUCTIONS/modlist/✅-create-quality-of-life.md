...
- **Procedência:** modlist(1).txt física atual de 20/09/2026 + runtime `createqol` 1.6.3-fix1 + CurseForge/Modrinth oficiais da release e changelogs já auditados. Reconciliação final: JAR/runtime permanecem exatamente `Create Quality of Life-1.21.1-1.6.3-fix1.jar` / `1.6.3-fix1`; Create 6.0.10 permanece authority das primitives base.
...
- **Atualização/Status:** PADRÃO ALEX'S MOBS REVALIDADO EM 20/09/2026 — reconciliação final física #114: `Create Quality of Life-1.21.1-1.6.3-fix1.jar` / `1.6.3-fix1` conferidos contra a modlist atual; QoL components, armor/elytra/propeller, trash/fluid behavior, config e regressões preservados.
