...
- **Fonte:** https://modrinth.com/mod/owo-lib
...
- **Dependências:** NeoForge 1.21.1. O JAR embute Endec/Jankson/Forgified Fabric API Base e componentes auxiliares em `META-INF/jars`; consumers reais precisam ser mapeados antes de qualquer remoção.
- **Compatibilidade/Riscos:** Build Beta Client & Server. Riscos: API drift, handshake/config-sync mismatch, client classloading, JarJar ambiguity e regression de DerivedComponentMap. Embedded Endec/Jankson/Forgified Fabric API Base não são top-level.
- **Sobreposição:** Biblioteca potencialmente load-bearing; não substitui gameplay. Forgified Fabric API Base interno não deve ser confundido com entrada top-level ou conflito automático.
- **Observações:** JAR físico `owo-lib-neoforge-0.12.15.5-beta.1+1.21.jar`, mod id `owo`, runtime 0.12.15.5-beta.1+1.21. Build Beta; changelog tenta corrigir equality de DerivedComponentMap.
- **Procedência:** modlist(1).txt física reconferida em 25/09/2026 + Modrinth oficial da build Beta + Wisp Forest Docs + hierarquia JarJar física.
- **Atualização/Status:** REVALIDADO EM 25/09/2026 — oωo/owo-lib 0.12.15.5-beta.1+1.21/JAR físico reconfirmado; Endec/Jankson/Forgified Fabric API Base permanecem embedded, não top-level.
- **Histórico da decisão:** 
- **Data da última decisão:** 2026-09-10

> **Autoridade física atual — 25/09/2026.** `modlist(1).txt` contém 587 entradas top-level incluindo o modloader; este item ocupa a ordem física #430: JAR `owo-lib-neoforge-0.12.15.5-beta.1+1.21.jar`, mod id `owo`, runtime `0.12.15.5-beta.1+1.21`, SHA-1 `48dda11a6710591cf162bdbedf982ea21dd1f2ed`.
