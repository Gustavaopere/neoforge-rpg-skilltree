...
- **Autoridade física:** `easy_model_entities-neoforge-1.21.1-2.3.0.jar`, mod id `easy_model_entities`; campo `mod version` vazio na modlist física; `2.3.0` é identidade filename/publicação
...
- **Observações:** Fail-closed preservado: `Versão 1.21.1` permanece vazia porque a metadata física não declara versão. O pack usa o artefato 2.3.0; upstream 2.4.0 adiciona sequences de animação, emissive blend, hand-item rendering/anchors e corrige clips explícitos em `body_type: static`/`animation.mode: none`. Atualizar Easy NPC para 7.12.1 também torna EME 2.4.0+ relevante para a integração opcional.
- **Procedência:** modlist.txt física atual — 595 entradas totais incluindo o modloader — confirma `easy_model_entities-neoforge-1.21.1-2.3.0.jar`, mod id `easy_model_entities` e campo de versão runtime vazio. CurseForge oficial revalidado em 12/09/2026 confirma 2.4.0 como Release 1.21.1 posterior; 2.3.0 continua sendo apenas a identidade do artefato físico por filename/publicação.
- **Fonte:** https://www.curseforge.com/minecraft/mc-mods/easy-model-entities/files/8706558
- **Atualização/Status:** PADRÃO ALEX'S MOBS REVALIDADO EM 12/09/2026 — lote físico #240: `easy_model_entities-neoforge-1.21.1-2.3.0.jar` permanece instalado, com metadata física de versão vazia; a distribuição 1.21.1 avançou para Release 2.4.0 em 10/09/2026. Não preencher a versão runtime por inferência.
...
Em 10/09/2026 o upstream publicou **Easy Model Entities 2.4.0 para NeoForge 1.21.1**. Essa release posterior adiciona sequences de animação de até 16 clips e API `playAnimationSequence`, blend `emissive`, renderização opt-in de itens nas mãos e anchors/fallbacks para modelos sem hand bones, além de corrigir clips explicitamente selecionados quando `body_type: static` ou `animation.mode: none`. Essas capacidades pertencem à 2.4.0 e **não são atribuídas ao artefato físico 2.3.0**.
...
## 19. Evidências
- modlist física atual: filename, mod id e campo de versão vazio;
- publicação oficial Easy Model Entities para NeoForge 1.21.1: artefato físico 2.3.0 e Release posterior 2.4.0 publicada em 10/09/2026;
- changelogs 2.1.0–2.4.0: API/model/animation, serializers/culling/render fixes, texture get, opacity/light/sync e, na 2.4.0 somente, animation sequences, emissive blend e hand-item anchors.
