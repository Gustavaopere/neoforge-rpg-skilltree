...
- **Compatibilidade/Riscos:** 5.4.4 declara compatibilidade com Sable Mod, relevante ao stack físico Sable/#399. GeckoLib é required; JEAsync é optional. Riscos principais: death/resurrection overlap, loot/body parts, pets/bodyguards, worldgen/Witherstein e ausência de source exato para auditar internals/persistência/networking.
- **Sobreposição:** Sobreposição temática/técnica com sistemas de necromancia, pets e corpos. Interseção mais concreta neste lote: #399 Sable mob ragdoll corpses e o stack Sable; 5.4.4 declara compatibilidade com Sable, mas o mecanismo interno não foi presumido sem source.
...
- **Procedência:** modlist(1).txt física anexada e reconferida em 25/09/2026 + CurseForge oficial Mobstein confirmando `mobstein-5.4.4-neoforge-1.21.1.jar` como release 1.21.1 + dossiê oficial de gameplay/dependências já auditado.
- **Atualização/Status:** PADRÃO ALEX'S MOBS REVALIDADO EM 25/09/2026 — Mobstein 5.4.4/JAR físico reconfirmado; compatibilidade oficial com Sable, dependência GeckoLib e limite por ausência de source binário exato preservados. Referência física ao Sable mob ragdoll corpses reconciliada para #399.
...
> **Autoridade física atual — 25/09/2026.** `modlist(1).txt` contém 587 entradas top-level incluindo o modloader; este item ocupa a ordem física #400: JAR `mobstein-5.4.4-neoforge-1.21.1.jar`, mod id `mobstein`, runtime `5.4.4`, SHA-1 `3672d88f940ddd474a5429d7066b099cd0ce0c29`.
...
Isso é particularmente relevante neste pack porque o stack Sable está fisicamente presente, inclusive `Sable`, `Sable Ragdolls`, `Ragdoll Reactions`, `Sable Ragdolls Patch` e #399 `Sable mob ragdoll corpses`.
...
## 17. Relação com Sable e #399
A release **5.4.4** foi publicada especificamente com a nota de que Mobstein agora é compatível com **Sable Mod**. No mesmo pack está #399 `Sable mob ragdoll corpses` 1.1.5, que intercepta fatal damage de mobs e mantém corpses físicos.
...
- não foi estabelecido formato de save/NBT/capability;
...
- **Death/resurrection overlap:** #399 Sable Corpse e outros death handlers podem compartilhar a mesma fase de morte de entidades.
...
- death + loot;
- save/restart e owner persistence;
- multiplayer com owner e segundo player.
### Corpos/órgãos
- obter partes por rota publicada, incluindo shears quando aplicável;
- criar/usar full body;
- Organ Extractor + Tweezers;
- confirmar resultado de um dos três órgãos;
- remover body do suporte conforme mecânica publicada.
### Surgery Stretch
- criar sujeitos variando Attack/Health/Speed nos ranges publicados;
- testar Template;
- validar tipos Turtle/Panda/Polar Bear/Spider/Enderman/Snow Golem;
- confirmar que player head não entra nesse fluxo.
### Subject Assembly Machine
- head comum;
- player head via Head Creator;
- head + torso + pants;
- confirmar mannequin sem habilidades de Surgery experiment.
### Igor/failed experiments
- localizar/spawnar Igor pela rota normal;
- Igor table;
- Suspicious Syringe;
- validar regras de primeira tentativa/limite;
- produzir exemplos 097, 062, 067, 077, 091, 012 e 007 quando possível;
- testar tame/ability individual conforme guia.
### Estruturas/boss
- encontrar Frankenstein Castle.
- encontrar Witherstein Ruins.
- encontrar Old Ruins.
- invocar Witherstein pela rota de Reviver Syringe.
- validar 3 stages e Wither Skeleton summons nos dois últimos.
- save/restart no meio do boss fight.
### Sable compatibility
- matar mob vanilla com Mobstein instalado + #399.
- matar mob ressuscitado/tamed.
- matar failed experiment.
- matar Dr Mobstenio/Igor em ambiente de teste.
- matar Witherstein em cada stage relevante.
- observar corpse, death completion, drops, advancement/progression e ausência de softlock.
- testar Reviver Syringe perto de corpse Sable ativo.
