...
> **Autoridade física atual — 23/09/2026.** `modlist(1).txt` contém **587 entradas top-level incluindo o modloader**; este item ocupa a ordem física **#156**: JAR `create_recycle_everything-2.1.0.jar`, mod id `create_recycle_everything`, runtime `1.1`, SHA-1 `bb1ccccc5e72410ebaf8c9f088186cc46060534e`. O filename/publicação física é `2.1.0`; a metadata/runtime top-level permanece `1.1`, divergência preservada para troubleshooting.
...
- **Dependências:** Create obrigatório; pack físico usa NeoForge 21.1.250 + Create 6.0.10. A build 2.1.0 adiciona integração de recipes para Create Stuff 'N Additions; outras integrações só contam quando o provider está presente.
...
O campo `Versão 1.21.1` usa a identidade do artefato físico/publicação (`2.1.0`). A metadata runtime literal `1.1` permanece documentada separadamente e não deve ser apagada do troubleshooting.
## 14. Riscos
...
## 16. Evidência
- modlist física atual de 16/09/2026: `create_recycle_everything-2.1.0.jar`, metadata runtime 1.1;
