...
- **Atualização/Status:** PADRÃO ALEX'S MOBS REVALIDADO EM 20/09/2026 — registro histórico da reconciliação física #86 na snapshot então vigente; posição física atual #87: `cataclysm_spellbooks-1.1.14-1.21.jar` / `1.1.14-1.21` conferidos contra a snapshot então vigente; delta 1.1.14 (Strange Disc + manuscripts→500 mB Timeless Slurry), baseline Beta 1.1.13, 65-spell release scope, Abyssal/Technomancy, required relations e boundary source público 1.1.11→binary 1.1.14 preservados. Decisão `Manter` mantida.
