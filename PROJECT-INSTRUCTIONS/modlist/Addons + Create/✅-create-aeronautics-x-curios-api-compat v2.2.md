...
> **Autoridade física atual — 23/09/2026.** `modlist(1).txt` contém **587 entradas top-level incluindo o modloader**; este item ocupa a ordem física **#171**: JAR `createaeronauticscurios-neoforge-1.21.1-2.2.jar`, mod id `aeronautics_curios_compat`, runtime `2.0`, SHA-1 `ded927b94810ff89791e1e57bcd20b468bd98a9f`. O filename/publicação física é `2.2`; a metadata/runtime top-level permanece `2.0`, divergência de packaging/runtime preservada para troubleshooting.
...
- **Dependências:** Pack físico: NeoForge 21.1.250 + Create 6.0.10 + Create Aeronautics 1.3.2 + Curios API 9.5.1+1.21.1. A bridge é Client & Server e atua apenas sobre os itens Aeronautics documentados.
...
> 🥽 **ESCOPO CANÔNICO.** Artefato físico: `createaeronauticscurios-neoforge-1.21.1-2.2.jar`. A publicação pública é **2.2**, mas o metadata/runtime extraído do JAR declara versão **2.0** e mod id `aeronautics_curios_compat`. A versão catalogada segue o artefato físico/publicação 2.2; a string runtime 2.0 permanece preservada separadamente para troubleshooting.
