...
- **Autoridade física atual:** `modlist(1).txt` anexada em 16/09/2026 — `create_stats-1.13.1.jar`, mod id `create_stats`, runtime `1.13.1`, mixin config `create_stats.mixins.json`
...
- **Fonte:** https://www.curseforge.com/minecraft/mc-mods/create-stats-power | https://modrinth.com/mod/create-graphs-power | https://github.com/Erbell9520/Create-Graphs-Additions-public
...
- **Dependências:** NeoForge + Create obrigatórios. A instalação física atual contém Create 6.0.10. O source público atual também declara LambDynamicLights como required dependency para a iluminação dinâmica de Stage Lights; GeckoLib e JEI aparecem como integrações opcionais; Sable Companion é embarcado via JarJar; integrações com Simulated/Create Aeronautics continuam condicionais à presença real.
- **Compatibilidade/Riscos:** Adiciona grid FE próprio, PDU, vehicle control e monitoramento; pode sobrepor energy bridges e control systems. O dossiê 1.5.1 preserva como regressões históricas os crashes de cables/PDU em certas configs, flicker de telas e layout do PDU, além de board chaining, outlets/Smart Home Panel e cable cornering. Na linha física atual 1.13.1, ampliar testes para Stage Light/LambDynamicLights, optional JEI/GeckoLib, Sable Companion embarcado e HUD/Stressometer.
...
- **Observações:** Runtime físico atual `1.13.1`. O antigo dossiê/Notion estava em 1.5.1. A página pública do projeto continua descrevendo o mesmo core funcional (power grid, monitoring, lighting, logistics, vehicles, heating e drivetrain control). O source público atual expõe dependências e integrações adicionais relevantes: LambDynamicLights required para Stage Light, GeckoLib/JEI opcionais e Sable Companion JarJar. O source também contém código P1.13 para configuração/HUD do Stressometer. Não foi localizado, nas fontes oficiais acessíveis nesta auditoria, um changelog público exato e completo da build `1.13.1`; por isso não são atribuídas mudanças específicas adicionais sem evidência.
- **Procedência:** modlist física atual anexada em 16/09/2026 + metadata física `create_stats` 1.13.1 + páginas oficiais CurseForge/Modrinth + source público atual Erbell9520/Create-Graphs-Additions-public. O filename 1.13.1 também aparece em agregador de distribuição sincronizado com CurseForge/Modrinth, mas a autoridade da versão instalada permanece a modlist física.
- **Histórico da decisão:** Sem decisão formal. Em 08/09/2026, Create: Stats & Power 1.5.1 foi reconfirmado como instalado; a antiga data 30/08 associada apenas à presença foi removida. Presença não foi convertida em decisão curatorial. Em 16/09/2026, a modlist física confirma `create_stats-1.13.1.jar`; a decisão permanece Sem decisão.
- **Atualização/Status:** PADRÃO ALEX'S MOBS REVALIDADO EM 16/09/2026 — runtime físico atualizado de 1.5.1 para 1.13.1; FE-grid/PDU authority, monitoring/logistics/vehicle-control surfaces e regressões 1.5.1 preservadas; dependências/integrations atuais do source público (LambDynamicLights, JEI, GeckoLib, Sable Companion) e HUD/Stressometer P1.13 adicionados. Changelog exato da build 1.13.1 não localizado; nenhuma mudança não comprovada foi inventada.
