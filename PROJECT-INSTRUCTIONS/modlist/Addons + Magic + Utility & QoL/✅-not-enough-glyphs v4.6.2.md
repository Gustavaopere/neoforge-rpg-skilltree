...
Na inspeção do host 4.6.1 existia `META-INF/jarjar/sauce-1.21.1-0.0.42.89.jar`. A release 4.6.2 declara update para Sauce `0.0.50+`; o JarJar interno exato do novo host não foi aberto nesta passagem, portanto `0.0.42.89` permanece apenas evidência histórica da 4.6.1.
Pelo protocolo:
...
## 16. Matriz de testes
- [ ] Dedicated server + cliente iniciam com Ars Nouveau 5.13.1 + NEG 4.6.2.
...
- Modlist física atual: `not_enough_glyphs-1.21.1-4.6.2.jar`, mod id/version e `not_enough_glyphs.mixins.json`; Sauce exato do novo JarJar não foi re-inspecionado.
- CurseForge oficial: project 1023517, release 4.6.2 NeoForge 1.21.1 de 14/09/2026; changelog com hotfix filter/runes, Propagate Underfoot em vehicle e Sauce 0.0.50+.
...
## 18. Atualização instalada — 4.6.2
A release oficial `not_enough_glyphs-1.21.1-4.6.2.jar`, publicada em 14/09/2026, registra três deltas materiais:
- hotfix para o filter que, após atualização de API, podia bloquear sempre o resolve em runes;
- **Propagate Underfoot** passa a poder atingir o vehicle da entidade na qual está resolvendo;
- Sauce atualizado para `0.0.50+`.
Regression gates adicionais:
- [ ] filtros em runes resolvem sem bloqueio indevido;
- [ ] Propagate Underfoot em entidade montada resolve no alvo/vehicle esperado sem duplicação;
- [ ] server/client iniciam com Ars Nouveau atual e a nova linha Sauce sem linkage error.
A versão exata do JarJar Sauce dentro do arquivo físico 4.6.2 não foi re-inspecionada; não se promove automaticamente a metadata interna da 4.6.1.
