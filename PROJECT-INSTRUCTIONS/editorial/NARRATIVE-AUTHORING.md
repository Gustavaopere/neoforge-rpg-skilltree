...
This is the repository-local working contract for AI-assisted campaign authoring. It is project-specific guidance, not a generic Minecraft Mod Factory skill.
...
- **Reusable narrative authoring tooling:** `Gustavaopere/minecraft-mod-factory/narrative/`, consumed through `historia/narrative-authoring-profile.json` and the repository consumer CI.
...
3. use the Factory narrative inventory with `historia/narrative-authoring-profile.json` to obtain a current editorial index before creating new stable entities;
4. treat inventory as spoiler-bearing/editorial output: do not expose it in player-facing or public logs when it contains undiscovered material;
...
Use the ID families declared in `historia/narrative-authoring-profile.json` (`HIST`, `ARC`, `NPC`, `QST`, `FAC`, `SET`, `LOC`, `EVT`, `EVD`, `END`, `DLG`). Reference entities by ID, not only by display name.
...
Validation is provided by `Gustavaopere/minecraft-mod-factory/narrative/tooling/`. The repository workflow `.github/workflows/narrative-factory-consumer.yml` pins a reviewed Factory revision and runs the validators without duplicating those scripts locally.

For deliberate local editorial debugging, with a compatible Factory checkout available as `<factory>`, use:

- `python <factory>/narrative/tooling/validate_story.py --profile historia/narrative-authoring-profile.json --root historia`;
- add `--strict-references` when validating a closed batch;
- `python <factory>/narrative/tooling/validate_dialogues.py --profile historia/narrative-authoring-profile.json --root historia/12-dialogos` for dialogue batches;
- `python <factory>/narrative/tooling/story_inventory.py --profile historia/narrative-authoring-profile.json --root historia --format json` for editorial inventory.

Validators are spoiler-safe by default. Use `--reveal` only for deliberate editorial debugging. Do not paste reveal-mode or inventory output into player-facing/public status reports unless explicitly requested.
...
Generic dialogue scaffolds belong to the Factory; concrete dialogue content and voice decisions for this campaign remain in `historia/`.
...
Use the existing `END-####` family and `historia/10-finais-e-epilogos/composicao-de-epilogos.md`. Generic ending scaffolds belong to the Factory.

Epilogue fragments must derive from persistent/derivable Narrative Core state rather than a second parallel ending-state system. Conflict/suppression between fragments must be explicit and ordering deterministic. Do not hardcode a universal morality score or require a single final A/B choice. Player knowledge and objective world state remain distinct when selecting or wording fragments.
...
Prefer validator default output. Reveal-mode validator output and inventory output are editorial surfaces and may contain spoiler-bearing identifiers/titles/paths.
