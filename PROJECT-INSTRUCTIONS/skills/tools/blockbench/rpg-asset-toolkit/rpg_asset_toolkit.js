(function (root, factory) {
  ...
  function issue(severity, code, message, context) { return { severity, code, message, context: context || null }; }
  function finiteVector3(value) { return Array.isArray(value) && value.length >= 3 && value.slice(0, 3).every(Number.isFinite); }
  function isPowerOfTwo(value) { return Number.isInteger(value) && value > 0 && (value & (value - 1)) === 0; }
  function normalizedName(value) { return typeof value === 'string' ? value.trim() : ''; }
      ...
      onload() {
        ...
        profileAction = new bb.Action('rpg_asset_toolkit_validate_profile', { name: 'Validate RPG Asset Against Contract Profile', description: 'Run the same checks plus optional required bones/animations/maxSpan from JSON.', icon: 'rule', click() {
          bb.Blockbench.textPrompt('RPG Asset Contract Profile (JSON)', '{}', (text) => {
            try { show(validateProject(bb.Blockbench.Project, parseProfileJson(text))); }
            catch (error) { bb.Blockbench.showMessageBox({ title: 'RPG Asset Toolkit — Invalid Profile', icon: 'error', message: String(error && error.message ? error.message : error), buttons: ['OK'] }); }
          });
        } });
        bb.MenuBar.menus.tools.addAction(auditAction);
        bb.MenuBar.menus.tools.addAction(profileAction);
      },
      onunload() { if (auditAction) auditAction.delete(); if (profileAction) profileAction.delete(); auditAction = null; profileAction = null; }
    });
  }
