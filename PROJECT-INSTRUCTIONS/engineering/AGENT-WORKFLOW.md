...
The root [`AGENTS.md`](../../AGENTS.md) is intentionally retained only as the repository discovery/bootstrap entrypoint. This file is the authoritative body of the working contract.
...
Before specialized Minecraft work, read [`../skills/ROUTER.md`](../skills/ROUTER.md) and apply [`../skills/VERSION-AUTHORITY.md`](../skills/VERSION-AUTHORITY.md). The repository library contains audited adaptations of the Minecraft skills supplied by the user; it is guidance, not a substitute for exact-version code/JAR evidence.

When a task requires the user to perform a manual action, also apply [`../skills/USER-GUIDED-WORKFLOW.md`](../skills/USER-GUIDED-WORKFLOW.md): give one atomic, verifiable manual step at a time and wait for the evidence needed before issuing the next manual action. Do not fragment work that the agent can execute directly.
...
See [`../../docs/MASTER_PLAN.md`](../../docs/MASTER_PLAN.md) for the exact phased order.