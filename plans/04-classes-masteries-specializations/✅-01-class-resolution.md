...
**Estado atual:** CONCLUÍDO — a autoridade live de classes baseada em `ProgressionState + ClassRuleCatalog` reconcilia deterministicamente após mutações e reload. A fronteira emergente `InvestmentState`/`ArchetypeResolver` projeta nodes comprados e milestones de Mastery somente a partir de metadata canônica explícita, sem inferência por ID, namespace, provider, posição ou nome.
...
- [x] Consumir somente snapshot canônico de perks/masteries.
  ...
  - [x] Cada rank comprado vale `1` ponto para cada tag `rpgskilltree:domain/<ProgressionDomain>` declarada; `rpgskilltree:domain/core` é neutra.
  ...
  - [x] Contribuições de Mastery vêm exclusivamente de recursos `data/*/mastery_investments/*.json`, com `lane`, `minimum_experience`, `domain_weights` e `tags` explícitos.
  - [x] `MasteryInvestmentMetadataParser` aceita apenas lanes reconhecidas por `MasteryLaneCatalog`, thresholds/pesos inteiros positivos exatos e rejeita duplicata `(lane, threshold)`, colisão de domínio após normalização e contribuição explicitamente vazia.
  - [x] `MasteryInvestmentMetadataCatalog` é o snapshot runtime autoritativo dessa metadata; ausência de declaração equivale a zero contribuição, sem fallback inferido.
  - [x] A única API pública canônica é `ClassResolutionRuntime.resolveCanonical(ProgressionState)`, que consome o catálogo publicado; chamadores de produção não podem fornecer pesos de Mastery ad hoc.
  - [x] Thresholds explícitos distintos para a mesma lane podem coexistir e os thresholds alcançados acumulam suas contribuições determinística e incrementalmente.
  - Os **valores concretos** de curvas, caps, thresholds e pesos de gameplay continuam pertencendo ao Stage 04.03. Este subplano define a autoridade e o mecanismo; não cria defaults de balanceamento.
...
- [x] Recalcular após compra, respec, mastery threshold e reload.
  ...
  - `MasteryInvestmentMetadataReloader` é auto-registrado no GAME bus e publica o snapshot somente após parse/validação completos.
...
- PR #365 adicionou `CanonicalInvestmentProjection`, fail-closed por node sem metadata, metadata revisionada e validação dos recursos canônicos.
- TDD RED de Mastery na PR #417: run `33994117681` falhou com os símbolos de catálogo/reloader/runtime ainda ausentes.
- TDD RED de hardening na PR #417: run `33996656425` executou 988 JUnit e falhou exatamente em quatro invariantes ainda ausentes: metadata no-op, colisão de domínio normalizado, números fracionários e overload público ad hoc.
- GREEN do código da PR #417: `RPG Skill Tree CI` `33996882580` passou Core, geração/wiki, 988+ JUnit, NeoForge JUnit adapters, NeoForge GameTests, Battle Mage provider-present GameTests, Compendium, validadores, build, verificação do JAR e dedicated-server smoke.

**Acceptance: satisfied.** A resolução de classes é determinística, catalog-backed e consome somente estado/metadata canônicos. A semântica estrutural de contribuição de Mastery está fechada sem heurísticas nem defaults inventados; balanceamento concreto e cobertura completa das fontes de Mastery permanecem corretamente no Stage 04.03.
