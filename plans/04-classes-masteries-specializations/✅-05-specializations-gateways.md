...
- A policy atual reconhece como adapters completos para esta camada `rpgskilltree`, `irons_spellbooks`, `ars_nouveau` e `epicfight`. `create`, `ae2`, `oritech` e providers desconhecidos permanecem fail-closed até uma integração runtime equivalente existir; isso impede que os gateways tecnológicos prometam mecânica ainda não implementada.
...
- Perda do provider ou regressão da capacidade do adapter remove somente a especialização derivada; o nó aprendido e a Mastery conquistada são preservados. O respec já existente remove o nó/reconcilia e, consequentemente, revoga o grant derivado sem criar uma segunda persistência.
...
- `SpecializationGatewayAvailabilityJUnitTest` cobre os 25 IDs/provides canônicos, distinção definição/provider/adapter, fail-closed do resolver, unlock/revogação de grant por disponibilidade e a policy explícita da build atual.
...
**Acceptance:** satisfied para o contrato de gateways/provider desta etapa. Gateways não anunciam integração mecânica de provider ausente ou sem adapter completo, e a disponibilidade é derivada da build/runtime atual em vez de ser persistida como verdade paralela.
