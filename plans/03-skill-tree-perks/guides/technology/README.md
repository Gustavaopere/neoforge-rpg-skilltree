# GUIA COMPLETO — Mods de Tecnologia | NeoForge 1.21.1

Create e addons, energia, indústria, logística, automação, transporte, computação e integrações tecnológicas.

**Fonte canônica no Notion:** https://app.notion.com/p/3c569db9f0db81a69e3ee1232ee636ff

**Referência de presença/JAR/versão usada por este snapshot:** `modlist 28.08.26.txt`.

## Como este guia está organizado

Cada arquivo abaixo contém uma **seção lógica completa**. Não existe continuação de parágrafo ou de capítulo em outro arquivo. Os arquivos podem ser lidos de forma independente; a ordem do índice apenas reproduz a ordem do guia canônico.

## Índice

- [Visão geral e escopo](00-visao-geral.md)
- [1. Sistemas tecnológicos principais](01-sistemas-tecnologicos-principais.md)
- [2. Applied Energistics 2 e automação digital](02-applied-energistics-2-e-automacao-digital.md)
- [3. Indústria, energia e processamento do Create](03-industria-energia-e-processamento-do-create.md)
- [4. Logística e fábricas conectadas](04-logistica-e-fabricas-conectadas.md)
- [5. Física, veículos e Create Aeronautics](05-fisica-veiculos-e-create-aeronautics.md)
- [6. Sable e infraestrutura de sublevels](06-sable-e-infraestrutura-de-sublevels.md)
- [6.1. Extensões físicas de ragdoll do Sable — atualização 28/08](07-1-extensoes-fisicas-de-ragdoll-do-sable-atualizacao-28-08.md)
- [7. Espaço e tecnologia de alto nível](08-espaco-e-tecnologia-de-alto-nivel.md)
- [8. Armas, defesa e engenharia militar do Create](09-armas-defesa-e-engenharia-militar-do-create.md)
- [9. Transporte ferroviário, marítimo e pessoal](10-transporte-ferroviario-maritimo-e-pessoal.md)
- [10. Armazenamento e logística geral](11-armazenamento-e-logistica-geral.md)
- [11. Pontes entre magia e tecnologia](12-pontes-entre-magia-e-tecnologia.md)
- [12. Addons Create de equipamentos, utilidades e construção](13-addons-create-de-equipamentos-utilidades-e-construcao.md)
- [13. Ferramentas de controle e interface Create](14-ferramentas-de-controle-e-interface-create.md)
- [14. Compatibilidades e infraestrutura técnica](15-compatibilidades-e-infraestrutura-tecnica.md)
- [15. Bibliotecas tecnológicas](16-bibliotecas-tecnologicas.md)
- [16. Mapa rápido dos ecossistemas](17-mapa-rapido-dos-ecossistemas.md)
- [17. Fontes principais de referência](18-fontes-principais-de-referencia.md)

## Regras de manutenção

- Nunca dividir um capítulo por quantidade de caracteres.
- Nunca deixar um `#`/`##` no meio de um parágrafo.
- Alterações futuras devem preservar uma seção inteira no mesmo arquivo.
- O Notion continua sendo a fonte canônica; este diretório é o snapshot auditável usado pelo repositório.

[← Voltar aos três guias](../README.md)
