...
- Quando a modlist contiver dois JARs com o mesmo mod ID, tratar como **bloqueio de startup** e registrar uma decisão explícita de qual implementação permanecerá. Para o snapshot de 2026-09-06, a decisão canônica é manter **Alex's Caves Continued 1.0.9** em `alexscaves` e **Alex's Mobs Continued 2.1.9** em `alexsmobs`; os JARs concorrentes permanecem fisicamente pendentes de remoção até uma nova modlist confirmar o estado.
