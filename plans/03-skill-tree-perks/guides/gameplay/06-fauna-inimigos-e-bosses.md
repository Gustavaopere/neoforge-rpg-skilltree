...
A modlist atual contém simultaneamente `alexsmobs-1.22.9.jar` (**Alex's Mobs**) e `alexsmobs-2.1.9-neoforge+1.21.1.jar` (**Alex's Mobs Continued**), ambos declarando `modid alexsmobs`. O segundo permanece descrito neste capítulo; o primeiro foi incorporado no [capítulo 16](16-novos-modulos-gameplay-e-sistemas.md). Para integrações, a presença do mod ID não distingue a implementação: resolver registry object/JAR concreto ou operar fail-closed.
