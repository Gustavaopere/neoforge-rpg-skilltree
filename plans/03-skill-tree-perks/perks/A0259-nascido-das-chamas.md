...
- **Fonte canônica:** [registro A0259 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81de81f0d4ccbfd510a1).
...
- **Dependências externas à faixa:** <code>A0161</code>, <code>A0162</code>. Elas permanecem sinalizadas e não são presumidas como concluídas.
...
| Hook | No gatilho elegível, deduplicar identidade causal e criar uma única janela expiry_tick=now+100; cooldown começa no término e expira 1200 ticks depois. Gatilho térmico exige exatamente ADVERSE_HOT_SEVERE do P-0054 e fica fail-closed sem adapter. No DamageMitigationResolver, aplicar ×0,10 somente a FIRE ou HEAT_DAMAGE reconhecido de outcomes posteriores; Cold Sweat HOT DamageSource pode alimentar HEAT_DAMAGE. No pipeline P-0057, aplicar ×0,50 somente a novos parcels térmicos positivos após modificadores específicos da fonte, sem escrever BODY/CORE/WORLD/thresholds/resistance/dampening. |
| Fallback | FAIL-CLOSED se Specialist Fire não estiver desbloqueada. Sem ADVERSE_HOT_SEVERE, omitir somente o gatilho térmico. Sem THERMAL_PARCEL_PIPELINE_V1, omitir somente ×0,50 sobre novos parcels. A janela ainda pode ser armada por outcome FIRE/HEAT_DAMAGE explícito quando o classificador/mitigação forem seguros. Nunca conceder imunidade universal nem escrever BODY/CORE/WORLD/threshold diretamente. |
| Regra | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Keystone INTERNA da Specialist Fire, não terminal exterior. FIRE, HEAT_DAMAGE, ADVERSE_HOT_SEVERE e novo deslocamento térmico são eixos distintos. A janela dura 100 ticks; a recarga de 1200 ticks inicia no fim da janela, logo novo armamento só após activation+1300. ×0,10 age somente em FIRE/HEAT reconhecidos posteriores; ×0,50 age somente em novo parcel térmico positivo via P-0057. Temperatura/penalidades já acumuladas não são removidas. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Fora da recarga, A0259 arma RPG_BORN_OF_FLAME por 100 ticks (5 s) quando ocorrer UM destes gatilhos server-side: (a) após o commit de um hostile_direct_damage_outcome positivo classificado FIRE ou HEAT_DAMAGE; ou (b) na transição real do estado corporal canônico para um estágio adverso de calor severo reconhecido pelo adapter da versão. O outcome que arma por dano não recebe a mitigação retroativamente. Durante a janela, componentes FIRE e HEAT_DAMAGE hostis reconhecidos recebem ×0,10, cada um mantendo seu bucket; novos parcels térmicos positivos que deslocariam o corpo para quente recebem ×0,50 antes de entrar no estado corporal. Temperatura/penalidades já acumuladas não são removidas.
...
SPECIALIST_UNLOCK:FIRE válido + requisitos locais + cooldown livre + (outcome hostil direto FIRE/HEAT_DAMAGE positivo OU transição real para ADVERSE_HOT_SEVERE fornecida por BODY_HEAT_STATE_V1). ADVERSE_HOT_SEVERE não pode ser substituído por BODY≥100, HOT DamageSource, WORLD>BURNING_POINT, FIRE damage ou comparação inventada. Enquanto P-0054 não comprovar essa semântica, somente o gatilho por dano explícito permanece elegível. Auto-dano, custo, fake player, callback duplicado e permanência contínua em calor não rearmam.

### Hook e ordem de execução

No gatilho elegível, deduplicar identidade causal e criar uma única janela expiry_tick=now+100; cooldown começa no término e expira 1200 ticks depois. Gatilho térmico exige exatamente ADVERSE_HOT_SEVERE do P-0054 e fica fail-closed sem adapter. No DamageMitigationResolver, aplicar ×0,10 somente a FIRE ou HEAT_DAMAGE reconhecido de outcomes posteriores; Cold Sweat HOT DamageSource pode alimentar HEAT_DAMAGE. No pipeline P-0057, aplicar ×0,50 somente a novos parcels térmicos positivos após modificadores específicos da fonte, sem escrever BODY/CORE/WORLD/thresholds/resistance/dampening.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Fire não estiver desbloqueada. Sem ADVERSE_HOT_SEVERE, omitir somente o gatilho térmico. Sem THERMAL_PARCEL_PIPELINE_V1, omitir somente ×0,50 sobre novos parcels. A janela ainda pode ser armada por outcome FIRE/HEAT_DAMAGE explícito quando o classificador/mitigação forem seguros. Nunca conceder imunidade universal nem escrever BODY/CORE/WORLD/threshold diretamente.
...
| Pré-requisito visual/estrutural | Specialist Fire desbloqueada (SPECIALIST_UNLOCK:FIRE) + A0250 ≥3 + A0256 ≥1 + A0161 Afinidade de Fogo + Fire Mastery ≥120. |
| Dependência semântica completa | SPECIALIST_UNLOCK:FIRE confirmado por Gate A/B/C + A0250 Resistência ao Calor ≥3 + A0256 Armadura Incandescente ≥1 + Fire Mastery ≥120 + A0161 Afinidade de Fogo. Estes são requisitos internos adicionais; não substituem ≥100 PP FIRE/fundamentos/terminal A0162. |
| Custo topológico | 3 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Keystone INTERNA da Specialist Fire, não terminal exterior. FIRE, HEAT_DAMAGE, ADVERSE_HOT_SEVERE e novo deslocamento térmico são eixos distintos. A janela dura 100 ticks; a recarga de 1200 ticks inicia no fim da janela, logo novo armamento só após activation+1300. ×0,10 age somente em FIRE/HEAT reconhecidos posteriores; ×0,50 age somente em novo parcel térmico positivo via P-0057. Temperatura/penalidades já acumuladas não são removidas. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0259 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0259.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Fire não estiver desbloqueada. Sem ADVERSE_HOT_SEVERE, omitir somente o gatilho térmico. Sem THERMAL_PARCEL_PIPELINE_V1, omitir somente ×0,50 sobre novos parcels. A janela ainda pode ser armada por outcome FIRE/HEAT_DAMAGE explícito quando o classificador/mitigação forem seguros. Nunca conceder imunidade universal nem escrever BODY/CORE/WORLD/threshold diretamente.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Janela defensiva: 100 ticks. FIRE hostil: ×0,10. HEAT_DAMAGE hostil: ×0,10. Novo deslocamento térmico positivo para quente: ×0,50. Recarga completa: 1200 ticks (60 s) iniciada APÓS o fim da janela, portanto nova ativação só é possível após activation_tick + 1300. Sem stacks ou extensão por novos gatilhos durante a janela/recarga..
...
- Página: [A0259 — Nascido das Chamas](https://app.notion.com/3c569db9f0db81de81f0d4ccbfd510a1)
...
**Resultado final do Chat 1 para A0259:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
