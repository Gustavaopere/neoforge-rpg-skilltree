...
- **Fonte canônica:** [registro A0208 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db8157bdf6d1730b8dd6a7).
...
|---|---|
| Código | A0208 |
| Nome | Resistência a Ender II |
| Domínio | VITALITY |
| Árvore | Principal — VITALITY ↔ AGILITY |
...
| Dependências Obrigatórias | A0207 Resistência a Ender I ≥ 3 ranks + realizar deslocamento dimensional próprio válido. Teleporte forçado por inimigo não arma o benefício. |
| Pré-requisitos | A0207 Resistência a Ender I ≥ 3 ranks. |
| Provider/Mods | RPG Skill Tree + serviço causal de deslocamento próprio + DamageMitigationResolver + adapter hostil ENDER de Fire's Ender Expansion 2.4.1/Somake 1.0.8 quando comprovado. NeoForge EntityTeleportEvent isolado é insuficiente; AGILITY não cria causalidade por si só. |
| Efeito | Após deslocamento dimensional próprio válido, A0208 arma RPG_ENDER_VEIL por 100 ticks (5 s). O próximo componente ENDER de um hostile_direct_damage_outcome recebido durante a janela é multiplicado por ×0,75 e consome o Véu atomicamente no mesmo evento. |
| Escalonamento | 1 rank. Janela RPG_ENDER_VEIL: 100 ticks. Próximo componente ENDER hostil direto elegível: ×0,75. O estado não acumula; novo deslocamento válido apenas renova a expiração para now+100 sem empilhar magnitude. |
| Gate | A0207 ≥3 legitimamente disponível + receipt causal de deslocamento dimensional próprio + hostile_direct_ender_component classificado. EntityTeleportEvent genérico, diferença de posição ou troca de dimensão não provam a ação. Sem os dois producers, A0208 é UNAVAILABLE_NODE/não comprável. |
| Hook | Após commit do displacement receipt próprio, criar/renovar um único RPG_ENDER_VEIL por 100 ticks. No próximo hostile_direct_ender_outcome, reservar ×0,75 no componente ENDER+consumo; commitar somente se o evento continuar elegível e não cancelado, rollback preserva o Véu. Deduplicar outcome_id e limpar em morte, logout, dimensão, rank/dependency loss, respec e rules reload. |
| Fallback | UNAVAILABLE_NODE enquanto A0207 estiver indisponível ou não existirem receipt causal de deslocamento próprio e classifier hostil ENDER. Não ativar por posição, dimensão, portal visual, comando, respawn, teleporte forçado, End/Void ou ausência de dano. |
| Regra | PP_REGION: VITALITY_AGILITY_ENDER_BRIDGE/DEFENSE. Availability transitiva A0207→A0208. RPG_ENDER_VEIL é mitigador transitório distinto do bucket base, nasce somente de receipt causal e usa reservation→commit/rollback. Teleporte forçado/comando/respawn não armam. Não terminal. |
...
Após deslocamento dimensional próprio válido, A0208 arma RPG_ENDER_VEIL por 100 ticks (5 s). O próximo componente ENDER de um hostile_direct_damage_outcome recebido durante a janela é multiplicado por ×0,75 e consome o Véu atomicamente no mesmo evento.

### Escalonamento aprovado

1 rank. Janela RPG_ENDER_VEIL: 100 ticks. Próximo componente ENDER hostil direto elegível: ×0,75. O estado não acumula; novo deslocamento válido apenas renova a expiração para now+100 sem empilhar magnitude.

### Gate de compra/ativação

A0207 ≥3 legitimamente disponível + receipt causal de deslocamento dimensional próprio + hostile_direct_ender_component classificado. EntityTeleportEvent genérico, diferença de posição ou troca de dimensão não provam a ação. Sem os dois producers, A0208 é UNAVAILABLE_NODE/não comprável.
...
UNAVAILABLE_NODE enquanto A0207 estiver indisponível ou não existirem receipt causal de deslocamento próprio e classifier hostil ENDER. Não ativar por posição, dimensão, portal visual, comando, respawn, teleporte forçado, End/Void ou ausência de dano.
...
| Item | Decisão |
|---|---|
| Região | Principal — VITALITY ↔ AGILITY / Ender — Defesa Dimensional |
| Camada e papel | Camada 5; Notable |
| Pré-requisito visual/estrutural | A0207 Resistência a Ender I ≥ 3 ranks. |
| Dependência semântica completa | A0207 Resistência a Ender I ≥ 3 ranks + realizar deslocamento dimensional próprio válido. Teleporte forçado por inimigo não arma o benefício. |
| Custo topológico | 1 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: VITALITY_AGILITY_ENDER_BRIDGE/DEFENSE. Availability transitiva A0207→A0208. RPG_ENDER_VEIL é mitigador transitório distinto do bucket base, nasce somente de receipt causal e usa reservation→commit/rollback. Teleporte forçado/comando/respawn não armam. Não terminal. |
...
RPG Skill Tree + serviço causal de deslocamento próprio + DamageMitigationResolver + adapter hostil ENDER de Fire's Ender Expansion 2.4.1/Somake 1.0.8 quando comprovado. NeoForge EntityTeleportEvent isolado é insuficiente; AGILITY não cria causalidade por si só.
...
| Projeto | Head auditado | Decisão para A0208 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0208.
...
- **Fallback normativo:** UNAVAILABLE_NODE enquanto A0207 estiver indisponível ou não existirem receipt causal de deslocamento próprio e classifier hostil ENDER. Não ativar por posição, dimensão, portal visual, comando, respawn, teleporte forçado, End/Void ou ausência de dano.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Janela RPG_ENDER_VEIL: 100 ticks. Próximo componente ENDER hostil direto elegível: ×0,75. O estado não acumula; novo deslocamento válido apenas renova a expiração para now+100 sem empilhar magnitude..
...
- Página: [A0208 — Resistência a Ender II](https://app.notion.com/3c569db9f0db8157bdf6d1730b8dd6a7)
...
**Resultado final do Chat 1 para A0208:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
