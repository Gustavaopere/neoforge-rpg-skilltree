...
- **Fonte canônica:** [registro A0254 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81269d13c20c9ee2d90f).
...
| Hook | Manter por jogador os cinco últimos créditos FIRE diretos elegíveis com timestamp e valor canônico FIRE pré-mitigação-do-alvo/pré-crítico. Ao alcançar cinco em 80 ticks, calcular a média snapshot, limpar preparação, criar RPG_EMBER_AURA e cooldown_expiry=now+240. A cada 20 ticks da aura, consultar inimigos no raio e emitir um único outcome derivado por alvo/pulso, deduplicado por aura_instance_id + pulse_index + target_uuid. |
| Fallback | FAIL-CLOSED se Specialist Fire não estiver desbloqueada ou se consulta espacial/pipeline de derived_fire_aura_pulse não forem seguros. Não converter a perk em bônus pessoal de dano, resistência, Ignição ou explosão para contornar a ausência do hook. |
| Regra | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. A snapshot usa exatamente a média das cinco parcelas FIRE diretas preparatórias na base canônica pré-mitigação-do-alvo/pré-crítico. Pulsos são outcomes derivados: sem crítico próprio, sem Ignição, Fire Mastery, Ímpeto, Mana Incandescente, Propagação, sustain ou proc de ação direta; um dano máximo por alvo/pulso. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Ao registrar 5 ações FIRE diretas distintas do próprio jogador dentro de uma janela móvel de 80 ticks (4 s), com a recarga livre, A0254 limpa o contador e arma RPG_EMBER_AURA por 80 / 120 ticks (4 / 6 s). Durante a aura, a cada 20 ticks é emitido um derived_fire_aura_pulse em raio de 4 blocos contra inimigos vivos elegíveis. O dano-base de cada pulso por alvo é 8% / 12% da média dos cinco componentes FIRE diretos que armaram a aura, medidos na base canônica pré-mitigação-do-alvo e pré-crítico. A snapshot é congelada na ativação. Pulsos passam apenas pela mitigação do alvo aplicável e não rerodam multiplicadores ofensivos do caster.
...
SPECIALIST_UNLOCK:FIRE válido + requisitos locais + cinco ações FIRE diretas distintas do próprio jogador em ≤80 ticks, deduplicadas por action_id, + cooldown livre + consulta espacial/pipeline derivado seguros. DoT, tick de Ignição, aura, propagação, derived components, summons, automação, fake player e callbacks repetidos não avançam preparação.

### Hook e ordem de execução

Manter por jogador os cinco últimos créditos FIRE diretos elegíveis com timestamp e valor canônico FIRE pré-mitigação-do-alvo/pré-crítico. Ao alcançar cinco em 80 ticks, calcular a média snapshot, limpar preparação, criar RPG_EMBER_AURA e cooldown_expiry=now+240. A cada 20 ticks da aura, consultar inimigos no raio e emitir um único outcome derivado por alvo/pulso, deduplicado por aura_instance_id + pulse_index + target_uuid.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Fire não estiver desbloqueada ou se consulta espacial/pipeline de derived_fire_aura_pulse não forem seguros. Não converter a perk em bônus pessoal de dano, resistência, Ignição ou explosão para contornar a ausência do hook.
...
| Pré-requisito visual/estrutural | Specialist Fire desbloqueada (SPECIALIST_UNLOCK:FIRE) + A0246 Calor Crescente ≥2 + A0252 Mana Incandescente ≥1. |
| Dependência semântica completa | SPECIALIST_UNLOCK:FIRE confirmado por Gate A/B/C + A0246 Calor Crescente ≥2 + A0252 Mana Incandescente ≥1. Os requisitos locais não substituem o unlock global da Specialist Fire. |
| Custo topológico | 2 PP por rank; 2 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. A snapshot usa exatamente a média das cinco parcelas FIRE diretas preparatórias na base canônica pré-mitigação-do-alvo/pré-crítico. Pulsos são outcomes derivados: sem crítico próprio, sem Ignição, Fire Mastery, Ímpeto, Mana Incandescente, Propagação, sustain ou proc de ação direta; um dano máximo por alvo/pulso. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0254 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>action_id</code>, <code>target_uuid</code>, <code>ledger</code>.
...
- **Chaves que o adapter precisa preservar:** <code>action_id</code>, <code>target_uuid</code>, <code>ledger</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0254.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Fire não estiver desbloqueada ou se consulta espacial/pipeline de derived_fire_aura_pulse não forem seguros. Não converter a perk em bônus pessoal de dano, resistência, Ignição ou explosão para contornar a ausência do hook.
...
6. **Causalidade:** correlacionar <code>action_id</code>, <code>target_uuid</code>, <code>ledger</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>action_id</code>, <code>target_uuid</code>, <code>ledger</code>. |
...
- Página: [A0254 — Aura de Brasas](https://app.notion.com/3c569db9f0db81269d13c20c9ee2d90f)
...
**Resultado final do Chat 1 para A0254:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
