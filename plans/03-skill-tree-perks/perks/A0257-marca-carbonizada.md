...
- **Fonte canônica:** [registro A0257 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db8149a14dc4a0323d6729).
...
| Hook | Manter estado por player_uuid + target_uuid; inserir no máximo uma marca por action_id/outcome_id com expiry_tick=now+160. Ao quinto crédito, remover marcas e armar ready com source_action_id e expiry=now+120. No próximo direct_fire_outcome distinto no mesmo alvo, calcular 18%/28% da parcela FIRE canônica pré-mitigação/pré-crítico, emitir burst derivado após commit e consumir ready. Dedupe pelo outcome consumidor. |
| Fallback | FAIL-CLOSED se Specialist Fire não estiver desbloqueada, se identidade jogador/alvo/ação não for estável ou se pipeline derivado seguro não existir. Não substituir por contador global, último alvo, explosão genérica, bônus pessoal ou coeficiente especial para bosses. |
| Regra | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Cada marca possui TTL independente de 160 ticks; a quinta limpa as marcas e arma ready por 120 ticks, que só uma ação FIRE direta distinta no mesmo alvo pode consumir. O burst derivado não possui crítico próprio, não chama Ignição, não cria Marca, Fire Mastery, Ímpeto, sustain, Aura, Propagação ou cadeia. Bosses usam a mesma fórmula. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Cada ação FIRE direta elegível do jogador contra um alvo vivo adiciona no máximo 1 RPG_CHARRED_MARK ao par jogador/alvo, até 5, deduplicada por action_id/outcome_id. Cada marca possui TTL independente de 160 ticks (8 s). Ao alcançar 5 marcas, limpar as cinco marcas e armar RPG_CHARRED_BURST_READY por 120 ticks (6 s). A próxima ação FIRE direta distinta do mesmo jogador contra o mesmo alvo durante essa janela consome o estado e emite um derived_fire_burst_outcome no próprio alvo igual a 18% / 28% do componente FIRE direto da ação consumidora medido pré-mitigação-do-alvo e pré-crítico.
...
SPECIALIST_UNLOCK:FIRE válido + requisitos locais + ação FIRE direta própria contra alvo vivo, deduplicada por action_id/outcome_id. DoT, tick de Ignição, Aura, Propagação, retaliação, derived components, summons, automação, fake player e callback duplicado não adicionam marcas nem consomem ready. A ação que cria a 5ª marca não pode consumi-la.

### Hook e ordem de execução

Manter estado por player_uuid + target_uuid; inserir no máximo uma marca por action_id/outcome_id com expiry_tick=now+160. Ao quinto crédito, remover marcas e armar ready com source_action_id e expiry=now+120. No próximo direct_fire_outcome distinto no mesmo alvo, calcular 18%/28% da parcela FIRE canônica pré-mitigação/pré-crítico, emitir burst derivado após commit e consumir ready. Dedupe pelo outcome consumidor.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Fire não estiver desbloqueada, se identidade jogador/alvo/ação não for estável ou se pipeline derivado seguro não existir. Não substituir por contador global, último alvo, explosão genérica, bônus pessoal ou coeficiente especial para bosses.
...
| Pré-requisito visual/estrutural | Specialist Fire desbloqueada (SPECIALIST_UNLOCK:FIRE) + A0244 Combustão ≥2 + A0248 Fogo em Alvo Ferido ≥1. |
| Dependência semântica completa | SPECIALIST_UNLOCK:FIRE confirmado por Gate A/B/C + A0244 Combustão ≥2 + A0248 Fogo em Alvo Ferido ≥1. Os requisitos locais não substituem o unlock global da Specialist Fire. |
| Custo topológico | 2 PP por rank; 2 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Cada marca possui TTL independente de 160 ticks; a quinta limpa as marcas e arma ready por 120 ticks, que só uma ação FIRE direta distinta no mesmo alvo pode consumir. O burst derivado não possui crítico próprio, não chama Ignição, não cria Marca, Fire Mastery, Ímpeto, sustain, Aura, Propagação ou cadeia. Bosses usam a mesma fórmula. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0257 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0257.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Fire não estiver desbloqueada, se identidade jogador/alvo/ação não for estável ou se pipeline derivado seguro não existir. Não substituir por contador global, último alvo, explosão genérica, bônus pessoal ou coeficiente especial para bosses.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 2 ranks. Máximo: 5 marcas por jogador/alvo. TTL de cada marca: 160 ticks, sem refresh das demais. Ao 5º crédito: estado pronto por 120 ticks. Próxima ação FIRE direta distinta no mesmo alvo: burst derivado de 18% / 28% da parcela FIRE direta pré-mitigação/pré-crítico da ação consumidora. Chefes usam a mesma fórmula; não existe coeficiente de boss indefinido..
...
- Página: [A0257 — Marca Carbonizada](https://app.notion.com/3c569db9f0db8149a14dc4a0323d6729)
...
**Resultado final do Chat 1 para A0257:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
