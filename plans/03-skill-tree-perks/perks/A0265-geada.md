...
- **Fonte canônica:** [registro A0265 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81c79903e43d2fcaf672).
...
| Hook | Usar CHILL_APPLICATION_RESOLVER_V1 uma única vez por direct_ice_outcome_id: agregar chance nativa integrável + contribuição A0265, limitar chance matemática a 100% e executar no máximo uma decisão server-side. Se o provider já aplicar CHILL deterministicamente naquele outcome, observar/registrar {action_id,outcome_id,owner,chill_state_id,base_duration,amplifier} sem reaplicar. Em sucesso probabilístico, aplicar/renovar exatamente o estado real definido pelo adapter, preservando duração/amplificador nativos; nunca criar Slowness genérica por conta própria. |
| Fallback | FAIL-CLOSED se Specialist Gelo não estiver desbloqueada, não houver chill_state_id real/compatível ou faltar hook seguro de decisão/aplicação na versão. Não criar minecraft:slowness genérica, freeze ticks, temperatura corporal ou segundo debuff como fallback; sem resolver seguro, A0265 fica inativa naquela fonte. |
| Regra | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Perk interna da Specialist Gelo. Geada não cria um debuff paralelo e não empilha amplificador indefinidamente. Há UMA decisão de CHILL por outcome direto; efeitos derivados/DoT/summons não rerrolam. irons_spellbooks:chilled é estado nativo elegível; CHILL não é Frozen/freeze buildup, temperatura corporal nem Resistência a Gelo. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Até 3 ranks. Contribuição A0265: +5 / +10 / +15 p.p. Base própria sem chance nativa: 0%. Chance matemática final limitada a 100%. Duração/amplificador permanecem nativos da aplicação compatível e não escalam com A0265.

### Gate de compra/ativação

SPECIALIST_UNLOCK:ICE válido + direct_ice_outcome_id atribuído ao jogador + adapter capaz de identificar/aplicar um chill_state_id real/compatível. Uma ação recebe no máximo UMA decisão de CHILL. DoT, tick de estado, derived component, summon, automação, fake player, callback duplicado e subeventos da mesma ação não recebem nova decisão.

### Hook e ordem de execução

Usar CHILL_APPLICATION_RESOLVER_V1 uma única vez por direct_ice_outcome_id: agregar chance nativa integrável + contribuição A0265, limitar chance matemática a 100% e executar no máximo uma decisão server-side. Se o provider já aplicar CHILL deterministicamente naquele outcome, observar/registrar {action_id,outcome_id,owner,chill_state_id,base_duration,amplifier} sem reaplicar. Em sucesso probabilístico, aplicar/renovar exatamente o estado real definido pelo adapter, preservando duração/amplificador nativos; nunca criar Slowness genérica por conta própria.
...
| Dependência semântica completa | SPECIALIST_UNLOCK:ICE confirmado server-side por Gate A/B/C: fundamentos ICE + ≥100 Passive Points válidos em SPECIALIST_REGION:ICE + terminal A0169. A0169 isolada não substitui o unlock global da Specialist Gelo. |
...
| Projeto | Head auditado | Decisão para A0265 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0265.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks. Contribuição A0265: +5 / +10 / +15 p.p. Base própria sem chance nativa: 0%. Chance matemática final limitada a 100%. Duração/amplificador permanecem nativos da aplicação compatível e não escalam com A0265..
...
- Página: [A0265 — Geada](https://app.notion.com/3c569db9f0db81c79903e43d2fcaf672)
...
**Resultado final do Chat 1 para A0265:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
