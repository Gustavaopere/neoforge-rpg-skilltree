...
- **Fonte canônica:** [registro A0240 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81a3bf2bd9a66661d6af).
...
| Dependências Obrigatórias | A0238 Adaptação Cruzada + pelo menos 9 pontos somados em resistências de 4 elementos distintos. Nenhuma resistência pode representar mais da metade do investimento. |
| Pré-requisitos | A0238 Adaptação Cruzada + investimento defensivo em pelo menos 4 elementos distintos. |
| Provider/Mods | RPG Skill Tree + FUTURE_PROVIDER_CONTRACTS ELEMENT_SIGNATURE_REGISTRY_V1 e ELEMENTAL_DIVERSITY_LEDGER_V1 + DamageMitigationResolver/Minecraft-NeoForge. O ledger defensivo prova ELEMENT_DEFENSE_PP[element_id] com ownership único por node; shared/general/bridge PP são excluídos por padrão. VERSION-STATUS: designs congelados, contratos não encontrados no runtime/main atual em 2026-08-29. |
| Efeito | A0240 coleta, por jogador, três element_signature_id hostis distintas de hostile_direct_damage_outcome em uma janela móvel de 240 ticks (12 s), uma assinatura por outcome. Ao completar, arma RPG_ADAPTIVE_MATRIX por 160 ticks (8 s). O próximo componente elemental de um dano hostil direto reconhecido, de qualquer assinatura, é multiplicado por ×0,75, consome a Matriz e inicia recarga interna de 360 ticks (18 s) após commit. |
| Escalonamento | 1 rank. Preparação: 3 assinaturas hostis distintas em 240 ticks. Matriz: 160 ticks. Próximo componente elemental hostil direto: ×0,75. Recarga após consumo: 360 ticks. |
| Gate | A0238 + ELEMENTAL_DIVERSITY_LEDGER_V1 comprovando ≥9 PP defensivos válidos em ≥4 element_id distintos + três hostile_direct_damage_outcome com assinaturas primárias canônicas distintas dentro da janela. PP shared/general/bridge, a mesma compra reciclada em mais de um elemento, dano autoinfligido, custos, ambiente sem fonte hostil, repetição, fonte ambígua, DoT derivado e callback duplicado não avançam a preparação. |
| Hook | Rastrear conjunto de assinaturas hostis por jogador, com timestamps e dedupe por outcome_id. Ao três assinaturas, limpar conjunto e armar Matriz por 160 ticks. No próximo outcome elemental hostil direto, aplicar ×0,75 como mitigador transitório no DamageMitigationResolver, consumir e iniciar cooldown now+360 após commit. |
| Fallback | FAIL-CLOSED: sem registry/ledger ou sem rastreamento seguro de assinatura/outcome, A0240 fica inativa. Não substituir por resistência elemental permanente, janela por dano genérico, total bruto de PP ou inferência de elemento. |
| Regra | PP_REGION: ELEMENTAL_SHARED_DEFENSE. DIVERSITY_LEDGER: requisito 9 PP/4 elementos usa apenas ELEMENT_DEFENSE_PP[element_id] element-specific; shared/general/bridge valem zero por padrão e uma compra nunca preenche múltiplas escolas. Preparação exige três assinaturas hostis primárias distintas em outcomes diretos; ao completar, RPG_ADAPTIVE_MATRIX é mitigador transitório semanticamente distinto dos buckets geral/específicos e o próximo componente elemental hostil direto recebe ×0,75 uma única vez. Não altera Afinidade térmica, não cria cap defensivo global oculto e não conta spam do mesmo elemento. Registry/ledger são contratos futuros, não APIs runtime existentes. Notable da Árvore Exterior; não é terminal. |
...
A0240 coleta, por jogador, três element_signature_id hostis distintas de hostile_direct_damage_outcome em uma janela móvel de 240 ticks (12 s), uma assinatura por outcome. Ao completar, arma RPG_ADAPTIVE_MATRIX por 160 ticks (8 s). O próximo componente elemental de um dano hostil direto reconhecido, de qualquer assinatura, é multiplicado por ×0,75, consome a Matriz e inicia recarga interna de 360 ticks (18 s) após commit.

### Escalonamento aprovado

1 rank. Preparação: 3 assinaturas hostis distintas em 240 ticks. Matriz: 160 ticks. Próximo componente elemental hostil direto: ×0,75. Recarga após consumo: 360 ticks.

### Gate de compra/ativação

A0238 + ELEMENTAL_DIVERSITY_LEDGER_V1 comprovando ≥9 PP defensivos válidos em ≥4 element_id distintos + três hostile_direct_damage_outcome com assinaturas primárias canônicas distintas dentro da janela. PP shared/general/bridge, a mesma compra reciclada em mais de um elemento, dano autoinfligido, custos, ambiente sem fonte hostil, repetição, fonte ambígua, DoT derivado e callback duplicado não avançam a preparação.

### Hook e ordem de execução

Rastrear conjunto de assinaturas hostis por jogador, com timestamps e dedupe por outcome_id. Ao três assinaturas, limpar conjunto e armar Matriz por 160 ticks. No próximo outcome elemental hostil direto, aplicar ×0,75 como mitigador transitório no DamageMitigationResolver, consumir e iniciar cooldown now+360 após commit.

### Fallback sem trocar a identidade

FAIL-CLOSED: sem registry/ledger ou sem rastreamento seguro de assinatura/outcome, A0240 fica inativa. Não substituir por resistência elemental permanente, janela por dano genérico, total bruto de PP ou inferência de elemento.
...
| Pré-requisito visual/estrutural | A0238 Adaptação Cruzada + investimento defensivo em pelo menos 4 elementos distintos. |
| Dependência semântica completa | A0238 Adaptação Cruzada + pelo menos 9 pontos somados em resistências de 4 elementos distintos. Nenhuma resistência pode representar mais da metade do investimento. |
| Custo topológico | 3 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: ELEMENTAL_SHARED_DEFENSE. DIVERSITY_LEDGER: requisito 9 PP/4 elementos usa apenas ELEMENT_DEFENSE_PP[element_id] element-specific; shared/general/bridge valem zero por padrão e uma compra nunca preenche múltiplas escolas. Preparação exige três assinaturas hostis primárias distintas em outcomes diretos; ao completar, RPG_ADAPTIVE_MATRIX é mitigador transitório semanticamente distinto dos buckets geral/específicos e o próximo componente elemental hostil direto recebe ×0,75 uma única vez. Não altera Afinidade térmica, não cria cap defensivo global oculto e não conta spam do mesmo elemento. Registry/ledger são contratos futuros, não APIs runtime existentes. Notable da Árvore Exterior; não é terminal. |
...
RPG Skill Tree + FUTURE_PROVIDER_CONTRACTS ELEMENT_SIGNATURE_REGISTRY_V1 e ELEMENTAL_DIVERSITY_LEDGER_V1 + DamageMitigationResolver/Minecraft-NeoForge. O ledger defensivo prova ELEMENT_DEFENSE_PP[element_id] com ownership único por node; shared/general/bridge PP são excluídos por padrão. VERSION-STATUS: designs congelados, contratos não encontrados no runtime/main atual em 2026-08-29.
...
| Projeto | Head auditado | Decisão para A0240 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>outcome_id</code>, <code>signature</code>, <code>element_id</code>, <code>ledger</code>.
...
- **Chaves que o adapter precisa preservar:** <code>outcome_id</code>, <code>signature</code>, <code>element_id</code>, <code>ledger</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0240.
...
- **Fallback normativo:** FAIL-CLOSED: sem registry/ledger ou sem rastreamento seguro de assinatura/outcome, A0240 fica inativa. Não substituir por resistência elemental permanente, janela por dano genérico, total bruto de PP ou inferência de elemento.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Preparação: 3 assinaturas hostis distintas em 240 ticks. Matriz: 160 ticks. Próximo componente elemental hostil direto: ×0,75. Recarga após consumo: 360 ticks..
...
6. **Causalidade:** correlacionar <code>outcome_id</code>, <code>signature</code>, <code>element_id</code>, <code>ledger</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>outcome_id</code>, <code>signature</code>, <code>element_id</code>, <code>ledger</code>. |
...
- Página: [A0240 — Matriz Adaptativa](https://app.notion.com/3c569db9f0db81a3bf2bd9a66661d6af)
...
**Resultado final do Chat 1 para A0240:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
