...
- **Fonte canônica:** [registro A0283 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81f58be1cb7e62f8ec3c).
...
| Ramo | Eletromancia — Condutividade |
| Camada | 1 |
...
| Dependências Obrigatórias | SPECIALIST_UNLOCK:LIGHTNING confirmado server-side por Gate A/B/C: Gate A = fundamentos exteriores ARCANE/POWER e ARCANE/LIGHTNING; Gate B = ≥100 Passive Points válidos em SPECIALIST_REGION:LIGHTNING; Gate C = A0176 Maestria de Raio. A0176 isolada, conexão visual por WATER ou topologia local não autorizam compra. |
| Pré-requisitos | Specialist Raio desbloqueada (SPECIALIST_UNLOCK:LIGHTNING) por Gate A/B/C. |
| Provider/Mods | RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + classificador LIGHTNING + FUTURE_PROVIDER_CONTRACT WET_STATE_V1 + compositor target-specific. OWNER: adapter ambiental/status do RPG; CONSUMER: A0283 e futuras perks de condução; QUERY: {entity_uuid, level/sublevel, tick} -> wet_state_id/source/expiry_or_context; BEHAVIOR: afirmar WET somente por estado/condição explicitamente mapeada. Minecraft/NeoForge pode alimentar o adapter quando a política versionada decidir que contato real com água/chuva gera WET, mas isInWaterOrRain, bioma ou chuva visual NÃO são diretamente o contrato. Iron's Spells 'n Spellbooks 3.16.3 e Ars Nouveau 5.13.1/Ars Elemental 0.7.10.1 são providers LIGHTNING somente quando a ação for classificada. VERSION-STATUS: nenhum provider persistente de WET foi comprovado na auditoria atual; WET_STATE_V1 permanece contrato futuro/fail-closed. |
| Efeito | Quando um componente de dano LIGHTNING elegível atinge um alvo cujo estado WET/Molhado foi confirmado no momento daquele impacto, A0283 aplica ao componente desse alvo o multiplicador LIGHTNING_WET ×1,05/×1,10/×1,15 conforme rank. É um modificador target-specific: não entra no snapshot reutilizável da ação raiz usado para calcular ramificações/hops; cada alvo WET recebe sua própria aplicação no respectivo outcome, antes da mitigação dele. |
| Escalonamento | Até 3 ranks. LIGHTNING_WET_MULTIPLIER=1+0,05×rank => ×1,05/×1,10/×1,15. Uma única aplicação por outcome_id + target_uuid. Pode valer para impacto direto ou hop/descarga LIGHTNING derivado quando esse outcome for elegível e o alvo concreto estiver WET; nunca é herdado do alvo anterior pelo snapshot. |
| Gate | SPECIALIST_UNLOCK:LIGHTNING válido + componente LIGHTNING positivo no outcome do alvo concreto + WET_STATE_V1=true para esse alvo no instante daquele outcome. Impacto direto ou hop LIGHTNING derivado podem ser elegíveis, mas cada target resolve seu próprio WET. Água/chuva ambiente só contam quando o adapter versionado publicar WET; bioma, chuva visual, proximidade de água, item molhado, WATER damage isolado, entidade aquática por espécie e estado WET de outro alvo não bastam. |
| Hook | No estágio target-specific do compositor, após formar a base LIGHTNING daquele outcome e antes da mitigação do alvo, consultar WET_STATE_V1 UMA vez e, se verdadeiro, aplicar canonical_modifier_id=LIGHTNING_WET ×1,05/×1,10/×1,15. Deduplicar por outcome_id+target_uuid. O snapshot raiz reutilizável de chains/hops NÃO incorpora LIGHTNING_WET do alvo original; cada novo alvo consulta sua própria condição e recebe no máximo uma aplicação. |
| Fallback | FAIL-CLOSED se Specialist Raio não estiver desbloqueada ou se WET_STATE_V1 não puder confirmar estado/condição molhada de forma inequívoca. Não inferir por isInWaterOrRain() fora do adapter, bioma, precipitação visual, proximidade de água, WATER damage ou nome de efeito; não converter A0283 em bônus LIGHTNING geral. |
| Regra | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna da Specialist Raio. LIGHTNING_WET é target-specific, aplicado uma vez por alvo/outcome e não duplica bônus WATER. O mesmo root action pode beneficiar vários alvos WET, mas a condição nunca é herdada entre eles e não aumenta a base reutilizável de derived damage, evitando multiplicação recursiva em chains. WET é contrato próprio, não sinônimo de estar em água/chuva até adapter versionado decidir isso explicitamente. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Até 3 ranks. LIGHTNING_WET_MULTIPLIER=1+0,05×rank => ×1,05/×1,10/×1,15. Uma única aplicação por outcome_id + target_uuid. Pode valer para impacto direto ou hop/descarga LIGHTNING derivado quando esse outcome for elegível e o alvo concreto estiver WET; nunca é herdado do alvo anterior pelo snapshot.

### Gate de compra/ativação

SPECIALIST_UNLOCK:LIGHTNING válido + componente LIGHTNING positivo no outcome do alvo concreto + WET_STATE_V1=true para esse alvo no instante daquele outcome. Impacto direto ou hop LIGHTNING derivado podem ser elegíveis, mas cada target resolve seu próprio WET. Água/chuva ambiente só contam quando o adapter versionado publicar WET; bioma, chuva visual, proximidade de água, item molhado, WATER damage isolado, entidade aquática por espécie e estado WET de outro alvo não bastam.

### Hook e ordem de execução

No estágio target-specific do compositor, após formar a base LIGHTNING daquele outcome e antes da mitigação do alvo, consultar WET_STATE_V1 UMA vez e, se verdadeiro, aplicar canonical_modifier_id=LIGHTNING_WET ×1,05/×1,10/×1,15. Deduplicar por outcome_id+target_uuid. O snapshot raiz reutilizável de chains/hops NÃO incorpora LIGHTNING_WET do alvo original; cada novo alvo consulta sua própria condição e recebe no máximo uma aplicação.
...
| Item | Decisão |
|---|---|
| Região | Especialista — Raio / Eletromancia — Condutividade |
...
| Dependência semântica completa | SPECIALIST_UNLOCK:LIGHTNING confirmado server-side por Gate A/B/C: Gate A = fundamentos exteriores ARCANE/POWER e ARCANE/LIGHTNING; Gate B = ≥100 Passive Points válidos em SPECIALIST_REGION:LIGHTNING; Gate C = A0176 Maestria de Raio. A0176 isolada, conexão visual por WATER ou topologia local não autorizam compra. |
| Custo topológico | 1 PP por rank; 3 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna da Specialist Raio. LIGHTNING_WET é target-specific, aplicado uma vez por alvo/outcome e não duplica bônus WATER. O mesmo root action pode beneficiar vários alvos WET, mas a condição nunca é herdada entre eles e não aumenta a base reutilizável de derived damage, evitando multiplicação recursiva em chains. WET é contrato próprio, não sinônimo de estar em água/chuva até adapter versionado decidir isso explicitamente. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + classificador LIGHTNING + FUTURE_PROVIDER_CONTRACT WET_STATE_V1 + compositor target-specific. OWNER: adapter ambiental/status do RPG; CONSUMER: A0283 e futuras perks de condução; QUERY: {entity_uuid, level/sublevel, tick} -> wet_state_id/source/expiry_or_context; BEHAVIOR: afirmar WET somente por estado/condição explicitamente mapeada. Minecraft/NeoForge pode alimentar o adapter quando a política versionada decidir que contato real com água/chuva gera WET, mas isInWaterOrRain, bioma ou chuva visual NÃO são diretamente o contrato. Iron's Spells 'n Spellbooks 3.16.3 e Ars Nouveau 5.13.1/Ars Elemental 0.7.10.1 são providers LIGHTNING somente quando a ação for classificada. VERSION-STATUS: nenhum provider persistente de WET foi comprovado na auditoria atual; WET_STATE_V1 permanece contrato futuro/fail-closed.
...
- **Contratos/capabilities nomeados no registro:** <code>SPECIALIST_GATE_RESOLVER_V1</code>, <code>WET_STATE_V1</code>.
- **Estado:** nenhum nome de API é tratado como existente apenas por aparecer no design; FUTURE_PROVIDER_CONTRACT permanece bloqueador até prova em código/API da versão exata.

### Matriz dos quatro projetos próprios

| Projeto | Head auditado | Decisão para A0283 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>outcome_id</code>, <code>target_uuid</code>, <code>canonical_modifier_id</code>.
...
- **Chaves que o adapter precisa preservar:** <code>outcome_id</code>, <code>target_uuid</code>, <code>canonical_modifier_id</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0283.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks. LIGHTNING_WET_MULTIPLIER=1+0,05×rank => ×1,05/×1,10/×1,15. Uma única aplicação por outcome_id + target_uuid. Pode valer para impacto direto ou hop/descarga LIGHTNING derivado quando esse outcome for elegível e o alvo concreto estiver WET; nunca é herdado do alvo anterior pelo snapshot..
...
6. **Causalidade:** correlacionar <code>outcome_id</code>, <code>target_uuid</code>, <code>canonical_modifier_id</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
- **Capabilities/contracts a provar:** <code>SPECIALIST_GATE_RESOLVER_V1</code>, <code>WET_STATE_V1</code>.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>outcome_id</code>, <code>target_uuid</code>, <code>canonical_modifier_id</code>. |
...
- Página: [A0283 — Condutividade](https://app.notion.com/3c569db9f0db81f58be1cb7e62f8ec3c)
...
**Resultado final do Chat 1 para A0283:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
