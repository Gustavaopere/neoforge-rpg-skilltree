...
- **Fonte canônica:** [registro A0268 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81cbaff6e40566096267).
...
- **Dependências externas à faixa:** <code>A0169</code>, <code>A0168</code>, <code>A0165</code>. Elas permanecem sinalizadas e não são presumidas como concluídas.
...
| Hook | No DAMAGE_MITIGATION_RESOLVER_V1, classificar ICE uma única vez, agregar A0268 (+7/+14/+21 p.p.) ao mesmo canonical_modifier_id=RPG_ICE_RESISTANCE usado pelos fundamentos e resolver esse bucket UMA vez no componente ICE antes de mitigadores semanticamente distintos. Se houver backend nativo de resistência ICE com adapter seguro, normalizá-lo no mesmo bucket; nunca aplicar nativo + RPG em duplicidade. |
| Fallback | FAIL-CLOSED se Specialist Gelo não estiver desbloqueada ou se o componente não puder ser classificado ICE com segurança. Não converter A0268 em resistência genérica a frio, COLD_DAMAGE, armadura, Afinidade, temperatura corporal, redução de CHILL/freeze buildup ou imunidade. |
| Regra | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Perk interna da Specialist Gelo. Pele Glacial = resistência a DANO ICE e usa o MESMO bucket RPG_ICE_RESISTANCE dos fundamentos, sem cap defensivo global oculto. A0168 Afinidade de Gelo, COLD_DAMAGE, CHILL, freeze buildup/Frozen e estado térmico corporal são eixos distintos e não são aliases. Backend equivalente deve ser unificado, nunca duplicado. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
A0268 adiciona +7% de resistência a dano ICE por rank (+7% / +14% / +21%) ao MESMO bucket canônico RPG_ICE_RESISTANCE já usado por A0165/A0166. Não reduz COLD_DAMAGE térmico, deslocamento corporal para frio, CHILL, freeze buildup ou penalidades ambientais.

### Escalonamento aprovado

Até 3 ranks. Contribuição ao RPG_ICE_RESISTANCE: +7% / +14% / +21%. Teto próprio de A0268: 21%. Somada aos demais nodes da família ICE no mesmo bucket, a contribuição é resolvida uma única vez; não existe cap defensivo global oculto.

### Gate de compra/ativação

SPECIALIST_UNLOCK:ICE válido + componente de dano legitimamente classificado ICE. COLD_DAMAGE térmico, hipotermia, água/neve fria, temperatura corporal Cold Sweat, CHILL, freeze buildup/Frozen, custo de recurso, dano físico e efeitos apenas visuais não entram por inferência.

### Hook e ordem de execução

No DAMAGE_MITIGATION_RESOLVER_V1, classificar ICE uma única vez, agregar A0268 (+7/+14/+21 p.p.) ao mesmo canonical_modifier_id=RPG_ICE_RESISTANCE usado pelos fundamentos e resolver esse bucket UMA vez no componente ICE antes de mitigadores semanticamente distintos. Se houver backend nativo de resistência ICE com adapter seguro, normalizá-lo no mesmo bucket; nunca aplicar nativo + RPG em duplicidade.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Gelo não estiver desbloqueada ou se o componente não puder ser classificado ICE com segurança. Não converter A0268 em resistência genérica a frio, COLD_DAMAGE, armadura, Afinidade, temperatura corporal, redução de CHILL/freeze buildup ou imunidade.
...
| Pré-requisito visual/estrutural | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) por Gate A/B/C. |
| Dependência semântica completa | SPECIALIST_UNLOCK:ICE confirmado server-side por Gate A/B/C. A0268 é perk interna defensiva e exige Specialist Gelo ativa; A0169 isolada não autoriza compra. A0268 reduz somente dano ICE, mantendo COLD_DAMAGE, CHILL, freeze buildup/Frozen e deslocamento térmico corporal em pipelines distintos. |
...
| Projeto | Head auditado | Decisão para A0268 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0268.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Gelo não estiver desbloqueada ou se o componente não puder ser classificado ICE com segurança. Não converter A0268 em resistência genérica a frio, COLD_DAMAGE, armadura, Afinidade, temperatura corporal, redução de CHILL/freeze buildup ou imunidade.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks. Contribuição ao RPG_ICE_RESISTANCE: +7% / +14% / +21%. Teto próprio de A0268: 21%. Somada aos demais nodes da família ICE no mesmo bucket, a contribuição é resolvida uma única vez; não existe cap defensivo global oculto..
...
- Página: [A0268 — Pele Glacial](https://app.notion.com/3c569db9f0db81cbaff6e40566096267)
...
**Resultado final do Chat 1 para A0268:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
