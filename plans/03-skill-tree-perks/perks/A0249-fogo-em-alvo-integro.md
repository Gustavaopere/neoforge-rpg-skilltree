...
- **Fonte canônica:** [registro A0249 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81afb26dfe164cce54e3).
...
| Hook | Antes do dano FIRE direto, ler vida pré-impacto e cooldown do par. Se elegível, aplicar FIRE_OPENER uma vez e iniciar cooldown_expiry=now+400 após commit. No rank 2, se o IgnitionResolver do mesmo outcome resultar em aplicação e expuser duração-base, multiplicar por ×1,10 antes do teto nativo explícito. Deduplicar por outcome_id. |
| Fallback | FAIL-CLOSED se Specialist Fire não estiver desbloqueada ou se vida/correlação/cooldown por alvo não forem confiáveis. Se somente a duração da Ignição não for modificável, preservar o bônus de abertura e omitir apenas essa parcela. |
| Regra | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Limite é estritamente vida pré-impacto >85%; exatamente 85% não qualifica. Rearme é exclusivamente pelo cooldown explícito de 400 ticks por jogador/alvo, não por microcura ou reset heurístico de combate. A0249 e A0248 são janelas complementares sem sobreposição. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
+7% de dano FIRE direto por rank contra alvo vivo cuja vida imediatamente antes do outcome esteja acima de 85% da vida máxima, desde que o par jogador/alvo não esteja na recarga de abertura. Multiplicador: ×1,07 / ×1,14. Um uso bem-sucedido inicia recarga por jogador/alvo de 400 ticks (20 s). No rank 2, se a MESMA decisão canônica de Ignição daquele outcome aplicar estado real e sua duração-base for modificável, multiplicar essa duração-base por ×1,10 uma única vez.

### Escalonamento aprovado

Até 2 ranks. Condição pré-impacto: vida >85%. Dano FIRE direto ×1,07 / ×1,14. Recarga de abertura por jogador/alvo: 400 ticks após uso bem-sucedido. Rank 2: duração-base da Ignição aplicada pelo mesmo outcome ×1,10 quando modificável.

### Gate de compra/ativação

SPECIALIST_UNLOCK:FIRE válido + requisito local + alvo vivo com vida autoritativa pré-impacto >85% + componente FIRE direto elegível + cooldown do par jogador/alvo livre. Microcura, regeneração, afastamento ou heurística de combate não ignoram os 400 ticks. A parcela de duração exige Ignição real aplicada pela mesma decisão/outcome.

### Hook e ordem de execução

Antes do dano FIRE direto, ler vida pré-impacto e cooldown do par. Se elegível, aplicar FIRE_OPENER uma vez e iniciar cooldown_expiry=now+400 após commit. No rank 2, se o IgnitionResolver do mesmo outcome resultar em aplicação e expuser duração-base, multiplicar por ×1,10 antes do teto nativo explícito. Deduplicar por outcome_id.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Fire não estiver desbloqueada ou se vida/correlação/cooldown por alvo não forem confiáveis. Se somente a duração da Ignição não for modificável, preservar o bônus de abertura e omitir apenas essa parcela.
...
| Pré-requisito visual/estrutural | Specialist Fire desbloqueada (SPECIALIST_UNLOCK:FIRE) + A0246 Calor Crescente ≥1 ou A0247 Fagulha Crítica ≥1. |
| Dependência semântica completa | SPECIALIST_UNLOCK:FIRE confirmado por Gate A/B/C + (A0246 Calor Crescente ≥1 OU A0247 Fagulha Crítica ≥1). Os requisitos locais não substituem o unlock global da Specialist Fire. |
| Custo topológico | 1 PP por rank; 2 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Limite é estritamente vida pré-impacto >85%; exatamente 85% não qualifica. Rearme é exclusivamente pelo cooldown explícito de 400 ticks por jogador/alvo, não por microcura ou reset heurístico de combate. A0249 e A0248 são janelas complementares sem sobreposição. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0249 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0249.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Fire não estiver desbloqueada ou se vida/correlação/cooldown por alvo não forem confiáveis. Se somente a duração da Ignição não for modificável, preservar o bônus de abertura e omitir apenas essa parcela.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 2 ranks. Condição pré-impacto: vida >85%. Dano FIRE direto ×1,07 / ×1,14. Recarga de abertura por jogador/alvo: 400 ticks após uso bem-sucedido. Rank 2: duração-base da Ignição aplicada pelo mesmo outcome ×1,10 quando modificável..
...
- Página: [A0249 — Fogo em Alvo Íntegro](https://app.notion.com/3c569db9f0db81afb26dfe164cce54e3)
...
**Resultado final do Chat 1 para A0249:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
