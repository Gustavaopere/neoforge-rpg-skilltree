...
- **Fonte canônica:** [registro A0269 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81ae9994c5a3d6b857db).
...
| Hook | No estágio server-authoritative de movimento no solo, resolver uma única effective_ground_surface via GROUND_SURFACE_CONTEXT_V1. Se SLIPPERY_SURFACE_REGISTRY_V1 a classificar como elegível, isolar a parcela de inércia/derrapagem adicional atribuível à superfície e reduzi-la por ×0,65/×0,40, além de aplicar +4%/+7% de movimento enquanto o contato persistir. Em Sable, usar coordenadas/contexto do sublevel e nunca consultar bloco equivalente no parent level. Cessar imediatamente ao perder a superfície. Não alterar gravidade, salto, velocidade aérea, colisão ou temperatura. |
| Fallback | FAIL-CLOSED se Specialist Gelo não estiver desbloqueada, a superfície efetiva não puder ser determinada ou o hook versionado não conseguir isolar de forma segura a influência de slipperiness/inércia. Não preservar apenas +velocidade, não aplicar Speed genérico, não inferir gelo por nome/textura e não aproximar sublevels Sable pelo parent level. |
| Regra | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Perk interna da Specialist Gelo. Passo Seguro no Gelo é CONTROLE LOCOMOTOR DE SUPERFÍCIE, não Afinidade, Resistência a Gelo, resistência ambiental, freeze immunity ou velocidade geral. Uma única superfície efetiva governa o modificador; múltiplos blocos sob o hitbox não empilham. Sable/Aeronautics só afetam resolução espacial do chão. Não gera Ice Mastery e não altera temperatura corporal. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Até 2 ranks. Excesso de inércia/derrapagem atribuível à superfície elegível: ×0,65 / ×0,40. Velocidade de movimento condicionada à mesma superfície: +4% / +7%. Os modificadores cessam imediatamente ao deixar a superfície; não acumulam com múltiplos blocos sob o hitbox.

### Gate de compra/ativação

SPECIALIST_UNLOCK:ICE válido + A0268 ≥2 + jogador grounded + uma única superfície efetiva de contato classificada SLIPPERY_SURFACE por registry/tag/adapter curado. Em sublevel Sable/Aeronautics, o contexto de chão deve ser resolvido no espaço correto; se isso não puder ser comprovado, fail-closed. Ar, queda, água, bloco lateral, gelo apenas visual, posição aproximada, nome contendo 'ice', superfície não allowlisted e múltiplos contatos usados para empilhar não ativam.

### Hook e ordem de execução

No estágio server-authoritative de movimento no solo, resolver uma única effective_ground_surface via GROUND_SURFACE_CONTEXT_V1. Se SLIPPERY_SURFACE_REGISTRY_V1 a classificar como elegível, isolar a parcela de inércia/derrapagem adicional atribuível à superfície e reduzi-la por ×0,65/×0,40, além de aplicar +4%/+7% de movimento enquanto o contato persistir. Em Sable, usar coordenadas/contexto do sublevel e nunca consultar bloco equivalente no parent level. Cessar imediatamente ao perder a superfície. Não alterar gravidade, salto, velocidade aérea, colisão ou temperatura.
...
| Pré-requisito visual/estrutural | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) + A0268 Pele Glacial ≥2 ranks. |
| Dependência semântica completa | SPECIALIST_UNLOCK:ICE confirmado por Gate A/B/C + A0268 Pele Glacial ≥2 ranks. A0268 e a topologia local não substituem fundamentos ICE + ≥100 PP válidos em SPECIALIST_REGION:ICE + terminal A0169. |
...
| Projeto | Head auditado | Decisão para A0269 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0269.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 2 ranks. Excesso de inércia/derrapagem atribuível à superfície elegível: ×0,65 / ×0,40. Velocidade de movimento condicionada à mesma superfície: +4% / +7%. Os modificadores cessam imediatamente ao deixar a superfície; não acumulam com múltiplos blocos sob o hitbox..
...
- Página: [A0269 — Passo Seguro no Gelo](https://app.notion.com/3c569db9f0db81ae9994c5a3d6b857db)
...
**Resultado final do Chat 1 para A0269:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
