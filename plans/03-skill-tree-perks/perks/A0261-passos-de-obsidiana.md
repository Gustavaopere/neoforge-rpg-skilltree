...
- **Fonte canônica:** [registro A0261 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81e39fd7ded8a6f0c6b4).
...
| Dependências Obrigatórias | SPECIALIST_UNLOCK:FIRE confirmado server-side por Gate A/B/C + A0259 Nascido das Chamas possuído e ATIVO + A0251 Passos Quentes ≥2 ranks. A0259/A0251 e a topologia local não substituem fundamentos FIRE + ≥100 PP válidos em SPECIALIST_REGION:FIRE + terminal A0162. |
| Pré-requisitos | Specialist Fire desbloqueada (SPECIALIST_UNLOCK:FIRE) + A0259 Nascido das Chamas + A0251 Passos Quentes ≥2 ranks. |
| Provider/Mods | RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + Minecraft/NeoForge 1.21.1 para estado de fluido e mutação server-side + FTB Chunks 2101.1.21/FTB Teams como provider real de claims/proteção territorial + Create Aeronautics: FTB Chunks 1.1.1 como bridge de claims/permissões em ships/sublevels + Sable 2.0.5 como infraestrutura de sublevels. FUTURE_PROVIDER_CONTRACT TEMPORARY_WORLD_MUTATION_GUARD_V1: OWNER adapter de mundo/proteção do RPG; CONSUMER A0261; QUERY {player, level/sublevel, pos, mutation_kind=TEMPORARY_LAVA_STEP}; BEHAVIOR autorizar a mutação somente quando claim/permissão e identidade de restauração forem seguras. VERSION-STATUS: contrato do RPG a implementar; nenhum hook A0261 correspondente foi encontrado na main auditada em 2026-08-29. |
| Efeito | Enquanto RPG_BORN_OF_FLAME de A0259 estiver ativo, quando o jogador mudar de célula horizontal e houver lava real diretamente sob a área dos pés, A0261 pode criar rpgskilltree:temporary_obsidian_step apenas sobre células de lava elegíveis num footprint de distância Manhattan ≤1 do ponto sob o jogador (centro + até 4 cardinais). Cada bloco temporário dura 40 ticks (2 s), não possui item/drop e registra o FluidState original para restauração. Limite: no máximo 5 novos blocos por janela móvel de 20 ticks e 15 blocos ativos por jogador. |
| Escalonamento | 1 rank. Footprint por passo: Manhattan ≤1, máximo 5 células elegíveis. Duração de cada plataforma: 40 ticks. Rate limit: 5 criações por 20 ticks; máximo 15 plataformas ativas por jogador. A mesma posição já temporária não conta como nova criação nem renova duração por permanência. |
| Gate | SPECIALIST_UNLOCK:FIRE válido + A0259 ativo + A0251 ≥2 + mudança real de célula horizontal + candidato contendo lava real + espaço de plataforma válido + TEMPORARY_WORLD_MUTATION_GUARD_V1=ALLOW. Em FTB Chunks, área sem permissão é inelegível. Em Sable/Aeronautics, a mutação só é permitida quando o adapter versionado comprovar identidade estável da posição, ownership/permissão e restauração no mesmo sublevel; sem essa prova, fail-closed. Célula já temporária, fake player, permanência AFK e posição em transição/migração de sublevel não criam novo bloco. |
| Hook | No movimento elegível, calcular candidatos Manhattan ≤1 e, para cada posição, consultar TEMPORARY_WORLD_MUTATION_GUARD_V1 antes de qualquer write. Se autorizado, snapshotar BlockState/FluidState originais, instalar rpgskilltree:temporary_obsidian_step com owner/instance/expiry e registrar restauração persistente. O bloco temporário não pode gerar item/drop, advancement/mining credit nem ser convertido em recurso; deve ser inelegível para piston/assembly/movimentação quando isso puder romper a identidade da restauração. Restaurar somente se a posição ainda contiver a instância temporária esperada; unload/restart preserva a pendência. Em sublevels Sable, nunca restaurar por coordenada do parent level nem espelhar mutação entre espaços. |
| Fallback | FAIL-CLOSED se Specialist Fire não estiver desbloqueada, A0259 não estiver ativo, o guard de proteção/restauração estiver ausente, FTB Chunks negar permissão ou a identidade espacial de um sublevel Sable/Aeronautics não puder ser preservada. Não criar obsidiana permanente, gelo, barreira persistente, item/drop, Speed/Levitation/voo ou outro bônus de movimento como substituto. |
| Regra | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Passos de Obsidiana é infraestrutura TEMPORÁRIA de mobilidade, não transmutação de recurso. FTB Chunks/FTB Teams governam claims; Create Aeronautics: FTB Chunks/Sable só entram pela camada de autorização/identidade espacial, nunca como fonte de lava ou de benefício. Não gera obsidiana coletável, drops, mining/advancement credit, Fire Mastery, Ímpeto, mana ou automação. Não reduz dano, HEAT_DAMAGE, FIRE ou temperatura da lava. Logout/dimensão/respec impedem novas criações, mas restaurações pendentes continuam até limpar todas as instâncias com segurança. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Enquanto RPG_BORN_OF_FLAME de A0259 estiver ativo, quando o jogador mudar de célula horizontal e houver lava real diretamente sob a área dos pés, A0261 pode criar rpgskilltree:temporary_obsidian_step apenas sobre células de lava elegíveis num footprint de distância Manhattan ≤1 do ponto sob o jogador (centro + até 4 cardinais). Cada bloco temporário dura 40 ticks (2 s), não possui item/drop e registra o FluidState original para restauração. Limite: no máximo 5 novos blocos por janela móvel de 20 ticks e 15 blocos ativos por jogador.

### Escalonamento aprovado

1 rank. Footprint por passo: Manhattan ≤1, máximo 5 células elegíveis. Duração de cada plataforma: 40 ticks. Rate limit: 5 criações por 20 ticks; máximo 15 plataformas ativas por jogador. A mesma posição já temporária não conta como nova criação nem renova duração por permanência.

### Gate de compra/ativação

SPECIALIST_UNLOCK:FIRE válido + A0259 ativo + A0251 ≥2 + mudança real de célula horizontal + candidato contendo lava real + espaço de plataforma válido + TEMPORARY_WORLD_MUTATION_GUARD_V1=ALLOW. Em FTB Chunks, área sem permissão é inelegível. Em Sable/Aeronautics, a mutação só é permitida quando o adapter versionado comprovar identidade estável da posição, ownership/permissão e restauração no mesmo sublevel; sem essa prova, fail-closed. Célula já temporária, fake player, permanência AFK e posição em transição/migração de sublevel não criam novo bloco.

### Hook e ordem de execução

No movimento elegível, calcular candidatos Manhattan ≤1 e, para cada posição, consultar TEMPORARY_WORLD_MUTATION_GUARD_V1 antes de qualquer write. Se autorizado, snapshotar BlockState/FluidState originais, instalar rpgskilltree:temporary_obsidian_step com owner/instance/expiry e registrar restauração persistente. O bloco temporário não pode gerar item/drop, advancement/mining credit nem ser convertido em recurso; deve ser inelegível para piston/assembly/movimentação quando isso puder romper a identidade da restauração. Restaurar somente se a posição ainda contiver a instância temporária esperada; unload/restart preserva a pendência. Em sublevels Sable, nunca restaurar por coordenada do parent level nem espelhar mutação entre espaços.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Fire não estiver desbloqueada, A0259 não estiver ativo, o guard de proteção/restauração estiver ausente, FTB Chunks negar permissão ou a identidade espacial de um sublevel Sable/Aeronautics não puder ser preservada. Não criar obsidiana permanente, gelo, barreira persistente, item/drop, Speed/Levitation/voo ou outro bônus de movimento como substituto.
...
| Pré-requisito visual/estrutural | Specialist Fire desbloqueada (SPECIALIST_UNLOCK:FIRE) + A0259 Nascido das Chamas + A0251 Passos Quentes ≥2 ranks. |
| Dependência semântica completa | SPECIALIST_UNLOCK:FIRE confirmado server-side por Gate A/B/C + A0259 Nascido das Chamas possuído e ATIVO + A0251 Passos Quentes ≥2 ranks. A0259/A0251 e a topologia local não substituem fundamentos FIRE + ≥100 PP válidos em SPECIALIST_REGION:FIRE + terminal A0162. |
| Custo topológico | 3 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Passos de Obsidiana é infraestrutura TEMPORÁRIA de mobilidade, não transmutação de recurso. FTB Chunks/FTB Teams governam claims; Create Aeronautics: FTB Chunks/Sable só entram pela camada de autorização/identidade espacial, nunca como fonte de lava ou de benefício. Não gera obsidiana coletável, drops, mining/advancement credit, Fire Mastery, Ímpeto, mana ou automação. Não reduz dano, HEAT_DAMAGE, FIRE ou temperatura da lava. Logout/dimensão/respec impedem novas criações, mas restaurações pendentes continuam até limpar todas as instâncias com segurança. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + Minecraft/NeoForge 1.21.1 para estado de fluido e mutação server-side + FTB Chunks 2101.1.21/FTB Teams como provider real de claims/proteção territorial + Create Aeronautics: FTB Chunks 1.1.1 como bridge de claims/permissões em ships/sublevels + Sable 2.0.5 como infraestrutura de sublevels. FUTURE_PROVIDER_CONTRACT TEMPORARY_WORLD_MUTATION_GUARD_V1: OWNER adapter de mundo/proteção do RPG; CONSUMER A0261; QUERY {player, level/sublevel, pos, mutation_kind=TEMPORARY_LAVA_STEP}; BEHAVIOR autorizar a mutação somente quando claim/permissão e identidade de restauração forem seguras. VERSION-STATUS: contrato do RPG a implementar; nenhum hook A0261 correspondente foi encontrado na main auditada em 2026-08-29.
...
| Projeto | Head auditado | Decisão para A0261 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0261.
...
- **Recursos/eixos tocados pelo contrato:** <code>MANA</code>, <code>PARCEL TÉRMICO</code>, <code>LAVA/WORLD STATE</code>, <code>RESOURCE_ID NATIVO</code>.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Fire não estiver desbloqueada, A0259 não estiver ativo, o guard de proteção/restauração estiver ausente, FTB Chunks negar permissão ou a identidade espacial de um sublevel Sable/Aeronautics não puder ser preservada. Não criar obsidiana permanente, gelo, barreira persistente, item/drop, Speed/Levitation/voo ou outro bônus de movimento como substituto.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Footprint por passo: Manhattan ≤1, máximo 5 células elegíveis. Duração de cada plataforma: 40 ticks. Rate limit: 5 criações por 20 ticks; máximo 15 plataformas ativas por jogador. A mesma posição já temporária não conta como nova criação nem renova duração por permanência..
...
- Página: [A0261 — Passos de Obsidiana](https://app.notion.com/3c569db9f0db81e39fd7ded8a6f0c6b4)
...
**Resultado final do Chat 1 para A0261:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
