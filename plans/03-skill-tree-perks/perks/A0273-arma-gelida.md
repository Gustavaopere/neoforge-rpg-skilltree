...
- **Fonte canônica:** [registro A0273 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81bf835bfb3e6b762f99).
...
- **Dependências externas à faixa:** <code>A0167</code>, <code>A0169</code>. Elas permanecem sinalizadas e não são presumidas como concluídas.
...
| Dependências Obrigatórias | SPECIALIST_UNLOCK:ICE confirmado por Gate A/B/C + A0167 Imbuimento de Gelo ≥2 ranks + A0266 Congelamento Progressivo ≥1 rank. O antigo requisito vago MARTIAL ≥6 foi removido: a qualificação marcial é herdada integralmente do gate atual de A0167, que exige família melee canônica elegível + mastery da própria lane ≥20. Requisitos locais não substituem fundamentos ICE + ≥100 PP válidos em SPECIALIST_REGION:ICE + terminal A0169. |
| Pré-requisitos | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) + A0167 Imbuimento de Gelo ≥2 + A0266 Congelamento Progressivo ≥1. A família melee/mastery elegível é a mesma resolvida por A0167. |
| Provider/Mods | RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + A0167/infra de derived_component:ICE no MESMO direct_melee_outcome + FREEZE_BUILDUP_ADAPTER_V1 + bucket único RPG_FREEZE_BUILDUP_MULTIPLIER. Epic Fight 21.17.3.1 e classificadores canônicos resolvem as lanes epic_sword, epic_axe, epic_spear melee, epic_dagger, epic_hammer; adapters canônicos resolvem combat_mace/combat_scythe; Weapons of Miracles 2.0.176 só participa quando a arma concreta cair inequivocamente em lane elegível de A0167. Iron's Spells runtime 1.21.1-3.16.3 e Ars Nouveau 5.13.1/Ars Elemental 0.7.10.1 fornecem o cast ICE de origem quando explicitamente classificado. combat_fist continua fail-closed conforme P-0032. VERSION-STATUS: A0273 depende dos mesmos contratos versionados de A0167/A0266; não cria lane ou medidor próprios. |
| Efeito | Em um direct_melee_outcome elegível para A0167 que efetivamente contenha o derived_component:ICE daquele imbuimento E já possua uma contribuição positiva de freeze_buildup nativo/compatível exposta pelo adapter, A0273 acrescenta +8% dessa contribuição por rank (+8% / +16% / +24%) ao MESMO RPG_FREEZE_BUILDUP_MULTIPLIER de A0266/A0267. A0273 não cria buildup quando o golpe/estado real não o fornece. |
| Escalonamento | Até 3 ranks. Em golpe melee imbuído elegível com buildup base positivo: contribuição adicional ao bucket = +0,08 / +0,16 / +0,24. Sem buildup real: 0. A0266, A0267 e A0273 somam contribuições no mesmo bucket e o valor final é resolvido uma única vez. |
| Gate | SPECIALIST_UNLOCK:ICE válido + A0167 ≥2 + A0266 ≥1 + direct_melee_outcome que satisfaça INTEGRALMENTE o gate atual de A0167, contenha exatamente o componente ICE daquele imbuimento e possua contribuição positiva real de freeze buildup exposta pelo adapter. Não existe gate adicional MARTIAL ≥6. Ranged, projétil, mão vazia, lança arremessada, sweep/cleave derivado não direto, DoT, summon, automação, fake player, derived component reprocessado e callback duplicado não contribuem. |
| Hook | No MESMO action_id/outcome_id melee de A0167, reutilizar lane/mastery/criticalHit já resolvidos e identificar o derived_component:ICE do imbuimento sem criar segundo dano. Se o provider expuser buildup base positivo para aquele outcome, agregar A0273 (+0,08/+0,16/+0,24) ao MESMO canonical_modifier_id=RPG_FREEZE_BUILDUP_MULTIPLIER de A0266/A0267 e resolver UMA única vez. Nunca criar novo DamageSource, segunda resolução, nova rolagem crítica, CHILL automático ou buildup inexistente. |
| Fallback | FAIL-CLOSED se Specialist Gelo não estiver desbloqueada, A0167 não puder resolver com segurança o mesmo outcome/lane/mastery, o componente ICE não puder ser identificado causalmente ou não houver freeze buildup real positivo. Não substituir por Slowness, CHILL aleatório, novo medidor, dano ICE extra adicional ou gate MARTIAL aproximado. |
| Regra | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Perk interna/ponte Spellblade da Specialist Gelo. A0273 reutiliza o MESMO outcome de A0167 e o MESMO buildup real de A0266/A0267; não cria crítico, CHILL, Congelamento, mastery, sustain ou proc. MARTIAL ≥6 legado foi removido por não ser um contrato canônico: a identidade marcial vem da lane+mastery de A0167. PP/hibridização não servem de atalho para Gate A/B/C da Specialist ICE. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Em um direct_melee_outcome elegível para A0167 que efetivamente contenha o derived_component:ICE daquele imbuimento E já possua uma contribuição positiva de freeze_buildup nativo/compatível exposta pelo adapter, A0273 acrescenta +8% dessa contribuição por rank (+8% / +16% / +24%) ao MESMO RPG_FREEZE_BUILDUP_MULTIPLIER de A0266/A0267. A0273 não cria buildup quando o golpe/estado real não o fornece.

### Escalonamento aprovado

Até 3 ranks. Em golpe melee imbuído elegível com buildup base positivo: contribuição adicional ao bucket = +0,08 / +0,16 / +0,24. Sem buildup real: 0. A0266, A0267 e A0273 somam contribuições no mesmo bucket e o valor final é resolvido uma única vez.

### Gate de compra/ativação

SPECIALIST_UNLOCK:ICE válido + A0167 ≥2 + A0266 ≥1 + direct_melee_outcome que satisfaça INTEGRALMENTE o gate atual de A0167, contenha exatamente o componente ICE daquele imbuimento e possua contribuição positiva real de freeze buildup exposta pelo adapter. Não existe gate adicional MARTIAL ≥6. Ranged, projétil, mão vazia, lança arremessada, sweep/cleave derivado não direto, DoT, summon, automação, fake player, derived component reprocessado e callback duplicado não contribuem.

### Hook e ordem de execução

No MESMO action_id/outcome_id melee de A0167, reutilizar lane/mastery/criticalHit já resolvidos e identificar o derived_component:ICE do imbuimento sem criar segundo dano. Se o provider expuser buildup base positivo para aquele outcome, agregar A0273 (+0,08/+0,16/+0,24) ao MESMO canonical_modifier_id=RPG_FREEZE_BUILDUP_MULTIPLIER de A0266/A0267 e resolver UMA única vez. Nunca criar novo DamageSource, segunda resolução, nova rolagem crítica, CHILL automático ou buildup inexistente.
...
| Item | Decisão |
|---|---|
| Região | Especialista — Gelo / Criomancia — Spellblade |
| Camada e papel | Camada 3; Ponte |
| Pré-requisito visual/estrutural | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) + A0167 Imbuimento de Gelo ≥2 + A0266 Congelamento Progressivo ≥1. A família melee/mastery elegível é a mesma resolvida por A0167. |
| Dependência semântica completa | SPECIALIST_UNLOCK:ICE confirmado por Gate A/B/C + A0167 Imbuimento de Gelo ≥2 ranks + A0266 Congelamento Progressivo ≥1 rank. O antigo requisito vago MARTIAL ≥6 foi removido: a qualificação marcial é herdada integralmente do gate atual de A0167, que exige família melee canônica elegível + mastery da própria lane ≥20. Requisitos locais não substituem fundamentos ICE + ≥100 PP válidos em SPECIALIST_REGION:ICE + terminal A0169. |
| Custo topológico | 1 PP por rank; 3 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Perk interna/ponte Spellblade da Specialist Gelo. A0273 reutiliza o MESMO outcome de A0167 e o MESMO buildup real de A0266/A0267; não cria crítico, CHILL, Congelamento, mastery, sustain ou proc. MARTIAL ≥6 legado foi removido por não ser um contrato canônico: a identidade marcial vem da lane+mastery de A0167. PP/hibridização não servem de atalho para Gate A/B/C da Specialist ICE. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + A0167/infra de derived_component:ICE no MESMO direct_melee_outcome + FREEZE_BUILDUP_ADAPTER_V1 + bucket único RPG_FREEZE_BUILDUP_MULTIPLIER. Epic Fight 21.17.3.1 e classificadores canônicos resolvem as lanes epic_sword, epic_axe, epic_spear melee, epic_dagger, epic_hammer; adapters canônicos resolvem combat_mace/combat_scythe; Weapons of Miracles 2.0.176 só participa quando a arma concreta cair inequivocamente em lane elegível de A0167. Iron's Spells runtime 1.21.1-3.16.3 e Ars Nouveau 5.13.1/Ars Elemental 0.7.10.1 fornecem o cast ICE de origem quando explicitamente classificado. combat_fist continua fail-closed conforme P-0032. VERSION-STATUS: A0273 depende dos mesmos contratos versionados de A0167/A0266; não cria lane ou medidor próprios.
...
| Projeto | Head auditado | Decisão para A0273 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>action_id</code>, <code>outcome_id</code>, <code>canonical_modifier_id</code>.
...
- **Chaves que o adapter precisa preservar:** <code>action_id</code>, <code>outcome_id</code>, <code>canonical_modifier_id</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0273.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks. Em golpe melee imbuído elegível com buildup base positivo: contribuição adicional ao bucket = +0,08 / +0,16 / +0,24. Sem buildup real: 0. A0266, A0267 e A0273 somam contribuições no mesmo bucket e o valor final é resolvido uma única vez..
...
6. **Causalidade:** correlacionar <code>action_id</code>, <code>outcome_id</code>, <code>canonical_modifier_id</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
- **Dependências fora desta faixa:** <code>A0167</code>, <code>A0169</code>.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>action_id</code>, <code>outcome_id</code>, <code>canonical_modifier_id</code>. |
...
- Página: [A0273 — Arma Gélida](https://app.notion.com/3c569db9f0db81bf835bfb3e6b762f99)
...
**Resultado final do Chat 1 para A0273:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
