...
- **Fonte canônica:** [registro A0247 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81d5b6c4d86142549204).
...
- **Dependências externas à faixa:** <code>A0151</code>. Elas permanecem sinalizadas e não são presumidas como concluídas.
...
| Hook | Antes da única decisão de Ignição daquele outcome, ler o criticalHit já resolvido pelo serviço crítico canônico. Se true, adicionar a contribuição de A0247 ao IgnitionResolver; realizar uma única decisão/aplicação total para o outcome. Nunca chamar um segundo RNG de crítico ou de Ignição. |
| Fallback | FAIL-CLOSED se Specialist Fire não estiver desbloqueada, criticalHit canônico não for confiável ou o resolver de Ignição não puder integrar a fonte. Não inferir crítico pelo dano final e não realizar tentativa separada de crítico/Ignição. |
| Regra | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Fagulha Crítica não aumenta chance crítica: condiciona contribuição à MESMA decisão de Ignição após criticalHit=true já resolvido. Uma decisão crítica + uma decisão de Ignição por outcome; sem reroll, segunda aplicação, mastery ou parcel térmico adicional. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Quando o ÚNICO sistema crítico canônico já determinou criticalHit=true para um resultado FIRE direto elegível, A0247 adiciona +10 pontos percentuais de chance de Ignição por rank (+10 / +20 p.p.) à MESMA decisão canônica de Ignição daquele direct_fire_outcome_id. Não existe segunda tentativa. Exemplo: A0243 rank 3 + A0247 rank 2 contribuem juntos com +12 +20 p.p. no mesmo resolver, além de eventual chance nativa integrável.

### Escalonamento aprovado

Até 2 ranks; em resultado FIRE direto com criticalHit=true, contribuição adicional à única decisão de Ignição: +10 / +20 p.p. A0247 não altera chance nem multiplicador crítico.

### Gate de compra/ativação

SPECIALIST_UNLOCK:FIRE válido + A0243 ≥1 + A0151 ≥2 + resultado FIRE direto elegível cuja decisão crítica canônica já seja criticalHit=true + FIRE_IGNITION_RESOLVER_V1 disponível para a mesma decisão de Ignição. Crítico derivado, DoT, summon, derived component, proc recursivo e resultado sem identidade crítica canônica não contribuem.

### Hook e ordem de execução

Antes da única decisão de Ignição daquele outcome, ler o criticalHit já resolvido pelo serviço crítico canônico. Se true, adicionar a contribuição de A0247 ao IgnitionResolver; realizar uma única decisão/aplicação total para o outcome. Nunca chamar um segundo RNG de crítico ou de Ignição.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Fire não estiver desbloqueada, criticalHit canônico não for confiável ou o resolver de Ignição não puder integrar a fonte. Não inferir crítico pelo dano final e não realizar tentativa separada de crítico/Ignição.
...
| Pré-requisito visual/estrutural | Specialist Fire desbloqueada (SPECIALIST_UNLOCK:FIRE) + A0243 Ignição ≥1 + A0151 Crítico Mágico ≥2. |
| Dependência semântica completa | SPECIALIST_UNLOCK:FIRE confirmado por Gate A/B/C + A0243 Ignição ≥1 rank + A0151 Crítico Mágico ≥2 ranks. Os requisitos locais não substituem o unlock global da Specialist Fire. |
| Custo topológico | 1 PP por rank; 2 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Fagulha Crítica não aumenta chance crítica: condiciona contribuição à MESMA decisão de Ignição após criticalHit=true já resolvido. Uma decisão crítica + uma decisão de Ignição por outcome; sem reroll, segunda aplicação, mastery ou parcel térmico adicional. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0247 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0247.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Fire não estiver desbloqueada, criticalHit canônico não for confiável ou o resolver de Ignição não puder integrar a fonte. Não inferir crítico pelo dano final e não realizar tentativa separada de crítico/Ignição.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 2 ranks; em resultado FIRE direto com criticalHit=true, contribuição adicional à única decisão de Ignição: +10 / +20 p.p. A0247 não altera chance nem multiplicador crítico..
...
- Página: [A0247 — Fagulha Crítica](https://app.notion.com/3c569db9f0db81d5b6c4d86142549204)
...
**Resultado final do Chat 1 para A0247:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
