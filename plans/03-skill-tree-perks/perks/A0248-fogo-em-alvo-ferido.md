...
- **Fonte canônica:** [registro A0248 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81299f3dec43dc1afd95).
...
|---|---|
| Código | A0248 |
| Nome | Fogo em Alvo Ferido |
| Domínio | ARCANE/FIRE |
| Árvore | Especialista — Fogo |
| Ramo | Piromancia — Crítico e Execução |
| Camada | 3 |
...
| Hook | Antes do dano FIRE direto, ler vida autoritativa pré-impacto e verificar autoria/estado de Ignição já persistente. Se ambos forem válidos, aplicar FIRE_EXECUTION uma única vez à camada FIRE direta daquele outcome; deduplicar por outcome_id. |
| Fallback | FAIL-CLOSED se Specialist Fire não estiver desbloqueada, se a vida pré-impacto não puder ser lida ou se a autoria da Ignição pré-existente não for segura. Não reduzir o requisito para 'alvo ferido', fogo ambiental ou combustão visual. |
| Regra | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Limite é estritamente vida pré-impacto <40%; exatamente 40% não qualifica. A Ignição deve preexistir ao outcome, impedindo ativação retroativa no golpe que a cria. Ticks de Ignição e outcomes derivados não recebem/rearmam FIRE_EXECUTION. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
+5% de dano FIRE direto por rank contra alvo vivo cuja vida imediatamente antes do outcome esteja abaixo de 40% da vida máxima E que já possua, antes daquela ação, estado de Ignição FIRE real atribuído ao jogador. Multiplicador: ×1,05 / ×1,10 / ×1,15.

### Escalonamento aprovado

Até 3 ranks; condição pré-impacto: vida <40% + Ignição atribuída já existente. Dano FIRE direto ×1,05 / ×1,10 / ×1,15. Acima ou exatamente em 40%, contribuição = 0.

### Gate de compra/ativação

SPECIALIST_UNLOCK:FIRE válido + requisito local + alvo vivo com vida autoritativa pré-impacto <40% + Ignição real do próprio jogador existente ANTES da ação + componente FIRE direto elegível. A ação que cria a primeira Ignição não recebe A0248; DoT, derived components, summons, automação e callbacks duplicados não se autoamplificam.

### Hook e ordem de execução

Antes do dano FIRE direto, ler vida autoritativa pré-impacto e verificar autoria/estado de Ignição já persistente. Se ambos forem válidos, aplicar FIRE_EXECUTION uma única vez à camada FIRE direta daquele outcome; deduplicar por outcome_id.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Fire não estiver desbloqueada, se a vida pré-impacto não puder ser lida ou se a autoria da Ignição pré-existente não for segura. Não reduzir o requisito para 'alvo ferido', fogo ambiental ou combustão visual.
...
| Item | Decisão |
|---|---|
| Região | Especialista — Fogo / Piromancia — Crítico e Execução |
| Camada e papel | Camada 3; Ramo |
| Pré-requisito visual/estrutural | Specialist Fire desbloqueada (SPECIALIST_UNLOCK:FIRE) + A0244 Combustão ≥1 ou A0247 Fagulha Crítica ≥1. |
| Dependência semântica completa | SPECIALIST_UNLOCK:FIRE confirmado por Gate A/B/C + (A0244 Combustão ≥1 OU A0247 Fagulha Crítica ≥1). Os requisitos locais não substituem o unlock global da Specialist Fire. |
| Custo topológico | 1 PP por rank; 3 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Limite é estritamente vida pré-impacto <40%; exatamente 40% não qualifica. A Ignição deve preexistir ao outcome, impedindo ativação retroativa no golpe que a cria. Ticks de Ignição e outcomes derivados não recebem/rearmam FIRE_EXECUTION. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0248 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0248.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Fire não estiver desbloqueada, se a vida pré-impacto não puder ser lida ou se a autoria da Ignição pré-existente não for segura. Não reduzir o requisito para 'alvo ferido', fogo ambiental ou combustão visual.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks; condição pré-impacto: vida <40% + Ignição atribuída já existente. Dano FIRE direto ×1,05 / ×1,10 / ×1,15. Acima ou exatamente em 40%, contribuição = 0..
...
- Página: [A0248 — Fogo em Alvo Ferido](https://app.notion.com/3c569db9f0db81299f3dec43dc1afd95)
...
**Resultado final do Chat 1 para A0248:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
