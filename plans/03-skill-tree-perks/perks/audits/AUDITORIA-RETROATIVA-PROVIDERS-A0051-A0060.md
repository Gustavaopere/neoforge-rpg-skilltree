...
- RPG Skill Tree `main@5e9dd777722014596641cb77d7be5c51df410e4e` na abertura; antes da branch, `main` avançou para `2e6cf57d5c12630d55280d1c4ff0177f536dce96` por correções Chat 3 em A0005/A0006 e Compendium. Esse delta não altera A0051–A0060; a branch deste lote foi criada sobre o HEAD novo.
...
- Delta desde o lote anterior: `7839db6... -> c26e97c...`, restrito ao ownership/projection lifecycle da integração RNS hidrotermal.
- `plans/STATUS.md` mantém Stage 06 parcialmente canônico; ownership transfer seletivo Cu/Fe/Au ainda é o trabalho aberto.
- Classificação para A0051–A0060: **NÃO DEVE SER INTEGRADO**. RNS/geologia/prospecção não classificam CROSSBOW/FIST, Cadência, Sequência, heavy/finalizer ou crítico.
...
|---|---|---|---|
| A0051 | APROVADO após correção | crítico físico presente | ledger CROSSBOW herdada de A0049 |
| A0052 | APROVADO após correção | PARCIAL | availability A0050 + mesma identidade de besta hit→reload |
| A0053 | APROVADO após correção | penetration path presente | availability A0050→A0052→A0053 |
| A0054 | APROVADO após correção | PARCIAL | consumo de Cadência antecipado no arm; availability/ledger |
...
| A0058 | APROVADO após correção | PARCIAL | heavy-impact reset ausente; body penalty opcional ausente |
...
- A0056, A0059 e A0060.
- Motivo: os contratos já estavam semanticamente adequados; gaps encontrados são de runtime/provider, não de design.

## Pendências destinadas ao Chat 2

1. **P-A0052-01:** propagar availability A0050→A0052; sem A0050 comprável, A0052 é indisponível/não comprável.
2. **P-A0052-02:** correlacionar hit e reload pela mesma identidade causal da besta/ItemStack; limpar receipt ao trocar/clonar.
3. **P-A0052-03:** testes de miss/cancel >50%/troca/reload real/estado externo/dedup.
4. **P-A0053-01:** propagar availability A0052→A0053.
5. **P-A0054-01:** não zerar Cadência ao armar Mecanismo Ajustado; consumir 3 no disparo que efetivamente usa a janela.
6. **P-A0054-02:** propagar availability A0050/A0052/A0053→A0054.
7. **P-A0054-03:** reconciliar `combat:crossbow` vs `epicfight:crossbow`; uma ledger apenas.
8. **P-A0055-01:** producer finite-discovery para `combat:fist`, +10 por tipo hostil inédito; 6→60, 8→80; suprimir/migrar `epicfight:fist` paralelo.
9. **P-A0055-02:** publicar/reconciliar `combat_fist` no `tree_architecture/combat.json` com modelo/Notion.
10. **P-A0055-03:** teste architecture↔model↔Notion↔producer + provider-present/absent FIST.
11. **P-A0058-01:** reset de Sequência por heavy impact recebido somente com receipt provider-native.
12. **P-A0058-02:** opcional body modulation por hunger/exhaustion real; ausência omite a parcela.
13. **P-A0059-01:** heavy/finalizer receipt provider-native para ativar Quebra de Ritmo.
14. **P-A0059-02:** guard-break receipt real + −8% movement/2s com lifecycle; sem break, omitir.
15. **P-A0060-01:** heavy/finalizer receipt provider-native para ativar Combinação Final.
16. **P-A0060-02:** Stamina refund apenas com ledger causal pós-consumo das cinco ações; sem receipt, 0% refund.
17. **P-A0060-03:** gate80 usa exclusivamente `combat:fist`; depende da reconciliação A0055.
18. **P-A0051-60-TEST-01:** GameTest/harness server-side CROSSBOW/FIST, availability, Mastery, same-weapon correlation, heavy/finalizer fail-closed, dedup, lifecycle e multiplayer.
...
- Authority/causalidade/dedup: PASS no contrato; gaps de implementação catalogados.
- Mastery anti-farm: PASS no design; CROSSBOW/FIST producer/namespace pendente no runtime.
- Recursos: Cadência/Sequência/Stamina mantêm owners separados; sem geração acidental.
