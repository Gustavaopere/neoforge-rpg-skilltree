...
- **Fonte canônica:** [registro A0216 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81a1aa7bdf4408d60fc9).
...
|---|---|
| Código | A0216 |
| Nome | Imbuimento de Terra |
| Domínio | ARCANE |
| Árvore | Principal — ARCANE ↔ MARTIAL/MINING |
| Ramo | Terra — Imbuimento Geomântico |
| Camada | 5 |
...
| Dependências Obrigatórias | A0212 Dano de Terra I ≥ 2 ranks + pelo menos uma mastery canônica de família melee elegível ≥20. Famílias ativas: epic_sword, epic_axe, epic_spear somente melee, epic_dagger, epic_hammer, combat_mace e combat_scythe; combat_fist somente após P-0032 e nunca mão vazia. |
| Pré-requisitos | A0212 ≥2 + mastery ≥20 na família melee usada. Ranged/projéteis excluídos; combat_fist fail-closed até P-0032. |
| Provider/Mods | GTBC's Geomancy Plus runtime 1.1.0-1.21.1 + Iron's Spells 'n Spellbooks 3.16.3 como framework e demais ações EARTH somente por adapter explícito + Epic Fight 21.17.3.1/classificador canônico de lanes melee. Weapons of Miracles 2.0.176 somente quando a arma estiver inequivocamente mapeada; combat_fist depende de P-0032. |
| Efeito | Após uma ação EARTH direta elegível, abre janela de Imbuimento por 120 ticks (6 s). Cada direct_melee_outcome legítimo com lane elegível e mastery ≥20 adiciona um único derived_component:EARTH igual a 5% do dano-base canônico elegível por rank: 5% / 10%. added_earth = base_weapon_damage_pre_target_mitigation_pre_critical_pre_added_elements × (0,05 × rank). |
| Escalonamento | Até 2 ranks; janela fixa 120 ticks; componente EARTH = 5% / 10% do dano-base elegível. Recast EARTH válido renova somente a expiração, sem empilhar magnitude. |
| Gate | A0212 ≥2 + janela ativa + direct_melee_outcome do próprio jogador + lane entre epic_sword, epic_axe, epic_spear melee, epic_dagger, epic_hammer, combat_mace, combat_scythe e mastery ≥20. combat_fist somente após P-0032. Ranged, projéteis, lança arremessada, mão vazia, sweep derivado não-direto, DoT, summons, automação, fake player e derived_component são inelegíveis. |
| Hook | Após commit da ação EARTH elegível, registrar expiração. Em cada outcome melee elegível, resolver lane/mastery, deduplicar outcome_id, calcular base pré-mitigação/pré-crítico/pré-elementos e anexar um único derived_component:EARTH ao mesmo evento. Herdar exatamente criticalHit e identidade causal do pai; nunca criar novo DamageSource, segunda resolução, mastery, sustain ou proc. |
| Fallback | Sem ação EARTH de origem explicitamente classificada, lane/mastery inequívoca ou composição no mesmo outcome, A0216 fica inativa. Não abrir janela por mineração, bloco/arma de pedra, grounded state ou dano físico genérico; não simular segundo ataque. combat_fist permanece fail-closed até P-0032. |
| Regra | PP_REGION: ARCANE_MARTIAL_MINING_EARTH_BRIDGE. BRIDGE_PP_POLICY: estes PP não contam simultaneamente para thresholds puros de ARCANE, MARTIAL e MINING; híbridos exigem gates completos dos pais. derived_component:EARTH pertence ao outcome melee pai, herda sua única decisão crítica e não cria segundo DamageSource, controle, impacto, mastery, sustain ou proc. Permanecer no solo/minerar não renova a janela. Ponte da Árvore Exterior; não é terminal. |
...
Após uma ação EARTH direta elegível, abre janela de Imbuimento por 120 ticks (6 s). Cada direct_melee_outcome legítimo com lane elegível e mastery ≥20 adiciona um único derived_component:EARTH igual a 5% do dano-base canônico elegível por rank: 5% / 10%. added_earth = base_weapon_damage_pre_target_mitigation_pre_critical_pre_added_elements × (0,05 × rank).

### Escalonamento aprovado

Até 2 ranks; janela fixa 120 ticks; componente EARTH = 5% / 10% do dano-base elegível. Recast EARTH válido renova somente a expiração, sem empilhar magnitude.

### Gate de compra/ativação

A0212 ≥2 + janela ativa + direct_melee_outcome do próprio jogador + lane entre epic_sword, epic_axe, epic_spear melee, epic_dagger, epic_hammer, combat_mace, combat_scythe e mastery ≥20. combat_fist somente após P-0032. Ranged, projéteis, lança arremessada, mão vazia, sweep derivado não-direto, DoT, summons, automação, fake player e derived_component são inelegíveis.
...
Sem ação EARTH de origem explicitamente classificada, lane/mastery inequívoca ou composição no mesmo outcome, A0216 fica inativa. Não abrir janela por mineração, bloco/arma de pedra, grounded state ou dano físico genérico; não simular segundo ataque. combat_fist permanece fail-closed até P-0032.
...
| Item | Decisão |
|---|---|
| Região | Principal — ARCANE ↔ MARTIAL/MINING / Terra — Imbuimento Geomântico |
| Camada e papel | Camada 5; Ponte |
| Pré-requisito visual/estrutural | A0212 ≥2 + mastery ≥20 na família melee usada. Ranged/projéteis excluídos; combat_fist fail-closed até P-0032. |
| Dependência semântica completa | A0212 Dano de Terra I ≥ 2 ranks + pelo menos uma mastery canônica de família melee elegível ≥20. Famílias ativas: epic_sword, epic_axe, epic_spear somente melee, epic_dagger, epic_hammer, combat_mace e combat_scythe; combat_fist somente após P-0032 e nunca mão vazia. |
| Custo topológico | 2 PP por rank; 2 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: ARCANE_MARTIAL_MINING_EARTH_BRIDGE. BRIDGE_PP_POLICY: estes PP não contam simultaneamente para thresholds puros de ARCANE, MARTIAL e MINING; híbridos exigem gates completos dos pais. derived_component:EARTH pertence ao outcome melee pai, herda sua única decisão crítica e não cria segundo DamageSource, controle, impacto, mastery, sustain ou proc. Permanecer no solo/minerar não renova a janela. Ponte da Árvore Exterior; não é terminal. |
...
GTBC's Geomancy Plus runtime 1.1.0-1.21.1 + Iron's Spells 'n Spellbooks 3.16.3 como framework e demais ações EARTH somente por adapter explícito + Epic Fight 21.17.3.1/classificador canônico de lanes melee. Weapons of Miracles 2.0.176 somente quando a arma estiver inequivocamente mapeada; combat_fist depende de P-0032.
...
| Projeto | Head auditado | Decisão para A0216 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0216.
...
- **Fallback normativo:** Sem ação EARTH de origem explicitamente classificada, lane/mastery inequívoca ou composição no mesmo outcome, A0216 fica inativa. Não abrir janela por mineração, bloco/arma de pedra, grounded state ou dano físico genérico; não simular segundo ataque. combat_fist permanece fail-closed até P-0032.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 2 ranks; janela fixa 120 ticks; componente EARTH = 5% / 10% do dano-base elegível. Recast EARTH válido renova somente a expiração, sem empilhar magnitude..
...
- Página: [A0216 — Imbuimento de Terra](https://app.notion.com/3c569db9f0db81a1aa7bdf4408d60fc9)
...
**Resultado final do Chat 1 para A0216:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
