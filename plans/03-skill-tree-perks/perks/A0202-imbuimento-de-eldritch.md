...
- **Fonte canônica:** [registro A0202 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db813da75fdf237582ef8f).
...
- **Dependências externas à faixa:** <code>A0198</code>. Elas permanecem sinalizadas e não são presumidas como concluídas.
...
|---|---|
| Código | A0202 |
| Nome | Imbuimento de Eldritch |
| Domínio | ARCANE |
| Árvore | Principal — ARCANE/OCCULT ↔ MARTIAL |
| Ramo | Eldritch — Imbuimento Profano |
| Camada | 5 |
...
| Dependências Obrigatórias | A0198 ≥2 + uma lane ELDRITCH exata e allowlisted ≥15 + mastery da lane melee efetivamente usada ≥20. Lanes melee canônicas: epicfight:sword, epicfight:axe, epicfight:spear (contato), epicfight:dagger, epicfight:heavy, combat:mace e combat:scythe; combat:fist somente quando sua cadeia estiver válida. IDs de gateway epic_* não são IDs de mastery. |
| Pré-requisitos | A0198 ≥2 + eldritch_mastery_lane_id exata ≥15 + uma mastery melee canônica da lista ≥20. A0198 é blocker externo; addons Iron's exigem normalização/catalogação de school id sem colisão. |
| Provider/Mods | Iron's 3.16.3 + Discerning The Eldritch 1.4.3 + Deeper and Darker: Spellbooks 1.3.3 somente após mapear SchoolType.getId() exato para lane MasteryLaneCatalog suportada; Epic Fight 21.17.3.1 e lanes combat:* canônicas; HealingResolver. Weapons of Miracles só por arma concreta mapeada. Black Arcana atual não fornece outcome ELDRITCH. |
| Efeito | Após uma ação/conjuração ELDRITCH direta elegível do jogador ser concluída com sucesso, A0202 abre uma janela de Imbuimento por 120 ticks (6 s). Durante a janela, cada direct_melee_outcome server-side legítimo com arma de lane elegível e mastery daquela lane ≥20 adiciona um único derived_component:ELDRITCH igual a 4% do dano-base canônico elegível por rank (4% / 8% / 12%): added_eldritch = base_weapon_damage_pre_target_mitigation_pre_critical_pre_added_elements × (0,04 × rank). Enquanto a janela estiver ativa, toda cura efetiva recebida pelo jogador é multiplicada por ×0,98 / ×0,96 / ×0,94 conforme rank. Benefício ofensivo e penalidade de cura são inseparáveis. |
| Escalonamento | Até 3 ranks; janela fixa 120 ticks. Componente ELDRITCH adicional = 4% / 8% / 12% do dano-base canônico do outcome melee. Cura efetiva recebida durante a janela = ×0,98 / ×0,96 / ×0,94. Recast ELDRITCH válido atualiza apenas a expiração; não empilha magnitude nem cria uma segunda instância da penalidade. |
| Gate | A0198 ≥2 disponível + lane ELDRITCH exata derivada do SchoolType.getId() e aceita por MasteryLaneCatalog ≥15 + lane melee canônica usada ≥20 + ação ELDRITCH direta concluída + HealingResolver server-side. O catálogo atual rejeita a forma namespace/escola produzida por addons Iron's; até corrigir e mapear IDs reais, A0202 é UNAVAILABLE_NODE/não comprável. combat:fist herda blockers próprios. |
| Hook | Após commit de cast/outcome ELDRITCH realmente bem-sucedido, criar/renovar estado único por 120 ticks; tentativa/cancelamento não abre janela. HealingResolver aplica ×0,98/0,96/0,94 uma vez. Em direct_melee_outcome, resolver lane canônica, deduplicar outcome_id e anexar um único derived_component:ELDRITCH sobre base pré-mitigação/pré-crítico/pré-added. Limpar em morte, logout, dimensão, rank/dependency loss, respec e rules reload. |
| Fallback | UNAVAILABLE_NODE enquanto faltar A0198, mapeamento exato da escola ELDRITCH, suporte do MasteryLaneCatalog a addon school id, producer de ação/outcome direto, hook melee ou HealingResolver. Benefício e penalidade de cura são inseparáveis; não substituir por mana, self-damage, segundo ataque ou tradeoff diferente. |
| Regra | PP_REGION: ARCANE_OCCULT_MARTIAL_ELDRITCH_BRIDGE. A Mastery não é a string genérica 'Eldritch Mastery': é a lane exata do school id allowlisted, sem agregar escolas diferentes. derived_component pertence ao outcome pai e não cria crítica, DamageSource, mastery, sustain ou proc. Availability é all-or-nothing para benefício+cura. Não terminal. |
...
Após uma ação/conjuração ELDRITCH direta elegível do jogador ser concluída com sucesso, A0202 abre uma janela de Imbuimento por 120 ticks (6 s). Durante a janela, cada direct_melee_outcome server-side legítimo com arma de lane elegível e mastery daquela lane ≥20 adiciona um único derived_component:ELDRITCH igual a 4% do dano-base canônico elegível por rank (4% / 8% / 12%): added_eldritch = base_weapon_damage_pre_target_mitigation_pre_critical_pre_added_elements × (0,04 × rank). Enquanto a janela estiver ativa, toda cura efetiva recebida pelo jogador é multiplicada por ×0,98 / ×0,96 / ×0,94 conforme rank. Benefício ofensivo e penalidade de cura são inseparáveis.

### Escalonamento aprovado

Até 3 ranks; janela fixa 120 ticks. Componente ELDRITCH adicional = 4% / 8% / 12% do dano-base canônico do outcome melee. Cura efetiva recebida durante a janela = ×0,98 / ×0,96 / ×0,94. Recast ELDRITCH válido atualiza apenas a expiração; não empilha magnitude nem cria uma segunda instância da penalidade.

### Gate de compra/ativação

A0198 ≥2 disponível + lane ELDRITCH exata derivada do SchoolType.getId() e aceita por MasteryLaneCatalog ≥15 + lane melee canônica usada ≥20 + ação ELDRITCH direta concluída + HealingResolver server-side. O catálogo atual rejeita a forma namespace/escola produzida por addons Iron's; até corrigir e mapear IDs reais, A0202 é UNAVAILABLE_NODE/não comprável. combat:fist herda blockers próprios.
...
UNAVAILABLE_NODE enquanto faltar A0198, mapeamento exato da escola ELDRITCH, suporte do MasteryLaneCatalog a addon school id, producer de ação/outcome direto, hook melee ou HealingResolver. Benefício e penalidade de cura são inseparáveis; não substituir por mana, self-damage, segundo ataque ou tradeoff diferente.
...
| Item | Decisão |
|---|---|
| Região | Principal — ARCANE/OCCULT ↔ MARTIAL / Eldritch — Imbuimento Profano |
| Camada e papel | Camada 5; Ponte |
| Pré-requisito visual/estrutural | A0198 ≥2 + eldritch_mastery_lane_id exata ≥15 + uma mastery melee canônica da lista ≥20. A0198 é blocker externo; addons Iron's exigem normalização/catalogação de school id sem colisão. |
| Dependência semântica completa | A0198 ≥2 + uma lane ELDRITCH exata e allowlisted ≥15 + mastery da lane melee efetivamente usada ≥20. Lanes melee canônicas: epicfight:sword, epicfight:axe, epicfight:spear (contato), epicfight:dagger, epicfight:heavy, combat:mace e combat:scythe; combat:fist somente quando sua cadeia estiver válida. IDs de gateway epic_* não são IDs de mastery. |
| Custo topológico | 1 PP por rank; 3 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: ARCANE_OCCULT_MARTIAL_ELDRITCH_BRIDGE. A Mastery não é a string genérica 'Eldritch Mastery': é a lane exata do school id allowlisted, sem agregar escolas diferentes. derived_component pertence ao outcome pai e não cria crítica, DamageSource, mastery, sustain ou proc. Availability é all-or-nothing para benefício+cura. Não terminal. |
...
Iron's 3.16.3 + Discerning The Eldritch 1.4.3 + Deeper and Darker: Spellbooks 1.3.3 somente após mapear SchoolType.getId() exato para lane MasteryLaneCatalog suportada; Epic Fight 21.17.3.1 e lanes combat:* canônicas; HealingResolver. Weapons of Miracles só por arma concreta mapeada. Black Arcana atual não fornece outcome ELDRITCH.
...
| Projeto | Head auditado | Decisão para A0202 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0202.
...
- **Fallback normativo:** UNAVAILABLE_NODE enquanto faltar A0198, mapeamento exato da escola ELDRITCH, suporte do MasteryLaneCatalog a addon school id, producer de ação/outcome direto, hook melee ou HealingResolver. Benefício e penalidade de cura são inseparáveis; não substituir por mana, self-damage, segundo ataque ou tradeoff diferente.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks; janela fixa 120 ticks. Componente ELDRITCH adicional = 4% / 8% / 12% do dano-base canônico do outcome melee. Cura efetiva recebida durante a janela = ×0,98 / ×0,96 / ×0,94. Recast ELDRITCH válido atualiza apenas a expiração; não empilha magnitude nem cria uma segunda instância da penalidade..
...
- **Dependências fora desta faixa:** <code>A0198</code>.
...
- Página: [A0202 — Imbuimento de Eldritch](https://app.notion.com/3c569db9f0db813da75fdf237582ef8f)
...
**Resultado final do Chat 1 para A0202:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
