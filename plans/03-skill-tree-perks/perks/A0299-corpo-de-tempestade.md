...
- **Fonte canônica:** [registro A0299 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db8106bc76e05891ba9723).
...
|---|---|
| Código | A0299 |
| Nome | Corpo de Tempestade |
| Domínio | ARCANE/LIGHTNING |
| Árvore | Especialista — Raio |
| Ramo | Eletromancia — Corpo de Tempestade |
| Camada | 6 |
...
| Dependências Obrigatórias | SPECIALIST_UNLOCK:LIGHTNING confirmado por Gate A/B/C + A0289 Resistência Elétrica =3 ranks + A0297 Passo de Relâmpago + Lightning Mastery ≥120 + pelo menos uma rota de ativação LIGHTNING tecnicamente operacional. Requisitos locais não substituem fundamentos LIGHTNING + ≥100 PP válidos em SPECIALIST_REGION:LIGHTNING + terminal A0176. |
| Pré-requisitos | Specialist Raio desbloqueada (SPECIALIST_UNLOCK:LIGHTNING) + A0289 3/3 + A0297 + Lightning Mastery ≥120 + rota defensiva e/ou ofensiva operacional. |
| Provider/Mods | RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + FUTURE_PROVIDER_CONTRACT HOSTILE_DAMAGE_RECEIPT_V1 para rota defensiva + A0288/A0297 para rota ofensiva + DAMAGE_MITIGATION_RESOLVER_V1 + TRANSIENT_ATTRIBUTE_MODIFIER_V1 + FUTURE_PROVIDER_CONTRACT opcional STAMINA_REGEN_MODIFIER_V1. Epic Fight 21.17.3.1 é provider de estamina apenas quando adapter expuser regen nativa positiva/modificável; ParCool não substitui estamina. Iron's/Ars apenas originam ações/danos LIGHTNING classificados. Oritech/Create/FE não ativam por serem tecnologia. |
| Efeito | A0299 possui duas rotas de ativação fora da recarga. Rota defensiva: após o jogador sofrer e sobreviver a um hostile_lightning_damage_outcome positivo, o estado começa DEPOIS do golpe gatilho. Rota ofensiva: enquanto RPG_LIGHTNING_STEP de A0297 estiver ativo, uma ação LIGHTNING direta que consuma com sucesso RPG_REFLEX_CHARGE de A0288 ativa o estado após o commit da ação. Em qualquer rota, RPG_STORM_BODY dura 100 ticks (5 s): aplica um mitigador LIGHTNING transitório distinto com multiplicador ×0,15 sobre o dano remanescente após RPG_LIGHTNING_RESISTANCE (equivalente a 85% de mitigação própria do estado), +18% de velocidade de movimento e ×1,10 somente sobre regeneração nativa positiva de estamina. Recarga compartilhada de 1200 ticks (60 s), iniciada na ativação. |
| Escalonamento | 1 rank. Duração: 100 ticks. Cooldown: 1200 ticks desde a ativação. Defesa transitória: canonical_modifier_id=RPG_STORM_BODY_LIGHTNING_MITIGATION, multiplicador de dano ×0,15, semanticamente distinto de RPG_LIGHTNING_RESISTANCE. Movimento: +18%. Stamina regen: ×1,10 somente se o provider expuser taxa nativa positiva/modificável; taxa zero permanece zero. |
| Gate | SPECIALIST_UNLOCK:LIGHTNING válido + requisitos locais + cooldown livre + pelo menos uma rota segura. DEFENSIVA: HOSTILE_DAMAGE_RECEIPT_V1 comprova componente LIGHTNING positivo realmente commitado ao jogador por fonte hostil externa elegível; auto-dano, owner/aliado, automação/máquina sem agressor hostil e callback duplicado não ativam. O golpe gatilho não recebe proteção retroativa. OFENSIVA: RPG_LIGHTNING_STEP de A0297 ativo + ação LIGHTNING DIRETA própria + claim/consumo real de RPG_REFLEX_CHARGE de A0288 no MESMO root action; ativação ocorre após commit da ação. Uma única recarga é compartilhada entre rotas. |
| Hook | Ambas as rotas chamam o mesmo activateStormBody() idempotente após seus commits causais. Criar uma única RPG_STORM_BODY(expires_at=now+100) e cooldown_expiry=now+1200; aplicar +18% movement por ID estável. Em outcomes LIGHTNING hostis POSTERIORES, resolver primeiro RPG_LIGHTNING_RESISTANCE e depois compor o mitigador semanticamente distinto RPG_STORM_BODY_LIGHTNING_MITIGATION×0,15. Se STAMINA_REGEN_MODIFIER_V1 existir, multiplicar somente regen nativa positiva de estamina por ×1,10; zero/bloqueada permanece zero. Expiração/respec/login/dimensão reconciliam todos os modificadores. |
| Fallback | FAIL-CLOSED se Specialist Raio não estiver desbloqueada ou nenhuma rota puder ser comprovada. Se uma rota estiver indisponível, preservar a outra quando segura. Sem stamina regen modificável, omitir somente ×1,10. Sem movimento idempotente ou mitigação segura, A0299 fica integralmente inativa porque ambos compõem a identidade do estado. Não adicionar +85 p.p. ao bucket permanente, não criar estamina, não retroagir ao golpe gatilho e não aceitar self/farm como hostilidade. |
| Regra | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Keystone INTERNA da Specialist Raio. Corpo de Tempestade dura 100 ticks e possui cooldown compartilhado de 1200 ticks desde a ativação. Sua defesa ×0,15 é bucket transitório DISTINTO e multiplicativo após RPG_LIGHTNING_RESISTANCE, não +85% aditivo nem imunidade. Movimento +18%; stamina regen é extensão opcional de provider. Ativações não geram Lightning Mastery por si. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
A0299 possui duas rotas de ativação fora da recarga. Rota defensiva: após o jogador sofrer e sobreviver a um hostile_lightning_damage_outcome positivo, o estado começa DEPOIS do golpe gatilho. Rota ofensiva: enquanto RPG_LIGHTNING_STEP de A0297 estiver ativo, uma ação LIGHTNING direta que consuma com sucesso RPG_REFLEX_CHARGE de A0288 ativa o estado após o commit da ação. Em qualquer rota, RPG_STORM_BODY dura 100 ticks (5 s): aplica um mitigador LIGHTNING transitório distinto com multiplicador ×0,15 sobre o dano remanescente após RPG_LIGHTNING_RESISTANCE (equivalente a 85% de mitigação própria do estado), +18% de velocidade de movimento e ×1,10 somente sobre regeneração nativa positiva de estamina. Recarga compartilhada de 1200 ticks (60 s), iniciada na ativação.
...
SPECIALIST_UNLOCK:LIGHTNING válido + requisitos locais + cooldown livre + pelo menos uma rota segura. DEFENSIVA: HOSTILE_DAMAGE_RECEIPT_V1 comprova componente LIGHTNING positivo realmente commitado ao jogador por fonte hostil externa elegível; auto-dano, owner/aliado, automação/máquina sem agressor hostil e callback duplicado não ativam. O golpe gatilho não recebe proteção retroativa. OFENSIVA: RPG_LIGHTNING_STEP de A0297 ativo + ação LIGHTNING DIRETA própria + claim/consumo real de RPG_REFLEX_CHARGE de A0288 no MESMO root action; ativação ocorre após commit da ação. Uma única recarga é compartilhada entre rotas.

### Hook e ordem de execução

Ambas as rotas chamam o mesmo activateStormBody() idempotente após seus commits causais. Criar uma única RPG_STORM_BODY(expires_at=now+100) e cooldown_expiry=now+1200; aplicar +18% movement por ID estável. Em outcomes LIGHTNING hostis POSTERIORES, resolver primeiro RPG_LIGHTNING_RESISTANCE e depois compor o mitigador semanticamente distinto RPG_STORM_BODY_LIGHTNING_MITIGATION×0,15. Se STAMINA_REGEN_MODIFIER_V1 existir, multiplicar somente regen nativa positiva de estamina por ×1,10; zero/bloqueada permanece zero. Expiração/respec/login/dimensão reconciliam todos os modificadores.
...
| Item | Decisão |
|---|---|
| Região | Especialista — Raio / Eletromancia — Corpo de Tempestade |
| Camada e papel | Camada 6; Keystone |
| Pré-requisito visual/estrutural | Specialist Raio desbloqueada (SPECIALIST_UNLOCK:LIGHTNING) + A0289 3/3 + A0297 + Lightning Mastery ≥120 + rota defensiva e/ou ofensiva operacional. |
| Dependência semântica completa | SPECIALIST_UNLOCK:LIGHTNING confirmado por Gate A/B/C + A0289 Resistência Elétrica =3 ranks + A0297 Passo de Relâmpago + Lightning Mastery ≥120 + pelo menos uma rota de ativação LIGHTNING tecnicamente operacional. Requisitos locais não substituem fundamentos LIGHTNING + ≥100 PP válidos em SPECIALIST_REGION:LIGHTNING + terminal A0176. |
| Custo topológico | 3 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Keystone INTERNA da Specialist Raio. Corpo de Tempestade dura 100 ticks e possui cooldown compartilhado de 1200 ticks desde a ativação. Sua defesa ×0,15 é bucket transitório DISTINTO e multiplicativo após RPG_LIGHTNING_RESISTANCE, não +85% aditivo nem imunidade. Movimento +18%; stamina regen é extensão opcional de provider. Ativações não geram Lightning Mastery por si. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + FUTURE_PROVIDER_CONTRACT HOSTILE_DAMAGE_RECEIPT_V1 para rota defensiva + A0288/A0297 para rota ofensiva + DAMAGE_MITIGATION_RESOLVER_V1 + TRANSIENT_ATTRIBUTE_MODIFIER_V1 + FUTURE_PROVIDER_CONTRACT opcional STAMINA_REGEN_MODIFIER_V1. Epic Fight 21.17.3.1 é provider de estamina apenas quando adapter expuser regen nativa positiva/modificável; ParCool não substitui estamina. Iron's/Ars apenas originam ações/danos LIGHTNING classificados. Oritech/Create/FE não ativam por serem tecnologia.
...
- **Contratos/capabilities nomeados no registro:** <code>SPECIALIST_GATE_RESOLVER_V1</code>, <code>HOSTILE_DAMAGE_RECEIPT_V1</code>, <code>DAMAGE_MITIGATION_RESOLVER_V1</code>, <code>TRANSIENT_ATTRIBUTE_MODIFIER_V1</code>, <code>STAMINA_REGEN_MODIFIER_V1</code>.
- **Estado:** nenhum nome de API é tratado como existente apenas por aparecer no design; FUTURE_PROVIDER_CONTRACT permanece bloqueador até prova em código/API da versão exata.

### Matriz dos quatro projetos próprios

| Projeto | Head auditado | Decisão para A0299 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>canonical_modifier_id</code>, <code>receipt</code>, <code>claim</code>.
...
- **Chaves que o adapter precisa preservar:** <code>canonical_modifier_id</code>, <code>receipt</code>, <code>claim</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0299.
...
- **Recursos/eixos tocados pelo contrato:** <code>STAMINA</code>.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Duração: 100 ticks. Cooldown: 1200 ticks desde a ativação. Defesa transitória: canonical_modifier_id=RPG_STORM_BODY_LIGHTNING_MITIGATION, multiplicador de dano ×0,15, semanticamente distinto de RPG_LIGHTNING_RESISTANCE. Movimento: +18%. Stamina regen: ×1,10 somente se o provider expuser taxa nativa positiva/modificável; taxa zero permanece zero..
...
6. **Causalidade:** correlacionar <code>canonical_modifier_id</code>, <code>receipt</code>, <code>claim</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
- **Capabilities/contracts a provar:** <code>SPECIALIST_GATE_RESOLVER_V1</code>, <code>HOSTILE_DAMAGE_RECEIPT_V1</code>, <code>DAMAGE_MITIGATION_RESOLVER_V1</code>, <code>TRANSIENT_ATTRIBUTE_MODIFIER_V1</code>, <code>STAMINA_REGEN_MODIFIER_V1</code>.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>canonical_modifier_id</code>, <code>receipt</code>, <code>claim</code>. |
...
- Página: [A0299 — Corpo de Tempestade](https://app.notion.com/3c569db9f0db8106bc76e05891ba9723)
...
**Resultado final do Chat 1 para A0299:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
