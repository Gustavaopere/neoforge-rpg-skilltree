# A0266 — Congelamento Progressivo
...
- **Fonte canônica:** [registro A0266 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81cdb69edb18c4f984e2).
...
| Hook | No ponto causal em que o provider calcula/aplica buildup, normalizar action_id/outcome_id, capturar a quantidade base positiva ANTES do write e agregar A0266 ao único canonical_modifier_id=RPG_FREEZE_BUILDUP_MULTIPLIER: +0,08/+0,16/+0,24. Resolver o bucket uma vez e entregar ao provider o delta final; no adapter Iron's, aplicar o delta final ao mesmo mecanismo nativo de freeze ticks. Não tocar getTicksRequiredToFreeze, threshold, decay, cap, estado Chilled ou temperatura corporal e não reprocessar polling de ticks congelados. |
| Fallback | FAIL-CLOSED se Specialist Gelo não estiver desbloqueada ou se a fonte não expuser uma contribuição causal positiva de freeze buildup que possa ser modificada com segurança. Não criar FREEZE_BUILDUP próprio, medidor, threshold, stun, Frozen artificial, Slowness, freeze ticks por polling ou bônus de dano como substituto. |
| Regra | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Perk interna da Specialist Gelo. A0266 modifica SOMENTE buildup real que a ação/provider já produz. Para Iron's, ticksFrozen é substrato nativo; A0266 modifica apenas o delta causal de ações allowlisted, nunca o valor total observado por tick. Não cria congelamento instantâneo, não reduz threshold, não gera mastery por tick e não converte CHILL, Slowness, Cold Sweat ou temperatura corporal em buildup. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Até 3 ranks. freeze_buildup_multiplier recebe contribuição aditiva de +0,08 / +0,16 / +0,24 sobre a quantidade positiva de buildup nativo/compatível daquela ação. Sem buildup nativo/compatível >0, contribuição = 0. Threshold, decay, cap e efeito de congelamento permanecem autoritativos do provider.

### Gate de compra/ativação

SPECIALIST_UNLOCK:ICE válido + A0265 ≥2 + ação ICE elegível atribuída ao jogador + contribuição positiva de freeze buildup REAL da própria ação exposta por FREEZE_BUILDUP_ADAPTER_V1. Para Iron's, isso significa delta causal positivo de freeze ticks no write da ação, não polling genérico de ticksFrozen. Tick passivo de CHILL/Slowness, ambiente, powder snow sem ação atribuível, temperatura corporal, derived component, summon, automação, callback duplicado e ação sem buildup não geram contribuição.

### Hook e ordem de execução

No ponto causal em que o provider calcula/aplica buildup, normalizar action_id/outcome_id, capturar a quantidade base positiva ANTES do write e agregar A0266 ao único canonical_modifier_id=RPG_FREEZE_BUILDUP_MULTIPLIER: +0,08/+0,16/+0,24. Resolver o bucket uma vez e entregar ao provider o delta final; no adapter Iron's, aplicar o delta final ao mesmo mecanismo nativo de freeze ticks. Não tocar getTicksRequiredToFreeze, threshold, decay, cap, estado Chilled ou temperatura corporal e não reprocessar polling de ticks congelados.
...
| Projeto | Head auditado | Decisão para A0266 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0266.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks. freeze_buildup_multiplier recebe contribuição aditiva de +0,08 / +0,16 / +0,24 sobre a quantidade positiva de buildup nativo/compatível daquela ação. Sem buildup nativo/compatível >0, contribuição = 0. Threshold, decay, cap e efeito de congelamento permanecem autoritativos do provider..
...
- Página: [A0266 — Congelamento Progressivo](https://app.notion.com/3c569db9f0db81cdb69edb18c4f984e2)
...
**Resultado final do Chat 1 para A0266:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
