...
**Runtime atual:** consumer/tag da perk ainda ausentes; availability deve permanecer fail-closed até implementação do Chat 2.  
...
A consolidação da PR #308 tornou Volcanoes um **subsistema nativo do mesmo JAR `rpgskilltree`**, mas não mudou sua authority semântica. `NativeVolcanoesServices` expõe somente consultas read-only de geologia, vulcanismo, tectônica, atmosfera e pressão; nenhuma dessas consultas classifica um `DamageSource` como elegível para A0103. O código histórico do repositório fonte Volcanoes permanece baseline de migração, não um segundo pipeline a ser executado em paralelo.
...
- não inferir elegibilidade por `NativeVolcanoesServices.atmosphereAt(...)` ou `atmosphericPressureAtm(...)`;
...
- RPG Skill Tree: `ProgressionService` governa gateway/PP; `DamageMitigationResolver` governa a única contribuição da perk; PR #308 internalizou Volcanoes sem transferir sua semântica para o resolver.
- Volcanoes nativo: pressão/gás/lava/calor/geologia permanecem provider-owned e não entram no allowlist atual; `NativeVolcanoesServices` é read-only.
...
9. Cobertura providers — PASS; projetos ambientais próprios e consolidação Volcanoes receberam disposição explícita.
...
- `P-A0103-05`: provar que a consolidação Volcanoes/`NativeVolcanoesServices` não cria classifier implícito nem segundo pipeline de mitigação.
