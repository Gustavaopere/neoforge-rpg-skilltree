...
- **Fonte canônica:** [registro A0291 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db814ba748ce8baa631578).
...
- **Dependências externas à faixa:** <code>A0145</code>, <code>A0176</code>. Elas permanecem sinalizadas e não são presumidas como concluídas.
...
| Ramo | Eletromancia — Eficiência Estática |
| Camada | 3 |
...
| Hook | Durante a ação, ROOT_ACTION_TARGET_LEDGER_V1 registra apenas targets nativos elegíveis por root_action_id, deduplicados por UUID. No fechamento autoritativo da ação, se unique_native_target_count>=3 e cooldown_expiry<=now, criar/renovar uma única RPG_STATIC_MANA(expires_at=now+80) e definir cooldown_expiry=now+140. Enquanto ativa, MANA_REGEN_MODIFIER_V1 multiplica SOMENTE regen nativa positiva do MESMO canal de mana do provider por ×1,04/×1,08/×1,12; regen zero/bloqueada permanece zero. Não escrever saldo e não reduzir custo. |
| Fallback | FAIL-CLOSED se Specialist Raio não estiver desbloqueada, o root action/targets não puderem ser agrupados com segurança ou não houver hook de regeneração nativa MANA. Não criar pulso/restituição fixa, regen paralela, Source/Soul/Spirit/FE, conversão de recurso ou contar procs RPG para satisfazer o limiar. |
| Regra | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna da Specialist Raio. Mana Estática recompensa multi-target REAL da ação original, não targets artificiais produzidos pelo Skill Tree. Janela=80 ticks; cooldown=140 desde a ativação; não força regen suspensa e não faz refill. O mesmo target conta uma vez por root. Lifecycle remove modificador órfão sem alterar saldo. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Quando um único root_action_id LIGHTNING legítimo do jogador registra pelo menos 3 alvos hostis distintos por seus componentes primários ou encadeamentos/área nativos do provider, A0291 ativa RPG_STATIC_MANA por 80 ticks (4 s): a regeneração nativa positiva de MANA daquele provider é multiplicada por ×1,04/×1,08/×1,12 conforme rank. Recarga interna de 140 ticks (7 s), iniciada na ativação. Regeneração nativa zero/bloqueada permanece zero.

### Escalonamento aprovado

Até 3 ranks. Janela: 80 ticks. Cooldown: 140 ticks desde a ativação. Multiplicador de regen nativa: 1,04/1,08/1,12. Contagem de alvo é por target_uuid único dentro do mesmo root_action_id; o mesmo alvo nunca conta duas vezes.

### Gate de compra/ativação

SPECIALIST_UNLOCK:LIGHTNING válido + requisitos locais + provider com MANA nativa positiva/modificável + root_action_id LIGHTNING legítimo encerrado com ≥3 target_uuid hostis únicos atingidos por componentes PRIMÁRIOS ou chain/AoE NATIVOS daquela ação. Mesmo alvo conta uma vez. A0290/A0292/A0293/A0295/A0296 e qualquer rpg_derived não contam; DoT posterior, máquina, summon, fake player, callback duplicado e targets sem outcome positivo também não.

### Hook e ordem de execução

Durante a ação, ROOT_ACTION_TARGET_LEDGER_V1 registra apenas targets nativos elegíveis por root_action_id, deduplicados por UUID. No fechamento autoritativo da ação, se unique_native_target_count>=3 e cooldown_expiry<=now, criar/renovar uma única RPG_STATIC_MANA(expires_at=now+80) e definir cooldown_expiry=now+140. Enquanto ativa, MANA_REGEN_MODIFIER_V1 multiplica SOMENTE regen nativa positiva do MESMO canal de mana do provider por ×1,04/×1,08/×1,12; regen zero/bloqueada permanece zero. Não escrever saldo e não reduzir custo.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Raio não estiver desbloqueada, o root action/targets não puderem ser agrupados com segurança ou não houver hook de regeneração nativa MANA. Não criar pulso/restituição fixa, regen paralela, Source/Soul/Spirit/FE, conversão de recurso ou contar procs RPG para satisfazer o limiar.
...
| Item | Decisão |
|---|---|
| Região | Especialista — Raio / Eletromancia — Eficiência Estática |
| Camada e papel | Camada 3; Ramo |
| Pré-requisito visual/estrutural | Specialist Raio desbloqueada (SPECIALIST_UNLOCK:LIGHTNING) + A0285 ≥2 + A0286 ≥1 + A0145 ≥2 + provider de mana com regen modificável. |
| Dependência semântica completa | SPECIALIST_UNLOCK:LIGHTNING confirmado por Gate A/B/C + A0285 Arco Elétrico ≥2 ranks + A0286 Sobrecarga ≥1 rank + A0145 Eficiência Arcana ≥2 ranks + provider com MANA nativa e regeneração positiva/modificável. Requisitos locais não substituem fundamentos LIGHTNING + ≥100 PP válidos em SPECIALIST_REGION:LIGHTNING + terminal A0176. |
| Custo topológico | 1 PP por rank; 3 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna da Specialist Raio. Mana Estática recompensa multi-target REAL da ação original, não targets artificiais produzidos pelo Skill Tree. Janela=80 ticks; cooldown=140 desde a ativação; não força regen suspensa e não faz refill. O mesmo target conta uma vez por root. Lifecycle remove modificador órfão sem alterar saldo. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0291 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0291.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Raio não estiver desbloqueada, o root action/targets não puderem ser agrupados com segurança ou não houver hook de regeneração nativa MANA. Não criar pulso/restituição fixa, regen paralela, Source/Soul/Spirit/FE, conversão de recurso ou contar procs RPG para satisfazer o limiar.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks. Janela: 80 ticks. Cooldown: 140 ticks desde a ativação. Multiplicador de regen nativa: 1,04/1,08/1,12. Contagem de alvo é por target_uuid único dentro do mesmo root_action_id; o mesmo alvo nunca conta duas vezes..
...
- Página: [A0291 — Mana Estática](https://app.notion.com/3c569db9f0db814ba748ce8baa631578)
...
**Resultado final do Chat 1 para A0291:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
