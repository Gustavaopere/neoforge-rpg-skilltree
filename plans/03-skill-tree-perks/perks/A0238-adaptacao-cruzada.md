...
- **Fonte canônica:** [registro A0238 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db8149a2a1e52b37809708).
...
| Dependências Obrigatórias | A0236 Memória Elemental + pelo menos 6 pontos somados em resistências de 3 elementos distintos. Uma única resistência profunda não satisfaz o requisito. |
| Pré-requisitos | A0236 Memória Elemental + investimento defensivo em pelo menos 3 elementos distintos. |
| Provider/Mods | RPG Skill Tree + FUTURE_PROVIDER_CONTRACTS ELEMENT_SIGNATURE_REGISTRY_V1 e ELEMENTAL_DIVERSITY_LEDGER_V1 + DamageMitigationResolver + adapters versionados de duração de estado elemental quando realmente exposta. O ledger defensivo usa ELEMENT_DEFENSE_PP[element_id]; shared/general/bridge PP são excluídos por padrão e cada node conta uma única vez para um único elemento. VERSION-STATUS: designs congelados, contratos não encontrados no runtime/main atual em 2026-08-29. |
| Efeito | Ao sofrer dano hostil direto elemental com element_signature_id reconhecida, A0238 arma Adaptação Cruzada por 160 ticks (8 s). O próximo dano hostil direto elemental de assinatura diferente durante a janela é multiplicado por ×0,80 e consome o estado. Se esse mesmo outcome aplicar um estado elemental real, hostil, removível e modificável allowlisted pelo adapter, sua duração-base é multiplicada por ×0,85. Após consumo bem-sucedido, recarga interna de 200 ticks (10 s). |
| Escalonamento | 1 rank. Janela: 160 ticks. Segundo elemento hostil distinto: componente elemental ×0,80. Estado elemental allowlisted do mesmo outcome: duração-base ×0,85. Recarga: 200 ticks após consumo. |
| Gate | A0236 + ELEMENTAL_DIVERSITY_LEDGER_V1 comprovando ≥6 PP defensivos válidos em ≥3 element_id distintos + dois outcomes hostis diretos com assinaturas primárias canônicas diferentes. PP shared/general/bridge, a mesma compra reciclada em vários elementos, dano autoinfligido, custos, ambiente sem fonte hostil, fonte ambígua, DoT derivado e callback duplicado não ativam. |
| Hook | Registrar a primeira assinatura por 160 ticks. No segundo outcome hostil direto distinto, aplicar ×0,80 como mitigador transitório no DamageMitigationResolver; se o mesmo outcome aplicar estado elemental allowlisted, ajustar somente sua duração-base na criação/renovação por ×0,85; consumir e iniciar cooldown now+200 após commit. |
| Fallback | FAIL-CLOSED: sem registry/ledger ou sem rastreamento seguro das assinaturas, A0238 fica inativa. Se somente a duração de estado não puder ser modificada, preservar a mitigação ×0,80 e omitir ×0,85 de duração. Não inferir estado elemental por debuff genérico, provider ou nome. |
| Regra | PP_REGION: ELEMENTAL_SHARED_DEFENSE. DIVERSITY_LEDGER: requisito 6 PP/3 elementos usa apenas ELEMENT_DEFENSE_PP[element_id] element-specific; shared/general/bridge valem zero por padrão e uma compra não preenche múltiplas escolas. A primeira ameaça memoriza uma assinatura primária; somente segunda ameaça hostil direta de assinatura distinta pode consumir ×0,80. Redução de duração atua apenas na duração-base de estado elemental real, hostil, removível e modificável aplicado/renovado pelo mesmo outcome, nunca por polling/tick. Não altera Afinidade térmica. Registry/ledger são contratos futuros, não APIs runtime existentes. |
...
Ao sofrer dano hostil direto elemental com element_signature_id reconhecida, A0238 arma Adaptação Cruzada por 160 ticks (8 s). O próximo dano hostil direto elemental de assinatura diferente durante a janela é multiplicado por ×0,80 e consome o estado. Se esse mesmo outcome aplicar um estado elemental real, hostil, removível e modificável allowlisted pelo adapter, sua duração-base é multiplicada por ×0,85. Após consumo bem-sucedido, recarga interna de 200 ticks (10 s).

### Escalonamento aprovado

1 rank. Janela: 160 ticks. Segundo elemento hostil distinto: componente elemental ×0,80. Estado elemental allowlisted do mesmo outcome: duração-base ×0,85. Recarga: 200 ticks após consumo.

### Gate de compra/ativação

A0236 + ELEMENTAL_DIVERSITY_LEDGER_V1 comprovando ≥6 PP defensivos válidos em ≥3 element_id distintos + dois outcomes hostis diretos com assinaturas primárias canônicas diferentes. PP shared/general/bridge, a mesma compra reciclada em vários elementos, dano autoinfligido, custos, ambiente sem fonte hostil, fonte ambígua, DoT derivado e callback duplicado não ativam.

### Hook e ordem de execução

Registrar a primeira assinatura por 160 ticks. No segundo outcome hostil direto distinto, aplicar ×0,80 como mitigador transitório no DamageMitigationResolver; se o mesmo outcome aplicar estado elemental allowlisted, ajustar somente sua duração-base na criação/renovação por ×0,85; consumir e iniciar cooldown now+200 após commit.

### Fallback sem trocar a identidade

FAIL-CLOSED: sem registry/ledger ou sem rastreamento seguro das assinaturas, A0238 fica inativa. Se somente a duração de estado não puder ser modificada, preservar a mitigação ×0,80 e omitir ×0,85 de duração. Não inferir estado elemental por debuff genérico, provider ou nome.
...
| Camada e papel | Camada 6; Notable |
| Pré-requisito visual/estrutural | A0236 Memória Elemental + investimento defensivo em pelo menos 3 elementos distintos. |
| Dependência semântica completa | A0236 Memória Elemental + pelo menos 6 pontos somados em resistências de 3 elementos distintos. Uma única resistência profunda não satisfaz o requisito. |
| Custo topológico | 2 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: ELEMENTAL_SHARED_DEFENSE. DIVERSITY_LEDGER: requisito 6 PP/3 elementos usa apenas ELEMENT_DEFENSE_PP[element_id] element-specific; shared/general/bridge valem zero por padrão e uma compra não preenche múltiplas escolas. A primeira ameaça memoriza uma assinatura primária; somente segunda ameaça hostil direta de assinatura distinta pode consumir ×0,80. Redução de duração atua apenas na duração-base de estado elemental real, hostil, removível e modificável aplicado/renovado pelo mesmo outcome, nunca por polling/tick. Não altera Afinidade térmica. Registry/ledger são contratos futuros, não APIs runtime existentes. |
...
RPG Skill Tree + FUTURE_PROVIDER_CONTRACTS ELEMENT_SIGNATURE_REGISTRY_V1 e ELEMENTAL_DIVERSITY_LEDGER_V1 + DamageMitigationResolver + adapters versionados de duração de estado elemental quando realmente exposta. O ledger defensivo usa ELEMENT_DEFENSE_PP[element_id]; shared/general/bridge PP são excluídos por padrão e cada node conta uma única vez para um único elemento. VERSION-STATUS: designs congelados, contratos não encontrados no runtime/main atual em 2026-08-29.
...
| Projeto | Head auditado | Decisão para A0238 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0238.
...
- **Fallback normativo:** FAIL-CLOSED: sem registry/ledger ou sem rastreamento seguro das assinaturas, A0238 fica inativa. Se somente a duração de estado não puder ser modificada, preservar a mitigação ×0,80 e omitir ×0,85 de duração. Não inferir estado elemental por debuff genérico, provider ou nome.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Janela: 160 ticks. Segundo elemento hostil distinto: componente elemental ×0,80. Estado elemental allowlisted do mesmo outcome: duração-base ×0,85. Recarga: 200 ticks após consumo..
...
- Página: [A0238 — Adaptação Cruzada](https://app.notion.com/3c569db9f0db8149a2a1e52b37809708)
...
**Resultado final do Chat 1 para A0238:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
