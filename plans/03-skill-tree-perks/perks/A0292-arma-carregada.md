...
- **Fonte canônica:** [registro A0292 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db811ab1aff5490c09b7db).
...
- **Dependências externas à faixa:** <code>A0174</code>, <code>A0176</code>. Elas permanecem sinalizadas e não são presumidas como concluídas.
...
| Hook | No commit do MESMO outcome de arma imbuída, reutilizar lane/mastery/criticalHit já resolvidos e snapshotar somente o componente LIGHTNING canônico pré-mitigação do alvo original. Executar uma única rolagem A0292 de 8/16/24% por weapon_root_action_id. Em sucesso, selecionar no máximo um segundo alvo hostil realmente atacável em raio 4 e LOS, excluindo visited targets, e emitir derived_lightning_weapon_discharge com base snapshot×0,35, criticalHit=false. O novo alvo resolve A0283/WET e mitigação própria; a descarga não reentra em A0292/A0284/A0151 nem gera mastery. |
| Fallback | FAIL-CLOSED se Specialist Raio não estiver desbloqueada, A0174 não puder resolver o mesmo outcome/lane/mastery, o componente LIGHTNING não puder ser identificado causalmente ou a seleção/emissão derivada não for segura. Não substituir por dano extra no alvo original, físico extra, AoE genérica, CHARGED gratuito ou gate MARTIAL aproximado. |
| Regra | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna/ponte Spellblade da Specialist Raio. A0292 reutiliza o MESMO outcome de A0174; MARTIAL ≥6 legado foi removido porque a identidade marcial já é definida por lane+mastery canônicas. A descarga é derivada: não pode critar, rerrolar A0292/A0284, aplicar CHARGED, gerar Lightning/weapon Mastery ou sustain. PP de ponte não servem de atalho para Gate A/B/C da Specialist. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Até 3 ranks. Chance: 8%/16%/24% por weapon_root_action_id. Um único segundo alvo, raio 4 blocos, linha de visão e filtros hostis normais. Base da descarga = 0,35 × direct_lightning_component_snapshot. A descarga não herda o dano físico da arma e criticalHit=false mesmo que o golpe original tenha sido crítico.

### Gate de compra/ativação

SPECIALIST_UNLOCK:LIGHTNING válido + A0174 ≥2 + A0284 ≥1 + direct_melee_outcome que satisfaça INTEGRALMENTE o gate atual de A0174 e contenha seu componente LIGHTNING positivo + ainda não rolled para o mesmo weapon_root_action_id. Não existe gate adicional MARTIAL ≥6. Ranged, projétil, mão vazia, lança arremessada, derived hit, DoT, summon, automação, fake player e callback duplicado não rerrolam.

### Hook e ordem de execução

No commit do MESMO outcome de arma imbuída, reutilizar lane/mastery/criticalHit já resolvidos e snapshotar somente o componente LIGHTNING canônico pré-mitigação do alvo original. Executar uma única rolagem A0292 de 8/16/24% por weapon_root_action_id. Em sucesso, selecionar no máximo um segundo alvo hostil realmente atacável em raio 4 e LOS, excluindo visited targets, e emitir derived_lightning_weapon_discharge com base snapshot×0,35, criticalHit=false. O novo alvo resolve A0283/WET e mitigação própria; a descarga não reentra em A0292/A0284/A0151 nem gera mastery.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Raio não estiver desbloqueada, A0174 não puder resolver o mesmo outcome/lane/mastery, o componente LIGHTNING não puder ser identificado causalmente ou a seleção/emissão derivada não for segura. Não substituir por dano extra no alvo original, físico extra, AoE genérica, CHARGED gratuito ou gate MARTIAL aproximado.
...
| Item | Decisão |
|---|---|
| Região | Especialista — Raio / Eletromancia — Spellblade |
| Camada e papel | Camada 3; Ponte |
| Pré-requisito visual/estrutural | Specialist Raio desbloqueada (SPECIALIST_UNLOCK:LIGHTNING) + A0174 Imbuimento de Raio ≥2 + A0284 Carga ≥1. A família melee/mastery é a mesma resolvida por A0174. |
| Dependência semântica completa | SPECIALIST_UNLOCK:LIGHTNING confirmado por Gate A/B/C + A0174 Imbuimento de Raio ≥2 ranks + A0284 Carga ≥1 rank. O antigo requisito vago MARTIAL ≥6 foi removido: a qualificação marcial é herdada integralmente do gate atual de A0174, que exige família melee canônica elegível + mastery da própria lane ≥20. Requisitos locais não substituem fundamentos LIGHTNING + ≥100 PP válidos em SPECIALIST_REGION:LIGHTNING + terminal A0176. |
| Custo topológico | 1 PP por rank; 3 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna/ponte Spellblade da Specialist Raio. A0292 reutiliza o MESMO outcome de A0174; MARTIAL ≥6 legado foi removido porque a identidade marcial já é definida por lane+mastery canônicas. A descarga é derivada: não pode critar, rerrolar A0292/A0284, aplicar CHARGED, gerar Lightning/weapon Mastery ou sustain. PP de ponte não servem de atalho para Gate A/B/C da Specialist. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0292 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>action_id</code>, <code>root_action_id</code>.
...
- **Chaves que o adapter precisa preservar:** <code>action_id</code>, <code>root_action_id</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0292.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Raio não estiver desbloqueada, A0174 não puder resolver o mesmo outcome/lane/mastery, o componente LIGHTNING não puder ser identificado causalmente ou a seleção/emissão derivada não for segura. Não substituir por dano extra no alvo original, físico extra, AoE genérica, CHARGED gratuito ou gate MARTIAL aproximado.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks. Chance: 8%/16%/24% por weapon_root_action_id. Um único segundo alvo, raio 4 blocos, linha de visão e filtros hostis normais. Base da descarga = 0,35 × direct_lightning_component_snapshot. A descarga não herda o dano físico da arma e criticalHit=false mesmo que o golpe original tenha sido crítico..
...
6. **Causalidade:** correlacionar <code>action_id</code>, <code>root_action_id</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>action_id</code>, <code>root_action_id</code>. |
...
- Página: [A0292 — Arma Carregada](https://app.notion.com/3c569db9f0db811ab1aff5490c09b7db)
...
**Resultado final do Chat 1 para A0292:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
