...
- **Fonte canônica:** [registro A0295 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81b9804adc2e74907c29).
...
| Ramo | Eletromancia — Campo Estático |
| Camada | 4 |
...
| Hook | Manter por jogador os cinco últimos créditos diretos {action_id/root_action_id,timestamp} e remover >80 ticks. Ao atingir cinco, limpar preparação, criar uma única RPG_STATIC_FIELD(instance_id,expires_at=now+80/120) e cooldown_expiry=now+360. A cada 20 ticks, COMBAT_TARGET_QUERY_V1 consulta entidades vivas realmente atacáveis em raio 4 NO MESMO level/sublevel do jogador, respeitando PvP, equipes/alianças, invulnerabilidade, blacklist e filtros do pipeline. Para cada alvo, se last_field_charge[target] <= now-40, inserir exatamente 1 stack do MESMO owner via CHARGED_STATE_LEDGER_V1, respeitando cap 3 e TTL individual; dedupe=field_instance_id+pulse_index+target_uuid. O pulso não é outcome de dano e não executa RNG A0284. |
| Fallback | FAIL-CLOSED se Specialist Raio não estiver desbloqueada, CHARGED_STATE_LEDGER_V1 não garantir owner/cap/TTL ou a consulta espacial/targetability não for segura. Em Sable/Aeronautics, incapacidade de resolver o mesmo sublevel também fecha o pulso. Não converter o campo em bônus de chance A0284, dano AoE, Speed, FE/energia, aura que afete aliados ou outro efeito pessoal. |
| Regra | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Notable interna da Specialist Raio. Pulsos NÃO causam dano, não rolam A0284, não geram Lightning Mastery, crítico, Mana Estática, nova preparação ou A0294 diretamente. A stack criada é CHARGED real do mesmo owner e pode ser consumida posteriormente por A0294 em uma ação elegível; o pulso em si nunca ativa A0294. Um alvo recebe no máximo uma stack do campo a cada 40 ticks. Morte/logout/dimensão/respec encerram campo/preparação; Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Ao registrar 5 direct_lightning_action_id distintas do próprio jogador dentro de uma janela móvel de 80 ticks (4 s), com a recarga livre, A0295 limpa a preparação e ativa RPG_STATIC_FIELD por 80/120 ticks (4/6 s). A cada 20 ticks, em raio de 4 blocos, o campo pode adicionar exatamente 1 stack CHARGED atribuída ao jogador a cada alvo hostil elegível; o mesmo alvo só pode receber nova stack deste campo após 40 ticks. Os pulsos não causam dano.

### Escalonamento aprovado

Até 2 ranks. Preparação: 5 ações LIGHTNING diretas distintas em até 80 ticks. Campo: 80/120 ticks. Pulso: 20 ticks. Raio: 4 blocos. Intervalo por alvo para stack derivada do campo: 40 ticks. Cooldown por jogador: 360 ticks (18 s), iniciado na ativação. Campo/preparação não acumulam nem rearmam durante campo/recarga.

### Gate de compra/ativação

SPECIALIST_UNLOCK:LIGHTNING válido + A0284 ≥2 + A0287 ≥1 + cinco ações LIGHTNING DIRETAS distintas do jogador em até 80 ticks + cooldown livre. Máximo um crédito por direct_lightning_action_id/root_action_id; DoT, chain hop, branch, weapon discharge, overload, campo anterior, aura, summon, automação, fake player e callbacks repetidos não avançam preparação. Preparação/campo/recarga não se acumulam nem rearmam entre si.

### Hook e ordem de execução

Manter por jogador os cinco últimos créditos diretos {action_id/root_action_id,timestamp} e remover >80 ticks. Ao atingir cinco, limpar preparação, criar uma única RPG_STATIC_FIELD(instance_id,expires_at=now+80/120) e cooldown_expiry=now+360. A cada 20 ticks, COMBAT_TARGET_QUERY_V1 consulta entidades vivas realmente atacáveis em raio 4 NO MESMO level/sublevel do jogador, respeitando PvP, equipes/alianças, invulnerabilidade, blacklist e filtros do pipeline. Para cada alvo, se last_field_charge[target] <= now-40, inserir exatamente 1 stack do MESMO owner via CHARGED_STATE_LEDGER_V1, respeitando cap 3 e TTL individual; dedupe=field_instance_id+pulse_index+target_uuid. O pulso não é outcome de dano e não executa RNG A0284.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Raio não estiver desbloqueada, CHARGED_STATE_LEDGER_V1 não garantir owner/cap/TTL ou a consulta espacial/targetability não for segura. Em Sable/Aeronautics, incapacidade de resolver o mesmo sublevel também fecha o pulso. Não converter o campo em bônus de chance A0284, dano AoE, Speed, FE/energia, aura que afete aliados ou outro efeito pessoal.
...
| Item | Decisão |
|---|---|
| Região | Especialista — Raio / Eletromancia — Campo Estático |
| Camada e papel | Camada 4; Notable |
| Pré-requisito visual/estrutural | Specialist Raio desbloqueada (SPECIALIST_UNLOCK:LIGHTNING) + A0284 Carga ≥2 + A0287 Impulso Elétrico ≥1. |
| Dependência semântica completa | SPECIALIST_UNLOCK:LIGHTNING confirmado por Gate A/B/C + A0284 Carga ≥2 ranks + A0287 Impulso Elétrico ≥1 rank. Requisitos locais não substituem fundamentos LIGHTNING + ≥100 PP válidos em SPECIALIST_REGION:LIGHTNING + terminal A0176. |
| Custo topológico | 2 PP por rank; 2 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Notable interna da Specialist Raio. Pulsos NÃO causam dano, não rolam A0284, não geram Lightning Mastery, crítico, Mana Estática, nova preparação ou A0294 diretamente. A stack criada é CHARGED real do mesmo owner e pode ser consumida posteriormente por A0294 em uma ação elegível; o pulso em si nunca ativa A0294. Um alvo recebe no máximo uma stack do campo a cada 40 ticks. Morte/logout/dimensão/respec encerram campo/preparação; Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0295 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>action_id</code>, <code>root_action_id</code>, <code>target_uuid</code>, <code>ledger</code>.
...
- **Chaves que o adapter precisa preservar:** <code>action_id</code>, <code>root_action_id</code>, <code>target_uuid</code>, <code>ledger</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0295.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Raio não estiver desbloqueada, CHARGED_STATE_LEDGER_V1 não garantir owner/cap/TTL ou a consulta espacial/targetability não for segura. Em Sable/Aeronautics, incapacidade de resolver o mesmo sublevel também fecha o pulso. Não converter o campo em bônus de chance A0284, dano AoE, Speed, FE/energia, aura que afete aliados ou outro efeito pessoal.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 2 ranks. Preparação: 5 ações LIGHTNING diretas distintas em até 80 ticks. Campo: 80/120 ticks. Pulso: 20 ticks. Raio: 4 blocos. Intervalo por alvo para stack derivada do campo: 40 ticks. Cooldown por jogador: 360 ticks (18 s), iniciado na ativação. Campo/preparação não acumulam nem rearmam durante campo/recarga..
...
6. **Causalidade:** correlacionar <code>action_id</code>, <code>root_action_id</code>, <code>target_uuid</code>, <code>ledger</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>action_id</code>, <code>root_action_id</code>, <code>target_uuid</code>, <code>ledger</code>. |
...
- Página: [A0295 — Campo Estático](https://app.notion.com/3c569db9f0db81b9804adc2e74907c29)
...
**Resultado final do Chat 1 para A0295:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
