...
- **Fonte canônica:** [registro A0287 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81c0ae1ffd810a31854b).
...
| Hook | No commit do outcome LIGHTNING direto, deduplicar action_id/outcome_id, revalidar rank atual e criar/renovar uma única instância RPG_LIGHTNING_IMPULSE com expires_at=now+50. Aplicar um modificador de movement_speed com ID canônico estável e magnitude +4/+8/+12%; renovação apenas move o expiry, não soma modificadores. Remover imediatamente em expiração, respec, perda do Specialist, morte/logout/dimensão conforme lifecycle; login/reload deve reconciliar e eliminar modificador órfão. |
| Fallback | FAIL-CLOSED se Specialist Raio não estiver desbloqueada ou se o modificador temporário de velocidade não puder ser aplicado/removido idempotentemente. Não substituir por sprint forçado, teleporte, velocity packet, Speed potion permanente ou alteração direta de movimento base. |
| Regra | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna da Specialist Raio. A0287 possui uma única instância, magnitude definida pelo rank atual e duração de 50 ticks desde a última ação LIGHTNING direta elegível. Efeitos derivados nunca renovam; movimentar-se não gera Lightning Mastery ou novo crédito. Lifecycle deve impedir qualquer atributo órfão após respec/login/dimensão. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Após um direct_lightning_outcome_id elegível do jogador causar componente LIGHTNING positivo, A0287 aplica RPG_LIGHTNING_IMPULSE por 50 ticks (2,5 s): +4% de velocidade de movimento por rank (+4/+8/+12%). O estado possui uma única instância; nova ação direta distinta durante a janela apenas redefine expires_at=now+50, sem somar magnitude.

### Escalonamento aprovado

Até 3 ranks. Movimento: +4%/+8%/+12%. Duração fixa: 50 ticks a partir do último direct_lightning_outcome_id elegível. Não existe cooldown separado de renovação: a fronteira anti-spam é a deduplicação por action_id/outcome_id; callbacks/subeventos da mesma ação nunca renovam duas vezes.

### Gate de compra/ativação

SPECIALIST_UNLOCK:LIGHTNING válido + A0284 ≥1 + direct_lightning_outcome_id atribuído ao jogador + componente LIGHTNING direto positivo. Chain hop, ramificação, aura/campo, DoT, summon, automação, fake player, derived component e callback duplicado não criam nem renovam RPG_LIGHTNING_IMPULSE. Exatamente uma renovação por ação direta distinta.

### Hook e ordem de execução

No commit do outcome LIGHTNING direto, deduplicar action_id/outcome_id, revalidar rank atual e criar/renovar uma única instância RPG_LIGHTNING_IMPULSE com expires_at=now+50. Aplicar um modificador de movement_speed com ID canônico estável e magnitude +4/+8/+12%; renovação apenas move o expiry, não soma modificadores. Remover imediatamente em expiração, respec, perda do Specialist, morte/logout/dimensão conforme lifecycle; login/reload deve reconciliar e eliminar modificador órfão.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Raio não estiver desbloqueada ou se o modificador temporário de velocidade não puder ser aplicado/removido idempotentemente. Não substituir por sprint forçado, teleporte, velocity packet, Speed potion permanente ou alteração direta de movimento base.
...
| Pré-requisito visual/estrutural | Specialist Raio desbloqueada (SPECIALIST_UNLOCK:LIGHTNING) + A0284 Carga ≥1 rank. |
| Dependência semântica completa | SPECIALIST_UNLOCK:LIGHTNING confirmado por Gate A/B/C + A0284 Carga ≥1 rank. Requisitos locais não substituem fundamentos LIGHTNING + ≥100 PP válidos em SPECIALIST_REGION:LIGHTNING + terminal A0176. |
| Custo topológico | 1 PP por rank; 3 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna da Specialist Raio. A0287 possui uma única instância, magnitude definida pelo rank atual e duração de 50 ticks desde a última ação LIGHTNING direta elegível. Efeitos derivados nunca renovam; movimentar-se não gera Lightning Mastery ou novo crédito. Lifecycle deve impedir qualquer atributo órfão após respec/login/dimensão. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0287 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0287.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Raio não estiver desbloqueada ou se o modificador temporário de velocidade não puder ser aplicado/removido idempotentemente. Não substituir por sprint forçado, teleporte, velocity packet, Speed potion permanente ou alteração direta de movimento base.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks. Movimento: +4%/+8%/+12%. Duração fixa: 50 ticks a partir do último direct_lightning_outcome_id elegível. Não existe cooldown separado de renovação: a fronteira anti-spam é a deduplicação por action_id/outcome_id; callbacks/subeventos da mesma ação nunca renovam duas vezes..
...
- Página: [A0287 — Impulso Elétrico](https://app.notion.com/3c569db9f0db81c0ae1ffd810a31854b)
...
**Resultado final do Chat 1 para A0287:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
