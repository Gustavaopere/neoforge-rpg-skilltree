...
- **Fonte canônica:** [registro A0244 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81c69e2ac65125adb15e).
...
| Ramo | Piromancia — Ignição e Combustão |
| Camada | 2 |
...
| Hook | No tick autoritativo do estado de Ignição, identificar ignition_state_id/source_action_id/owner, deduplicar a resolução daquele tick e aplicar a camada A0244 somente ao parcel de dano nativo do estado. Não rerodar Potência Mágica, A0156, crítico ou modifiers do impacto inicial. |
| Fallback | FAIL-CLOSED se Specialist Fire não estiver desbloqueada ou se autoria/correlação da Ignição não for confiável. Não presumir combustão vanilla pela entidade estar em chamas e não rerodar multiplicadores do impacto inicial. |
| Regra | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Ticks de Ignição recebem somente a camada periódica A0244 uma vez; não reaplicam Potência Mágica, crítico, modifiers do hit inicial, nova Ignição, mastery, Ímpeto ou procs de ação direta. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
+6% de dano do estado real de Ignição atribuído ao jogador por rank (×1,06 / ×1,12 / ×1,18 sobre o parcel de dano próprio do estado). A0244 modifica somente o tick pertencente à própria Ignição e nunca reaplica os modificadores gerais/diretos que já foram resolvidos no evento que originou o estado.

### Escalonamento aprovado

Até 3 ranks; multiplicador do parcel de dano periódico da Ignição elegível: ×1,06 / ×1,12 / ×1,18. Teto próprio: +18%. Frequência de ticks e duração não são alteradas por A0244.

### Gate de compra/ativação

SPECIALIST_UNLOCK:FIRE válido + A0243 ≥2 + tick de estado FIRE de Ignição real com autoria do jogador preservada por FIRE_IGNITION_RESOLVER_V1/adapter equivalente. Fogo ambiental, estado sem owner/correlação, DoT não relacionado e efeito derivado de outra origem são inelegíveis.

### Hook e ordem de execução

No tick autoritativo do estado de Ignição, identificar ignition_state_id/source_action_id/owner, deduplicar a resolução daquele tick e aplicar a camada A0244 somente ao parcel de dano nativo do estado. Não rerodar Potência Mágica, A0156, crítico ou modifiers do impacto inicial.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Fire não estiver desbloqueada ou se autoria/correlação da Ignição não for confiável. Não presumir combustão vanilla pela entidade estar em chamas e não rerodar multiplicadores do impacto inicial.
...
| Pré-requisito visual/estrutural | Specialist Fire desbloqueada (SPECIALIST_UNLOCK:FIRE) + A0243 Ignição rank ≥2. |
| Dependência semântica completa | SPECIALIST_UNLOCK:FIRE confirmado por Gate A/B/C + A0243 Ignição rank ≥2. A0243/local topology não substitui o unlock global da Specialist Fire. |
| Custo topológico | 1 PP por rank; 3 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Ticks de Ignição recebem somente a camada periódica A0244 uma vez; não reaplicam Potência Mágica, crítico, modifiers do hit inicial, nova Ignição, mastery, Ímpeto ou procs de ação direta. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0244 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0244.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Fire não estiver desbloqueada ou se autoria/correlação da Ignição não for confiável. Não presumir combustão vanilla pela entidade estar em chamas e não rerodar multiplicadores do impacto inicial.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks; multiplicador do parcel de dano periódico da Ignição elegível: ×1,06 / ×1,12 / ×1,18. Teto próprio: +18%. Frequência de ticks e duração não são alteradas por A0244..
...
- Página: [A0244 — Combustão](https://app.notion.com/3c569db9f0db81c69e2ac65125adb15e)
...
**Resultado final do Chat 1 para A0244:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
