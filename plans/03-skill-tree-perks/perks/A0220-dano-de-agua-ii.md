...
- **Fonte canônica:** [registro A0220 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81db9853c4c3c28393e7).
...
|---|---|
| Código | A0220 |
| Nome | Dano de Água II |
| Domínio | ARCANE |
| Árvore | Principal — ARCANE ↔ AGILITY |
| Ramo | Água — Wet e Pressão |
| Camada | 5 |
...
| Dependências Obrigatórias | A0219 Dano de Água I ≥ 3 ranks + Water Mastery ≥ 20 + acesso a estado WET/WATER aplicado por habilidade WATER válida do jogador. |
| Pré-requisitos | A0219 Dano de Água I ≥ 3 ranks. |
| Provider/Mods | Somake Spells 1.0.8-1.21.1-fix como provider WATER/Aqua principal dentro de Iron's Spells 'n Spellbooks 3.16.3 + Ars Elemental 0.7.10.1 e outros providers somente por adapter WATER explícito + registry canônico de water_state_id e hook de duração-base. Integrações Lightning/WATER só existem quando cada ação/estado possuir classificação causal própria. |
| Efeito | Quando uma ação WATER direta do jogador aplicar um water_state_id real e explicitamente allowlisted por adapter versionado, A0220 marca o alvo com RPG_TIDAL_PRESSURE por 120 ticks (6 s). A próxima ação WATER direta com action_id/signature diferente contra o mesmo alvo durante a janela consome a marca, aplica dano WATER direto ×1,15 e, se o mesmo water_state_id for renovado/aplicado pelo outcome consumidor com duração-base modificável, acrescenta 40 ticks (2 s) à duração-base antes de respeitar teto nativo explícito do provider. |
| Escalonamento | 1 rank. Marca por alvo: 120 ticks. Consumidor WATER direto distinto: dano WATER direto ×1,15. Estado WATER allowlisted aplicado/renovado pelo mesmo outcome: duração-base +40 ticks quando modificável. Uma marca por jogador/alvo; não acumula. |
| Gate | A0219 ≥3 + Water Mastery ≥20 + water_state_id real aplicado por ação WATER direta atribuída ao jogador. A consumidora deve ser WATER direta e distinta contra o mesmo alvo. Chuva, imersão ambiental, balde, fonte sem owner, automação, DoT, summons, callback duplicado e repetição da mesma ação não armam nem consomem. |
| Hook | Ao confirmar aplicação do estado WATER allowlisted, registrar source_action_id/signature, alvo e expiração de 120 ticks. No próximo direct_water_outcome distinto contra o mesmo alvo, deduplicar por outcome_id, aplicar ×1,15 à camada WATER direta e, se aplicável, base_duration + 40 ticks ao mesmo estado na criação/renovação; consumir após commit. |
| Fallback | Sem water_state_id real, allowlisted e atribuível a uma ação WATER do jogador, A0220 fica inativa. Se somente a duração não for modificável, preservar o dano ×1,15 e omitir +40 ticks. Não usar chuva, imersão, balde, natação, WET ambiental, umidade ou Lightning como vulnerabilidade WATER universal; não inventar estado WET. |
| Regra | PP_REGION: ARCANE/WATER. RPG_TIDAL_PRESSURE só nasce de water_state_id real aplicado por ação WATER direta atribuída ao jogador. A mesma ação não arma e consome; o consumidor deve ser action_id/signature WATER distinta contra o mesmo alvo. Chuva, imersão, balde, ambiente, automação, summons, DoT e callbacks duplicados não qualificam. Duração só é alterada na criação/renovação do estado, respeitando teto nativo explícito, nunca por polling/tick. Notable da Árvore Exterior; não é terminal. |
...
Quando uma ação WATER direta do jogador aplicar um water_state_id real e explicitamente allowlisted por adapter versionado, A0220 marca o alvo com RPG_TIDAL_PRESSURE por 120 ticks (6 s). A próxima ação WATER direta com action_id/signature diferente contra o mesmo alvo durante a janela consome a marca, aplica dano WATER direto ×1,15 e, se o mesmo water_state_id for renovado/aplicado pelo outcome consumidor com duração-base modificável, acrescenta 40 ticks (2 s) à duração-base antes de respeitar teto nativo explícito do provider.

### Escalonamento aprovado

1 rank. Marca por alvo: 120 ticks. Consumidor WATER direto distinto: dano WATER direto ×1,15. Estado WATER allowlisted aplicado/renovado pelo mesmo outcome: duração-base +40 ticks quando modificável. Uma marca por jogador/alvo; não acumula.

### Gate de compra/ativação

A0219 ≥3 + Water Mastery ≥20 + water_state_id real aplicado por ação WATER direta atribuída ao jogador. A consumidora deve ser WATER direta e distinta contra o mesmo alvo. Chuva, imersão ambiental, balde, fonte sem owner, automação, DoT, summons, callback duplicado e repetição da mesma ação não armam nem consomem.

### Hook e ordem de execução

Ao confirmar aplicação do estado WATER allowlisted, registrar source_action_id/signature, alvo e expiração de 120 ticks. No próximo direct_water_outcome distinto contra o mesmo alvo, deduplicar por outcome_id, aplicar ×1,15 à camada WATER direta e, se aplicável, base_duration + 40 ticks ao mesmo estado na criação/renovação; consumir após commit.

### Fallback sem trocar a identidade

Sem water_state_id real, allowlisted e atribuível a uma ação WATER do jogador, A0220 fica inativa. Se somente a duração não for modificável, preservar o dano ×1,15 e omitir +40 ticks. Não usar chuva, imersão, balde, natação, WET ambiental, umidade ou Lightning como vulnerabilidade WATER universal; não inventar estado WET.
...
| Item | Decisão |
|---|---|
| Região | Principal — ARCANE ↔ AGILITY / Água — Wet e Pressão |
| Camada e papel | Camada 5; Notable |
| Pré-requisito visual/estrutural | A0219 Dano de Água I ≥ 3 ranks. |
| Dependência semântica completa | A0219 Dano de Água I ≥ 3 ranks + Water Mastery ≥ 20 + acesso a estado WET/WATER aplicado por habilidade WATER válida do jogador. |
| Custo topológico | 1 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: ARCANE/WATER. RPG_TIDAL_PRESSURE só nasce de water_state_id real aplicado por ação WATER direta atribuída ao jogador. A mesma ação não arma e consome; o consumidor deve ser action_id/signature WATER distinta contra o mesmo alvo. Chuva, imersão, balde, ambiente, automação, summons, DoT e callbacks duplicados não qualificam. Duração só é alterada na criação/renovação do estado, respeitando teto nativo explícito, nunca por polling/tick. Notable da Árvore Exterior; não é terminal. |
...
Somake Spells 1.0.8-1.21.1-fix como provider WATER/Aqua principal dentro de Iron's Spells 'n Spellbooks 3.16.3 + Ars Elemental 0.7.10.1 e outros providers somente por adapter WATER explícito + registry canônico de water_state_id e hook de duração-base. Integrações Lightning/WATER só existem quando cada ação/estado possuir classificação causal própria.
...
| Projeto | Head auditado | Decisão para A0220 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0220.
...
- **Fallback normativo:** Sem water_state_id real, allowlisted e atribuível a uma ação WATER do jogador, A0220 fica inativa. Se somente a duração não for modificável, preservar o dano ×1,15 e omitir +40 ticks. Não usar chuva, imersão, balde, natação, WET ambiental, umidade ou Lightning como vulnerabilidade WATER universal; não inventar estado WET.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Marca por alvo: 120 ticks. Consumidor WATER direto distinto: dano WATER direto ×1,15. Estado WATER allowlisted aplicado/renovado pelo mesmo outcome: duração-base +40 ticks quando modificável. Uma marca por jogador/alvo; não acumula..
...
- Página: [A0220 — Dano de Água II](https://app.notion.com/3c569db9f0db81db9853c4c3c28393e7)
...
**Resultado final do Chat 1 para A0220:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
