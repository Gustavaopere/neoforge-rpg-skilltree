...
- **Fonte canônica:** [registro A0221 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81bdb4afdfc7115a06cb).
...
| Dependências Obrigatórias | A0219 Dano de Água I ≥ 1 rank OU Gateway VITALITY ativo. A0221 é ponte defensiva; a rota defensiva isolada não conta como prova de domínio para Maestria de Água. |
| Pré-requisitos | A0219 Dano de Água I ≥1 ou Gateway VITALITY. |
| Provider/Mods | Somake Spells 1.0.8-1.21.1-fix (Aqua School) + Iron's Spells 'n Spellbooks runtime 1.21.1-3.16.3 como framework + Ars Elemental 0.7.10.1 e outros providers somente quando a ação/componente estiver explicitamente mapeado WATER + DamageMitigationResolver canônico + VITALITY. Quando houver resistência WATER/Aqua nativa segura do provider, o adapter deve unificá-la ao canonical_modifier_id=RPG_WATER_RESISTANCE; nunca aplicar backend nativo e bucket RPG duas vezes no mesmo componente. |
| Efeito | +3% de Resistência a Água por rank (máx. +12%) contra componentes WATER elegíveis. A0221 contribui para um único bucket canônico RPG_WATER_RESISTANCE; não existe cap defensivo global implícito. |
| Escalonamento | Até 4 ranks; contribuição ao bucket RPG_WATER_RESISTANCE: +3% / +6% / +9% / +12%. Teto próprio de A0221: 12%. |
| Gate | (A0219 ≥1 OU Gateway VITALITY ativo) + componente de dano legitimamente classificado WATER. Afogamento, pressão ambiental, sede, frio, criatura aquática e deslocamento térmico não entram por inferência. |
| Hook | DamageMitigationResolver: classificar WATER uma vez, somar A0221 ao canonical_modifier_id=RPG_WATER_RESISTANCE e resolver esse bucket uma única vez no componente WATER antes de compor mitigadores semanticamente distintos. |
| Fallback | Sem classificação WATER explícita e segura, A0221 fica inativa naquele componente. Não converter afogamento, pressão ambiental, respiração, sede, frio, WET ambiental, criatura aquática ou proteção térmica em RPG_WATER_RESISTANCE. Se um provider já resolver resistência Aqua nativa e não houver adapter seguro para unificação, a contribuição de A0221 fica fail-closed naquele provider em vez de duplicar mitigação. |
| Regra | FUNDAMENTO_EXTERIOR: VITALITY/WATER_RESISTANCE. PP_REGION: VITALITY_ARCANE_WATER_BRIDGE/RESISTANCE. BRIDGE_PP_POLICY: estes PP não contam simultaneamente para thresholds puros de VITALITY e ARCANE/WATER; uma Specialist pode whitelistar no máximo um lado semântico. RPG_WATER_RESISTANCE é resistência a componente WATER explícito, distinta de sobrevivência aquática, WET ambiental, temperatura, sede, pressão e respiração. BACKEND_UNICO: provider-native Aqua/WATER resistance OU bucket RPG, nunca ambos para a mesma contribuição. Ponte da Árvore Exterior; não é terminal. |
...
+3% de Resistência a Água por rank (máx. +12%) contra componentes WATER elegíveis. A0221 contribui para um único bucket canônico RPG_WATER_RESISTANCE; não existe cap defensivo global implícito.

### Escalonamento aprovado

Até 4 ranks; contribuição ao bucket RPG_WATER_RESISTANCE: +3% / +6% / +9% / +12%. Teto próprio de A0221: 12%.

### Gate de compra/ativação

(A0219 ≥1 OU Gateway VITALITY ativo) + componente de dano legitimamente classificado WATER. Afogamento, pressão ambiental, sede, frio, criatura aquática e deslocamento térmico não entram por inferência.

### Hook e ordem de execução

DamageMitigationResolver: classificar WATER uma vez, somar A0221 ao canonical_modifier_id=RPG_WATER_RESISTANCE e resolver esse bucket uma única vez no componente WATER antes de compor mitigadores semanticamente distintos.

### Fallback sem trocar a identidade

Sem classificação WATER explícita e segura, A0221 fica inativa naquele componente. Não converter afogamento, pressão ambiental, respiração, sede, frio, WET ambiental, criatura aquática ou proteção térmica em RPG_WATER_RESISTANCE. Se um provider já resolver resistência Aqua nativa e não houver adapter seguro para unificação, a contribuição de A0221 fica fail-closed naquele provider em vez de duplicar mitigação.
...
| Item | Decisão |
|---|---|
| Região | Principal — VITALITY ↔ ARCANE / Água — Defesa Fluida |
| Camada e papel | Camada 4; Ponte |
| Pré-requisito visual/estrutural | A0219 Dano de Água I ≥1 ou Gateway VITALITY. |
| Dependência semântica completa | A0219 Dano de Água I ≥ 1 rank OU Gateway VITALITY ativo. A0221 é ponte defensiva; a rota defensiva isolada não conta como prova de domínio para Maestria de Água. |
| Custo topológico | 1 PP por rank; 4 rank(s); extra 0 |
| Regra de região/PP | FUNDAMENTO_EXTERIOR: VITALITY/WATER_RESISTANCE. PP_REGION: VITALITY_ARCANE_WATER_BRIDGE/RESISTANCE. BRIDGE_PP_POLICY: estes PP não contam simultaneamente para thresholds puros de VITALITY e ARCANE/WATER; uma Specialist pode whitelistar no máximo um lado semântico. RPG_WATER_RESISTANCE é resistência a componente WATER explícito, distinta de sobrevivência aquática, WET ambiental, temperatura, sede, pressão e respiração. BACKEND_UNICO: provider-native Aqua/WATER resistance OU bucket RPG, nunca ambos para a mesma contribuição. Ponte da Árvore Exterior; não é terminal. |
...
| Projeto | Head auditado | Decisão para A0221 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0221.
...
- **Fallback normativo:** Sem classificação WATER explícita e segura, A0221 fica inativa naquele componente. Não converter afogamento, pressão ambiental, respiração, sede, frio, WET ambiental, criatura aquática ou proteção térmica em RPG_WATER_RESISTANCE. Se um provider já resolver resistência Aqua nativa e não houver adapter seguro para unificação, a contribuição de A0221 fica fail-closed naquele provider em vez de duplicar mitigação.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 4 ranks; contribuição ao bucket RPG_WATER_RESISTANCE: +3% / +6% / +9% / +12%. Teto próprio de A0221: 12%..
...
- Página: [A0221 — Resistência a Água I](https://app.notion.com/3c569db9f0db81bdb4afdfc7115a06cb)
...
**Resultado final do Chat 1 para A0221:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
