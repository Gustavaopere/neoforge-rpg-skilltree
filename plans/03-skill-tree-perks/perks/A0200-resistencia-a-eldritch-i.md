...
- **Fonte canônica:** [registro A0200 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db814f91ffc69842ed2ec0).
...
| Dependências Obrigatórias | A0198 Dano de Eldritch I ≥1 OU Gateway VITALITY, mais capability server-side disponível para classificar componente hostil ELDRITCH e vinculá-lo ao DamageMitigationResolver. A rota A0198 permanece fechada até A0198 ser concluída; VITALITY não substitui o adapter. |
| Pré-requisitos | A0198 Dano de Eldritch I ≥1 ou Gateway VITALITY. |
| Provider/Mods | Minecraft/NeoForge + DamageMitigationResolver/RPG_ELDRITCH_RESISTANCE + adapters ELDRITCH versionados. Iron's 3.16.3, Goety 3.1.4, Malum 1.8.2 e Eidolon 0.5.0.2 só participam quando um resultado hostil concreto for classificado. A integração atual do Black Arcana (hazard/progression/mastery e forecast de Arcane Resistance) NÃO publica BLACK_ARCANA_ELDRITCH_OUTCOME e não alimenta este bucket. |
| Efeito | +4% de Resistência a Eldritch por rank (máx. +16%) somente contra dano ELDRITCH elegível. A0200 contribui para um único bucket canônico RPG_ELDRITCH_RESISTANCE; não existe cap defensivo global implícito. |
| Escalonamento | Até 4 ranks; contribuição ao bucket RPG_ELDRITCH_RESISTANCE: +4% / +8% / +12% / +16%. Teto próprio de A0200: 16%. |
| Gate | (A0198 ≥1 legitimamente disponível OU Gateway VITALITY ativo) + adapter versionado que publique componente hostil ELDRITCH e binding canônico RPG_ELDRITCH_RESISTANCE. Sem ambos, A0200 é UNAVAILABLE_NODE e não pode ser comprada; o gateway visual nunca habilita um no-op. |
| Hook | AvailabilityResolver exige adapter hostil ELDRITCH + DamageMitigationResolver antes da compra. Em outcome elegível, classificar uma vez, somar A0200 ao canonical_modifier_id=RPG_ELDRITCH_RESISTANCE e aplicar o bucket uma única vez somente ao componente ELDRITCH. Reavaliar availability em login, datapack/rules reload, mudança de provider e perda de dependência. |
| Fallback | UNAVAILABLE_NODE/não comprável enquanto nenhum classificador hostil ELDRITCH confiável e nenhum binding DamageMitigationResolver existirem. Com o node disponível, componente não classificado falha fechado. Não converter curse, Arcane/Corruption Resistance, Strain, Backlash, Soul Energy, tema ou namespace em ELDRITCH. |
| Regra | FUNDAMENTO_EXTERIOR: VITALITY/ELDRITCH_RESISTANCE. PP_REGION: VITALITY_OCCULT_ELDRITCH_BRIDGE/RESISTANCE. UNAVAILABLE_NODE é obrigatório sem classifier+binding. A0198 é dependência externa adiantada e não é presumida concluída. Arcane Resistance, Corruption Resistance, curses e estados continuam buckets distintos. Não terminal. |
...
+4% de Resistência a Eldritch por rank (máx. +16%) somente contra dano ELDRITCH elegível. A0200 contribui para um único bucket canônico RPG_ELDRITCH_RESISTANCE; não existe cap defensivo global implícito.

### Escalonamento aprovado

Até 4 ranks; contribuição ao bucket RPG_ELDRITCH_RESISTANCE: +4% / +8% / +12% / +16%. Teto próprio de A0200: 16%.

### Gate de compra/ativação

(A0198 ≥1 legitimamente disponível OU Gateway VITALITY ativo) + adapter versionado que publique componente hostil ELDRITCH e binding canônico RPG_ELDRITCH_RESISTANCE. Sem ambos, A0200 é UNAVAILABLE_NODE e não pode ser comprada; o gateway visual nunca habilita um no-op.

### Hook e ordem de execução

AvailabilityResolver exige adapter hostil ELDRITCH + DamageMitigationResolver antes da compra. Em outcome elegível, classificar uma vez, somar A0200 ao canonical_modifier_id=RPG_ELDRITCH_RESISTANCE e aplicar o bucket uma única vez somente ao componente ELDRITCH. Reavaliar availability em login, datapack/rules reload, mudança de provider e perda de dependência.

### Fallback sem trocar a identidade

UNAVAILABLE_NODE/não comprável enquanto nenhum classificador hostil ELDRITCH confiável e nenhum binding DamageMitigationResolver existirem. Com o node disponível, componente não classificado falha fechado. Não converter curse, Arcane/Corruption Resistance, Strain, Backlash, Soul Energy, tema ou namespace em ELDRITCH.
...
| Pré-requisito visual/estrutural | A0198 Dano de Eldritch I ≥1 ou Gateway VITALITY. |
| Dependência semântica completa | A0198 Dano de Eldritch I ≥1 OU Gateway VITALITY, mais capability server-side disponível para classificar componente hostil ELDRITCH e vinculá-lo ao DamageMitigationResolver. A rota A0198 permanece fechada até A0198 ser concluída; VITALITY não substitui o adapter. |
| Custo topológico | 1 PP por rank; 4 rank(s); extra 0 |
| Regra de região/PP | FUNDAMENTO_EXTERIOR: VITALITY/ELDRITCH_RESISTANCE. PP_REGION: VITALITY_OCCULT_ELDRITCH_BRIDGE/RESISTANCE. UNAVAILABLE_NODE é obrigatório sem classifier+binding. A0198 é dependência externa adiantada e não é presumida concluída. Arcane Resistance, Corruption Resistance, curses e estados continuam buckets distintos. Não terminal. |
...
| Projeto | Head auditado | Decisão para A0200 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0200.
...
- **Fallback normativo:** UNAVAILABLE_NODE/não comprável enquanto nenhum classificador hostil ELDRITCH confiável e nenhum binding DamageMitigationResolver existirem. Com o node disponível, componente não classificado falha fechado. Não converter curse, Arcane/Corruption Resistance, Strain, Backlash, Soul Energy, tema ou namespace em ELDRITCH.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 4 ranks; contribuição ao bucket RPG_ELDRITCH_RESISTANCE: +4% / +8% / +12% / +16%. Teto próprio de A0200: 16%..
...
- Página: [A0200 — Resistência a Eldritch I](https://app.notion.com/3c569db9f0db814f91ffc69842ed2ec0)
...
**Resultado final do Chat 1 para A0200:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
