...
- **Fonte canônica:** [registro A0278 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81f6824fef7c7947267a).
...
| Hook | Manter last_hostile_damage_tick e ledger de origem server-side. Quando now-last_hostile_damage_tick >=160, definir target_A0278_absorption=0,08×current_max_health e completar SOMENTE a contribuição source_id=A0278 até esse valor, preservando todas as demais origens. Em mudança legítima de max health, reavaliar o teto da parcela própria sem curar nem converter excesso em vida; se o novo teto for menor, remover apenas excesso comprovadamente pertencente a A0278. O consumo de dano deve ser reconciliado pelo ledger/receipt do pipeline real; nunca inferir que toda queda de Absorption pertence a A0278. Após consumo parcial/total da parcela própria, nova recomposição exige novamente 160 ticks íntegros sem dano hostil. |
| Fallback | FAIL-CLOSED se Specialist Gelo não estiver desbloqueada ou se não houver rastreamento seguro de origem/consumo da Absorption. Não sobrescrever absorptionAmount, não remover ou reclassificar Absorption externa, não usar Resistance/Armor/vida temporária custom como aproximação e não transformar os 8% em cura. |
| Regra | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Perk interna da Specialist Gelo. A0278 mantém UMA única parcela defensiva regenerável de 8% da vida máxima atual; não cura, não empilha consigo mesma e não possui prioridade implícita sobre Absorption de outras fontes. O ledger de origem é obrigatório porque o valor vanilla é agregado. Dano hostil interrompe a reconstrução; dano não hostil não pode ser explorado como reset/armamento. Logout/dimensão/morte/respec reconciliam/encerram somente a contribuição A0278; nenhuma recomposição gera mastery, sustain ou proc adicional. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Após 160 ticks (8 s) sem receber dano hostil, A0278 recompõe uma parcela própria de Absorption equivalente a 8% da vida máxima atual. Essa parcela é identificada por ledger do RPG Skill Tree, não empilha com cópias de A0278 e nunca substitui/remove Absorption de outras fontes. Quando a parcela própria é consumida, só pode começar a recompor novamente após nova janela íntegra de 160 ticks sem dano hostil.

### Escalonamento aprovado

1 rank. Valor-alvo da parcela A0278 = 8% da vida máxima atual no momento da recomposição. Reavaliar após mudança legítima de max_health, sem gerar cura nem conservar excesso antigo acima do novo alvo. Não existe cap absoluto adicional oculto; o próprio limite de 8% é o cap deste node.

### Gate de compra/ativação

SPECIALIST_UNLOCK:ICE válido + A0268 ≥3 + A0274 ≥1 + 160 ticks consecutivos desde o último hostile_damage_outcome recebido + ABSORPTION_SOURCE_LEDGER_V1 operacional. Apenas dano hostil real reinicia a janela. Dano ambiental/não hostil não arma nem reseta artificialmente. Se a parcela própria de A0278 já estiver completa no alvo atual de 8% da vida máxima, não gerar nova camada.

### Hook e ordem de execução

Manter last_hostile_damage_tick e ledger de origem server-side. Quando now-last_hostile_damage_tick >=160, definir target_A0278_absorption=0,08×current_max_health e completar SOMENTE a contribuição source_id=A0278 até esse valor, preservando todas as demais origens. Em mudança legítima de max health, reavaliar o teto da parcela própria sem curar nem converter excesso em vida; se o novo teto for menor, remover apenas excesso comprovadamente pertencente a A0278. O consumo de dano deve ser reconciliado pelo ledger/receipt do pipeline real; nunca inferir que toda queda de Absorption pertence a A0278. Após consumo parcial/total da parcela própria, nova recomposição exige novamente 160 ticks íntegros sem dano hostil.
...
| Pré-requisito visual/estrutural | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) + A0268 Pele Glacial ≥3 + A0274 Retaliação Gélida ≥1. |
| Dependência semântica completa | SPECIALIST_UNLOCK:ICE confirmado server-side por Gate A/B/C + A0268 Pele Glacial ≥3 ranks + A0274 Retaliação Gélida ≥1 rank. Requisitos locais não substituem fundamentos ICE + ≥100 PP válidos em SPECIALIST_REGION:ICE + terminal A0169. |
...
| Projeto | Head auditado | Decisão para A0278 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>source_id</code>, <code>receipt</code>, <code>ledger</code>.
...
- **Chaves que o adapter precisa preservar:** <code>source_id</code>, <code>receipt</code>, <code>ledger</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0278.
...
- **Recursos/eixos tocados pelo contrato:** <code>ABSORPTION</code>.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Valor-alvo da parcela A0278 = 8% da vida máxima atual no momento da recomposição. Reavaliar após mudança legítima de max_health, sem gerar cura nem conservar excesso antigo acima do novo alvo. Não existe cap absoluto adicional oculto; o próprio limite de 8% é o cap deste node..
...
6. **Causalidade:** correlacionar <code>source_id</code>, <code>receipt</code>, <code>ledger</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>source_id</code>, <code>receipt</code>, <code>ledger</code>. |
...
- Página: [A0278 — Escudo de Gelo](https://app.notion.com/3c569db9f0db81f6824fef7c7947267a)
...
**Resultado final do Chat 1 para A0278:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
