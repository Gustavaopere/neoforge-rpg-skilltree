...
- **Fonte canônica:** [registro A0207 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db813f89acdd6cca8e23bb).
...
|---|---|
| Código | A0207 |
| Nome | Resistência a Ender I |
| Domínio | VITALITY |
| Árvore | Principal — VITALITY ↔ ARCANE/AGILITY |
| Ramo | Ender — Defesa Dimensional |
| Camada | 4 |
...
| Dependências Obrigatórias | A0205 ≥1 disponível OU Gateway VITALITY, mais capability server-side para classificar componente hostil ENDER e vinculá-lo ao DamageMitigationResolver. A rota A0205 permanece fechada enquanto A0205 estiver indisponível; VITALITY não substitui o adapter. |
| Pré-requisitos | A0205 Dano de Ender I ≥ 1 rank ou Gateway VITALITY. |
| Provider/Mods | Minecraft/NeoForge + DamageMitigationResolver/RPG_ENDER_RESISTANCE + Fire's Ender Expansion 2.4.1/Somake 1.0.8 somente após adapter hostil ENDER exato. Nenhum binding desse tipo existe hoje no RPG. Void, cold, teleport e origem no End permanecem eixos distintos. |
| Efeito | +3% de Resistência a Ender por rank (máx. +12%) contra componentes ENDER elegíveis. A0207 contribui para um único bucket canônico RPG_ENDER_RESISTANCE; não existe cap defensivo global implícito. |
| Escalonamento | Até 4 ranks; contribuição ao bucket RPG_ENDER_RESISTANCE: +3% / +6% / +9% / +12%. Teto próprio de A0207: 12%. |
| Gate | (A0205 ≥1 legitimamente disponível OU Gateway VITALITY ativo) + adapter versionado de hostile_direct_ender_component + binding RPG_ENDER_RESISTANCE. Sem classifier/binding, A0207 é UNAVAILABLE_NODE e não pode ser comprada. VITALITY visual não habilita no-op. |
| Hook | AvailabilityResolver exige adapter hostil ENDER + DamageMitigationResolver. Em outcome elegível, classificar uma vez, somar A0207 ao canonical_modifier_id=RPG_ENDER_RESISTANCE e aplicar uma vez só ao componente ENDER. Reavaliar em login, provider/datapack reload, respec e perda de A0205/gateway. |
| Fallback | UNAVAILABLE_NODE/não comprável enquanto não houver classificador hostil ENDER confiável e bucket DamageMitigationResolver. Com node disponível, componente desconhecido falha fechado. Não converter Void, cold, teleport, entidade/dimensão do End, magic resistance, armor ou proteção de deslocamento. |
| Regra | FUNDAMENTO_EXTERIOR: VITALITY/ENDER_RESISTANCE; PP_REGION: VITALITY_AGILITY_ENDER_BRIDGE/RESISTANCE. Availability transitiva de A0205 na rota ofensiva. Sem classifier+binding, UNAVAILABLE_NODE. RPG_ENDER_RESISTANCE não é Void/cold/teleport/proteção dimensional. Não terminal. |
...
+3% de Resistência a Ender por rank (máx. +12%) contra componentes ENDER elegíveis. A0207 contribui para um único bucket canônico RPG_ENDER_RESISTANCE; não existe cap defensivo global implícito.

### Escalonamento aprovado

Até 4 ranks; contribuição ao bucket RPG_ENDER_RESISTANCE: +3% / +6% / +9% / +12%. Teto próprio de A0207: 12%.

### Gate de compra/ativação

(A0205 ≥1 legitimamente disponível OU Gateway VITALITY ativo) + adapter versionado de hostile_direct_ender_component + binding RPG_ENDER_RESISTANCE. Sem classifier/binding, A0207 é UNAVAILABLE_NODE e não pode ser comprada. VITALITY visual não habilita no-op.

### Hook e ordem de execução

AvailabilityResolver exige adapter hostil ENDER + DamageMitigationResolver. Em outcome elegível, classificar uma vez, somar A0207 ao canonical_modifier_id=RPG_ENDER_RESISTANCE e aplicar uma vez só ao componente ENDER. Reavaliar em login, provider/datapack reload, respec e perda de A0205/gateway.

### Fallback sem trocar a identidade

UNAVAILABLE_NODE/não comprável enquanto não houver classificador hostil ENDER confiável e bucket DamageMitigationResolver. Com node disponível, componente desconhecido falha fechado. Não converter Void, cold, teleport, entidade/dimensão do End, magic resistance, armor ou proteção de deslocamento.
...
| Item | Decisão |
|---|---|
| Região | Principal — VITALITY ↔ ARCANE/AGILITY / Ender — Defesa Dimensional |
| Camada e papel | Camada 4; Ponte |
| Pré-requisito visual/estrutural | A0205 Dano de Ender I ≥ 1 rank ou Gateway VITALITY. |
| Dependência semântica completa | A0205 ≥1 disponível OU Gateway VITALITY, mais capability server-side para classificar componente hostil ENDER e vinculá-lo ao DamageMitigationResolver. A rota A0205 permanece fechada enquanto A0205 estiver indisponível; VITALITY não substitui o adapter. |
| Custo topológico | 1 PP por rank; 4 rank(s); extra 0 |
| Regra de região/PP | FUNDAMENTO_EXTERIOR: VITALITY/ENDER_RESISTANCE; PP_REGION: VITALITY_AGILITY_ENDER_BRIDGE/RESISTANCE. Availability transitiva de A0205 na rota ofensiva. Sem classifier+binding, UNAVAILABLE_NODE. RPG_ENDER_RESISTANCE não é Void/cold/teleport/proteção dimensional. Não terminal. |
...
Minecraft/NeoForge + DamageMitigationResolver/RPG_ENDER_RESISTANCE + Fire's Ender Expansion 2.4.1/Somake 1.0.8 somente após adapter hostil ENDER exato. Nenhum binding desse tipo existe hoje no RPG. Void, cold, teleport e origem no End permanecem eixos distintos.
...
| Projeto | Head auditado | Decisão para A0207 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0207.
...
- **Fallback normativo:** UNAVAILABLE_NODE/não comprável enquanto não houver classificador hostil ENDER confiável e bucket DamageMitigationResolver. Com node disponível, componente desconhecido falha fechado. Não converter Void, cold, teleport, entidade/dimensão do End, magic resistance, armor ou proteção de deslocamento.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 4 ranks; contribuição ao bucket RPG_ENDER_RESISTANCE: +3% / +6% / +9% / +12%. Teto próprio de A0207: 12%..
...
- Página: [A0207 — Resistência a Ender I](https://app.notion.com/3c569db9f0db813f89acdd6cca8e23bb)
...
**Resultado final do Chat 1 para A0207:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
