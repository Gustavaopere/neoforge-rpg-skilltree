...
- **Fonte canônica:** [registro A0262 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81158332c2c08e0970fe).
...
| Dependências Obrigatórias | SPECIALIST_UNLOCK:FIRE confirmado server-side por Gate A/B/C + A0259 Nascido das Chamas possuído e ATIVO + A0246 Calor Crescente ≥3 ranks. Estes requisitos locais não substituem fundamentos FIRE + ≥100 Passive Points válidos em SPECIALIST_REGION:FIRE + terminal A0162. |
| Pré-requisitos | Specialist Fire desbloqueada (SPECIALIST_UNLOCK:FIRE) + A0259 Nascido das Chamas + A0246 Calor Crescente ≥3 ranks. |
| Provider/Mods | RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + A0259 + o mesmo ledger RPG_FIRE_MOMENTUM de A0246 + FUTURE_PROVIDER_CONTRACT DAMAGE_MITIGATION_RESOLVER_V1 com receipt causal por modificador. OWNER: RPG core/adapter de mitigação; CONSUMER: A0262; QUERY/RECEIPT: {outcome_id, modifier_id=A0259, damage_kind, mitigated_amount} após commit; BEHAVIOR: creditar no máximo uma carga normal de A0246 quando A0259 tiver mitigado quantidade positiva de FIRE/HEAT_DAMAGE hostil direto. Cold Sweat 2.4.2 pode fornecer HOT→HEAT_DAMAGE somente via mapping versionado; Iron's/Ars e demais providers FIRE só entram por classificador explícito já aceito pelo pipeline. VERSION-STATUS: DamageMitigationResolver/receipt causal é contrato de design; API correspondente não foi encontrada na main auditada em 2026-08-29. |
| Efeito | Enquanto RPG_BORN_OF_FLAME de A0259 estiver ativo, o primeiro hostile_direct_damage_outcome FIRE ou HEAT_DAMAGE em que a camada A0259 tenha reduzido uma quantidade positiva daquele componente pode conceder exatamente 1 carga normal de RPG_FIRE_MOMENTUM de A0246, respeitando o limite de 5 e o TTL normal da carga. Após uma concessão, A0262 entra em recarga interna de 15 ticks (0,75 s). Um mesmo outcome, mesmo que contenha FIRE e HEAT_DAMAGE, concede no máximo 1 carga. |
| Escalonamento | 1 rank. Crédito: +1 carga existente de A0246 por outcome hostil direto efetivamente mitigado por A0259. Intervalo mínimo: 15 ticks por jogador. Limite e TTL permanecem os de A0246: máximo 5 cargas, cada nova carga expira 160 ticks após aquisição. A0262 não cria um segundo tipo de Ímpeto. |
| Gate | SPECIALIST_UNLOCK:FIRE válido + A0259 ativo + A0246 ≥3 + hostile_direct_damage_outcome com componente FIRE ou HEAT_DAMAGE reconhecido + receipt causal comprovando mitigated_amount_by_A0259 > 0 + recarga interna livre. Um mesmo outcome_id, mesmo com FIRE e HEAT_DAMAGE, concede no máximo 1 carga. Auto-dano, custos, temperatura corporal, lava/ambiente sem ameaça hostil direta, DoT/derived component, fake player, automação e callback duplicado não geram crédito. |
| Hook | Após commit do DamageMitigationResolver, consultar o receipt causal de A0259 e deduplicar por outcome_id ANTES de considerar buckets. Se a soma efetivamente mitigada por A0259 em FIRE/HEAT_DAMAGE for positiva e now >= next_credit_tick, inserir exatamente uma carga no ledger existente RPG_FIRE_MOMENTUM de A0246 com expiry_tick=now+160, respeitando máximo 5, e definir next_credit_tick=now+15. Não criar segundo ledger, não renovar cargas antigas e não atribuir autoria ofensiva ao crédito defensivo. |
| Fallback | FAIL-CLOSED se Specialist Fire não estiver desbloqueada, A0259 não estiver ativo, o classificador FIRE/HEAT não for seguro, o receipt causal de mitigação não existir ou o ledger A0246 não puder ser atualizado atomicamente. Não conceder carga por simples presença de RPG_BORN_OF_FLAME, por tick em lava/fogo, por temperatura corporal, por redução inferida de dano ou por heurística de provider. |
| Regra | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Coração de Magma converte um evento defensivo REAL, causalmente mitigado por A0259, em uma carga do recurso transitório JÁ EXISTENTE de A0246; não cria cura, mana, nova barra, mastery, dano ou segundo tipo de Ímpeto. Cargas obtidas aqui podem completar o limite de 5, porém NÃO contam como atividade FIRE ofensiva recente para A0263 e nunca ativam o Capstone sem uma nova ação FIRE direta ofensiva elegível. Uma carga no máximo por outcome; Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Enquanto RPG_BORN_OF_FLAME de A0259 estiver ativo, o primeiro hostile_direct_damage_outcome FIRE ou HEAT_DAMAGE em que a camada A0259 tenha reduzido uma quantidade positiva daquele componente pode conceder exatamente 1 carga normal de RPG_FIRE_MOMENTUM de A0246, respeitando o limite de 5 e o TTL normal da carga. Após uma concessão, A0262 entra em recarga interna de 15 ticks (0,75 s). Um mesmo outcome, mesmo que contenha FIRE e HEAT_DAMAGE, concede no máximo 1 carga.

### Escalonamento aprovado

1 rank. Crédito: +1 carga existente de A0246 por outcome hostil direto efetivamente mitigado por A0259. Intervalo mínimo: 15 ticks por jogador. Limite e TTL permanecem os de A0246: máximo 5 cargas, cada nova carga expira 160 ticks após aquisição. A0262 não cria um segundo tipo de Ímpeto.

### Gate de compra/ativação

SPECIALIST_UNLOCK:FIRE válido + A0259 ativo + A0246 ≥3 + hostile_direct_damage_outcome com componente FIRE ou HEAT_DAMAGE reconhecido + receipt causal comprovando mitigated_amount_by_A0259 > 0 + recarga interna livre. Um mesmo outcome_id, mesmo com FIRE e HEAT_DAMAGE, concede no máximo 1 carga. Auto-dano, custos, temperatura corporal, lava/ambiente sem ameaça hostil direta, DoT/derived component, fake player, automação e callback duplicado não geram crédito.
...
FAIL-CLOSED se Specialist Fire não estiver desbloqueada, A0259 não estiver ativo, o classificador FIRE/HEAT não for seguro, o receipt causal de mitigação não existir ou o ledger A0246 não puder ser atualizado atomicamente. Não conceder carga por simples presença de RPG_BORN_OF_FLAME, por tick em lava/fogo, por temperatura corporal, por redução inferida de dano ou por heurística de provider.
...
| Pré-requisito visual/estrutural | Specialist Fire desbloqueada (SPECIALIST_UNLOCK:FIRE) + A0259 Nascido das Chamas + A0246 Calor Crescente ≥3 ranks. |
| Dependência semântica completa | SPECIALIST_UNLOCK:FIRE confirmado server-side por Gate A/B/C + A0259 Nascido das Chamas possuído e ATIVO + A0246 Calor Crescente ≥3 ranks. Estes requisitos locais não substituem fundamentos FIRE + ≥100 Passive Points válidos em SPECIALIST_REGION:FIRE + terminal A0162. |
| Custo topológico | 3 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Coração de Magma converte um evento defensivo REAL, causalmente mitigado por A0259, em uma carga do recurso transitório JÁ EXISTENTE de A0246; não cria cura, mana, nova barra, mastery, dano ou segundo tipo de Ímpeto. Cargas obtidas aqui podem completar o limite de 5, porém NÃO contam como atividade FIRE ofensiva recente para A0263 e nunca ativam o Capstone sem uma nova ação FIRE direta ofensiva elegível. Uma carga no máximo por outcome; Gate A/B/C permanece obrigatório durante ownership/respec. |
...
RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + A0259 + o mesmo ledger RPG_FIRE_MOMENTUM de A0246 + FUTURE_PROVIDER_CONTRACT DAMAGE_MITIGATION_RESOLVER_V1 com receipt causal por modificador. OWNER: RPG core/adapter de mitigação; CONSUMER: A0262; QUERY/RECEIPT: {outcome_id, modifier_id=A0259, damage_kind, mitigated_amount} após commit; BEHAVIOR: creditar no máximo uma carga normal de A0246 quando A0259 tiver mitigado quantidade positiva de FIRE/HEAT_DAMAGE hostil direto. Cold Sweat 2.4.2 pode fornecer HOT→HEAT_DAMAGE somente via mapping versionado; Iron's/Ars e demais providers FIRE só entram por classificador explícito já aceito pelo pipeline. VERSION-STATUS: DamageMitigationResolver/receipt causal é contrato de design; API correspondente não foi encontrada na main auditada em 2026-08-29.
...
| Projeto | Head auditado | Decisão para A0262 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>outcome_id</code>, <code>receipt</code>, <code>ledger</code>.
...
- **Chaves que o adapter precisa preservar:** <code>outcome_id</code>, <code>receipt</code>, <code>ledger</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0262.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Fire não estiver desbloqueada, A0259 não estiver ativo, o classificador FIRE/HEAT não for seguro, o receipt causal de mitigação não existir ou o ledger A0246 não puder ser atualizado atomicamente. Não conceder carga por simples presença de RPG_BORN_OF_FLAME, por tick em lava/fogo, por temperatura corporal, por redução inferida de dano ou por heurística de provider.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Crédito: +1 carga existente de A0246 por outcome hostil direto efetivamente mitigado por A0259. Intervalo mínimo: 15 ticks por jogador. Limite e TTL permanecem os de A0246: máximo 5 cargas, cada nova carga expira 160 ticks após aquisição. A0262 não cria um segundo tipo de Ímpeto..
...
6. **Causalidade:** correlacionar <code>outcome_id</code>, <code>receipt</code>, <code>ledger</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
- **Referências internas posteriores:** <code>A0263</code>.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>outcome_id</code>, <code>receipt</code>, <code>ledger</code>. |
...
- Página: [A0262 — Coração de Magma](https://app.notion.com/3c569db9f0db81158332c2c08e0970fe)
...
**Resultado final do Chat 1 para A0262:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
