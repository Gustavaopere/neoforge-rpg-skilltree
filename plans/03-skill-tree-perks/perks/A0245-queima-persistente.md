...
- **Fonte canônica:** [registro A0245 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db813e8238f778675ff3be).
...
| Hook | Na criação/renovação legítima do estado, obter a duração que o provider aplicaria sem A0245, multiplicar uma única vez conforme rank e respeitar o limite nativo explícito. Nunca multiplicar tempo restante, executar por tick ou alterar frequência dos ticks. |
| Fallback | FAIL-CLOSED se Specialist Fire não estiver desbloqueada ou se a duração-base real/modificável da Ignição não estiver disponível. Não prolongar fogo visual/genérico, não criar teto global e não converter duração em dano. |
| Regra | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Duração e dano da Ignição são camadas separadas; criação/renovação sempre parte da duração-base nativa daquela aplicação, nunca do tempo restante já ampliado. Sem crescimento exponencial, polling por tick, mastery ou proc. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
+8% de duração-base da Ignição real por rank (×1,08 / ×1,16 / ×1,24), aplicada somente no momento de criação ou renovação legítima do estado atribuído ao jogador. A duração final respeita apenas pisos/tetos nativos explícitos do provider; não existe cap global inventado.

### Escalonamento aprovado

Até 3 ranks; duração-base da aplicação/renovação elegível ×1,08 / ×1,16 / ×1,24. O cálculo sempre parte da duração nativa daquela aplicação, nunca do tempo restante já ampliado.

### Gate de compra/ativação

SPECIALIST_UNLOCK:FIRE válido + A0243 ≥2 + aplicação/renovação de estado de Ignição real atribuído ao jogador + duração-base modificável exposta por FIRE_IGNITION_RESOLVER_V1/adapter equivalente. Estado fixo, sem autoria, ambiental ou duração não modificável é inelegível.

### Hook e ordem de execução

Na criação/renovação legítima do estado, obter a duração que o provider aplicaria sem A0245, multiplicar uma única vez conforme rank e respeitar o limite nativo explícito. Nunca multiplicar tempo restante, executar por tick ou alterar frequência dos ticks.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Fire não estiver desbloqueada ou se a duração-base real/modificável da Ignição não estiver disponível. Não prolongar fogo visual/genérico, não criar teto global e não converter duração em dano.
...
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Duração e dano da Ignição são camadas separadas; criação/renovação sempre parte da duração-base nativa daquela aplicação, nunca do tempo restante já ampliado. Sem crescimento exponencial, polling por tick, mastery ou proc. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0245 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0245.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Fire não estiver desbloqueada ou se a duração-base real/modificável da Ignição não estiver disponível. Não prolongar fogo visual/genérico, não criar teto global e não converter duração em dano.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks; duração-base da aplicação/renovação elegível ×1,08 / ×1,16 / ×1,24. O cálculo sempre parte da duração nativa daquela aplicação, nunca do tempo restante já ampliado..
...
- Página: [A0245 — Queima Persistente](https://app.notion.com/3c569db9f0db813e8238f778675ff3be)
...
**Resultado final do Chat 1 para A0245:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
