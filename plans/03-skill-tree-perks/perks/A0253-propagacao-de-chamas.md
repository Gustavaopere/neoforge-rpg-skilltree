...
- **Fonte canônica:** [registro A0253 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81c7b954d7a5f4ca524a).
...
|---|---|
| Código | A0253 |
| Nome | Propagação de Chamas |
| Domínio | ARCANE/FIRE |
| Árvore | Especialista — Fogo |
| Ramo | Piromancia — Campo e Propagação |
| Camada | 4 |
...
| Hook | No commit do death_outcome elegível, correlacionar owner, ignition_state_id, source/origin e estado restante; deduplicar pela identidade da morte. Consultar uma vez entidades hostis vivas em 4 blocos, selecionar no máximo 3 de forma determinística e aplicar propagated_ignition marcada com propagation_depth=1. A aplicação derivada é determinística: não chama IgnitionResolver, não rerrola crítico e não cria dano instantâneo. Iniciar cooldown now+40 somente se ao menos um estado for aplicado. |
| Fallback | FAIL-CLOSED se Specialist Fire não estiver desbloqueada, se autoria da morte/Ignição-fonte for insegura ou se não existir hook real para aplicar o mesmo estado em forma derivada. Não substituir por dano instantâneo, explosão, fogo vanilla genérico ou bônus pessoal. |
| Regra | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. propagated_ignition tem propagation_depth=1, nunca repropaga, usa 60% do tempo restante e 60% do dano-base nativo da Ignição-fonte sem reaplicar A0244/A0245. Não arma A0252/A0254, não gera Fire Mastery, crítico, Ímpeto, sustain ou proc de ação direta. Multiplayer usa ownership causal. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Quando um alvo vivo que possuía antes do golpe fatal uma Ignição real atribuída ao jogador morrer por um death_outcome causalmente atribuível a uma ação direta do próprio jogador ou a um tick daquela própria Ignição não-propagada, A0253 pode propagar o estado uma única vez. Se a recarga global estiver livre, selecionar até 3 inimigos vivos elegíveis num raio de 4 blocos e aplicar a cada um uma propagated_ignition derivada: duração = quantize_down(0,60 × remaining_ignition_duration_at_death) e dano-base periódico = 0,60 × provider_native_base_tick_damage da Ignição-fonte, sem reaplicar A0244/A0245. Resultado de duração zero não cria estado.
...
SPECIALIST_UNLOCK:FIRE válido + requisitos locais + vítima com Ignição real NÃO propagada atribuída ao jogador antes do death outcome + morte causada por ação direta própria ou tick da própria Ignição + cooldown livre + pipeline seguro para aplicar estado derivado. Ignição propagada, summon, máquina, fake player, ambiente, terceiro, callback duplicado e estado sem ownership não propagam.

### Hook e ordem de execução

No commit do death_outcome elegível, correlacionar owner, ignition_state_id, source/origin e estado restante; deduplicar pela identidade da morte. Consultar uma vez entidades hostis vivas em 4 blocos, selecionar no máximo 3 de forma determinística e aplicar propagated_ignition marcada com propagation_depth=1. A aplicação derivada é determinística: não chama IgnitionResolver, não rerrola crítico e não cria dano instantâneo. Iniciar cooldown now+40 somente se ao menos um estado for aplicado.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Fire não estiver desbloqueada, se autoria da morte/Ignição-fonte for insegura ou se não existir hook real para aplicar o mesmo estado em forma derivada. Não substituir por dano instantâneo, explosão, fogo vanilla genérico ou bônus pessoal.
...
| Item | Decisão |
|---|---|
| Região | Especialista — Fogo / Piromancia — Campo e Propagação |
| Camada e papel | Camada 4; Notable |
| Pré-requisito visual/estrutural | Specialist Fire desbloqueada (SPECIALIST_UNLOCK:FIRE) + A0244 Combustão ≥2 + A0245 Queima Persistente ≥2. |
| Dependência semântica completa | SPECIALIST_UNLOCK:FIRE confirmado por Gate A/B/C + A0244 Combustão ≥2 + A0245 Queima Persistente ≥2. Os requisitos locais não substituem o unlock global da Specialist Fire. |
| Custo topológico | 2 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. propagated_ignition tem propagation_depth=1, nunca repropaga, usa 60% do tempo restante e 60% do dano-base nativo da Ignição-fonte sem reaplicar A0244/A0245. Não arma A0252/A0254, não gera Fire Mastery, crítico, Ímpeto, sustain ou proc de ação direta. Multiplayer usa ownership causal. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0253 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0253.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Fire não estiver desbloqueada, se autoria da morte/Ignição-fonte for insegura ou se não existir hook real para aplicar o mesmo estado em forma derivada. Não substituir por dano instantâneo, explosão, fogo vanilla genérico ou bônus pessoal.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Raio: 4 blocos. Máximo: 3 alvos por morte elegível. Duração propagada: 60% do tempo restante da Ignição-fonte no instante da morte, quantizada para baixo na unidade de ticks do provider. Dano-base periódico propagado: 60% do dano-base nativo periódico da Ignição-fonte. Recarga global por jogador: 40 ticks (2 s), iniciada após propagação bem-sucedida..
...
- Página: [A0253 — Propagação de Chamas](https://app.notion.com/3c569db9f0db81c7b954d7a5f4ca524a)
...
**Resultado final do Chat 1 para A0253:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
