...
- **Fonte canônica:** [registro A0239 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81508ca8d11630698a8e).
...
| Dependências Obrigatórias | A0237 Cadeia Prismática + pelo menos 9 pontos somados em bônus ofensivos de 4 elementos distintos, com 2 × max(pontos_de_uma_escola) ≤ total_de_pontos_contados. |
| Pré-requisitos | A0237 + ≥9 pontos ofensivos em ≥4 elementos distintos; nenhuma escola pode representar mais da metade do investimento contado (2×max ≤ total). |
| Provider/Mods | RPG Skill Tree + FUTURE_PROVIDER_CONTRACTS ELEMENT_SIGNATURE_REGISTRY_V1 e ELEMENTAL_DIVERSITY_LEDGER_V1 + pipeline canônico de penetração específica quando existir. O ledger ofensivo fornece ELEMENT_OFFENSE_PP[element_id] com ownership único por node; shared/general/bridge PP são excluídos por padrão. VERSION-STATUS: designs congelados, contratos não encontrados no runtime/main atual em 2026-08-29. |
| Efeito | No mesmo par jogador/alvo, A0239 coleta quatro element_signature_id canônicos distintos de ações elementais diretas em até 240 ticks (12 s), uma assinatura no máximo por ação. Ao completar, arma RPG_OFFENSIVE_CONVERGENCE por 160 ticks (8 s). A próxima ação elemental direta com uma quinta assinatura diferente das quatro coletadas recebe dano direto ×1,20 e, se suportado, 8% de penetração elemental específica; então consome todo o estado. |
| Escalonamento | 1 rank. Coleta: 4 assinaturas distintas em 240 ticks. Estado preparado: 160 ticks. Quinta assinatura diferente: dano elemental direto ×1,20; penetração específica opcional 8%. |
| Gate | A0237 + ELEMENTAL_DIVERSITY_LEDGER_V1 comprovando ≥9 PP ofensivos válidos em ≥4 element_id distintos e 2 × max(ELEMENT_OFFENSE_PP[element]) ≤ total_contado + mesmo alvo + ações diretas com assinatura primária única. PP shared/general/bridge, a mesma compra reciclada em mais de um elemento, DoT, summons sem autoria, automação, derived components e callbacks duplicados não contam. O consumidor deve possuir uma quinta assinatura diferente das quatro preparatórias. |
| Hook | Rastrear conjunto por jogador/alvo com timestamps e dedupe. Ao quatro assinaturas, congelar conjunto e armar 160 ticks. No quinto elemento distinto, aplicar ×1,20 e, se suportado, penetração de 8% somente sobre resistência específica positiva (positive_specific_resistance ×0,92), sem bypass de mitigadores gerais; consumir após commit. |
| Fallback | FAIL-CLOSED: sem registry/ledger ou sem rastreamento seguro por jogador/alvo, A0239 fica inativa. Se apenas penetração específica estiver indisponível, preservar dano ×1,20. Não aproximar diversidade por total bruto de PP, quatro elementos quando não houver quinto utilizável, ou janela pessoal global. |
| Regra | PP_REGION: ELEMENTAL_SHARED_OFFENSE. DIVERSITY_LEDGER: requisito 9 PP/4 elementos e a regra 2×max≤total usam apenas ELEMENT_OFFENSE_PP[element_id] element-specific; shared/general/bridge valem zero por padrão e uma compra nunca preenche duas escolas. Convergência coleta quatro assinaturas primárias distintas no mesmo alvo e exige uma quinta assinatura diferente para consumir. Penetração opcional de 8% atua somente sobre resistência específica positiva do quinto elemento, nunca mitigação geral, armadura aplicável, invulnerabilidade ou resistência negativa. Registry/ledger são contratos futuros, não APIs runtime existentes. Notable da Árvore Exterior; não é terminal. |
...
No mesmo par jogador/alvo, A0239 coleta quatro element_signature_id canônicos distintos de ações elementais diretas em até 240 ticks (12 s), uma assinatura no máximo por ação. Ao completar, arma RPG_OFFENSIVE_CONVERGENCE por 160 ticks (8 s). A próxima ação elemental direta com uma quinta assinatura diferente das quatro coletadas recebe dano direto ×1,20 e, se suportado, 8% de penetração elemental específica; então consome todo o estado.

### Escalonamento aprovado

1 rank. Coleta: 4 assinaturas distintas em 240 ticks. Estado preparado: 160 ticks. Quinta assinatura diferente: dano elemental direto ×1,20; penetração específica opcional 8%.

### Gate de compra/ativação

A0237 + ELEMENTAL_DIVERSITY_LEDGER_V1 comprovando ≥9 PP ofensivos válidos em ≥4 element_id distintos e 2 × max(ELEMENT_OFFENSE_PP[element]) ≤ total_contado + mesmo alvo + ações diretas com assinatura primária única. PP shared/general/bridge, a mesma compra reciclada em mais de um elemento, DoT, summons sem autoria, automação, derived components e callbacks duplicados não contam. O consumidor deve possuir uma quinta assinatura diferente das quatro preparatórias.

### Hook e ordem de execução

Rastrear conjunto por jogador/alvo com timestamps e dedupe. Ao quatro assinaturas, congelar conjunto e armar 160 ticks. No quinto elemento distinto, aplicar ×1,20 e, se suportado, penetração de 8% somente sobre resistência específica positiva (positive_specific_resistance ×0,92), sem bypass de mitigadores gerais; consumir após commit.

### Fallback sem trocar a identidade

FAIL-CLOSED: sem registry/ledger ou sem rastreamento seguro por jogador/alvo, A0239 fica inativa. Se apenas penetração específica estiver indisponível, preservar dano ×1,20. Não aproximar diversidade por total bruto de PP, quatro elementos quando não houver quinto utilizável, ou janela pessoal global.
...
| Pré-requisito visual/estrutural | A0237 + ≥9 pontos ofensivos em ≥4 elementos distintos; nenhuma escola pode representar mais da metade do investimento contado (2×max ≤ total). |
| Dependência semântica completa | A0237 Cadeia Prismática + pelo menos 9 pontos somados em bônus ofensivos de 4 elementos distintos, com 2 × max(pontos_de_uma_escola) ≤ total_de_pontos_contados. |
| Custo topológico | 3 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: ELEMENTAL_SHARED_OFFENSE. DIVERSITY_LEDGER: requisito 9 PP/4 elementos e a regra 2×max≤total usam apenas ELEMENT_OFFENSE_PP[element_id] element-specific; shared/general/bridge valem zero por padrão e uma compra nunca preenche duas escolas. Convergência coleta quatro assinaturas primárias distintas no mesmo alvo e exige uma quinta assinatura diferente para consumir. Penetração opcional de 8% atua somente sobre resistência específica positiva do quinto elemento, nunca mitigação geral, armadura aplicável, invulnerabilidade ou resistência negativa. Registry/ledger são contratos futuros, não APIs runtime existentes. Notable da Árvore Exterior; não é terminal. |
...
RPG Skill Tree + FUTURE_PROVIDER_CONTRACTS ELEMENT_SIGNATURE_REGISTRY_V1 e ELEMENTAL_DIVERSITY_LEDGER_V1 + pipeline canônico de penetração específica quando existir. O ledger ofensivo fornece ELEMENT_OFFENSE_PP[element_id] com ownership único por node; shared/general/bridge PP são excluídos por padrão. VERSION-STATUS: designs congelados, contratos não encontrados no runtime/main atual em 2026-08-29.
...
| Projeto | Head auditado | Decisão para A0239 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>signature</code>, <code>element_id</code>, <code>ledger</code>.
...
- **Chaves que o adapter precisa preservar:** <code>signature</code>, <code>element_id</code>, <code>ledger</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0239.
...
- **Fallback normativo:** FAIL-CLOSED: sem registry/ledger ou sem rastreamento seguro por jogador/alvo, A0239 fica inativa. Se apenas penetração específica estiver indisponível, preservar dano ×1,20. Não aproximar diversidade por total bruto de PP, quatro elementos quando não houver quinto utilizável, ou janela pessoal global.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Coleta: 4 assinaturas distintas em 240 ticks. Estado preparado: 160 ticks. Quinta assinatura diferente: dano elemental direto ×1,20; penetração específica opcional 8%..
...
6. **Causalidade:** correlacionar <code>signature</code>, <code>element_id</code>, <code>ledger</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>signature</code>, <code>element_id</code>, <code>ledger</code>. |
...
- Página: [A0239 — Convergência Ofensiva](https://app.notion.com/3c569db9f0db81508ca8d11630698a8e)
...
**Resultado final do Chat 1 para A0239:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
