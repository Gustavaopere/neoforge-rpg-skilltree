...
- **Fonte canônica:** [registro A0290 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db814f8456e1d0a54c7cd3).
...
- **Dependências externas à faixa:** <code>A0151</code>, <code>A0176</code>. Elas permanecem sinalizadas e não são presumidas como concluídas.
...
| Hook | Após consolidar o componente LIGHTNING direto do root e seu criticalHit imutável, snapshotar a parcela LIGHTNING após modificadores ofensivos/crítico e ANTES da mitigação do alvo inicial. Se root_action_id ainda não tiver claim A0290, selecionar no máximo um alvo novo usando LIGHTNING_CHAIN_QUERY_V1, respeitando visited set, LOS, range, PvP/alianças, invulnerabilidade e blacklist. Emitir exatamente um derived_lightning_chain_hop com base snapshot×0,35/0,50, criticalHit=false e identidade root_action_id+A0290+target_uuid. O novo alvo resolve A0283/WET e sua mitigação normalmente. O hop não reroda A0151/A0284/A0290 nem multiplicadores ofensivos raiz. |
| Fallback | FAIL-CLOSED se Specialist Raio não estiver desbloqueada, criticalHit canônico não estiver disponível ou não houver criação/seleção segura de hop adicional. Não substituir por aumento de dano do alvo inicial ou dos hops nativos existentes, pois isso sobrepõe A0286 e muda a identidade do node. |
| Regra | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna da Specialist Raio. Crítico Condutor converte um crítico ROOT real em no máximo um hop adicional; o hop não pode critar, gerar Lightning Mastery, A0284, sustain ou recursão. A base é pré-mitigação do alvo inicial para evitar dupla dependência de mitigação; target-specific A0283/WET continua resolvido no novo alvo. Um root action = no máximo um claim A0290. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
SPECIALIST_UNLOCK:LIGHTNING válido + A0285 ≥2 + A0151 ≥2 + direct_lightning_outcome_id com criticalHit=true já resolvido pela única decisão crítica canônica + componente LIGHTNING direto positivo + consulta segura de alvo secundário + alvo novo realmente atacável. Crítico de hop derivado, DoT, aura/campo, summon, automação, fake player, callback duplicado e root sem target query segura não ativam.

### Hook e ordem de execução

Após consolidar o componente LIGHTNING direto do root e seu criticalHit imutável, snapshotar a parcela LIGHTNING após modificadores ofensivos/crítico e ANTES da mitigação do alvo inicial. Se root_action_id ainda não tiver claim A0290, selecionar no máximo um alvo novo usando LIGHTNING_CHAIN_QUERY_V1, respeitando visited set, LOS, range, PvP/alianças, invulnerabilidade e blacklist. Emitir exatamente um derived_lightning_chain_hop com base snapshot×0,35/0,50, criticalHit=false e identidade root_action_id+A0290+target_uuid. O novo alvo resolve A0283/WET e sua mitigação normalmente. O hop não reroda A0151/A0284/A0290 nem multiplicadores ofensivos raiz.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Raio não estiver desbloqueada, criticalHit canônico não estiver disponível ou não houver criação/seleção segura de hop adicional. Não substituir por aumento de dano do alvo inicial ou dos hops nativos existentes, pois isso sobrepõe A0286 e muda a identidade do node.
...
| Pré-requisito visual/estrutural | Specialist Raio desbloqueada (SPECIALIST_UNLOCK:LIGHTNING) + A0285 ≥2 + A0151 Crítico Mágico ≥2. |
| Dependência semântica completa | SPECIALIST_UNLOCK:LIGHTNING confirmado por Gate A/B/C + A0285 Arco Elétrico ≥2 ranks + A0151 Crítico Mágico ≥2 ranks. Requisitos locais não substituem fundamentos LIGHTNING + ≥100 PP válidos em SPECIALIST_REGION:LIGHTNING + terminal A0176. |
| Custo topológico | 2 PP por rank; 2 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna da Specialist Raio. Crítico Condutor converte um crítico ROOT real em no máximo um hop adicional; o hop não pode critar, gerar Lightning Mastery, A0284, sustain ou recursão. A base é pré-mitigação do alvo inicial para evitar dupla dependência de mitigação; target-specific A0283/WET continua resolvido no novo alvo. Um root action = no máximo um claim A0290. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0290 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>action_id</code>, <code>outcome_id</code>, <code>root_action_id</code>, <code>target_uuid</code>, <code>claim</code>.
...
- **Chaves que o adapter precisa preservar:** <code>action_id</code>, <code>outcome_id</code>, <code>root_action_id</code>, <code>target_uuid</code>, <code>claim</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0290.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Raio não estiver desbloqueada, criticalHit canônico não estiver disponível ou não houver criação/seleção segura de hop adicional. Não substituir por aumento de dano do alvo inicial ou dos hops nativos existentes, pois isso sobrepõe A0286 e muda a identidade do node.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 2 ranks. Coeficiente do hop adicional: 0,35/0,50 da parcela LIGHTNING direta snapshot pré-mitigação-do-alvo. Um único hop A0290 por root_action_id; alvo deve ser hostil/elegível, diferente de todos os alvos já registrados na ação e dentro da consulta/range/linha de visão válidos. O hop é derived_lightning_chain_hop e criticalHit=false..
...
6. **Causalidade:** correlacionar <code>action_id</code>, <code>outcome_id</code>, <code>root_action_id</code>, <code>target_uuid</code>, <code>claim</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>action_id</code>, <code>outcome_id</code>, <code>root_action_id</code>, <code>target_uuid</code>, <code>claim</code>. |
...
- Página: [A0290 — Crítico Condutor](https://app.notion.com/3c569db9f0db814f8456e1d0a54c7cd3)
...
**Resultado final do Chat 1 para A0290:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
