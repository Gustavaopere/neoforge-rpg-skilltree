...
- **Fonte canônica:** [registro A0293 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81aa8e14c1f3ea21eba2).
...
| Hook | Após consolidar a parcela LIGHTNING direta do root, snapshotar a base após modificadores ofensivos/crítico e ANTES da mitigação do alvo inicial. Executar uma única rolagem A0293 por root_action_id: 30%/45%. Em sucesso, selecionar no máximo um novo alvo em raio 4/LOS, respeitando visited set, PvP/alianças/invulnerabilidade/blacklist, e emitir derived_lightning_branch com base snapshot×0,45/0,55, criticalHit=false, identidade root_action_id+A0293+target_uuid. O novo alvo resolve A0283/WET e mitigação própria. A branch não reentra em A0151/A0284/A0290/A0292/A0293 nem em multiplicadores ofensivos raiz. |
| Fallback | FAIL-CLOSED se Specialist Raio não estiver desbloqueada, a cardinalidade original não puder ser comprovada SINGLE_TARGET ou consulta/outcome derivado não forem seguros. Não substituir por explosão AoE, dano no alvo original, aumento de chain nativo ou classificação baseada apenas no número de targets efetivamente atingidos em uma execução. |
| Regra | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Notable interna da Specialist Raio. A0293 adiciona ramificação somente a uma ação originalmente single-target; uma ação multi-target que por acaso acertou um único alvo continua inelegível. A branch é derivada, não critica, não aplica CHARGED, não gera Lightning Mastery e não conta como multi-target nativo para A0291. Base pré-mitigação evita dupla dependência do primeiro alvo. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Para um direct_lightning_outcome_id cuja ação original seja legitimamente de alvo único, A0293 realiza uma única rolagem de 30%/45%. No sucesso, pode ramificar para exatamente 1 novo alvo hostil dentro de 4 blocos e linha de visão, causando 45%/55% do componente LIGHTNING direto canônico da ação original após modificadores ofensivos e resolução crítica, mas antes da mitigação do alvo inicial. O novo alvo aplica sua própria mitigação.

### Escalonamento aprovado

Até 2 ranks. Chance: 30%/45% por root_action_id. Base da ramificação: 45%/55% da parcela LIGHTNING direta snapshot pré-mitigação-do-alvo. Um único alvo secundário novo, raio 4 blocos, LOS e filtros normais. A ramificação é derived_lightning_branch com criticalHit=false.

### Gate de compra/ativação

SPECIALIST_UNLOCK:LIGHTNING válido + A0285 ≥2 + A0286 ≥2 + ROOT_ACTION_CARDINALITY_V1=SINGLE_TARGET para a ação LIGHTNING original + componente LIGHTNING direto positivo + ainda não rolled A0293 para o root_action_id + segundo alvo hostil realmente atacável disponível. Qualquer ação com chain/AoE/múltiplos componentes primários nativos, cardinalidade UNKNOWN, hop derivado, A0290/A0292, DoT, summon, automação, fake player ou callback duplicado é inelegível.

### Hook e ordem de execução

Após consolidar a parcela LIGHTNING direta do root, snapshotar a base após modificadores ofensivos/crítico e ANTES da mitigação do alvo inicial. Executar uma única rolagem A0293 por root_action_id: 30%/45%. Em sucesso, selecionar no máximo um novo alvo em raio 4/LOS, respeitando visited set, PvP/alianças/invulnerabilidade/blacklist, e emitir derived_lightning_branch com base snapshot×0,45/0,55, criticalHit=false, identidade root_action_id+A0293+target_uuid. O novo alvo resolve A0283/WET e mitigação própria. A branch não reentra em A0151/A0284/A0290/A0292/A0293 nem em multiplicadores ofensivos raiz.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Raio não estiver desbloqueada, a cardinalidade original não puder ser comprovada SINGLE_TARGET ou consulta/outcome derivado não forem seguros. Não substituir por explosão AoE, dano no alvo original, aumento de chain nativo ou classificação baseada apenas no número de targets efetivamente atingidos em uma execução.
...
| Pré-requisito visual/estrutural | Specialist Raio desbloqueada (SPECIALIST_UNLOCK:LIGHTNING) + A0285 ≥2 + A0286 ≥2. |
| Dependência semântica completa | SPECIALIST_UNLOCK:LIGHTNING confirmado por Gate A/B/C + A0285 Arco Elétrico ≥2 ranks + A0286 Sobrecarga ≥2 ranks. Requisitos locais não substituem fundamentos LIGHTNING + ≥100 PP válidos em SPECIALIST_REGION:LIGHTNING + terminal A0176. |
| Custo topológico | 2 PP por rank; 2 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Notable interna da Specialist Raio. A0293 adiciona ramificação somente a uma ação originalmente single-target; uma ação multi-target que por acaso acertou um único alvo continua inelegível. A branch é derivada, não critica, não aplica CHARGED, não gera Lightning Mastery e não conta como multi-target nativo para A0291. Base pré-mitigação evita dupla dependência do primeiro alvo. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0293 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>action_id</code>, <code>outcome_id</code>, <code>root_action_id</code>, <code>target_uuid</code>.
...
- **Chaves que o adapter precisa preservar:** <code>action_id</code>, <code>outcome_id</code>, <code>root_action_id</code>, <code>target_uuid</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0293.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Raio não estiver desbloqueada, a cardinalidade original não puder ser comprovada SINGLE_TARGET ou consulta/outcome derivado não forem seguros. Não substituir por explosão AoE, dano no alvo original, aumento de chain nativo ou classificação baseada apenas no número de targets efetivamente atingidos em uma execução.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 2 ranks. Chance: 30%/45% por root_action_id. Base da ramificação: 45%/55% da parcela LIGHTNING direta snapshot pré-mitigação-do-alvo. Um único alvo secundário novo, raio 4 blocos, LOS e filtros normais. A ramificação é derived_lightning_branch com criticalHit=false..
...
6. **Causalidade:** correlacionar <code>action_id</code>, <code>outcome_id</code>, <code>root_action_id</code>, <code>target_uuid</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>action_id</code>, <code>outcome_id</code>, <code>root_action_id</code>, <code>target_uuid</code>. |
...
- Página: [A0293 — Raio Ramificado](https://app.notion.com/3c569db9f0db81aa8e14c1f3ea21eba2)
...
**Resultado final do Chat 1 para A0293:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
