...
- **Fonte canônica:** [registro A0258 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81b89f0ac6ba638315d2).
...
|---|---|
| Código | A0258 |
| Nome | Combustão Súbita |
| Domínio | ARCANE/FIRE |
| Árvore | Especialista — Fogo |
| Ramo | Piromancia — Marca e Explosão |
| Camada | 5 |
...
| Hook | Antes da aplicação/renovação final de Ignição daquele direct_fire_outcome_id, ler o estado preexistente e ownership uma única vez. Sem realizar novo RNG, se o alvo já estiver sob Ignição do jogador e cooldown livre, snapshotar a parcela FIRE direta pré-mitigação/pré-crítico. Após commit do outcome/decisão canônica, emitir burst de 20% e iniciar cooldown_expiry=now+120. O fluxo normal de renovação da Ignição segue apenas o contrato do IgnitionResolver/provider. |
| Fallback | FAIL-CLOSED se Specialist Fire não estiver desbloqueada, ownership da Ignição pré-existente não for confiável, a decisão causal de Ignição não puder ser observada ou pipeline de burst derivado não existir. Não inferir por fogo visual, dano final, proximidade ou segunda rolagem. |
| Regra | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. A0258 adiciona exatamente um burst derivado de 20% à ação de reaplicação elegível e nunca cria segunda tentativa crítica/Ignição nem duplica a renovação nativa. O burst não ativa A0253/A0257, não aplica Marca e não gera mastery, sustain, recurso, Ímpeto ou procs de ação direta. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Quando uma ação FIRE direta elegível do jogador chega à única resolução canônica de Ignição contra um alvo que já possuía, antes daquele outcome, uma Ignição real atribuída ao mesmo jogador, A0258 pode disparar RPG_SUDDEN_COMBUSTION se a recarga do par estiver livre. O burst é um derived_fire_burst_outcome no próprio alvo igual a 20% do componente FIRE direto da ação gatilho medido pré-mitigação-do-alvo e pré-crítico. A0258 não cria uma segunda tentativa de Ignição e não altera por si só a renovação normal que o IgnitionResolver/provider já faria.

### Escalonamento aprovado

1 rank. Burst: 20% da parcela FIRE direta pré-mitigação/pré-crítico da ação gatilho. Recarga por jogador/alvo: 120 ticks (6 s), iniciada após burst commitado. Uma ação/outcome pode disparar no máximo uma vez.

### Gate de compra/ativação

SPECIALIST_UNLOCK:FIRE válido + requisitos locais + ação FIRE direta própria + alvo já sob Ignição real desse jogador ANTES do outcome + o outcome participa da resolução causal de aplicação/renovação de Ignição + cooldown do par livre. DoT, Propagação, Aura, retaliação, derived components, summons, automação e callbacks duplicados não ativam.

### Hook e ordem de execução

Antes da aplicação/renovação final de Ignição daquele direct_fire_outcome_id, ler o estado preexistente e ownership uma única vez. Sem realizar novo RNG, se o alvo já estiver sob Ignição do jogador e cooldown livre, snapshotar a parcela FIRE direta pré-mitigação/pré-crítico. Após commit do outcome/decisão canônica, emitir burst de 20% e iniciar cooldown_expiry=now+120. O fluxo normal de renovação da Ignição segue apenas o contrato do IgnitionResolver/provider.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Fire não estiver desbloqueada, ownership da Ignição pré-existente não for confiável, a decisão causal de Ignição não puder ser observada ou pipeline de burst derivado não existir. Não inferir por fogo visual, dano final, proximidade ou segunda rolagem.
...
| Item | Decisão |
|---|---|
| Região | Especialista — Fogo / Piromancia — Marca e Explosão |
| Camada e papel | Camada 5; Notable |
| Pré-requisito visual/estrutural | Specialist Fire desbloqueada (SPECIALIST_UNLOCK:FIRE) + A0244 Combustão ≥3 + A0245 Queima Persistente ≥2. |
| Dependência semântica completa | SPECIALIST_UNLOCK:FIRE confirmado por Gate A/B/C + A0244 Combustão ≥3 + A0245 Queima Persistente ≥2. Os requisitos locais não substituem o unlock global da Specialist Fire. |
| Custo topológico | 3 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. A0258 adiciona exatamente um burst derivado de 20% à ação de reaplicação elegível e nunca cria segunda tentativa crítica/Ignição nem duplica a renovação nativa. O burst não ativa A0253/A0257, não aplica Marca e não gera mastery, sustain, recurso, Ímpeto ou procs de ação direta. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0258 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0258.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Fire não estiver desbloqueada, ownership da Ignição pré-existente não for confiável, a decisão causal de Ignição não puder ser observada ou pipeline de burst derivado não existir. Não inferir por fogo visual, dano final, proximidade ou segunda rolagem.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Burst: 20% da parcela FIRE direta pré-mitigação/pré-crítico da ação gatilho. Recarga por jogador/alvo: 120 ticks (6 s), iniciada após burst commitado. Uma ação/outcome pode disparar no máximo uma vez..
...
- Página: [A0258 — Combustão Súbita](https://app.notion.com/3c569db9f0db81b89f0ac6ba638315d2)
...
**Resultado final do Chat 1 para A0258:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
