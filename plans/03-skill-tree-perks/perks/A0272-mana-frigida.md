...
- **Fonte canônica:** [registro A0272 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81c0b336f02ac5986d1a).
...
- **Dependências externas à faixa:** <code>A0145</code>, <code>A0169</code>. Elas permanecem sinalizadas e não são presumidas como concluídas.
...
| Ramo | Criomancia — Eficiência Frígida |
| Camada | 3 |
...
| Dependências Obrigatórias | SPECIALIST_UNLOCK:ICE confirmado por Gate A/B/C + A0266 Congelamento Progressivo ≥2 ranks + A0145 Eficiência Arcana ≥2 ranks + provider com mana nativa e regeneração positiva/modificável. Requisitos locais não substituem fundamentos ICE + ≥100 PP válidos em SPECIALIST_REGION:ICE + terminal A0169. |
| Pré-requisitos | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) + A0266 ≥2 + A0145 Eficiência Arcana ≥2 + provider de mana com regen modificável. |
| Provider/Mods | RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + FULL_FREEZE_STATE_V1 + FUTURE_PROVIDER_CONTRACT FULL_FREEZE_TRANSITION_RECEIPT_V1 + FUTURE_PROVIDER_CONTRACT MANA_REGEN_MODIFIER_V1. Iron's Spells 'n Spellbooks runtime 1.21.1-3.16.3 é provider ICE/full-freeze e de mana quando o adapter da versão puder correlacionar a ação que levou o alvo de não-full-freeze para FULLY_FROZEN e expuser regen positiva; Ars Nouveau 5.13.1 participa somente para mana/ações ICE quando adapters correspondentes existirem. Source, Soul Energy, spirits e outros recursos não são mana intercambiável. VERSION-STATUS: receipts/regen modifier são contratos RPG a implementar; não presumir API runtime existente. |
| Efeito | Quando uma ação ICE elegível do jogador causar a transição real de um alvo de estado não-congelado/com buildup incompleto para um full_freeze_state_id completo reconhecido pelo adapter da versão, A0272 arma RPG_FRIGID_MANA por 80 ticks (4 s) se a recarga estiver livre. Durante a janela, a regeneração nativa positiva de mana do provider é multiplicada por ×1,04 / ×1,08 / ×1,12. Regeneração zero/bloqueada permanece zero; não há restauração direta. |
| Escalonamento | Até 3 ranks. Janela: 80 ticks. Regeneração nativa positiva de mana: ×1,04 / ×1,08 / ×1,12. Recarga interna por jogador: 140 ticks (7 s), iniciada somente após transição elegível/armamento. Manter, renovar ou reaplicar o mesmo Congelamento durante a recarga não renova a janela. |
| Gate | SPECIALIST_UNLOCK:ICE válido + A0266 ≥2 + A0145 ≥2 + ação ICE atribuída ao jogador que cause uma TRANSIÇÃO comprovada de was_fully_frozen=false para is_fully_frozen=true no alvo + recarga livre + provider com regeneração nativa positiva/modificável de mana. Manutenção, refresh, atingir buildup sem completar, recongelar alvo já full-frozen, ambiente sem autoria, DoT/derived sem receipt causal, máquina, summon, fake player e callback duplicado não armam. |
| Hook | No commit da transição reconhecida, consumir FULL_FREEZE_TRANSITION_RECEIPT_V1 {action_id,outcome_id,target,was_fully_frozen=false,is_fully_frozen=true} e deduplicar por outcome_id+target. Se cooldown livre, criar RPG_FRIGID_MANA com expiry_tick=now+80 e cooldown_expiry_tick=now+140. Enquanto ativo, MANA_REGEN_MODIFIER_V1 multiplica SOMENTE regeneração nativa positiva por ×1,04/×1,08/×1,12; regen zero/bloqueada permanece zero. Não escrever saldo, não reduzir custo e não converter recursos. |
| Fallback | FAIL-CLOSED se Specialist Gelo não estiver desbloqueada, a transição para full freeze não puder ser atribuída causalmente à ação ou a regeneração nativa de mana não tiver hook seguro. Não usar freeze buildup próprio, polling de ticks, restauração fixa, pulsos, redução de custo, Source ou outro recurso como fallback. |
| Regra | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Perk interna da Specialist Gelo. Mana Frígida recompensa uma TRANSIÇÃO real de controle, não manutenção de estado. Não cria/usa medidor paralelo, não converte recursos e não gera mana se o provider bloquear regen. Uma mesma transição arma no máximo uma vez; morte/logout/dimensão/respec limpam a janela e Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Até 3 ranks. Janela: 80 ticks. Regeneração nativa positiva de mana: ×1,04 / ×1,08 / ×1,12. Recarga interna por jogador: 140 ticks (7 s), iniciada somente após transição elegível/armamento. Manter, renovar ou reaplicar o mesmo Congelamento durante a recarga não renova a janela.

### Gate de compra/ativação

SPECIALIST_UNLOCK:ICE válido + A0266 ≥2 + A0145 ≥2 + ação ICE atribuída ao jogador que cause uma TRANSIÇÃO comprovada de was_fully_frozen=false para is_fully_frozen=true no alvo + recarga livre + provider com regeneração nativa positiva/modificável de mana. Manutenção, refresh, atingir buildup sem completar, recongelar alvo já full-frozen, ambiente sem autoria, DoT/derived sem receipt causal, máquina, summon, fake player e callback duplicado não armam.

### Hook e ordem de execução

No commit da transição reconhecida, consumir FULL_FREEZE_TRANSITION_RECEIPT_V1 {action_id,outcome_id,target,was_fully_frozen=false,is_fully_frozen=true} e deduplicar por outcome_id+target. Se cooldown livre, criar RPG_FRIGID_MANA com expiry_tick=now+80 e cooldown_expiry_tick=now+140. Enquanto ativo, MANA_REGEN_MODIFIER_V1 multiplica SOMENTE regeneração nativa positiva por ×1,04/×1,08/×1,12; regen zero/bloqueada permanece zero. Não escrever saldo, não reduzir custo e não converter recursos.
...
| Item | Decisão |
|---|---|
| Região | Especialista — Gelo / Criomancia — Eficiência Frígida |
| Camada e papel | Camada 3; Ramo |
| Pré-requisito visual/estrutural | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) + A0266 ≥2 + A0145 Eficiência Arcana ≥2 + provider de mana com regen modificável. |
| Dependência semântica completa | SPECIALIST_UNLOCK:ICE confirmado por Gate A/B/C + A0266 Congelamento Progressivo ≥2 ranks + A0145 Eficiência Arcana ≥2 ranks + provider com mana nativa e regeneração positiva/modificável. Requisitos locais não substituem fundamentos ICE + ≥100 PP válidos em SPECIALIST_REGION:ICE + terminal A0169. |
...
RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + FULL_FREEZE_STATE_V1 + FUTURE_PROVIDER_CONTRACT FULL_FREEZE_TRANSITION_RECEIPT_V1 + FUTURE_PROVIDER_CONTRACT MANA_REGEN_MODIFIER_V1. Iron's Spells 'n Spellbooks runtime 1.21.1-3.16.3 é provider ICE/full-freeze e de mana quando o adapter da versão puder correlacionar a ação que levou o alvo de não-full-freeze para FULLY_FROZEN e expuser regen positiva; Ars Nouveau 5.13.1 participa somente para mana/ações ICE quando adapters correspondentes existirem. Source, Soul Energy, spirits e outros recursos não são mana intercambiável. VERSION-STATUS: receipts/regen modifier são contratos RPG a implementar; não presumir API runtime existente.
...
- **Contratos/capabilities nomeados no registro:** <code>SPECIALIST_GATE_RESOLVER_V1</code>, <code>FULL_FREEZE_STATE_V1</code>, <code>FULL_FREEZE_TRANSITION_RECEIPT_V1</code>, <code>MANA_REGEN_MODIFIER_V1</code>.
- **Estado:** nenhum nome de API é tratado como existente apenas por aparecer no design; FUTURE_PROVIDER_CONTRACT permanece bloqueador até prova em código/API da versão exata.

### Matriz dos quatro projetos próprios

| Projeto | Head auditado | Decisão para A0272 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>action_id</code>, <code>outcome_id</code>, <code>receipt</code>.
...
- **Chaves que o adapter precisa preservar:** <code>action_id</code>, <code>outcome_id</code>, <code>receipt</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0272.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks. Janela: 80 ticks. Regeneração nativa positiva de mana: ×1,04 / ×1,08 / ×1,12. Recarga interna por jogador: 140 ticks (7 s), iniciada somente após transição elegível/armamento. Manter, renovar ou reaplicar o mesmo Congelamento durante a recarga não renova a janela..
...
6. **Causalidade:** correlacionar <code>action_id</code>, <code>outcome_id</code>, <code>receipt</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
- **Capabilities/contracts a provar:** <code>SPECIALIST_GATE_RESOLVER_V1</code>, <code>FULL_FREEZE_STATE_V1</code>, <code>FULL_FREEZE_TRANSITION_RECEIPT_V1</code>, <code>MANA_REGEN_MODIFIER_V1</code>.
- **Dependências fora desta faixa:** <code>A0145</code>, <code>A0169</code>.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>action_id</code>, <code>outcome_id</code>, <code>receipt</code>. |
...
- Página: [A0272 — Mana Frígida](https://app.notion.com/3c569db9f0db81c0b336f02ac5986d1a)
...
**Resultado final do Chat 1 para A0272:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
