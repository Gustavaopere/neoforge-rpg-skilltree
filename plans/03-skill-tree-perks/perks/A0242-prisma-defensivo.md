...
- **Fonte canônica:** [registro A0242 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db8197a983d46ba94ee061).
...
| Dependências Obrigatórias | A0240 Matriz Adaptativa + ELEMENTAL_DIVERSITY_LEDGER_V1 comprovando ≥12 PP defensivos element-specific em ≥5 elementos distintos + mastery canônica ≥20 em pelo menos 3 desses elementos. PP shared/general/bridge não contam; uma compra não pode ser creditada em mais de um elemento. |
| Pré-requisitos | A0240 Matriz Adaptativa + domínio defensivo diversificado de pelo menos 5 elementos. |
| Provider/Mods | RPG Skill Tree + FUTURE_PROVIDER_CONTRACTS ELEMENT_SIGNATURE_REGISTRY_V1 e ELEMENTAL_DIVERSITY_LEDGER_V1 + ledger canônico de mastery + pipeline defensivo Minecraft/NeoForge + adapters de duração de estado elemental somente quando o provider expuser alteração real. VERSION-STATUS: contratos de assinatura/ledger são design congelado e não foram encontrados como APIs runtime na main auditada em 2026-08-29. |
| Efeito | A0242 coleta três element_signature_id hostis distintas de danos diretos em até 240 ticks (12 s), uma assinatura por outcome. Ao completar, arma RPG_DEFENSIVE_PRISM por 160 ticks (8 s). O próximo componente elemental de um dano hostil direto é multiplicado por ×0,70. Se o mesmo outcome aplicar um estado elemental real, hostil, removível e modificável allowlisted pelo adapter, sua duração-base é multiplicada por ×0,70. O Prisma é consumido e inicia recarga de 400 ticks (20 s) após commit. |
| Escalonamento | 1 rank. Preparação: 3 assinaturas hostis distintas em 240 ticks. Prisma: 160 ticks. Próximo componente elemental hostil direto: ×0,70. Estado elemental allowlisted do mesmo outcome: duração-base ×0,70. Recarga após consumo: 400 ticks. |
| Gate | A0240 + ledger autoritativo com ≥12 ELEMENT_DEFENSE_PP distribuídos em ≥5 element_id, mastery ≥20 em ≥3 deles + três outcomes hostis diretos com um único primary_element_signature_id canônico cada. Shared/bridge PP, a mesma compra reciclada, auto-dano, custos, ambiente sem fonte hostil, DoT derivado, callbacks duplicados e fonte multi-elemento ambígua não preparam. Consumo exige componente elemental hostil direto reconhecido. |
| Hook | Rastrear três assinaturas hostis distintas por jogador. Ao completar, armar Prisma por 160 ticks. No próximo outcome elemental hostil direto, aplicar ×0,70 como mitigador transitório no DamageMitigationResolver; se houver estado elemental allowlisted aplicado pelo mesmo outcome, ajustar somente sua duração-base por ×0,70; consumir e iniciar cooldown now+400 após commit. |
| Fallback | FAIL-CLOSED sem registry/ledger autoritativos ou rastreamento seguro de assinatura/outcome. Se somente o hook de duração do estado elemental não existir, preservar a mitigação ×0,70 e omitir essa parcela. Não aproximar diversidade por PP bruto, proximidade topológica ou resistência generalista. |
| Regra | PP_REGION: ELEMENTAL_SHARED_DEFENSE/PRISMATIC. O requisito de 12 PP/5 elementos usa somente ELEMENT_DEFENSE_PP[element_id] element-specific com ownership semântico único; shared/general/bridge valem zero por padrão e uma compra nunca preenche duas escolas. Cada outcome hostil fornece no máximo uma assinatura primária. RPG_DEFENSIVE_PRISM é mitigador transitório semanticamente distinto dos buckets geral/específicos e não cria cap defensivo global oculto. Duração só pode ser alterada na criação/renovação de estado elemental real, hostil, removível e modificável do mesmo outcome, nunca por polling. Keystone da Árvore Exterior; não pertence às 30 perks de uma Specialist elemental específica. |
...
A0242 coleta três element_signature_id hostis distintas de danos diretos em até 240 ticks (12 s), uma assinatura por outcome. Ao completar, arma RPG_DEFENSIVE_PRISM por 160 ticks (8 s). O próximo componente elemental de um dano hostil direto é multiplicado por ×0,70. Se o mesmo outcome aplicar um estado elemental real, hostil, removível e modificável allowlisted pelo adapter, sua duração-base é multiplicada por ×0,70. O Prisma é consumido e inicia recarga de 400 ticks (20 s) após commit.

### Escalonamento aprovado

1 rank. Preparação: 3 assinaturas hostis distintas em 240 ticks. Prisma: 160 ticks. Próximo componente elemental hostil direto: ×0,70. Estado elemental allowlisted do mesmo outcome: duração-base ×0,70. Recarga após consumo: 400 ticks.

### Gate de compra/ativação

A0240 + ledger autoritativo com ≥12 ELEMENT_DEFENSE_PP distribuídos em ≥5 element_id, mastery ≥20 em ≥3 deles + três outcomes hostis diretos com um único primary_element_signature_id canônico cada. Shared/bridge PP, a mesma compra reciclada, auto-dano, custos, ambiente sem fonte hostil, DoT derivado, callbacks duplicados e fonte multi-elemento ambígua não preparam. Consumo exige componente elemental hostil direto reconhecido.

### Hook e ordem de execução

Rastrear três assinaturas hostis distintas por jogador. Ao completar, armar Prisma por 160 ticks. No próximo outcome elemental hostil direto, aplicar ×0,70 como mitigador transitório no DamageMitigationResolver; se houver estado elemental allowlisted aplicado pelo mesmo outcome, ajustar somente sua duração-base por ×0,70; consumir e iniciar cooldown now+400 após commit.

### Fallback sem trocar a identidade

FAIL-CLOSED sem registry/ledger autoritativos ou rastreamento seguro de assinatura/outcome. Se somente o hook de duração do estado elemental não existir, preservar a mitigação ×0,70 e omitir essa parcela. Não aproximar diversidade por PP bruto, proximidade topológica ou resistência generalista.
...
| Pré-requisito visual/estrutural | A0240 Matriz Adaptativa + domínio defensivo diversificado de pelo menos 5 elementos. |
| Dependência semântica completa | A0240 Matriz Adaptativa + ELEMENTAL_DIVERSITY_LEDGER_V1 comprovando ≥12 PP defensivos element-specific em ≥5 elementos distintos + mastery canônica ≥20 em pelo menos 3 desses elementos. PP shared/general/bridge não contam; uma compra não pode ser creditada em mais de um elemento. |
| Custo topológico | 4 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: ELEMENTAL_SHARED_DEFENSE/PRISMATIC. O requisito de 12 PP/5 elementos usa somente ELEMENT_DEFENSE_PP[element_id] element-specific com ownership semântico único; shared/general/bridge valem zero por padrão e uma compra nunca preenche duas escolas. Cada outcome hostil fornece no máximo uma assinatura primária. RPG_DEFENSIVE_PRISM é mitigador transitório semanticamente distinto dos buckets geral/específicos e não cria cap defensivo global oculto. Duração só pode ser alterada na criação/renovação de estado elemental real, hostil, removível e modificável do mesmo outcome, nunca por polling. Keystone da Árvore Exterior; não pertence às 30 perks de uma Specialist elemental específica. |
...
RPG Skill Tree + FUTURE_PROVIDER_CONTRACTS ELEMENT_SIGNATURE_REGISTRY_V1 e ELEMENTAL_DIVERSITY_LEDGER_V1 + ledger canônico de mastery + pipeline defensivo Minecraft/NeoForge + adapters de duração de estado elemental somente quando o provider expuser alteração real. VERSION-STATUS: contratos de assinatura/ledger são design congelado e não foram encontrados como APIs runtime na main auditada em 2026-08-29.
...
| Projeto | Head auditado | Decisão para A0242 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0242.
...
- **Fallback normativo:** FAIL-CLOSED sem registry/ledger autoritativos ou rastreamento seguro de assinatura/outcome. Se somente o hook de duração do estado elemental não existir, preservar a mitigação ×0,70 e omitir essa parcela. Não aproximar diversidade por PP bruto, proximidade topológica ou resistência generalista.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Preparação: 3 assinaturas hostis distintas em 240 ticks. Prisma: 160 ticks. Próximo componente elemental hostil direto: ×0,70. Estado elemental allowlisted do mesmo outcome: duração-base ×0,70. Recarga após consumo: 400 ticks..
...
- Página: [A0242 — Prisma Defensivo](https://app.notion.com/3c569db9f0db8197a983d46ba94ee061)
...
**Resultado final do Chat 1 para A0242:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
