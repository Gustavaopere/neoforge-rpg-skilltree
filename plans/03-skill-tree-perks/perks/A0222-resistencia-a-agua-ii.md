...
- **Fonte canônica:** [registro A0222 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db813182b0c2701b301156).
...
|---|---|
| Código | A0222 |
| Nome | Resistência a Água II |
| Domínio | VITALITY |
| Árvore | Principal — VITALITY ↔ AGILITY/ARCANE |
| Ramo | Água — Defesa Fluida |
| Camada | 5 |
...
| Dependências Obrigatórias | A0221 Resistência a Água I ≥ 3 ranks + estado WET/submerso real reconhecido. |
| Pré-requisitos | A0221 Resistência a Água I ≥ 3 ranks. |
| Provider/Mods | Minecraft/NeoForge — estado de fluido/submersão autoritativo + Somake Spells 1.0.8-1.21.1-fix/Iron's Spells runtime 1.21.1-3.16.3 somente para componentes recebidos explicitamente classificados WATER + DamageMitigationResolver canônico + VITALITY/AGILITY como regiões semânticas. WET do provider só participa quando houver water_state_id real allowlisted; chuva visual não é equivalente. |
| Efeito | Enquanto o jogador estiver em estado WET real allowlisted ou realmente submerso por leitura autoritativa, o primeiro componente WATER de um dano hostil direto com recarga livre é multiplicado por ×0,75. A mitigação e o início da recarga são commitados atomicamente no mesmo outcome. Recarga interna: 200 ticks (10 s). |
| Escalonamento | 1 rank. Condição: WET real allowlisted ou submersão autoritativa. Dano WATER hostil direto: ×0,75. Recarga interna: 200 ticks iniciada somente após consumo bem-sucedido. Permanecer WET/submerso não renova nem reduz a recarga. |
| Gate | A0221 ≥3 + WET real allowlisted ou submersão autoritativa + hostile_direct_damage_outcome com componente WATER positivo + recarga livre. Chuva visual, bioma, posição aproximada, ambiente sem componente WATER hostil, custo e callback duplicado não ativam. |
| Hook | No DamageMitigationResolver, verificar condição WET/submersa antes do outcome; se elegível e recarga livre, tratar Contrafluxo como mitigador transitório distinto, aplicar ×0,75 ao componente WATER e iniciar cooldown now+200 somente após commit do evento. Deduplicar por outcome_id. |
| Fallback | Sem leitura autoritativa de submersão/WET allowlisted ou sem classificação WATER segura, A0222 fica inativa. Não estimar por chuva, bioma, partículas, coordenada, umidade ou aproximação ambiental; condição aquática nunca transforma dano não-WATER em WATER. |
| Regra | PP_REGION: VITALITY_AGILITY_WATER_BRIDGE/DEFENSE. BRIDGE_PP_POLICY: estes PP não contam simultaneamente para thresholds puros de VITALITY, AGILITY e ARCANE/WATER; uma Specialist pode whitelistar no máximo um lado semântico. WET/submersão é somente contexto de ativação defensiva: não classifica o outcome como WATER, não gera Water Mastery e não concede crédito por tick. Contrafluxo é mitigador transitório distinto de RPG_WATER_RESISTANCE, consumido uma vez por outcome elegível e com cooldown iniciado somente após commit. Notable/bridge da Árvore Exterior; não é terminal. |
...
1 rank. Condição: WET real allowlisted ou submersão autoritativa. Dano WATER hostil direto: ×0,75. Recarga interna: 200 ticks iniciada somente após consumo bem-sucedido. Permanecer WET/submerso não renova nem reduz a recarga.

### Gate de compra/ativação

A0221 ≥3 + WET real allowlisted ou submersão autoritativa + hostile_direct_damage_outcome com componente WATER positivo + recarga livre. Chuva visual, bioma, posição aproximada, ambiente sem componente WATER hostil, custo e callback duplicado não ativam.

### Hook e ordem de execução

No DamageMitigationResolver, verificar condição WET/submersa antes do outcome; se elegível e recarga livre, tratar Contrafluxo como mitigador transitório distinto, aplicar ×0,75 ao componente WATER e iniciar cooldown now+200 somente após commit do evento. Deduplicar por outcome_id.

### Fallback sem trocar a identidade

Sem leitura autoritativa de submersão/WET allowlisted ou sem classificação WATER segura, A0222 fica inativa. Não estimar por chuva, bioma, partículas, coordenada, umidade ou aproximação ambiental; condição aquática nunca transforma dano não-WATER em WATER.
...
| Item | Decisão |
|---|---|
| Região | Principal — VITALITY ↔ AGILITY/ARCANE / Água — Defesa Fluida |
| Camada e papel | Camada 5; Notable |
| Pré-requisito visual/estrutural | A0221 Resistência a Água I ≥ 3 ranks. |
| Dependência semântica completa | A0221 Resistência a Água I ≥ 3 ranks + estado WET/submerso real reconhecido. |
| Custo topológico | 1 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: VITALITY_AGILITY_WATER_BRIDGE/DEFENSE. BRIDGE_PP_POLICY: estes PP não contam simultaneamente para thresholds puros de VITALITY, AGILITY e ARCANE/WATER; uma Specialist pode whitelistar no máximo um lado semântico. WET/submersão é somente contexto de ativação defensiva: não classifica o outcome como WATER, não gera Water Mastery e não concede crédito por tick. Contrafluxo é mitigador transitório distinto de RPG_WATER_RESISTANCE, consumido uma vez por outcome elegível e com cooldown iniciado somente após commit. Notable/bridge da Árvore Exterior; não é terminal. |
...
Minecraft/NeoForge — estado de fluido/submersão autoritativo + Somake Spells 1.0.8-1.21.1-fix/Iron's Spells runtime 1.21.1-3.16.3 somente para componentes recebidos explicitamente classificados WATER + DamageMitigationResolver canônico + VITALITY/AGILITY como regiões semânticas. WET do provider só participa quando houver water_state_id real allowlisted; chuva visual não é equivalente.
...
| Projeto | Head auditado | Decisão para A0222 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0222.
...
- **Fallback normativo:** Sem leitura autoritativa de submersão/WET allowlisted ou sem classificação WATER segura, A0222 fica inativa. Não estimar por chuva, bioma, partículas, coordenada, umidade ou aproximação ambiental; condição aquática nunca transforma dano não-WATER em WATER.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Condição: WET real allowlisted ou submersão autoritativa. Dano WATER hostil direto: ×0,75. Recarga interna: 200 ticks iniciada somente após consumo bem-sucedido. Permanecer WET/submerso não renova nem reduz a recarga..
...
- Página: [A0222 — Resistência a Água II](https://app.notion.com/3c569db9f0db813182b0c2701b301156)
...
**Resultado final do Chat 1 para A0222:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
