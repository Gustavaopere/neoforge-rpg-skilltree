...
- **Fonte canônica:** [registro A0288 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81b2ae91f563031556fa).
...
|---|---|
| Código | A0288 |
| Nome | Reflexo Carregado |
| Domínio | ARCANE/LIGHTNING |
| Árvore | Especialista — Raio |
| Ramo | Eletromancia — Mobilidade Reflexa |
| Camada | 3 |
...
| Hook | Ao receber DODGE_AVOID_RECEIPT_V1 {player_uuid,threat_id/attack_id,provider,dodge_id,avoided=true}, deduplicar por receipt e, se cooldown_expiry<=now, criar exatamente uma RPG_REFLEX_CHARGE(expires_at=now+80) e cooldown_expiry=now+100. No primeiro direct_lightning_outcome_id elegível antes da expiração, fazer claim atômico da carga; aplicar ×1,15 uma única vez ao componente LIGHTNING direto. Se a ação já possuir LIGHTNING_CHAIN_QUERY_V1, aplicar também ×1,15 somente ao raio do PRIMEIRO hop antes de A0285 e caps/LOS/filtros do provider. Consumir a carga após o claim mesmo que a ação não tenha chain; hop derivado nunca a consome. |
| Fallback | FAIL-CLOSED se Specialist Raio não estiver desbloqueada ou se nenhuma integração puder provar uma esquiva que evitou uma ameaça concreta. Se o consumo LIGHTNING for seguro mas a chain query não existir, preservar apenas ×1,15 de dano direto. Não criar chain artificial, descarga automática, CHARGED, Speed permanente ou bônus por animação de dodge. |
| Regra | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna/ponte AGILITY da Specialist Raio. RPG_REFLEX_CHARGE é binária, claim-once, não acumula e não é CHARGED. A carga dura 80 ticks e o cooldown de 100 ticks começa na concessão. O consumo afeta somente a primeira ação LIGHTNING direta elegível; não gera Lightning Mastery extra nem recursa em hops. O bônus de alcance não cria saltos nem ignora filtros. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Quando uma esquiva validada pelo provider realmente evita um ataque hostil e A0288 está fora da recarga, concede uma única RPG_REFLEX_CHARGE por 80 ticks (4 s). A próxima ação LIGHTNING direta elegível do jogador consome essa carga uma vez: o componente LIGHTNING direto recebe ×1,15 e, se a própria ação já possuir encadeamento nativo/compatível com consulta mutável, o raio do primeiro hop recebe ×1,15 antes dos limites normais. Recarga de 100 ticks (5 s), iniciada no momento em que a carga é concedida.

### Escalonamento aprovado

1 rank. Carga Reflexa: 80 ticks. Cooldown compartilhado: 100 ticks desde a concessão; a carga não é renovada/reempilhada durante a própria duração ou cooldown. Bônus no consumo: +15% somente ao componente LIGHTNING direto da ação; primeiro-hop range ×1,15 somente quando já existe chain query segura.

### Gate de compra/ativação

SPECIALIST_UNLOCK:LIGHTNING válido + A0287 ≥2 + AGILITY ≥6 + receipt canônico de esquiva que prove ataque hostil realmente evitado + cooldown livre. Animação de dodge sem ameaça, i-frame sem correlação, sair do alcance, parry, block, knockback, invulnerabilidade genérica ou simplesmente não receber dano não concedem carga. RPG_REFLEX_CHARGE é recurso transitório do jogador e não é CHARGED aplicado em alvo.

### Hook e ordem de execução

Ao receber DODGE_AVOID_RECEIPT_V1 {player_uuid,threat_id/attack_id,provider,dodge_id,avoided=true}, deduplicar por receipt e, se cooldown_expiry<=now, criar exatamente uma RPG_REFLEX_CHARGE(expires_at=now+80) e cooldown_expiry=now+100. No primeiro direct_lightning_outcome_id elegível antes da expiração, fazer claim atômico da carga; aplicar ×1,15 uma única vez ao componente LIGHTNING direto. Se a ação já possuir LIGHTNING_CHAIN_QUERY_V1, aplicar também ×1,15 somente ao raio do PRIMEIRO hop antes de A0285 e caps/LOS/filtros do provider. Consumir a carga após o claim mesmo que a ação não tenha chain; hop derivado nunca a consome.
...
| Item | Decisão |
|---|---|
| Região | Especialista — Raio / Eletromancia — Mobilidade Reflexa |
| Camada e papel | Camada 3; Ponte |
| Pré-requisito visual/estrutural | Specialist Raio desbloqueada (SPECIALIST_UNLOCK:LIGHTNING) + A0287 ≥2 + AGILITY ≥6 + provider de esquiva com ataque realmente evitado. |
| Dependência semântica completa | SPECIALIST_UNLOCK:LIGHTNING confirmado por Gate A/B/C + A0287 Impulso Elétrico ≥2 ranks + atributo canônico AGILITY ≥6 + DODGE_AVOID_RECEIPT_V1 disponível. Requisitos locais não substituem fundamentos LIGHTNING + ≥100 PP válidos em SPECIALIST_REGION:LIGHTNING + terminal A0176. |
| Custo topológico | 2 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna/ponte AGILITY da Specialist Raio. RPG_REFLEX_CHARGE é binária, claim-once, não acumula e não é CHARGED. A carga dura 80 ticks e o cooldown de 100 ticks começa na concessão. O consumo afeta somente a primeira ação LIGHTNING direta elegível; não gera Lightning Mastery extra nem recursa em hops. O bônus de alcance não cria saltos nem ignora filtros. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0288 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>outcome_id</code>, <code>receipt</code>, <code>claim</code>.
...
- **Chaves que o adapter precisa preservar:** <code>outcome_id</code>, <code>receipt</code>, <code>claim</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0288.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Carga Reflexa: 80 ticks. Cooldown compartilhado: 100 ticks desde a concessão; a carga não é renovada/reempilhada durante a própria duração ou cooldown. Bônus no consumo: +15% somente ao componente LIGHTNING direto da ação; primeiro-hop range ×1,15 somente quando já existe chain query segura..
...
6. **Causalidade:** correlacionar <code>outcome_id</code>, <code>receipt</code>, <code>claim</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>outcome_id</code>, <code>receipt</code>, <code>claim</code>. |
...
- Página: [A0288 — Reflexo Carregado](https://app.notion.com/3c569db9f0db81b2ae91f563031556fa)
...
**Resultado final do Chat 1 para A0288:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
