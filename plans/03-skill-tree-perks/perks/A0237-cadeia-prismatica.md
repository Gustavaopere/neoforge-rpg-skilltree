...
- **Fonte canônica:** [registro A0237 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db8122a3d0eac9e85184cf).
...
| Dependências Obrigatórias | A0235 Ressonância Elemental + pelo menos 6 pontos somados em bônus ofensivos de 3 elementos distintos. Nenhuma escola isolada pode satisfazer o requisito. |
| Pré-requisitos | A0235 Ressonância Elemental + investimento ofensivo em pelo menos 3 elementos distintos. |
| Provider/Mods | RPG Skill Tree + FUTURE_PROVIDER_CONTRACTS ELEMENT_SIGNATURE_REGISTRY_V1 e ELEMENTAL_DIVERSITY_LEDGER_V1 + pipeline canônico de penetração específica quando existir. O ledger ofensivo prova ELEMENT_OFFENSE_PP[element_id]; shared/general/bridge PP são excluídos por padrão e cada node conta uma única vez para um único elemento. VERSION-STATUS: designs congelados, contratos não encontrados no runtime/main atual em 2026-08-29. |
| Efeito | No mesmo par jogador/alvo, A0237 coleta até três element_signature_id canônicos distintos produzidos por ações elementais diretas em uma janela móvel de 200 ticks (10 s). Cada action_id/outcome_id contribui com no máximo uma assinatura. Ao atingir três assinaturas, prepara RPG_PRISMATIC_CHAIN por 120 ticks (6 s). A próxima ação elemental direta usando uma das três assinaturas já coletadas recebe dano direto ×1,12 e, se existir estágio seguro de penetração elemental, 5% de penetração; então consome o estado. |
| Escalonamento | 1 rank. Coleta: 3 assinaturas distintas em 200 ticks. Estado preparado: 120 ticks. Consumidor com assinatura do conjunto: dano elemental direto ×1,12; penetração específica opcional de 5% quando suportada. |
| Gate | A0235 + ELEMENTAL_DIVERSITY_LEDGER_V1 comprovando ≥6 PP ofensivos válidos em ≥3 element_id distintos + mesmo alvo + ações diretas com um único primary_element_signature_id. PP shared/general/bridge, compra híbrida sem ownership único, DoT, summons sem autoria, automação, derived components e callbacks duplicados não adicionam assinaturas. O consumidor deve usar assinatura contida no conjunto congelado. |
| Hook | Manter conjunto por jogador/alvo com timestamps e dedupe por action_id/outcome_id. Ao alcançar três assinaturas, congelar o conjunto e armar por 120 ticks. No próximo outcome direto com assinatura contida, aplicar ×1,12 e, se suportado, penetração de 5% somente sobre resistência específica positiva do elemento consumidor (positive_specific_resistance ×0,95), sem bypass de mitigadores gerais; consumir após commit. |
| Fallback | FAIL-CLOSED: sem registry/ledger ou sem rastreamento seguro por jogador/alvo, A0237 fica inativa. Se somente a penetração específica não puder ser implementada com segurança, preservar dano ×1,12 e omitir apenas essa parcela; não substituir por Armor Negation, shred geral ou bypass de mitigação. |
| Regra | PP_REGION: ELEMENTAL_SHARED_OFFENSE. DIVERSITY_LEDGER: requisito 6 PP/3 elementos usa apenas ELEMENT_OFFENSE_PP[element_id] element-specific; shared/general/bridge valem zero por padrão e a mesma compra nunca preenche duas escolas. Cadeia coleta três assinaturas primárias distintas no mesmo alvo e congela o conjunto; consumidor deve usar uma dessas assinaturas. Penetração opcional de 5% atua somente sobre resistência específica positiva do elemento consumidor, nunca RPG_ELEMENTAL_GENERAL_RESISTANCE, armadura aplicável, invulnerabilidade ou resistência negativa. Registry/ledger são contratos futuros, não APIs runtime existentes. Notable da Árvore Exterior; não é terminal. |
...
No mesmo par jogador/alvo, A0237 coleta até três element_signature_id canônicos distintos produzidos por ações elementais diretas em uma janela móvel de 200 ticks (10 s). Cada action_id/outcome_id contribui com no máximo uma assinatura. Ao atingir três assinaturas, prepara RPG_PRISMATIC_CHAIN por 120 ticks (6 s). A próxima ação elemental direta usando uma das três assinaturas já coletadas recebe dano direto ×1,12 e, se existir estágio seguro de penetração elemental, 5% de penetração; então consome o estado.

### Escalonamento aprovado

1 rank. Coleta: 3 assinaturas distintas em 200 ticks. Estado preparado: 120 ticks. Consumidor com assinatura do conjunto: dano elemental direto ×1,12; penetração específica opcional de 5% quando suportada.

### Gate de compra/ativação

A0235 + ELEMENTAL_DIVERSITY_LEDGER_V1 comprovando ≥6 PP ofensivos válidos em ≥3 element_id distintos + mesmo alvo + ações diretas com um único primary_element_signature_id. PP shared/general/bridge, compra híbrida sem ownership único, DoT, summons sem autoria, automação, derived components e callbacks duplicados não adicionam assinaturas. O consumidor deve usar assinatura contida no conjunto congelado.

### Hook e ordem de execução

Manter conjunto por jogador/alvo com timestamps e dedupe por action_id/outcome_id. Ao alcançar três assinaturas, congelar o conjunto e armar por 120 ticks. No próximo outcome direto com assinatura contida, aplicar ×1,12 e, se suportado, penetração de 5% somente sobre resistência específica positiva do elemento consumidor (positive_specific_resistance ×0,95), sem bypass de mitigadores gerais; consumir após commit.

### Fallback sem trocar a identidade

FAIL-CLOSED: sem registry/ledger ou sem rastreamento seguro por jogador/alvo, A0237 fica inativa. Se somente a penetração específica não puder ser implementada com segurança, preservar dano ×1,12 e omitir apenas essa parcela; não substituir por Armor Negation, shred geral ou bypass de mitigação.
...
| Pré-requisito visual/estrutural | A0235 Ressonância Elemental + investimento ofensivo em pelo menos 3 elementos distintos. |
| Dependência semântica completa | A0235 Ressonância Elemental + pelo menos 6 pontos somados em bônus ofensivos de 3 elementos distintos. Nenhuma escola isolada pode satisfazer o requisito. |
| Custo topológico | 2 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: ELEMENTAL_SHARED_OFFENSE. DIVERSITY_LEDGER: requisito 6 PP/3 elementos usa apenas ELEMENT_OFFENSE_PP[element_id] element-specific; shared/general/bridge valem zero por padrão e a mesma compra nunca preenche duas escolas. Cadeia coleta três assinaturas primárias distintas no mesmo alvo e congela o conjunto; consumidor deve usar uma dessas assinaturas. Penetração opcional de 5% atua somente sobre resistência específica positiva do elemento consumidor, nunca RPG_ELEMENTAL_GENERAL_RESISTANCE, armadura aplicável, invulnerabilidade ou resistência negativa. Registry/ledger são contratos futuros, não APIs runtime existentes. Notable da Árvore Exterior; não é terminal. |
...
RPG Skill Tree + FUTURE_PROVIDER_CONTRACTS ELEMENT_SIGNATURE_REGISTRY_V1 e ELEMENTAL_DIVERSITY_LEDGER_V1 + pipeline canônico de penetração específica quando existir. O ledger ofensivo prova ELEMENT_OFFENSE_PP[element_id]; shared/general/bridge PP são excluídos por padrão e cada node conta uma única vez para um único elemento. VERSION-STATUS: designs congelados, contratos não encontrados no runtime/main atual em 2026-08-29.
...
| Projeto | Head auditado | Decisão para A0237 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0237.
...
- **Fallback normativo:** FAIL-CLOSED: sem registry/ledger ou sem rastreamento seguro por jogador/alvo, A0237 fica inativa. Se somente a penetração específica não puder ser implementada com segurança, preservar dano ×1,12 e omitir apenas essa parcela; não substituir por Armor Negation, shred geral ou bypass de mitigação.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Coleta: 3 assinaturas distintas em 200 ticks. Estado preparado: 120 ticks. Consumidor com assinatura do conjunto: dano elemental direto ×1,12; penetração específica opcional de 5% quando suportada..
...
- Página: [A0237 — Cadeia Prismática](https://app.notion.com/3c569db9f0db8122a3d0eac9e85184cf)
...
**Resultado final do Chat 1 para A0237:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
