# A0234 — Fundamento Defensivo Elemental
...
- **Fonte canônica:** [registro A0234 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db819c8ed2cf7cef3de80a).
...
|---|---|
| Código | A0234 |
| Nome | Fundamento Defensivo Elemental |
| Domínio | VITALITY |
...
| Dependências Obrigatórias | Acesso efetivamente desbloqueado ao corredor VITALITY de resistências elementais. Fonte recebida precisa possuir classificação elemental reconhecida. |
| Pré-requisitos | Acesso ao corredor VITALITY de resistências elementais. |
| Provider/Mods | RPG Skill Tree + FUTURE_PROVIDER_CONTRACT ELEMENT_SIGNATURE_REGISTRY_V1 + DamageMitigationResolver canônico + Minecraft/NeoForge e adapters elementais versionados. O registry fornece somente classificação explícita de componentes/assinaturas; providers nativos de resistência elementar precisam ser unificados por adapter para evitar backend duplicado. VERSION-STATUS: design congelado, element_signature_id não encontrado no runtime/main atual em 2026-08-29. |
| Efeito | +2% de resistência contra cada componente elemental elegível reconhecido. A0234 contribui para o bucket canônico RPG_ELEMENTAL_GENERAL_RESISTANCE e é resolvido uma única vez por componente; resistências específicas permanecem buckets distintos. |
| Escalonamento | 1 rank. Contribuição ao RPG_ELEMENTAL_GENERAL_RESISTANCE: +2% por componente elemental elegível. Teto próprio de A0234: 2%; não existe cap defensivo global implícito. |
| Gate | Acesso VITALITY de resistências elementais + componente com element_id reconhecido. Não cobre dano físico, projétil genérico, ambiente, Vazio, temperatura corporal ou fonte ambígua sem classificação elemental. |
| Hook | DamageMitigationResolver: aplicar RPG_ELEMENTAL_GENERAL_RESISTANCE uma única vez por componente elemental e então compor multiplicativamente com o bucket específico daquele elemento por serem mitigadores semanticamente distintos. Nunca aplicar o general novamente a subcomponentes derivados do mesmo parcel. |
| Fallback | Sem ELEMENT_SIGNATURE_REGISTRY_V1 implementado ou sem classificação elemental explícita do componente, A0234 fica inativa naquele componente. Não inferir por entidade, bioma, dimensão, partícula, provider ou efeito visual; não aplicar a dano físico, Vazio não registrado, temperatura corporal ou fonte ambígua. Se a mesma mitigação geral já for resolvida nativamente e não houver unificação segura, a parcela RPG fica fail-closed em vez de duplicar. |
| Regra | FUNDAMENTO_EXTERIOR: ELEMENTAL_SHARED_DEFENSE. PP_REGION: ELEMENTAL_SHARED_DEFENSE. A0234 é fundamento compartilhado generalista da Árvore Exterior e seus PP não contam automaticamente para todos os Gate B de FIRE/ICE/LIGHTNING/etc. RPG_ELEMENTAL_GENERAL_RESISTANCE é semanticamente distinto das resistências específicas, mas cada bucket é resolvido uma única vez por componente; backend nativo equivalente deve ser unificado, nunca duplicado. Para convergência defensiva, um outcome hostil fornece no máximo uma assinatura primária canônica. ELEMENT_SIGNATURE_REGISTRY_V1 é contrato futuro, não API runtime existente. |
...
+2% de resistência contra cada componente elemental elegível reconhecido. A0234 contribui para o bucket canônico RPG_ELEMENTAL_GENERAL_RESISTANCE e é resolvido uma única vez por componente; resistências específicas permanecem buckets distintos.

### Escalonamento aprovado

1 rank. Contribuição ao RPG_ELEMENTAL_GENERAL_RESISTANCE: +2% por componente elemental elegível. Teto próprio de A0234: 2%; não existe cap defensivo global implícito.

### Gate de compra/ativação

Acesso VITALITY de resistências elementais + componente com element_id reconhecido. Não cobre dano físico, projétil genérico, ambiente, Vazio, temperatura corporal ou fonte ambígua sem classificação elemental.

### Hook e ordem de execução

DamageMitigationResolver: aplicar RPG_ELEMENTAL_GENERAL_RESISTANCE uma única vez por componente elemental e então compor multiplicativamente com o bucket específico daquele elemento por serem mitigadores semanticamente distintos. Nunca aplicar o general novamente a subcomponentes derivados do mesmo parcel.

### Fallback sem trocar a identidade

Sem ELEMENT_SIGNATURE_REGISTRY_V1 implementado ou sem classificação elemental explícita do componente, A0234 fica inativa naquele componente. Não inferir por entidade, bioma, dimensão, partícula, provider ou efeito visual; não aplicar a dano físico, Vazio não registrado, temperatura corporal ou fonte ambígua. Se a mesma mitigação geral já for resolvida nativamente e não houver unificação segura, a parcela RPG fica fail-closed em vez de duplicar.
...
| Pré-requisito visual/estrutural | Acesso ao corredor VITALITY de resistências elementais. |
| Dependência semântica completa | Acesso efetivamente desbloqueado ao corredor VITALITY de resistências elementais. Fonte recebida precisa possuir classificação elemental reconhecida. |
| Custo topológico | 1 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | FUNDAMENTO_EXTERIOR: ELEMENTAL_SHARED_DEFENSE. PP_REGION: ELEMENTAL_SHARED_DEFENSE. A0234 é fundamento compartilhado generalista da Árvore Exterior e seus PP não contam automaticamente para todos os Gate B de FIRE/ICE/LIGHTNING/etc. RPG_ELEMENTAL_GENERAL_RESISTANCE é semanticamente distinto das resistências específicas, mas cada bucket é resolvido uma única vez por componente; backend nativo equivalente deve ser unificado, nunca duplicado. Para convergência defensiva, um outcome hostil fornece no máximo uma assinatura primária canônica. ELEMENT_SIGNATURE_REGISTRY_V1 é contrato futuro, não API runtime existente. |
...
| Projeto | Head auditado | Decisão para A0234 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>signature</code>, <code>element_id</code>.
...
- **Chaves que o adapter precisa preservar:** <code>signature</code>, <code>element_id</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0234.
...
- **Fallback normativo:** Sem ELEMENT_SIGNATURE_REGISTRY_V1 implementado ou sem classificação elemental explícita do componente, A0234 fica inativa naquele componente. Não inferir por entidade, bioma, dimensão, partícula, provider ou efeito visual; não aplicar a dano físico, Vazio não registrado, temperatura corporal ou fonte ambígua. Se a mesma mitigação geral já for resolvida nativamente e não houver unificação segura, a parcela RPG fica fail-closed em vez de duplicar.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Contribuição ao RPG_ELEMENTAL_GENERAL_RESISTANCE: +2% por componente elemental elegível. Teto próprio de A0234: 2%; não existe cap defensivo global implícito..
...
6. **Causalidade:** correlacionar <code>signature</code>, <code>element_id</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>signature</code>, <code>element_id</code>. |
...
- Página: [A0234 — Fundamento Defensivo Elemental](https://app.notion.com/3c569db9f0db819c8ed2cf7cef3de80a)
...
**Resultado final do Chat 1 para A0234:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
