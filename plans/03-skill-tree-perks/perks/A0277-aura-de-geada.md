...
- **Fonte canônica:** [registro A0277 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81b98720d3122b9ee0fd).
...
| Ramo | Criomancia — Campo e Controle |
| Camada | 5 |
...
| Dependências Obrigatórias | SPECIALIST_UNLOCK:ICE confirmado por Gate A/B/C + A0270 Frio Persistente ≥2 ranks + A0266 Congelamento Progressivo ≥2 ranks. Requisitos locais não substituem fundamentos ICE + ≥100 PP válidos em SPECIALIST_REGION:ICE + terminal A0169. |
| Pré-requisitos | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) + A0270 Frio Persistente ≥2 + A0266 Congelamento Progressivo ≥2. |
| Provider/Mods | RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + receipts de ações ICE/freeze buildup via FREEZE_BUILDUP_ADAPTER_V1 + CHILL_STATE_REGISTRY_V1/CHILL_APPLICATION_RESOLVER_V1 + FUTURE_PROVIDER_CONTRACT DERIVED_COMBAT_OUTCOME_PIPELINE_V1 + consulta espacial/targetability server-side. Iron's Spells 'n Spellbooks runtime 1.21.1-3.16.3 é provider CHILL real (irons_spellbooks:chilled) e de freeze buildup quando o adapter comprovar delta causal; Ars Nouveau 5.13.1/Ars Elemental 0.7.10.1 e demais providers ICE só entram por adapters explícitos. Sable 2.0.5/Aeronautics exigem consulta espacial no sublevel correto quando caster/alvo estiverem nesses espaços; não são providers ICE. VERSION-STATUS: aura/receipts são contratos RPG a implementar/adaptar; nenhum medidor de congelamento paralelo é criado. |
| Efeito | Ao registrar 5 ações ICE diretas distintas do próprio jogador dentro de 80 ticks (4 s), cada uma com contribuição positiva de freeze_buildup nativo/compatível realmente entregue ao provider, e com a recarga livre, A0277 limpa a preparação e arma RPG_FROST_AURA por 100/140 ticks (5/7 s). A cada 20 ticks, em raio de 4 blocos, um derived_ice_aura_pulse tenta aplicar/renovar somente um chill_state_id real/compatível por adapter seguro. O pulso NÃO cria nem acrescenta freeze_buildup próprio do Skill Tree. |
| Escalonamento | Até 2 ranks. Preparação: 5 action_id ICE diretas distintas com freeze_buildup > 0 confirmado em até 80 ticks. Aura: 100/140 ticks. Pulso: 20 ticks, raio 4 blocos. Recarga por jogador: 280 ticks (14 s), iniciada na ativação. Aura/preparação não acumulam nem rearmam durante aura/recarga. |
| Gate | SPECIALIST_UNLOCK:ICE válido + A0270 ≥2 + A0266 ≥2 + recarga livre + cinco ações ICE DIRETAS distintas do jogador em até 80 ticks, cada uma com receipt que confirme freeze_buildup_base_or_final > 0 realmente entregue/aceito pelo provider. Máximo um crédito por action_id/outcome_id. DoT, tick de CHILL, aura, retaliação, derived component, summon, automação, fake player, callback repetido e ação cujo buildup foi bloqueado/zerado não avançam preparação. |
| Hook | Manter por jogador os cinco últimos receipts elegíveis {action_id,outcome_id,timestamp}; remover >80 ticks. Ao alcançar cinco, limpar a preparação, criar uma única RPG_FROST_AURA por 100/140 ticks e cooldown_expiry=now+280. Durante aura/preparação/recarga não rearmar nem acumular segunda instância. A cada 20 ticks, consultar entidades vivas realmente atacáveis em raio 4 NO MESMO espaço/sublevel do jogador e emitir no máximo um derived_ice_aura_pulse por alvo, id=aura_instance_id+pulse_index+target_uuid. O pulso usa CHILL_APPLICATION_RESOLVER_V1 para aplicar/renovar CHILL real preservando regras de merge/ownership do provider, respeita PvP/alianças/invulnerabilidade e NUNCA chama RPG_FREEZE_BUILDUP_MULTIPLIER, crítico, mastery ou resolvers de ação direta. |
| Fallback | FAIL-CLOSED se Specialist Gelo não estiver desbloqueada, os receipts de buildup não forem confiáveis, a consulta espacial/targetability não puder ser feita com segurança ou não houver chill_state_id real/compatível aplicável. Em Sable/Aeronautics, incapacidade de resolver o mesmo sublevel também fecha a aura. Não substituir por Slowness genérica, bônus pessoal de buildup, temperatura corporal, dano ICE, medidor próprio ou aura que ignore PvP/alianças. |
| Regra | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Perk interna da Specialist Gelo. A0277 usa SOMENTE o fato de que cinco ações diretas entregaram buildup real para PREPARAR a aura; não soma esses valores num segundo medidor. Pulsos são DERIVADOS e servem apenas para CHILL real: não geram Ice Mastery, Mana Frígida, sequência, A0266/A0275, nova preparação, crítico, sustain ou outros procs diretos. Cada alvo recebe no máximo uma tentativa por pulso e regras de attackability/PvP/aliança permanecem canônicas. Morte/logout/dimensão/respec encerram aura/preparação; Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Ao registrar 5 ações ICE diretas distintas do próprio jogador dentro de 80 ticks (4 s), cada uma com contribuição positiva de freeze_buildup nativo/compatível realmente entregue ao provider, e com a recarga livre, A0277 limpa a preparação e arma RPG_FROST_AURA por 100/140 ticks (5/7 s). A cada 20 ticks, em raio de 4 blocos, um derived_ice_aura_pulse tenta aplicar/renovar somente um chill_state_id real/compatível por adapter seguro. O pulso NÃO cria nem acrescenta freeze_buildup próprio do Skill Tree.

### Escalonamento aprovado

Até 2 ranks. Preparação: 5 action_id ICE diretas distintas com freeze_buildup > 0 confirmado em até 80 ticks. Aura: 100/140 ticks. Pulso: 20 ticks, raio 4 blocos. Recarga por jogador: 280 ticks (14 s), iniciada na ativação. Aura/preparação não acumulam nem rearmam durante aura/recarga.

### Gate de compra/ativação

SPECIALIST_UNLOCK:ICE válido + A0270 ≥2 + A0266 ≥2 + recarga livre + cinco ações ICE DIRETAS distintas do jogador em até 80 ticks, cada uma com receipt que confirme freeze_buildup_base_or_final > 0 realmente entregue/aceito pelo provider. Máximo um crédito por action_id/outcome_id. DoT, tick de CHILL, aura, retaliação, derived component, summon, automação, fake player, callback repetido e ação cujo buildup foi bloqueado/zerado não avançam preparação.

### Hook e ordem de execução

Manter por jogador os cinco últimos receipts elegíveis {action_id,outcome_id,timestamp}; remover >80 ticks. Ao alcançar cinco, limpar a preparação, criar uma única RPG_FROST_AURA por 100/140 ticks e cooldown_expiry=now+280. Durante aura/preparação/recarga não rearmar nem acumular segunda instância. A cada 20 ticks, consultar entidades vivas realmente atacáveis em raio 4 NO MESMO espaço/sublevel do jogador e emitir no máximo um derived_ice_aura_pulse por alvo, id=aura_instance_id+pulse_index+target_uuid. O pulso usa CHILL_APPLICATION_RESOLVER_V1 para aplicar/renovar CHILL real preservando regras de merge/ownership do provider, respeita PvP/alianças/invulnerabilidade e NUNCA chama RPG_FREEZE_BUILDUP_MULTIPLIER, crítico, mastery ou resolvers de ação direta.
...
| Item | Decisão |
|---|---|
| Região | Especialista — Gelo / Criomancia — Campo e Controle |
| Camada e papel | Camada 5; Notable |
| Pré-requisito visual/estrutural | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) + A0270 Frio Persistente ≥2 + A0266 Congelamento Progressivo ≥2. |
| Dependência semântica completa | SPECIALIST_UNLOCK:ICE confirmado por Gate A/B/C + A0270 Frio Persistente ≥2 ranks + A0266 Congelamento Progressivo ≥2 ranks. Requisitos locais não substituem fundamentos ICE + ≥100 PP válidos em SPECIALIST_REGION:ICE + terminal A0169. |
...
RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + receipts de ações ICE/freeze buildup via FREEZE_BUILDUP_ADAPTER_V1 + CHILL_STATE_REGISTRY_V1/CHILL_APPLICATION_RESOLVER_V1 + FUTURE_PROVIDER_CONTRACT DERIVED_COMBAT_OUTCOME_PIPELINE_V1 + consulta espacial/targetability server-side. Iron's Spells 'n Spellbooks runtime 1.21.1-3.16.3 é provider CHILL real (irons_spellbooks:chilled) e de freeze buildup quando o adapter comprovar delta causal; Ars Nouveau 5.13.1/Ars Elemental 0.7.10.1 e demais providers ICE só entram por adapters explícitos. Sable 2.0.5/Aeronautics exigem consulta espacial no sublevel correto quando caster/alvo estiverem nesses espaços; não são providers ICE. VERSION-STATUS: aura/receipts são contratos RPG a implementar/adaptar; nenhum medidor de congelamento paralelo é criado.
...
- **Contratos/capabilities nomeados no registro:** <code>SPECIALIST_GATE_RESOLVER_V1</code>, <code>FREEZE_BUILDUP_ADAPTER_V1</code>, <code>CHILL_STATE_REGISTRY_V1</code>, <code>CHILL_APPLICATION_RESOLVER_V1</code>, <code>DERIVED_COMBAT_OUTCOME_PIPELINE_V1</code>.
- **Estado:** nenhum nome de API é tratado como existente apenas por aparecer no design; FUTURE_PROVIDER_CONTRACT permanece bloqueador até prova em código/API da versão exata.

### Matriz dos quatro projetos próprios

| Projeto | Head auditado | Decisão para A0277 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0277.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 2 ranks. Preparação: 5 action_id ICE diretas distintas com freeze_buildup > 0 confirmado em até 80 ticks. Aura: 100/140 ticks. Pulso: 20 ticks, raio 4 blocos. Recarga por jogador: 280 ticks (14 s), iniciada na ativação. Aura/preparação não acumulam nem rearmam durante aura/recarga..
...
- **Capabilities/contracts a provar:** <code>SPECIALIST_GATE_RESOLVER_V1</code>, <code>FREEZE_BUILDUP_ADAPTER_V1</code>, <code>CHILL_STATE_REGISTRY_V1</code>, <code>CHILL_APPLICATION_RESOLVER_V1</code>, <code>DERIVED_COMBAT_OUTCOME_PIPELINE_V1</code>.
...
- Página: [A0277 — Aura de Geada](https://app.notion.com/3c569db9f0db81b98720d3122b9ee0fd)
...
**Resultado final do Chat 1 para A0277:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
