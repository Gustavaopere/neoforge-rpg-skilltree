...
- **Fonte canônica:** [registro A0223 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db8131831fd90263aecc3b).
...
|---|---|
| Código | A0223 |
| Nome | Imbuimento de Água |
| Domínio | ARCANE |
| Árvore | Principal — ARCANE ↔ MARTIAL/AGILITY |
| Ramo | Água — Imbuimento Fluido |
| Camada | 5 |
...
| Dependências Obrigatórias | A0219 Dano de Água I ≥ 2 ranks + pelo menos uma mastery canônica de família melee elegível ≥20. Famílias ativas: epic_sword, epic_axe, epic_spear somente melee, epic_dagger, epic_hammer, combat_mace e combat_scythe; combat_fist somente após P-0032 e nunca mão vazia. |
| Pré-requisitos | A0219 ≥2 + mastery ≥20 na família melee usada. Ranged/projéteis excluídos; combat_fist fail-closed até P-0032. |
| Provider/Mods | Somake Spells 1.0.8-1.21.1-fix (Aqua School) + Iron's Spells 'n Spellbooks runtime 1.21.1-3.16.3 como framework + Ars Elemental 0.7.10.1/outros somente quando a ação de origem estiver explicitamente WATER + Epic Fight 21.17.3.1/classificador canônico das lanes melee. Weapons of Miracles 2.0.176 só participa com arma concretamente mapeada; combat_fist depende de P-0032. |
| Efeito | Após uma ação WATER direta elegível, abre janela de Imbuimento por 120 ticks (6 s). Cada direct_melee_outcome legítimo com lane elegível e mastery ≥20 adiciona um único derived_component:WATER igual a 5% do dano-base canônico elegível por rank: 5% / 10%. added_water = base_weapon_damage_pre_target_mitigation_pre_critical_pre_added_elements × (0,05 × rank). |
| Escalonamento | Até 2 ranks; janela fixa 120 ticks; componente WATER = 5% / 10% do dano-base elegível. Recast WATER válido renova somente a expiração, sem empilhar magnitude. |
| Gate | A0219 ≥2 + janela ativa + direct_melee_outcome do próprio jogador + lane entre epic_sword, epic_axe, epic_spear melee, epic_dagger, epic_hammer, combat_mace, combat_scythe e mastery ≥20. combat_fist só após P-0032. Ranged, projéteis, lança arremessada, mão vazia, sweep derivado não-direto, DoT, summons, automação, fake player e derived_component são inelegíveis. |
| Hook | Após commit da ação WATER elegível, registrar expiração. Em cada outcome melee elegível, resolver lane/mastery, deduplicar outcome_id, calcular base pré-mitigação/pré-crítico/pré-elementos e anexar um único derived_component:WATER ao mesmo evento. Herdar exatamente criticalHit e identidade causal do pai; nunca criar novo DamageSource, segunda resolução, mastery, sustain ou proc. |
| Fallback | Sem ação WATER de origem explicitamente classificada, lane/mastery inequívoca ou composição segura no mesmo outcome, A0223 fica inativa. Não abrir janela por WET ambiental, chuva, imersão, natação ou dano físico genérico; não simular segundo ataque. combat_fist permanece fail-closed até P-0032. |
| Regra | PP_REGION: ARCANE_MARTIAL_AGILITY_WATER_BRIDGE. BRIDGE_PP_POLICY: estes PP não contam simultaneamente para thresholds puros de ARCANE/WATER, MARTIAL e AGILITY; híbridos exigem os gates completos dos pais. derived_component:WATER pertence ao mesmo action_id/outcome_id melee, herda a única decisão crítica e não cria segundo DamageSource, WET, mastery, sustain, proc ou parcel térmico. WET/imersão não renovam a janela. Ponte da Árvore Exterior; não é terminal. |
...
Após uma ação WATER direta elegível, abre janela de Imbuimento por 120 ticks (6 s). Cada direct_melee_outcome legítimo com lane elegível e mastery ≥20 adiciona um único derived_component:WATER igual a 5% do dano-base canônico elegível por rank: 5% / 10%. added_water = base_weapon_damage_pre_target_mitigation_pre_critical_pre_added_elements × (0,05 × rank).

### Escalonamento aprovado

Até 2 ranks; janela fixa 120 ticks; componente WATER = 5% / 10% do dano-base elegível. Recast WATER válido renova somente a expiração, sem empilhar magnitude.

### Gate de compra/ativação

A0219 ≥2 + janela ativa + direct_melee_outcome do próprio jogador + lane entre epic_sword, epic_axe, epic_spear melee, epic_dagger, epic_hammer, combat_mace, combat_scythe e mastery ≥20. combat_fist só após P-0032. Ranged, projéteis, lança arremessada, mão vazia, sweep derivado não-direto, DoT, summons, automação, fake player e derived_component são inelegíveis.
...
Sem ação WATER de origem explicitamente classificada, lane/mastery inequívoca ou composição segura no mesmo outcome, A0223 fica inativa. Não abrir janela por WET ambiental, chuva, imersão, natação ou dano físico genérico; não simular segundo ataque. combat_fist permanece fail-closed até P-0032.
...
| Item | Decisão |
|---|---|
| Região | Principal — ARCANE ↔ MARTIAL/AGILITY / Água — Imbuimento Fluido |
| Camada e papel | Camada 5; Ponte |
| Pré-requisito visual/estrutural | A0219 ≥2 + mastery ≥20 na família melee usada. Ranged/projéteis excluídos; combat_fist fail-closed até P-0032. |
| Dependência semântica completa | A0219 Dano de Água I ≥ 2 ranks + pelo menos uma mastery canônica de família melee elegível ≥20. Famílias ativas: epic_sword, epic_axe, epic_spear somente melee, epic_dagger, epic_hammer, combat_mace e combat_scythe; combat_fist somente após P-0032 e nunca mão vazia. |
| Custo topológico | 2 PP por rank; 2 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: ARCANE_MARTIAL_AGILITY_WATER_BRIDGE. BRIDGE_PP_POLICY: estes PP não contam simultaneamente para thresholds puros de ARCANE/WATER, MARTIAL e AGILITY; híbridos exigem os gates completos dos pais. derived_component:WATER pertence ao mesmo action_id/outcome_id melee, herda a única decisão crítica e não cria segundo DamageSource, WET, mastery, sustain, proc ou parcel térmico. WET/imersão não renovam a janela. Ponte da Árvore Exterior; não é terminal. |
...
Somake Spells 1.0.8-1.21.1-fix (Aqua School) + Iron's Spells 'n Spellbooks runtime 1.21.1-3.16.3 como framework + Ars Elemental 0.7.10.1/outros somente quando a ação de origem estiver explicitamente WATER + Epic Fight 21.17.3.1/classificador canônico das lanes melee. Weapons of Miracles 2.0.176 só participa com arma concretamente mapeada; combat_fist depende de P-0032.
...
| Projeto | Head auditado | Decisão para A0223 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0223.
...
- **Fallback normativo:** Sem ação WATER de origem explicitamente classificada, lane/mastery inequívoca ou composição segura no mesmo outcome, A0223 fica inativa. Não abrir janela por WET ambiental, chuva, imersão, natação ou dano físico genérico; não simular segundo ataque. combat_fist permanece fail-closed até P-0032.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 2 ranks; janela fixa 120 ticks; componente WATER = 5% / 10% do dano-base elegível. Recast WATER válido renova somente a expiração, sem empilhar magnitude..
...
- Página: [A0223 — Imbuimento de Água](https://app.notion.com/3c569db9f0db8131831fd90263aecc3b)
...
**Resultado final do Chat 1 para A0223:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
