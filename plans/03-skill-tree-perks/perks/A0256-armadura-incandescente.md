...
- **Fonte canônica:** [registro A0256 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db8114b478f704108bff8d).
...
- **Dependências externas à faixa:** <code>A0158</code>. Elas permanecem sinalizadas e não são presumidas como concluídas.
...
| Hook | Após commit do outcome gatilho, registrar por jogador source_outcome_id, bucket, expiry_tick=now+100, cooldown_expiry_tick e retaliation_available=true. No próximo outcome distinto do mesmo bucket, tratar A0256 como mitigador transitório semanticamente distinto no DamageMitigationResolver, aplicar ×0,80/×0,70 e consumir somente a parcela defensiva após commit. No primeiro outcome melee subsequente elegível, após dano final positivo, emitir um derived_fire_retaliation_outcome para o atacante com 15%/20% do dano realmente recebido e consumir a disponibilidade de retaliação. Deduplicar ambos por outcome_id. |
| Fallback | FAIL-CLOSED se Specialist Fire não estiver desbloqueada ou a natureza FIRE/HEAT não for segura. Sem atacante/correlação melee ou pipeline derivado, preservar somente a mitigação reativa do mesmo bucket. Não inferir HEAT por BODY/WORLD/FIRE e não converter entre FIRE/HOT. |
| Regra | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. FIRE e HEAT_DAMAGE nunca são aliases; o bucket que arma determina qual bucket pode consumir a mitigação. Outcome armador não recebe mitigação retroativa. Retaliação é FIRE derivado sem crítico, Ignição, mastery, sustain, Ímpeto, Aura, Marca ou recursão. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Ao concluir um hostile_direct_damage_outcome positivo classificado inequivocamente como FIRE ou HEAT_DAMAGE, fora da recarga e não autoinfligido, A0256 arma RPG_INCANDESCENT_ARMOR por 100 ticks (5 s), registrando exatamente o bucket gatilho. O outcome armador não recebe a mitigação. O próximo outcome hostil direto distinto do MESMO bucket durante a janela recebe multiplicador ×0,80 / ×0,70 naquele componente e consome essa parcela defensiva. Durante a mesma janela, o primeiro hostile_direct_melee_outcome subsequente de um atacante vivo identificável que cause dano final positivo pode gerar uma única retaliação FIRE derivada contra esse atacante, igual a 15% / 20% do dano final efetivamente recebido nesse golpe melee. Recarga: 240 / 180 ticks (12 / 9 s), iniciada ao armar.
...
SPECIALIST_UNLOCK:FIRE válido + requisitos locais + outcome hostil direto positivo explicitamente FIRE ou HEAT_DAMAGE + cooldown livre. Cold Sweat HOT só alimenta HEAT_DAMAGE por adapter versionado; FIRE e HEAT permanecem buckets distintos. Auto-dano, custos, BODY/WORLD sem damage event, DoT derivado e callback duplicado não armam. Mitigação exige outcome distinto do mesmo bucket; retaliação exige primeiro melee subsequente com atacante vivo confiável e dano final >0.
...
FAIL-CLOSED se Specialist Fire não estiver desbloqueada ou a natureza FIRE/HEAT não for segura. Sem atacante/correlação melee ou pipeline derivado, preservar somente a mitigação reativa do mesmo bucket. Não inferir HEAT por BODY/WORLD/FIRE e não converter entre FIRE/HOT.
...
| Pré-requisito visual/estrutural | Specialist Fire desbloqueada (SPECIALIST_UNLOCK:FIRE) + A0250 Resistência ao Calor ≥3 + A0158 Resistência a Fogo ≥2. |
| Dependência semântica completa | SPECIALIST_UNLOCK:FIRE confirmado por Gate A/B/C + A0250 Resistência ao Calor ≥3 + A0158 Resistência a Fogo ≥2. Exige dano hostil elegível; auto-dano não arma. Requisitos locais não substituem o unlock global da Specialist Fire. |
| Custo topológico | 2 PP por rank; 2 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. FIRE e HEAT_DAMAGE nunca são aliases; o bucket que arma determina qual bucket pode consumir a mitigação. Outcome armador não recebe mitigação retroativa. Retaliação é FIRE derivado sem crítico, Ignição, mastery, sustain, Ímpeto, Aura, Marca ou recursão. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0256 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0256.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Fire não estiver desbloqueada ou a natureza FIRE/HEAT não for segura. Sem atacante/correlação melee ou pipeline derivado, preservar somente a mitigação reativa do mesmo bucket. Não inferir HEAT por BODY/WORLD/FIRE e não converter entre FIRE/HOT.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 2 ranks. Janela: 100 ticks. Próximo componente do mesmo bucket: ×0,80 / ×0,70. Retaliação opcional no primeiro golpe melee subsequente elegível: 15% / 20% do dano final positivo recebido daquele golpe, como FIRE derivado contra o atacante. Recarga: 240 / 180 ticks iniciada no armamento. FIRE e HEAT_DAMAGE permanecem buckets distintos..
...
- Página: [A0256 — Armadura Incandescente](https://app.notion.com/3c569db9f0db8114b478f704108bff8d)
...
**Resultado final do Chat 1 para A0256:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
