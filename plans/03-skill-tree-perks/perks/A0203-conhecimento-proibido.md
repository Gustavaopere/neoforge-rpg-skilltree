...
- **Fonte canônica:** [registro A0203 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81aabcdcc423d00abb2c).
...
- **Dependências externas à faixa:** <code>A0198</code>, <code>A0199</code>. Elas permanecem sinalizadas e não são presumidas como concluídas.
...
| Domínio | ARCANE |
| Árvore | Principal — ARCANE ↔ OCCULT |
| Ramo | Eldritch — Conhecimento Proibido |
| Camada | 6 |
...
| Dependências Obrigatórias | A0198 ≥3 + uma lane ELDRITCH exata/allowlisted ≥30 + ≥3 Passive Points válidos em PP_REGION:OCCULT + (A0199=1 OU A0201=1 OU A0202≥2). A0198/A0199 são blockers externos e nenhum requisito é presumido concluído. |
| Pré-requisitos | A0198 ≥3 + eldritch_mastery_lane_id exata ≥30 + PP_REGION:OCCULT ≥3 + (A0199=1 OU A0201=1 OU A0202≥2), todos disponíveis. |
| Provider/Mods | RPG Skill Tree + Iron's Spells 'n Spellbooks 3.16.3 + Discerning The Eldritch runtime 1.4.3-1.21 + Deeper and Darker: Spellbooks runtime 1.3.3-1.21.1 somente através de registry/adapters ELDRITCH versionados + HealingResolver e state-duration hooks canônicos. Dreamless/Empty school não é reclassificada ELDRITCH por tema de vazio. |
| Efeito | Ao registrar três categorias ELDRITCH distintas e explicitamente allowlisted por adapters versionados em uma janela móvel de 200 ticks (10 s), A0203 entra em RPG_FORBIDDEN_KNOWLEDGE por 160 ticks (8 s). Categorias elegíveis são IDs canônicos definidos pelo registry da versão, por exemplo direct_damage e estados ELDRITCH concretos distintos expostos pelo provider; nomes como Ruína, Fratura ou Loucura não são presumidos. A mesma action_id/outcome_id pode creditar no máximo uma categoria. Durante a janela ativa: dano ELDRITCH direto elegível ×1,12; duração-base de estados ELDRITCH allowlisted, removíveis e modificáveis ×1,15; cura efetiva recebida ×0,85. A penalidade de cura é inseparável do bônus de dano. |
| Escalonamento | 1 rank. Janela de coleta: 200 ticks; requisito: 3 eldritch_category_id distintos. Estado ativo: 160 ticks. Dano ELDRITCH direto ×1,12; duração-base de estado ELDRITCH elegível ×1,15 na criação/renovação; cura efetiva recebida ×0,85. Ao ativar, limpar o conjunto de categorias; outcomes da própria janela ativa não criam ativação recursiva. |
| Gate | Todos os pré-requisitos disponíveis + registry versionado com pelo menos 3 eldritch_category_id reais + outcome producer causal + HealingResolver. OCCULT ≥3 significa PP semânticos válidos em PP_REGION:OCCULT, não ranks, mana ou proximidade. Enquanto faltar qualquer eixo obrigatório ou a lane addon não for aceita pelo MasteryLaneCatalog, A0203 é UNAVAILABLE_NODE/não comprável. |
| Hook | Ledger por jogador: timestamps de categorias na janela móvel de 200 ticks, no máximo uma categoria por action_id/outcome_id. Ao alcançar 3 IDs distintos, limpar ledger e ativar 160 ticks. Aplicar ×1,12 à camada ELDRITCH direta, ×1,15 somente à duração-base allowlisted e ×0,85 no HealingResolver, cada qual uma vez. Limpar ledger/estado em morte, logout, dimensão, rank/dependency loss, respec e rules reload. |
| Fallback | UNAVAILABLE_NODE enquanto não houver três categorias reais distinguíveis, lane ELDRITCH exata, HealingResolver e prerequisites disponíveis. Se apenas o hook de duração faltar, omitir somente ×1,15; dano ×1,12 e cura ×0,85 permanecem inseparáveis. Não inferir categoria por nome, partícula, VFX, namespace ou mod. |
| Regra | PP_REGION: ARCANE/ELDRITCH; OCCULT gate resolve PP_REGION:OCCULT. Keystone exterior, não terminal. 'Eldritch Mastery' resolve uma lane canônica exata de SchoolType, nunca soma escolas. Availability transitiva de A0198/A0199/A0201/A0202. Uma ação credita no máximo uma categoria; efeitos derivados não contam. |
...
Ao registrar três categorias ELDRITCH distintas e explicitamente allowlisted por adapters versionados em uma janela móvel de 200 ticks (10 s), A0203 entra em RPG_FORBIDDEN_KNOWLEDGE por 160 ticks (8 s). Categorias elegíveis são IDs canônicos definidos pelo registry da versão, por exemplo direct_damage e estados ELDRITCH concretos distintos expostos pelo provider; nomes como Ruína, Fratura ou Loucura não são presumidos. A mesma action_id/outcome_id pode creditar no máximo uma categoria. Durante a janela ativa: dano ELDRITCH direto elegível ×1,12; duração-base de estados ELDRITCH allowlisted, removíveis e modificáveis ×1,15; cura efetiva recebida ×0,85. A penalidade de cura é inseparável do bônus de dano.
...
Ledger por jogador: timestamps de categorias na janela móvel de 200 ticks, no máximo uma categoria por action_id/outcome_id. Ao alcançar 3 IDs distintos, limpar ledger e ativar 160 ticks. Aplicar ×1,12 à camada ELDRITCH direta, ×1,15 somente à duração-base allowlisted e ×0,85 no HealingResolver, cada qual uma vez. Limpar ledger/estado em morte, logout, dimensão, rank/dependency loss, respec e rules reload.

### Fallback sem trocar a identidade

UNAVAILABLE_NODE enquanto não houver três categorias reais distinguíveis, lane ELDRITCH exata, HealingResolver e prerequisites disponíveis. Se apenas o hook de duração faltar, omitir somente ×1,15; dano ×1,12 e cura ×0,85 permanecem inseparáveis. Não inferir categoria por nome, partícula, VFX, namespace ou mod.
...
| Item | Decisão |
|---|---|
| Região | Principal — ARCANE ↔ OCCULT / Eldritch — Conhecimento Proibido |
| Camada e papel | Camada 6; Keystone |
| Pré-requisito visual/estrutural | A0198 ≥3 + eldritch_mastery_lane_id exata ≥30 + PP_REGION:OCCULT ≥3 + (A0199=1 OU A0201=1 OU A0202≥2), todos disponíveis. |
| Dependência semântica completa | A0198 ≥3 + uma lane ELDRITCH exata/allowlisted ≥30 + ≥3 Passive Points válidos em PP_REGION:OCCULT + (A0199=1 OU A0201=1 OU A0202≥2). A0198/A0199 são blockers externos e nenhum requisito é presumido concluído. |
| Custo topológico | 2 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: ARCANE/ELDRITCH; OCCULT gate resolve PP_REGION:OCCULT. Keystone exterior, não terminal. 'Eldritch Mastery' resolve uma lane canônica exata de SchoolType, nunca soma escolas. Availability transitiva de A0198/A0199/A0201/A0202. Uma ação credita no máximo uma categoria; efeitos derivados não contam. |
...
RPG Skill Tree + Iron's Spells 'n Spellbooks 3.16.3 + Discerning The Eldritch runtime 1.4.3-1.21 + Deeper and Darker: Spellbooks runtime 1.3.3-1.21.1 somente através de registry/adapters ELDRITCH versionados + HealingResolver e state-duration hooks canônicos. Dreamless/Empty school não é reclassificada ELDRITCH por tema de vazio.
...
| Projeto | Head auditado | Decisão para A0203 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>action_id</code>, <code>outcome_id</code>, <code>ledger</code>.
...
- **Chaves que o adapter precisa preservar:** <code>action_id</code>, <code>outcome_id</code>, <code>ledger</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0203.
...
- **Fallback normativo:** UNAVAILABLE_NODE enquanto não houver três categorias reais distinguíveis, lane ELDRITCH exata, HealingResolver e prerequisites disponíveis. Se apenas o hook de duração faltar, omitir somente ×1,15; dano ×1,12 e cura ×0,85 permanecem inseparáveis. Não inferir categoria por nome, partícula, VFX, namespace ou mod.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Janela de coleta: 200 ticks; requisito: 3 eldritch_category_id distintos. Estado ativo: 160 ticks. Dano ELDRITCH direto ×1,12; duração-base de estado ELDRITCH elegível ×1,15 na criação/renovação; cura efetiva recebida ×0,85. Ao ativar, limpar o conjunto de categorias; outcomes da própria janela ativa não criam ativação recursiva..
...
6. **Causalidade:** correlacionar <code>action_id</code>, <code>outcome_id</code>, <code>ledger</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
- **Dependências fora desta faixa:** <code>A0198</code>, <code>A0199</code>.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>action_id</code>, <code>outcome_id</code>, <code>ledger</code>. |
...
- Página: [A0203 — Conhecimento Proibido](https://app.notion.com/3c569db9f0db81aabcdcc423d00abb2c)
...
**Resultado final do Chat 1 para A0203:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
