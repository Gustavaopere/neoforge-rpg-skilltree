...
- **Fonte canônica:** [registro A0217 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81fa8d6bd98c9471ac8d).
...
|---|---|
| Código | A0217 |
| Nome | Domínio Geomântico |
| Domínio | ARCANE |
| Árvore | Principal — ARCANE ↔ MINING |
| Ramo | Terra — Domínio Geomântico |
| Camada | 6 |
...
| Dependências Obrigatórias | A0212 Dano de Terra I ≥ 3 ranks + Earth Mastery ≥ 30 + pelo menos um dos seguintes: A0213 = 1 rank, A0215 = 1 rank ou A0216 ≥ 1 rank. Exige histórico real de dano e/ou controle EARTH; mineração não substitui domínio mágico. |
| Pré-requisitos | A0212 Dano de Terra I ≥ 3 ranks + pelo menos um caminho secundário entre A0213/A0215/A0216. |
| Provider/Mods | RPG Skill Tree + GTBC's Geomancy Plus runtime 1.1.0-1.21.1 + Iron's Spells 'n Spellbooks 3.16.3 como framework + adapters EARTH/Geomancy explicitamente mapeados + hooks reais de earth_control_state_id, impacto/pressão e duração-base. Ars Elemental 0.7.10.1/Somake 1.0.8 só participam quando a ação concreta for EARTH/GEO explícita. |
| Efeito | Completar, em qualquer ordem válida, uma ação EARTH direta de controle reconhecido e uma ação EARTH direta ofensiva distinta dentro de 120 ticks (6 s) arma RPG_GEOMANTIC_FOUNDATION por 160 ticks (8 s). A próxima ação EARTH elegível consome o Fundamento: se for ofensiva direta e o provider expuser impacto/pressão de guarda modificável, essa métrica recebe ×1,15; se for controle e a duração-base do estado EARTH allowlisted for modificável, recebe ×1,20. Uma ação recebe somente o benefício correspondente à sua categoria. |
| Escalonamento | 1 rank. Janela de sequência: 120 ticks; estado RPG_GEOMANTIC_FOUNDATION: 160 ticks. Payoff ofensivo: impacto/pressão nativa ×1,15. Payoff de controle: duração-base do estado EARTH elegível ×1,20. Sem acumulação. |
| Gate | A0212 ≥3 + Earth Mastery ≥30 + caminho secundário exigido + uma ação EARTH de controle real e uma ação EARTH ofensiva direta distinta dentro de 120 ticks. Mineração, terreno, automação, DoT, derived_component, callback duplicado e repetição da mesma ação não completam a sequência. A ação consumidora precisa possuir um payoff real suportado pela sua categoria. |
| Hook | Registrar por jogador a categoria EARTH válida e action_id/signature; exigir a categoria complementar por ação distinta dentro de 120 ticks. Ao completar, armar Fundamento por 160 ticks. Na próxima ação EARTH com payoff suportado, aplicar somente ×1,15 ao impacto/pressão nativa OU ×1,20 à duração-base do controle, e consumir após commit. |
| Fallback | Sem classificador seguro de controle EARTH, A0217 fica inativa. Se apenas uma rota de payoff não possuir hook real, essa categoria não pode consumir o Fundamento; se nenhuma rota tiver payoff seguro, o node fica integralmente fail-closed. Não substituir impacto por dano, duração por desconto de custo ou mineração por ação EARTH. |
| Regra | PP_REGION: ARCANE/EARTH. Keystone da Árvore Exterior, não Specialist e não terminal. RPG_GEOMANTIC_FOUNDATION exige duas ações EARTH distintas e reais: uma ofensiva direta e uma de controle allowlisted, em qualquer ordem dentro da janela. Mineração, terreno, ticks, automação e derived components não completam sequência. O payoff altera somente impacto/pressão nativa OU duração-base real conforme a categoria consumidora. SPECIALIST_REGION:EARTH = núcleo ARCANE compartilhado elegível + PP ARCANE/EARTH; MINING/bridges só contam quando explicitamente mapeados e nunca em dois thresholds. |
...
Completar, em qualquer ordem válida, uma ação EARTH direta de controle reconhecido e uma ação EARTH direta ofensiva distinta dentro de 120 ticks (6 s) arma RPG_GEOMANTIC_FOUNDATION por 160 ticks (8 s). A próxima ação EARTH elegível consome o Fundamento: se for ofensiva direta e o provider expuser impacto/pressão de guarda modificável, essa métrica recebe ×1,15; se for controle e a duração-base do estado EARTH allowlisted for modificável, recebe ×1,20. Uma ação recebe somente o benefício correspondente à sua categoria.

### Escalonamento aprovado

1 rank. Janela de sequência: 120 ticks; estado RPG_GEOMANTIC_FOUNDATION: 160 ticks. Payoff ofensivo: impacto/pressão nativa ×1,15. Payoff de controle: duração-base do estado EARTH elegível ×1,20. Sem acumulação.

### Gate de compra/ativação

A0212 ≥3 + Earth Mastery ≥30 + caminho secundário exigido + uma ação EARTH de controle real e uma ação EARTH ofensiva direta distinta dentro de 120 ticks. Mineração, terreno, automação, DoT, derived_component, callback duplicado e repetição da mesma ação não completam a sequência. A ação consumidora precisa possuir um payoff real suportado pela sua categoria.

### Hook e ordem de execução

Registrar por jogador a categoria EARTH válida e action_id/signature; exigir a categoria complementar por ação distinta dentro de 120 ticks. Ao completar, armar Fundamento por 160 ticks. Na próxima ação EARTH com payoff suportado, aplicar somente ×1,15 ao impacto/pressão nativa OU ×1,20 à duração-base do controle, e consumir após commit.

### Fallback sem trocar a identidade

Sem classificador seguro de controle EARTH, A0217 fica inativa. Se apenas uma rota de payoff não possuir hook real, essa categoria não pode consumir o Fundamento; se nenhuma rota tiver payoff seguro, o node fica integralmente fail-closed. Não substituir impacto por dano, duração por desconto de custo ou mineração por ação EARTH.
...
| Item | Decisão |
|---|---|
| Região | Principal — ARCANE ↔ MINING / Terra — Domínio Geomântico |
| Camada e papel | Camada 6; Keystone |
| Pré-requisito visual/estrutural | A0212 Dano de Terra I ≥ 3 ranks + pelo menos um caminho secundário entre A0213/A0215/A0216. |
| Dependência semântica completa | A0212 Dano de Terra I ≥ 3 ranks + Earth Mastery ≥ 30 + pelo menos um dos seguintes: A0213 = 1 rank, A0215 = 1 rank ou A0216 ≥ 1 rank. Exige histórico real de dano e/ou controle EARTH; mineração não substitui domínio mágico. |
| Custo topológico | 2 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: ARCANE/EARTH. Keystone da Árvore Exterior, não Specialist e não terminal. RPG_GEOMANTIC_FOUNDATION exige duas ações EARTH distintas e reais: uma ofensiva direta e uma de controle allowlisted, em qualquer ordem dentro da janela. Mineração, terreno, ticks, automação e derived components não completam sequência. O payoff altera somente impacto/pressão nativa OU duração-base real conforme a categoria consumidora. SPECIALIST_REGION:EARTH = núcleo ARCANE compartilhado elegível + PP ARCANE/EARTH; MINING/bridges só contam quando explicitamente mapeados e nunca em dois thresholds. |
...
RPG Skill Tree + GTBC's Geomancy Plus runtime 1.1.0-1.21.1 + Iron's Spells 'n Spellbooks 3.16.3 como framework + adapters EARTH/Geomancy explicitamente mapeados + hooks reais de earth_control_state_id, impacto/pressão e duração-base. Ars Elemental 0.7.10.1/Somake 1.0.8 só participam quando a ação concreta for EARTH/GEO explícita.
...
| Projeto | Head auditado | Decisão para A0217 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>action_id</code>, <code>signature</code>.
...
- **Chaves que o adapter precisa preservar:** <code>action_id</code>, <code>signature</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0217.
...
- **Fallback normativo:** Sem classificador seguro de controle EARTH, A0217 fica inativa. Se apenas uma rota de payoff não possuir hook real, essa categoria não pode consumir o Fundamento; se nenhuma rota tiver payoff seguro, o node fica integralmente fail-closed. Não substituir impacto por dano, duração por desconto de custo ou mineração por ação EARTH.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Janela de sequência: 120 ticks; estado RPG_GEOMANTIC_FOUNDATION: 160 ticks. Payoff ofensivo: impacto/pressão nativa ×1,15. Payoff de controle: duração-base do estado EARTH elegível ×1,20. Sem acumulação..
...
6. **Causalidade:** correlacionar <code>action_id</code>, <code>signature</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>action_id</code>, <code>signature</code>. |
...
- Página: [A0217 — Domínio Geomântico](https://app.notion.com/3c569db9f0db81fa8d6bd98c9471ac8d)
...
**Resultado final do Chat 1 para A0217:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
