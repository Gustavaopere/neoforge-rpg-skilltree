...
- **Fonte canônica:** [registro A0231 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81e88654fa1d3082dd6e).
...
|---|---|
| Código | A0231 |
| Nome | Domínio Aerocinético |
| Domínio | ARCANE |
| Árvore | Principal — ARCANE ↔ AGILITY |
| Ramo | Vento — Domínio Aerocinético |
| Camada | 6 |
...
| Dependências Obrigatórias | A0226 Dano de Vento I ≥ 3 ranks + Wind Mastery ≥ 30 + pelo menos um dos seguintes: A0227 = 1 rank, A0229 = 1 rank ou A0230 ≥ 1 rank. Exige histórico real de controle/mobilidade WIND; movimento comum não substitui domínio. |
| Pré-requisitos | A0226 Dano de Vento I ≥ 3 ranks + pelo menos um caminho secundário entre A0227/A0229/A0230. |
| Provider/Mods | RPG Skill Tree + SnackPirate's Aeromancy Additions 1.2.8 + Wind's Spellbooks 1.0.5/Iron's Spells runtime 1.21.1-3.16.3 e demais ações WIND somente por adapter explícito + Wind Mastery canônica. ParCool 4.0.0.2/Epic ParCool 21.0.0/Epic Fight 21.17.3.1 fornecem apenas eventos de mobilidade reconhecida, nunca autoridade WIND. FUTURE_PROVIDER_CONTRACT RESOURCE_COST_MODIFIER_V1: OWNER provider do recurso/adapter; CONSUMER A0231; QUERY ação+resource_id+custo final pré-débito; BEHAVIOR modificar atomicamente somente esse custo no mesmo recurso; VERSION-STATUS design congelado, contrato não encontrado no runtime/main atual em 2026-08-29. |
| Efeito | Completar, em qualquer ordem, um controle/deslocamento WIND real e uma mobilidade ativa explicitamente classificada WIND, por ações distintas dentro de 120 ticks (6 s), arma RPG_AEROKINETIC_IMPULSE por 160 ticks (8 s). A próxima ação WIND elegível consome o Impulso: se ofensiva direta, dano WIND ×1,12 e knockback/launch nativo ×1,20 quando modificável; se mobilidade/suporte e possuir custo nativo final modificável, esse custo no próprio recurso é multiplicado por ×0,85 antes do débito. |
| Escalonamento | 1 rank. Janela de sequência: 120 ticks; Impulso: 160 ticks. Rota ofensiva: dano WIND direto ×1,12 e knockback/launch nativo ×1,20 quando disponível. Rota mobilidade/suporte: custo nativo final ×0,85 no mesmo recurso. Uma ação recebe somente o benefício da própria categoria. |
| Gate | A0226 ≥3 + Wind Mastery ≥30 + caminho secundário exigido + dois eventos WIND distintos: controle/deslocamento real e mobilidade ativa WIND. Sprint, salto, elytra comum, velocidade genérica, movimento forçado, DoT, derived_component, automação e repetição da mesma ação não completam. A consumidora precisa possuir hook real do payoff de sua categoria. |
| Hook | Registrar as duas categorias WIND com action_id/signature e exigir a complementar dentro de 120 ticks; ao completar, armar Impulso por 160 ticks. Na próxima ação elegível, aplicar somente rota ofensiva (×1,12 dano + ×1,20 knockback quando disponível) OU rota de custo (final native cost ×0,85 no próprio recurso), e consumir após commit. |
| Fallback | Sem classificadores seguros de controle e mobilidade WIND, A0231 fica inativa. Se knockback/launch não existir, a rota ofensiva preserva apenas dano ×1,12. Sem RESOURCE_COST_MODIFIER_V1 implementado antes do débito nativo, a rota mobilidade/suporte é inelegível e não consome o Impulso; não substituir por reembolso pós-hoc, diferença de barra, recurso diferente ou bônus alternativo. |
| Regra | PP_REGION: ARCANE/WIND. Keystone da Árvore Exterior, não Specialist e não terminal. As duas categorias preparatórias devem vir de ações distintas: controle/deslocamento WIND real e mobilidade ativa explicitamente WIND. ParCool/Epic Fight podem fornecer o evento de mobilidade, mas não a classificação WIND. Na rota de custo, recursos tipados permanecem independentes: apenas o custo nativo da própria ação e do próprio resource_id pode receber ×0,85. RESOURCE_COST_MODIFIER_V1 é FUTURE_PROVIDER_CONTRACT, não API runtime existente; enquanto ausente, somente a rota de custo fica fail-closed. |
...
Completar, em qualquer ordem, um controle/deslocamento WIND real e uma mobilidade ativa explicitamente classificada WIND, por ações distintas dentro de 120 ticks (6 s), arma RPG_AEROKINETIC_IMPULSE por 160 ticks (8 s). A próxima ação WIND elegível consome o Impulso: se ofensiva direta, dano WIND ×1,12 e knockback/launch nativo ×1,20 quando modificável; se mobilidade/suporte e possuir custo nativo final modificável, esse custo no próprio recurso é multiplicado por ×0,85 antes do débito.

### Escalonamento aprovado

1 rank. Janela de sequência: 120 ticks; Impulso: 160 ticks. Rota ofensiva: dano WIND direto ×1,12 e knockback/launch nativo ×1,20 quando disponível. Rota mobilidade/suporte: custo nativo final ×0,85 no mesmo recurso. Uma ação recebe somente o benefício da própria categoria.

### Gate de compra/ativação

A0226 ≥3 + Wind Mastery ≥30 + caminho secundário exigido + dois eventos WIND distintos: controle/deslocamento real e mobilidade ativa WIND. Sprint, salto, elytra comum, velocidade genérica, movimento forçado, DoT, derived_component, automação e repetição da mesma ação não completam. A consumidora precisa possuir hook real do payoff de sua categoria.

### Hook e ordem de execução

Registrar as duas categorias WIND com action_id/signature e exigir a complementar dentro de 120 ticks; ao completar, armar Impulso por 160 ticks. Na próxima ação elegível, aplicar somente rota ofensiva (×1,12 dano + ×1,20 knockback quando disponível) OU rota de custo (final native cost ×0,85 no próprio recurso), e consumir após commit.

### Fallback sem trocar a identidade

Sem classificadores seguros de controle e mobilidade WIND, A0231 fica inativa. Se knockback/launch não existir, a rota ofensiva preserva apenas dano ×1,12. Sem RESOURCE_COST_MODIFIER_V1 implementado antes do débito nativo, a rota mobilidade/suporte é inelegível e não consome o Impulso; não substituir por reembolso pós-hoc, diferença de barra, recurso diferente ou bônus alternativo.
...
| Item | Decisão |
|---|---|
| Região | Principal — ARCANE ↔ AGILITY / Vento — Domínio Aerocinético |
| Camada e papel | Camada 6; Keystone |
| Pré-requisito visual/estrutural | A0226 Dano de Vento I ≥ 3 ranks + pelo menos um caminho secundário entre A0227/A0229/A0230. |
| Dependência semântica completa | A0226 Dano de Vento I ≥ 3 ranks + Wind Mastery ≥ 30 + pelo menos um dos seguintes: A0227 = 1 rank, A0229 = 1 rank ou A0230 ≥ 1 rank. Exige histórico real de controle/mobilidade WIND; movimento comum não substitui domínio. |
| Custo topológico | 2 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: ARCANE/WIND. Keystone da Árvore Exterior, não Specialist e não terminal. As duas categorias preparatórias devem vir de ações distintas: controle/deslocamento WIND real e mobilidade ativa explicitamente WIND. ParCool/Epic Fight podem fornecer o evento de mobilidade, mas não a classificação WIND. Na rota de custo, recursos tipados permanecem independentes: apenas o custo nativo da própria ação e do próprio resource_id pode receber ×0,85. RESOURCE_COST_MODIFIER_V1 é FUTURE_PROVIDER_CONTRACT, não API runtime existente; enquanto ausente, somente a rota de custo fica fail-closed. |
...
RPG Skill Tree + SnackPirate's Aeromancy Additions 1.2.8 + Wind's Spellbooks 1.0.5/Iron's Spells runtime 1.21.1-3.16.3 e demais ações WIND somente por adapter explícito + Wind Mastery canônica. ParCool 4.0.0.2/Epic ParCool 21.0.0/Epic Fight 21.17.3.1 fornecem apenas eventos de mobilidade reconhecida, nunca autoridade WIND. FUTURE_PROVIDER_CONTRACT RESOURCE_COST_MODIFIER_V1: OWNER provider do recurso/adapter; CONSUMER A0231; QUERY ação+resource_id+custo final pré-débito; BEHAVIOR modificar atomicamente somente esse custo no mesmo recurso; VERSION-STATUS design congelado, contrato não encontrado no runtime/main atual em 2026-08-29.
...
- **Contratos/capabilities nomeados no registro:** <code>RESOURCE_COST_MODIFIER_V1</code>.
- **Estado:** nenhum nome de API é tratado como existente apenas por aparecer no design; FUTURE_PROVIDER_CONTRACT permanece bloqueador até prova em código/API da versão exata.

### Matriz dos quatro projetos próprios

| Projeto | Head auditado | Decisão para A0231 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>action_id</code>, <code>signature</code>, <code>source_id</code>, <code>resource_id</code>.
...
- **Chaves que o adapter precisa preservar:** <code>action_id</code>, <code>signature</code>, <code>source_id</code>, <code>resource_id</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0231.
...
- **Fallback normativo:** Sem classificadores seguros de controle e mobilidade WIND, A0231 fica inativa. Se knockback/launch não existir, a rota ofensiva preserva apenas dano ×1,12. Sem RESOURCE_COST_MODIFIER_V1 implementado antes do débito nativo, a rota mobilidade/suporte é inelegível e não consome o Impulso; não substituir por reembolso pós-hoc, diferença de barra, recurso diferente ou bônus alternativo.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Janela de sequência: 120 ticks; Impulso: 160 ticks. Rota ofensiva: dano WIND direto ×1,12 e knockback/launch nativo ×1,20 quando disponível. Rota mobilidade/suporte: custo nativo final ×0,85 no mesmo recurso. Uma ação recebe somente o benefício da própria categoria..
...
6. **Causalidade:** correlacionar <code>action_id</code>, <code>signature</code>, <code>source_id</code>, <code>resource_id</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
- **Capabilities/contracts a provar:** <code>RESOURCE_COST_MODIFIER_V1</code>.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>action_id</code>, <code>signature</code>, <code>source_id</code>, <code>resource_id</code>. |
...
- Página: [A0231 — Domínio Aerocinético](https://app.notion.com/3c569db9f0db81e88654fa1d3082dd6e)
...
**Resultado final do Chat 1 para A0231:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
