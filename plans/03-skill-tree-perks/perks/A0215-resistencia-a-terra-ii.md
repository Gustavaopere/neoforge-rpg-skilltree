...
- **Fonte canônica:** [registro A0215 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db8196a97eddb2ffc107dd).
...
|---|---|
| Código | A0215 |
| Nome | Resistência a Terra II |
| Domínio | VITALITY |
| Árvore | Principal — VITALITY ↔ MINING/ARCANE |
| Ramo | Terra — Postura Ancorada |
| Camada | 5 |
...
| Dependências Obrigatórias | A0214 Resistência a Terra I ≥ 3 ranks + estado de apoio no solo confiável. |
| Pré-requisitos | A0214 Resistência a Terra I ≥ 3 ranks. |
| Provider/Mods | Minecraft/NeoForge authoritative grounded state + DamageMitigationResolver/RPG_EARTH_RESISTANCE + GTBC's Geomancy Plus runtime 1.1.0-1.21.1 somente para componentes EARTH explicitamente classificados + VITALITY/MINING bridges canônicos. Knockback só é modificado quando o evento expuser métrica nativa segura. |
| Efeito | Após permanecer 30 ticks (1,5 s) continuamente em apoio real no solo, sem voar, nadar, montar ou sofrer deslocamento vertical forçado, A0215 arma RPG_EARTH_ANCHORED. O estado é perdido imediatamente ao deixar a condição válida. O próximo componente EARTH de um dano hostil direto é multiplicado por ×0,75 e, se o mesmo evento expuser knockback modificável, sua magnitude é multiplicada por ×0,60; então a Postura é consumida. |
| Escalonamento | 1 rank. Tempo contínuo de apoio: 30 ticks. Próximo componente EARTH hostil direto: ×0,75. Knockback do mesmo evento, quando modificável: ×0,60. Uma Postura por jogador; não acumula. |
| Gate | A0214 ≥3 + apoio real contínuo por 30 ticks + presença ativa. Voo, natação, montaria, queda, movimento forçado e mera existência de bloco abaixo sem estado grounded confiável não armam. Consumo exige dano hostil direto com componente EARTH positivo. |
| Hook | Rastrear grounded_since_tick somente enquanto a condição autoritativa for válida; ao completar 30 ticks, armar um único estado. Ao sair do solo, limpar. No próximo outcome EARTH hostil direto, tratar a mitigação como estado transitório distinto no DamageMitigationResolver, aplicar ×0,75 e, quando disponível, ×0,60 à repulsão do mesmo evento; consumir após commit. |
| Fallback | Sem estado grounded autoritativo contínuo, A0215 fica inativa. Se somente knockback não puder ser modificado, preservar a mitigação EARTH ×0,75 e omitir essa parcela. Não inferir apoio por bloco abaixo/posição, não criar peso/postura paralela e não conceder mastery por permanecer parado. |
| Regra | PP_REGION: VITALITY_MINING_EARTH_BRIDGE/DEFENSE. BRIDGE_PP_POLICY: estes PP não contam simultaneamente para VITALITY, MINING e ARCANE. Exige 30 ticks contínuos de grounded autoritativo; voo, natação, montaria, queda ou deslocamento vertical forçado invalidam imediatamente. RPG_EARTH_ANCHORED é mitigação reativa distinta do bucket base e não gera mastery por tick/AFK. Knockback só muda se a métrica nativa existir. Notable/bridge da Árvore Exterior; não é terminal. |
...
Após permanecer 30 ticks (1,5 s) continuamente em apoio real no solo, sem voar, nadar, montar ou sofrer deslocamento vertical forçado, A0215 arma RPG_EARTH_ANCHORED. O estado é perdido imediatamente ao deixar a condição válida. O próximo componente EARTH de um dano hostil direto é multiplicado por ×0,75 e, se o mesmo evento expuser knockback modificável, sua magnitude é multiplicada por ×0,60; então a Postura é consumida.

### Escalonamento aprovado

1 rank. Tempo contínuo de apoio: 30 ticks. Próximo componente EARTH hostil direto: ×0,75. Knockback do mesmo evento, quando modificável: ×0,60. Uma Postura por jogador; não acumula.

### Gate de compra/ativação

A0214 ≥3 + apoio real contínuo por 30 ticks + presença ativa. Voo, natação, montaria, queda, movimento forçado e mera existência de bloco abaixo sem estado grounded confiável não armam. Consumo exige dano hostil direto com componente EARTH positivo.

### Hook e ordem de execução

Rastrear grounded_since_tick somente enquanto a condição autoritativa for válida; ao completar 30 ticks, armar um único estado. Ao sair do solo, limpar. No próximo outcome EARTH hostil direto, tratar a mitigação como estado transitório distinto no DamageMitigationResolver, aplicar ×0,75 e, quando disponível, ×0,60 à repulsão do mesmo evento; consumir após commit.

### Fallback sem trocar a identidade

Sem estado grounded autoritativo contínuo, A0215 fica inativa. Se somente knockback não puder ser modificado, preservar a mitigação EARTH ×0,75 e omitir essa parcela. Não inferir apoio por bloco abaixo/posição, não criar peso/postura paralela e não conceder mastery por permanecer parado.
...
| Item | Decisão |
|---|---|
| Região | Principal — VITALITY ↔ MINING/ARCANE / Terra — Postura Ancorada |
| Camada e papel | Camada 5; Notable |
| Pré-requisito visual/estrutural | A0214 Resistência a Terra I ≥ 3 ranks. |
| Dependência semântica completa | A0214 Resistência a Terra I ≥ 3 ranks + estado de apoio no solo confiável. |
| Custo topológico | 1 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: VITALITY_MINING_EARTH_BRIDGE/DEFENSE. BRIDGE_PP_POLICY: estes PP não contam simultaneamente para VITALITY, MINING e ARCANE. Exige 30 ticks contínuos de grounded autoritativo; voo, natação, montaria, queda ou deslocamento vertical forçado invalidam imediatamente. RPG_EARTH_ANCHORED é mitigação reativa distinta do bucket base e não gera mastery por tick/AFK. Knockback só muda se a métrica nativa existir. Notable/bridge da Árvore Exterior; não é terminal. |
...
Minecraft/NeoForge authoritative grounded state + DamageMitigationResolver/RPG_EARTH_RESISTANCE + GTBC's Geomancy Plus runtime 1.1.0-1.21.1 somente para componentes EARTH explicitamente classificados + VITALITY/MINING bridges canônicos. Knockback só é modificado quando o evento expuser métrica nativa segura.
...
| Projeto | Head auditado | Decisão para A0215 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0215.
...
- **Fallback normativo:** Sem estado grounded autoritativo contínuo, A0215 fica inativa. Se somente knockback não puder ser modificado, preservar a mitigação EARTH ×0,75 e omitir essa parcela. Não inferir apoio por bloco abaixo/posição, não criar peso/postura paralela e não conceder mastery por permanecer parado.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Tempo contínuo de apoio: 30 ticks. Próximo componente EARTH hostil direto: ×0,75. Knockback do mesmo evento, quando modificável: ×0,60. Uma Postura por jogador; não acumula..
...
- Página: [A0215 — Resistência a Terra II](https://app.notion.com/3c569db9f0db8196a97eddb2ffc107dd)
...
**Resultado final do Chat 1 para A0215:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
