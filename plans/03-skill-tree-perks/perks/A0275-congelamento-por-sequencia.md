# A0275 — Congelamento por Sequência
...
- **Fonte canônica:** [registro A0275 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81d580c3f93d8f763ec6).
...
- **Dependências externas à faixa:** <code>A0169</code>, <code>A0167</code>. Elas permanecem sinalizadas e não são presumidas como concluídas.
...
|---|---|
| Código | A0275 |
| Nome | Congelamento por Sequência |
| Domínio | ARCANE/ICE |
...
| Dependências Obrigatórias | SPECIALIST_UNLOCK:ICE confirmado por Gate A/B/C + A0273 Arma Gélida ≥2 ranks + A0266 Congelamento Progressivo ≥2 ranks. Requisitos locais não substituem fundamentos ICE + ≥100 PP válidos em SPECIALIST_REGION:ICE + terminal A0169. |
| Pré-requisitos | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) + A0273 Arma Gélida ≥2 + A0266 Congelamento Progressivo ≥2. |
| Provider/Mods | RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + pipeline de ações ICE diretas + A0167/A0273 para melee imbuído + FREEZE_BUILDUP_ADAPTER_V1. Iron's Spells 'n Spellbooks runtime 1.21.1-3.16.3 é provider de freeze buildup quando a ação expuser delta causal positivo de freeze ticks e threshold nativo (getTicksRequiredToFreeze()); Ars Nouveau 5.13.1/Ars Elemental 0.7.10.1 e demais providers ICE só contam quando adapter comprovar ação direta e buildup real aceito pelo provider. Epic Fight 21.17.3.1 participa somente da identidade do direct_melee_outcome de A0167/A0273. VERSION-STATUS: sequência é estado RPG próprio de preparação, mas o payoff escreve SOMENTE no buildup nativo via adapter; não existe medidor de congelamento paralelo. |
| Efeito | A0275 rastreia por jogador/alvo ações diretas ICE elegíveis e distintas em uma janela móvel de 100 ticks (5 s): magia ICE direta ou direct_melee_outcome que contenha o componente ICE de A0167. No rank 1, 6 ações; no rank 2, 5 ações. Ao atingir o limiar, se o provider expuser freeze buildup atual, threshold nativo positivo e operação segura de incremento, adicionar 35% / 50% do threshold nativo como buildup, limpar a sequência e iniciar recarga por jogador/alvo de 80 ticks (4 s). |
| Escalonamento | Até 2 ranks. Janela: 100 ticks. Ações necessárias: 6 / 5. Crédito por action_id/outcome_id: máximo 1 por ação. Payoff: +35% / +50% do threshold nativo de freeze buildup na unidade do provider. Recarga por jogador/alvo: 80 ticks após payoff commitado. O payoff pode completar naturalmente o threshold do provider, mas A0275 não aplica estado de Congelamento por fora da mecânica nativa. |
| Gate | SPECIALIST_UNLOCK:ICE válido + A0273 ≥2 + A0266 ≥2 + recarga do par livre + mesma vítima + ações diretas ICE elegíveis e DISTINTAS em até 100 ticks. Magia ICE direta conta no máximo uma vez por action_id/outcome_id; melee conta SOMENTE pelo outcome pai que contém o componente ICE de A0167/A0273, nunca pelo derived_component separadamente. DoT, tick de estado, subhit do mesmo action_id, aura, retaliação, summon, automação, fake player e callback duplicado não contam. |
| Hook | Manter deque/conjunto por player_uuid+target_uuid com {action_id,outcome_id,timestamp} e remover créditos expirados >100 ticks. Limiar: 6 ações no rank 1, 5 no rank 2. Ao alcançar o limiar, consultar FREEZE_BUILDUP_ADAPTER_V1; somente se houver threshold nativo positivo e operação causal segura, aplicar +0,35/+0,50 × threshold na unidade nativa, deixar o provider decidir naturalmente se isso completa FULLY_FROZEN, limpar a sequência e registrar cooldown_expiry=now+80. Nunca escrever estado completo diretamente. Morte/despawn do alvo, logout/dimensão/respec do jogador limpam preparação transitória correspondente. |
| Fallback | FAIL-CLOSED se Specialist Gelo não estiver desbloqueada, o rastreamento por jogador/alvo não for estável ou o provider não expuser threshold/incremento nativo seguro. Não usar contador global, FREEZE_BUILDUP próprio, incremento fixo arbitrário, CHILL/Slowness, dano ICE ou temperatura corporal como substituto. Sem payoff nativo seguro, a preparação não deve produzir efeito. |
| Regra | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Perk interna/Spellblade da Specialist Gelo. Congelamento por Sequência converte CADÊNCIA REAL em buildup NATIVO; a sequência é apenas memória de preparação, não segundo medidor de congelamento. É por alvo, não transferível, e uma ação concede no máximo um crédito. O componente ICE derivado de A0167/A0273 não conta separadamente do outcome melee pai. Não gera mastery por contagem e não aplica FULLY_FROZEN por fora do provider. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
SPECIALIST_UNLOCK:ICE válido + A0273 ≥2 + A0266 ≥2 + recarga do par livre + mesma vítima + ações diretas ICE elegíveis e DISTINTAS em até 100 ticks. Magia ICE direta conta no máximo uma vez por action_id/outcome_id; melee conta SOMENTE pelo outcome pai que contém o componente ICE de A0167/A0273, nunca pelo derived_component separadamente. DoT, tick de estado, subhit do mesmo action_id, aura, retaliação, summon, automação, fake player e callback duplicado não contam.

### Hook e ordem de execução

Manter deque/conjunto por player_uuid+target_uuid com {action_id,outcome_id,timestamp} e remover créditos expirados >100 ticks. Limiar: 6 ações no rank 1, 5 no rank 2. Ao alcançar o limiar, consultar FREEZE_BUILDUP_ADAPTER_V1; somente se houver threshold nativo positivo e operação causal segura, aplicar +0,35/+0,50 × threshold na unidade nativa, deixar o provider decidir naturalmente se isso completa FULLY_FROZEN, limpar a sequência e registrar cooldown_expiry=now+80. Nunca escrever estado completo diretamente. Morte/despawn do alvo, logout/dimensão/respec do jogador limpam preparação transitória correspondente.
...
| Pré-requisito visual/estrutural | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) + A0273 Arma Gélida ≥2 + A0266 Congelamento Progressivo ≥2. |
| Dependência semântica completa | SPECIALIST_UNLOCK:ICE confirmado por Gate A/B/C + A0273 Arma Gélida ≥2 ranks + A0266 Congelamento Progressivo ≥2 ranks. Requisitos locais não substituem fundamentos ICE + ≥100 PP válidos em SPECIALIST_REGION:ICE + terminal A0169. |
| Custo topológico | 2 PP por rank; 2 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Perk interna/Spellblade da Specialist Gelo. Congelamento por Sequência converte CADÊNCIA REAL em buildup NATIVO; a sequência é apenas memória de preparação, não segundo medidor de congelamento. É por alvo, não transferível, e uma ação concede no máximo um crédito. O componente ICE derivado de A0167/A0273 não conta separadamente do outcome melee pai. Não gera mastery por contagem e não aplica FULLY_FROZEN por fora do provider. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + pipeline de ações ICE diretas + A0167/A0273 para melee imbuído + FREEZE_BUILDUP_ADAPTER_V1. Iron's Spells 'n Spellbooks runtime 1.21.1-3.16.3 é provider de freeze buildup quando a ação expuser delta causal positivo de freeze ticks e threshold nativo (getTicksRequiredToFreeze()); Ars Nouveau 5.13.1/Ars Elemental 0.7.10.1 e demais providers ICE só contam quando adapter comprovar ação direta e buildup real aceito pelo provider. Epic Fight 21.17.3.1 participa somente da identidade do direct_melee_outcome de A0167/A0273. VERSION-STATUS: sequência é estado RPG próprio de preparação, mas o payoff escreve SOMENTE no buildup nativo via adapter; não existe medidor de congelamento paralelo.
...
| Projeto | Head auditado | Decisão para A0275 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>action_id</code>, <code>outcome_id</code>, <code>target_uuid</code>.
...
- **Chaves que o adapter precisa preservar:** <code>action_id</code>, <code>outcome_id</code>, <code>target_uuid</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0275.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 2 ranks. Janela: 100 ticks. Ações necessárias: 6 / 5. Crédito por action_id/outcome_id: máximo 1 por ação. Payoff: +35% / +50% do threshold nativo de freeze buildup na unidade do provider. Recarga por jogador/alvo: 80 ticks após payoff commitado. O payoff pode completar naturalmente o threshold do provider, mas A0275 não aplica estado de Congelamento por fora da mecânica nativa..
...
6. **Causalidade:** correlacionar <code>action_id</code>, <code>outcome_id</code>, <code>target_uuid</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
- **Dependências fora desta faixa:** <code>A0169</code>, <code>A0167</code>.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>action_id</code>, <code>outcome_id</code>, <code>target_uuid</code>. |
...
- Página: [A0275 — Congelamento por Sequência](https://app.notion.com/3c569db9f0db81d580c3f93d8f763ec6)
...
**Resultado final do Chat 1 para A0275:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
