...
- **Fonte canônica:** [registro A0209 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db816c8969c9a4b1d635a1).
...
|---|---|
| Código | A0209 |
| Nome | Imbuimento de Ender |
| Domínio | ARCANE |
| Árvore | Principal — ARCANE ↔ MARTIAL/AGILITY |
| Ramo | Ender — Imbuimento Dimensional |
| Camada | 5 |
...
| Dependências Obrigatórias | A0205 ≥2 disponível + mastery da lane melee usada ≥20. Lanes canônicas: epicfight:sword, epicfight:axe, epicfight:spear (contato), epicfight:dagger, epicfight:heavy, combat:mace e combat:scythe; combat:fist somente com sua cadeia válida. Exige também producer causal de ação/deslocamento ENDER. |
| Pré-requisitos | A0205 ≥2 disponível + uma mastery melee canônica da lista ≥20 + adapter de ação/deslocamento ENDER. IDs epic_* de gateway não são IDs de mastery. |
| Provider/Mods | Fire's Ender Expansion 2.4.1/Somake 1.0.8 + Iron's 3.16.3 somente após action/SchoolType id exato e adapter ENDER; Epic Fight 21.17.3.1 e lanes combat:* canônicas. Weapons of Miracles só por arma mapeada. Generic teleport não é provider. |
| Efeito | Após uma ação/deslocamento ENDER direto elegível do jogador, abre janela de Imbuimento por 120 ticks (6 s). Durante a janela, cada direct_melee_outcome legítimo com lane elegível e mastery da lane ≥20 adiciona um único derived_component:ENDER igual a 5% do dano-base canônico elegível por rank: 5% / 10%. added_ender = base_weapon_damage_pre_target_mitigation_pre_critical_pre_added_elements × (0,05 × rank). O componente pertence ao mesmo action_id/outcome_id pai. |
| Escalonamento | Até 2 ranks; janela fixa 120 ticks. Componente ENDER adicional = 5% / 10% do dano-base canônico elegível. Recast ENDER válido renova somente a expiração; não empilha magnitude. |
| Gate | A0205 ≥2 disponível + producer versionado de ação ou deslocamento ENDER causal + janela ativa + direct_melee_outcome e lane canônica ≥20. Sem A0205, producer ENDER, hook melee ou normalização de lane, A0209 é UNAVAILABLE_NODE/não comprável. Teleporte genérico e diferença de posição não abrem janela; combat:fist herda blockers próprios. |
| Hook | Após commit de ação/deslocamento ENDER comprovado, criar/renovar janela única 120 ticks; tentativa/cancelamento não arma. Em direct_melee_outcome, resolver lane canônica, deduplicar outcome_id, calcular base pré-mitigação/pré-crítico/pré-added e anexar um único derived_component:ENDER ao pai. Limpar em morte, logout, dimensão, rank/dependency loss, respec e rules reload. |
| Fallback | UNAVAILABLE_NODE enquanto faltar A0205, ação/deslocamento ENDER confirmado, direct melee hook ou lane canônica. Não abrir por teleporte genérico, End/Void, gear/tema, nome ou posição; não simular segundo ataque. combat:fist permanece fechado quando sua cadeia não estiver disponível. |
| Regra | PP_REGION: ARCANE_MARTIAL_AGILITY_ENDER_BRIDGE. IDs de mastery melee são os IDs canônicos, não gateways epic_*. derived_component:ENDER pertence ao mesmo action_id/outcome_id, herda a única crítica e não cria DamageSource, teleporte, Ruptura, mastery, sustain ou proc. Availability transitiva de A0205. Não terminal. |
...
Após uma ação/deslocamento ENDER direto elegível do jogador, abre janela de Imbuimento por 120 ticks (6 s). Durante a janela, cada direct_melee_outcome legítimo com lane elegível e mastery da lane ≥20 adiciona um único derived_component:ENDER igual a 5% do dano-base canônico elegível por rank: 5% / 10%. added_ender = base_weapon_damage_pre_target_mitigation_pre_critical_pre_added_elements × (0,05 × rank). O componente pertence ao mesmo action_id/outcome_id pai.

### Escalonamento aprovado

Até 2 ranks; janela fixa 120 ticks. Componente ENDER adicional = 5% / 10% do dano-base canônico elegível. Recast ENDER válido renova somente a expiração; não empilha magnitude.

### Gate de compra/ativação

A0205 ≥2 disponível + producer versionado de ação ou deslocamento ENDER causal + janela ativa + direct_melee_outcome e lane canônica ≥20. Sem A0205, producer ENDER, hook melee ou normalização de lane, A0209 é UNAVAILABLE_NODE/não comprável. Teleporte genérico e diferença de posição não abrem janela; combat:fist herda blockers próprios.
...
UNAVAILABLE_NODE enquanto faltar A0205, ação/deslocamento ENDER confirmado, direct melee hook ou lane canônica. Não abrir por teleporte genérico, End/Void, gear/tema, nome ou posição; não simular segundo ataque. combat:fist permanece fechado quando sua cadeia não estiver disponível.
...
| Item | Decisão |
|---|---|
| Região | Principal — ARCANE ↔ MARTIAL/AGILITY / Ender — Imbuimento Dimensional |
| Camada e papel | Camada 5; Ponte |
| Pré-requisito visual/estrutural | A0205 ≥2 disponível + uma mastery melee canônica da lista ≥20 + adapter de ação/deslocamento ENDER. IDs epic_* de gateway não são IDs de mastery. |
| Dependência semântica completa | A0205 ≥2 disponível + mastery da lane melee usada ≥20. Lanes canônicas: epicfight:sword, epicfight:axe, epicfight:spear (contato), epicfight:dagger, epicfight:heavy, combat:mace e combat:scythe; combat:fist somente com sua cadeia válida. Exige também producer causal de ação/deslocamento ENDER. |
| Custo topológico | 2 PP por rank; 2 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: ARCANE_MARTIAL_AGILITY_ENDER_BRIDGE. IDs de mastery melee são os IDs canônicos, não gateways epic_*. derived_component:ENDER pertence ao mesmo action_id/outcome_id, herda a única crítica e não cria DamageSource, teleporte, Ruptura, mastery, sustain ou proc. Availability transitiva de A0205. Não terminal. |
...
Fire's Ender Expansion 2.4.1/Somake 1.0.8 + Iron's 3.16.3 somente após action/SchoolType id exato e adapter ENDER; Epic Fight 21.17.3.1 e lanes combat:* canônicas. Weapons of Miracles só por arma mapeada. Generic teleport não é provider.
...
| Projeto | Head auditado | Decisão para A0209 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0209.
...
- **Fallback normativo:** UNAVAILABLE_NODE enquanto faltar A0205, ação/deslocamento ENDER confirmado, direct melee hook ou lane canônica. Não abrir por teleporte genérico, End/Void, gear/tema, nome ou posição; não simular segundo ataque. combat:fist permanece fechado quando sua cadeia não estiver disponível.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 2 ranks; janela fixa 120 ticks. Componente ENDER adicional = 5% / 10% do dano-base canônico elegível. Recast ENDER válido renova somente a expiração; não empilha magnitude..
...
- Página: [A0209 — Imbuimento de Ender](https://app.notion.com/3c569db9f0db816c8969c9a4b1d635a1)
...
**Resultado final do Chat 1 para A0209:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
