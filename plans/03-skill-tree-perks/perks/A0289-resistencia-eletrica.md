...
- **Fonte canônica:** [registro A0289 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81a9a6f9df9e38551c67).
...
- **Dependências externas à faixa:** <code>A0176</code>, <code>A0172</code>, <code>A0173</code>. Elas permanecem sinalizadas e não são presumidas como concluídas.
...
| Ramo | Eletromancia — Defesa Elétrica |
| Camada | 1 |
...
| Dependências Obrigatórias | SPECIALIST_UNLOCK:LIGHTNING confirmado server-side por Gate A/B/C. A0289 é perk interna defensiva e exige Specialist Raio ativa; A0176 isolada não autoriza compra. |
| Pré-requisitos | Specialist Raio desbloqueada (SPECIALIST_UNLOCK:LIGHTNING) por Gate A/B/C. |
| Provider/Mods | RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + FUTURE_PROVIDER_CONTRACT DAMAGE_MITIGATION_RESOLVER_V1 + classificador LIGHTNING + bucket único RPG_LIGHTNING_RESISTANCE. Iron's Spells 'n Spellbooks 3.16.3 e Ars Nouveau 5.13.1/Ars Elemental 0.7.10.1 participam somente quando o componente recebido estiver explicitamente classificado LIGHTNING por adapter seguro. A0172/A0173/A0289 contribuem ao MESMO bucket; qualquer resistência LIGHTNING nativa integrável deve ser normalizada no mesmo backend, nunca aplicada novamente em paralelo. Oritech/Create New Age/FE não entram salvo dano efetivamente classificado LIGHTNING por adapter. |
| Efeito | A0289 adiciona +7% de resistência a dano LIGHTNING por rank (+7/+14/+21%) ao mesmo bucket canônico RPG_LIGHTNING_RESISTANCE já usado pela família A0172/A0173. O bucket é resolvido uma única vez por componente LIGHTNING elegível; dano físico associado, temperatura corporal, FE/energia tecnológica e estados não elétricos permanecem fora desta resistência. |
| Escalonamento | Até 3 ranks. Contribuição própria de A0289 ao RPG_LIGHTNING_RESISTANCE: +7%/+14%/+21%. O teto próprio desta perk é 21%. Não existe cap defensivo global oculto; contribuições que compartilham este mesmo canonical_modifier_id são agregadas como uma única família antes da aplicação, enquanto mitigadores semanticamente distintos compõem conforme o DamageMitigationResolver. |
| Gate | SPECIALIST_UNLOCK:LIGHTNING válido + componente de dano legitimamente classificado LIGHTNING. Dano físico com partículas elétricas, aquecimento corporal, FE/energia tecnológica, estado CHARGED sem dano, choque de máquina não classificado, knockback/postura e WATER não entram por inferência. |
| Hook | No DAMAGE_MITIGATION_RESOLVER_V1, classificar LIGHTNING uma vez, agregar A0289 (+7/+14/+21 p.p.) ao mesmo canonical_modifier_id=RPG_LIGHTNING_RESISTANCE dos fundamentos e resolver esse bucket UMA vez antes de mitigadores semanticamente distintos. Se houver backend nativo equivalente com adapter seguro, unificá-lo no mesmo bucket; nunca aplicar resistência nativa + RPG em duplicidade. |
| Fallback | FAIL-CLOSED se Specialist Raio não estiver desbloqueada ou se o componente não puder ser classificado LIGHTNING com segurança. Não converter em resistência mágica geral, Armor, Absorption, Afinidade, resistência térmica, proteção contra WET/CHARGED, FE storage ou economia energética. |
| Regra | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna da Specialist Raio. A0289 mitiga SOMENTE dano LIGHTNING e compartilha o bucket RPG_LIGHTNING_RESISTANCE com A0172/A0173; não são três mitigadores separados. Não concede imunidade, não reduz WET, CHARGED, postura ou temperatura e não cria cap defensivo global implícito. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
A0289 adiciona +7% de resistência a dano LIGHTNING por rank (+7/+14/+21%) ao mesmo bucket canônico RPG_LIGHTNING_RESISTANCE já usado pela família A0172/A0173. O bucket é resolvido uma única vez por componente LIGHTNING elegível; dano físico associado, temperatura corporal, FE/energia tecnológica e estados não elétricos permanecem fora desta resistência.
...
SPECIALIST_UNLOCK:LIGHTNING válido + componente de dano legitimamente classificado LIGHTNING. Dano físico com partículas elétricas, aquecimento corporal, FE/energia tecnológica, estado CHARGED sem dano, choque de máquina não classificado, knockback/postura e WATER não entram por inferência.

### Hook e ordem de execução

No DAMAGE_MITIGATION_RESOLVER_V1, classificar LIGHTNING uma vez, agregar A0289 (+7/+14/+21 p.p.) ao mesmo canonical_modifier_id=RPG_LIGHTNING_RESISTANCE dos fundamentos e resolver esse bucket UMA vez antes de mitigadores semanticamente distintos. Se houver backend nativo equivalente com adapter seguro, unificá-lo no mesmo bucket; nunca aplicar resistência nativa + RPG em duplicidade.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Raio não estiver desbloqueada ou se o componente não puder ser classificado LIGHTNING com segurança. Não converter em resistência mágica geral, Armor, Absorption, Afinidade, resistência térmica, proteção contra WET/CHARGED, FE storage ou economia energética.
...
| Item | Decisão |
|---|---|
| Região | Especialista — Raio / Eletromancia — Defesa Elétrica |
...
| Dependência semântica completa | SPECIALIST_UNLOCK:LIGHTNING confirmado server-side por Gate A/B/C. A0289 é perk interna defensiva e exige Specialist Raio ativa; A0176 isolada não autoriza compra. |
| Custo topológico | 1 PP por rank; 3 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna da Specialist Raio. A0289 mitiga SOMENTE dano LIGHTNING e compartilha o bucket RPG_LIGHTNING_RESISTANCE com A0172/A0173; não são três mitigadores separados. Não concede imunidade, não reduz WET, CHARGED, postura ou temperatura e não cria cap defensivo global implícito. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + FUTURE_PROVIDER_CONTRACT DAMAGE_MITIGATION_RESOLVER_V1 + classificador LIGHTNING + bucket único RPG_LIGHTNING_RESISTANCE. Iron's Spells 'n Spellbooks 3.16.3 e Ars Nouveau 5.13.1/Ars Elemental 0.7.10.1 participam somente quando o componente recebido estiver explicitamente classificado LIGHTNING por adapter seguro. A0172/A0173/A0289 contribuem ao MESMO bucket; qualquer resistência LIGHTNING nativa integrável deve ser normalizada no mesmo backend, nunca aplicada novamente em paralelo. Oritech/Create New Age/FE não entram salvo dano efetivamente classificado LIGHTNING por adapter.
...
| Projeto | Head auditado | Decisão para A0289 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0289.
...
- **Recursos/eixos tocados pelo contrato:** <code>ABSORPTION</code>, <code>PARCEL TÉRMICO</code>.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Raio não estiver desbloqueada ou se o componente não puder ser classificado LIGHTNING com segurança. Não converter em resistência mágica geral, Armor, Absorption, Afinidade, resistência térmica, proteção contra WET/CHARGED, FE storage ou economia energética.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks. Contribuição própria de A0289 ao RPG_LIGHTNING_RESISTANCE: +7%/+14%/+21%. O teto próprio desta perk é 21%. Não existe cap defensivo global oculto; contribuições que compartilham este mesmo canonical_modifier_id são agregadas como uma única família antes da aplicação, enquanto mitigadores semanticamente distintos compõem conforme o DamageMitigationResolver..
...
- **Dependências fora desta faixa:** <code>A0176</code>, <code>A0172</code>, <code>A0173</code>.
...
- Página: [A0289 — Resistência Elétrica](https://app.notion.com/3c569db9f0db81a9a6f9df9e38551c67)
...
**Resultado final do Chat 1 para A0289:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
