...
- **Fonte canônica:** [registro A0270 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db8157909fd4398d79745e).
...
| Hook | No momento causal de criação/renovação, obter base_duration nativa ANTES de A0270, aplicar uma única contribuição CHILL_DURATION_MULTIPLIER ×1,10/×1,20/×1,30 e então respeitar apenas pisos/tetos NATIVOS explícitos do provider. Renovação parte da nova duração-base nativa da operação; nunca multiplica o tempo restante já ampliado, nunca roda por tick e não altera amplificador/chance/buildup. |
| Fallback | FAIL-CLOSED se Specialist Gelo não estiver desbloqueada ou se o chill_state_id não possuir duração-base modificável por hook seguro. Não ampliar Slowness genérica, freeze ticks/Frozen, temperatura corporal ou debuff aproximado; não converter duração indisponível em dano, buildup ou outro bônus. |
| Regra | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Perk interna da Specialist Gelo. Frio Persistente modifica SOMENTE a duração-base de CHILL real/compatível na criação/renovação. Não há cap global inventado, não aumenta amplificador/chance, não cria estado paralelo, não modifica freeze buildup/Frozen e não afeta Cold Sweat/temperatura corporal. irons_spellbooks:chilled é provider real elegível. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
A0270 multiplica em +10% de duração-base por rank a aplicação/renovação de um chill_state_id real/compatível atribuído ao jogador: ×1,10 / ×1,20 / ×1,30. O cálculo parte sempre da duração nativa que o provider aplicaria naquela operação e respeita somente pisos/tetos nativos explícitos. minecraft:slowness só participa quando aquela aplicação ICE concreta estiver mapeada pelo adapter como CHILL compatível.

### Escalonamento aprovado

Até 3 ranks. Duração-base de CHILL elegível na criação/renovação: ×1,10 / ×1,20 / ×1,30. Não altera amplificador, chance, freeze buildup ou tick rate. Renovação recalcula da duração nativa correspondente; não multiplica tempo restante já ampliado.

### Gate de compra/ativação

SPECIALIST_UNLOCK:ICE válido + A0265 ≥2 + criação ou renovação legítima de um chill_state_id allowlisted atribuída ao jogador + duração-base da OPERAÇÃO exposta de forma modificável. Para Iron's, irons_spellbooks:chilled é elegível quando o adapter interceptar sua aplicação/renovação causal. Estado sem owner quando ownership for exigido pelo mapping, Slowness não mapeada, temperatura corporal, ambiente, tick passivo e estado de duração fixa não modificável são inelegíveis.

### Hook e ordem de execução

No momento causal de criação/renovação, obter base_duration nativa ANTES de A0270, aplicar uma única contribuição CHILL_DURATION_MULTIPLIER ×1,10/×1,20/×1,30 e então respeitar apenas pisos/tetos NATIVOS explícitos do provider. Renovação parte da nova duração-base nativa da operação; nunca multiplica o tempo restante já ampliado, nunca roda por tick e não altera amplificador/chance/buildup.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Gelo não estiver desbloqueada ou se o chill_state_id não possuir duração-base modificável por hook seguro. Não ampliar Slowness genérica, freeze ticks/Frozen, temperatura corporal ou debuff aproximado; não converter duração indisponível em dano, buildup ou outro bônus.
...
| Pré-requisito visual/estrutural | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) + A0265 Geada ≥2 ranks. |
| Dependência semântica completa | SPECIALIST_UNLOCK:ICE confirmado por Gate A/B/C + A0265 Geada ≥2 ranks. A0265 e a topologia local não substituem fundamentos ICE + ≥100 PP válidos em SPECIALIST_REGION:ICE + terminal A0169. |
...
| Projeto | Head auditado | Decisão para A0270 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0270.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Gelo não estiver desbloqueada ou se o chill_state_id não possuir duração-base modificável por hook seguro. Não ampliar Slowness genérica, freeze ticks/Frozen, temperatura corporal ou debuff aproximado; não converter duração indisponível em dano, buildup ou outro bônus.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks. Duração-base de CHILL elegível na criação/renovação: ×1,10 / ×1,20 / ×1,30. Não altera amplificador, chance, freeze buildup ou tick rate. Renovação recalcula da duração nativa correspondente; não multiplica tempo restante já ampliado..
...
- Página: [A0270 — Frio Persistente](https://app.notion.com/3c569db9f0db8157909fd4398d79745e)
...
**Resultado final do Chat 1 para A0270:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
