...
- **Fonte canônica:** [registro A0271 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db811a9706ef5b44a27ff2).
...
| Hook | Antes da resolução do outcome direto, consultar FULL_FREEZE_STATE_V1 UMA vez e congelar target_was_fully_frozen_before_outcome. Se true, aplicar FROZEN_TARGET_DAMAGE ×1,06/×1,12/×1,18 uma única vez aos componentes PHYSICAL e/ou ICE diretos elegíveis do mesmo action_id/outcome_id. A0271 é read-only sobre o estado: não consome, renova, prolonga, reduz threshold nem escreve freeze ticks. |
| Fallback | FAIL-CLOSED se Specialist Gelo não estiver desbloqueada ou se não houver estado completo real/compatível reconhecível com segurança. Não usar freeze_buildup próprio, qualquer valor parcial de ticks, CHILL, Slowness, Cold Sweat, partícula/visual ou temperatura baixa como substituto; sem full freeze real, contribuição = 0. |
| Regra | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Perk interna da Specialist Gelo. Quebra de Gelo recompensa um estado COMPLETO real já existente e não o cria nem o consome. A0266 modifica buildup causal; A0271 consulta somente o resultado completo pré-existente. Cada outcome recebe a camada uma única vez; derivados não são reprocessados. FULLY_FROZEN, CHILL, buildup, Resistência a Gelo e temperatura corporal são eixos distintos. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Contra alvo que, antes do outcome, esteja em um estado de Congelamento completo real e explicitamente reconhecido pelo adapter da versão, A0271 concede +6% por rank aos componentes diretos PHYSICAL e ICE atribuídos ao jogador: ×1,06 / ×1,12 / ×1,18. CHILL, Slowness, buildup parcial ou temperatura corporal baixa não satisfazem o gate.

### Escalonamento aprovado

Até 3 ranks. Componentes diretos PHYSICAL/ICE elegíveis contra alvo já em Congelamento completo: ×1,06 / ×1,12 / ×1,18. O bônus não consome, renova nem prolonga o estado e não se aplica a DoT/derived já resolvidos.

### Gate de compra/ativação

SPECIALIST_UNLOCK:ICE válido + A0266 ≥2 + alvo já em FULLY_FROZEN real/compatível ANTES do outcome_id + componente direto PHYSICAL ou ICE atribuído ao jogador. Para Iron's/vanilla, a consulta deve refletir o estado completo nativo de congelamento, não apenas algum freeze tick positivo. CHILL, Slowness, buildup parcial, temperatura corporal, ambiente, DoT, derived component, summon e estado que só se completa pela própria ação gatilho não retroagem o gate.

### Hook e ordem de execução

Antes da resolução do outcome direto, consultar FULL_FREEZE_STATE_V1 UMA vez e congelar target_was_fully_frozen_before_outcome. Se true, aplicar FROZEN_TARGET_DAMAGE ×1,06/×1,12/×1,18 uma única vez aos componentes PHYSICAL e/ou ICE diretos elegíveis do mesmo action_id/outcome_id. A0271 é read-only sobre o estado: não consome, renova, prolonga, reduz threshold nem escreve freeze ticks.
...
| Pré-requisito visual/estrutural | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) + A0266 Congelamento Progressivo ≥2 ranks. |
| Dependência semântica completa | SPECIALIST_UNLOCK:ICE confirmado por Gate A/B/C + A0266 Congelamento Progressivo ≥2 ranks. Requisitos locais não substituem fundamentos ICE + ≥100 PP válidos em SPECIALIST_REGION:ICE + terminal A0169. |
...
| Projeto | Head auditado | Decisão para A0271 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0271.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks. Componentes diretos PHYSICAL/ICE elegíveis contra alvo já em Congelamento completo: ×1,06 / ×1,12 / ×1,18. O bônus não consome, renova nem prolonga o estado e não se aplica a DoT/derived já resolvidos..
...
- Página: [A0271 — Quebra de Gelo](https://app.notion.com/3c569db9f0db811a9706ef5b44a27ff2)
...
**Resultado final do Chat 1 para A0271:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
