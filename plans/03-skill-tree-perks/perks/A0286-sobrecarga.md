...
- **Fonte canônica:** [registro A0286 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db8179b781e978879277b5).
...
| Hook | No compositor do hop, calcular c_final=max(c_native,min(c_native×(1+0,07×rank),0,90)) e aplicar uma única contribuição RPG_LIGHTNING_CHAIN_DAMAGE à base do hop. Se c_native>=0,90, contribuição A0286=0. Deduplicar por root_action_id+hop_index+target_uuid. O novo alvo resolve depois seus modificadores target-specific (incluindo A0283/WET quando aplicável) e sua mitigação. Não rerodar multiplicadores ofensivos raiz, crítico, mastery ou perks de ação direta. |
| Fallback | FAIL-CLOSED se Specialist Raio não estiver desbloqueada ou se c_native/base causal do hop não puderem ser conhecidos com segurança. Não inventar coeficiente a partir do dano final, não converter qualquer dano secundário em chain, não criar segundo hit e não substituir por bônus LIGHTNING genérico. |
| Regra | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna da Specialist Raio. A0286 modifica SOMENTE hops secundários e nunca o alvo inicial. Nunca piora coeficiente nativo já maior e sua contribuição não permite ultrapassar 90% da base elegível do hop anterior. Hops continuam derivados: não ganham nova rolagem crítica, Lightning Mastery, A0284, Mana ou procs de ação direta. A0283/WET é resolvido depois por alvo e não compõe a base reutilizável da chain. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Para cada salto secundário LIGHTNING cujo provider exponha o coeficiente de dano relativo ao hop anterior, A0286 multiplica esse coeficiente por ×1,07/×1,14/×1,21 conforme rank, mas nunca reduz o coeficiente nativo e nunca permite que a contribuição desta perk eleve o salto acima de 90% do dano-base elegível do hop anterior.

### Escalonamento aprovado

Até 3 ranks. Seja c_native o coeficiente nativo/compatível do hop secundário em relação ao hop anterior. c_final = max(c_native, min(c_native × (1 + 0,07×rank), 0,90)). Se c_native >= 0,90, A0286 não adiciona dano naquele hop. O novo alvo ainda aplica sua própria mitigação normalmente.

### Gate de compra/ativação

SPECIALIST_UNLOCK:LIGHTNING válido + A0285 ≥1 + hop LIGHTNING secundário explicitamente classificado + relação c_native e base pré-mitigação do hop anterior conhecidas ANTES da mitigação do novo alvo. Alvo inicial, DoT, proc não-chain, AoE derivada sem hop index, salto com dano absoluto opaco e callback duplicado não recebem A0286.
...
FAIL-CLOSED se Specialist Raio não estiver desbloqueada ou se c_native/base causal do hop não puderem ser conhecidos com segurança. Não inventar coeficiente a partir do dano final, não converter qualquer dano secundário em chain, não criar segundo hit e não substituir por bônus LIGHTNING genérico.
...
| Pré-requisito visual/estrutural | Specialist Raio desbloqueada (SPECIALIST_UNLOCK:LIGHTNING) + A0285 Arco Elétrico ≥1 rank. |
| Dependência semântica completa | SPECIALIST_UNLOCK:LIGHTNING confirmado por Gate A/B/C + A0285 Arco Elétrico ≥1 rank. Requisitos locais não substituem fundamentos LIGHTNING + ≥100 PP válidos em SPECIALIST_REGION:LIGHTNING + terminal A0176. |
| Custo topológico | 1 PP por rank; 3 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Perk interna da Specialist Raio. A0286 modifica SOMENTE hops secundários e nunca o alvo inicial. Nunca piora coeficiente nativo já maior e sua contribuição não permite ultrapassar 90% da base elegível do hop anterior. Hops continuam derivados: não ganham nova rolagem crítica, Lightning Mastery, A0284, Mana ou procs de ação direta. A0283/WET é resolvido depois por alvo e não compõe a base reutilizável da chain. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0286 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0286.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks. Seja c_native o coeficiente nativo/compatível do hop secundário em relação ao hop anterior. c_final = max(c_native, min(c_native × (1 + 0,07×rank), 0,90)). Se c_native >= 0,90, A0286 não adiciona dano naquele hop. O novo alvo ainda aplica sua própria mitigação normalmente..
...
- Página: [A0286 — Sobrecarga](https://app.notion.com/3c569db9f0db8179b781e978879277b5)
...
**Resultado final do Chat 1 para A0286:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
