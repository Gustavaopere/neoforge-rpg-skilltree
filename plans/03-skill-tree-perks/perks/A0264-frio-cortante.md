...
- **Fonte canônica:** [registro A0264 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81c0864ad7f14e76b2ea).
...
| Ramo | Criomancia — Calafrio e Controle |
| Camada | 1 |
...
| Dependências Obrigatórias | SPECIALIST_UNLOCK:ICE confirmado server-side por Gate A/B/C: Gate A = fundamentos exteriores ICE canônicos; Gate B = ≥100 Passive Points válidos em SPECIALIST_REGION:ICE; Gate C = A0169 Maestria de Gelo. A0169 isolada, conexão visual ou topologia local não permitem comprar A0264. |
| Pré-requisitos | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) por Gate A/B/C: fundamentos ICE + ≥100 PP válidos em SPECIALIST_REGION:ICE + terminal A0169. |
| Provider/Mods | RPG Skill Tree + pipeline canônica TreeUnlockResolver + TreeUnlockDefinition + Stage 04.01 + classificador ICE + FUTURE_PROVIDER_CONTRACT CHILL_STATE_REGISTRY_V1. Provider CHILL comprovado: Iron's Spells 'n Spellbooks runtime 1.21.1-3.16.3, estado real irons_spellbooks:chilled; o próprio provider preserva sua duração/amplificador e semântica nativa. Ars Nouveau 5.13.1/Ars Elemental 0.7.10.1 e outros providers ICE entram somente quando um adapter versionado mapear um estado CHILL concreto com autoria; dano ICE sozinho não cria CHILL. minecraft:slowness só participa quando a aplicação específica de uma fonte ICE for explicitamente registrada como representação compatível. Cold Sweat 2.4.2 é provider de estado térmico corporal e NÃO provider CHILL de alvo. VERSION-STATUS: CHILL_STATE_REGISTRY_V1 é contrato do RPG a implementar; irons_spellbooks:chilled é estado real comprovado no provider. |
| Efeito | +5% de dano ICE direto por rank (×1,05 / ×1,10 / ×1,15) contra alvo que, antes do outcome, possua um chill_state_id real e hostil atribuído ao jogador, explicitamente allowlisted pelo adapter da versão. minecraft:slowness só é elegível quando o adapter de uma fonte ICE concreta declarar aquela aplicação específica como representação semântica de CHILL, com autoria preservada; Slowness genérica nunca basta. |
| Escalonamento | Até 3 ranks. Dano ICE direto elegível contra alvo com CHILL pré-existente do jogador: ×1,05 / ×1,10 / ×1,15. Exatamente em ausência do estado antes da ação: contribuição 0. A0264 não aumenta duração, amplificador, buildup ou chance de CHILL. |
| Gate | SPECIALIST_UNLOCK:ICE válido + componente ICE direto atribuído ao jogador + alvo possuindo ANTES daquele outcome_id um chill_state_id allowlisted e com autoria preservada. irons_spellbooks:chilled é elegível quando aplicado pelo jogador/ação atribuível conforme adapter. Slowness genérica, freeze ticks sem origem CHILL mapeada, temperatura corporal Cold Sweat, lentidão ambiental, efeito visual, baixa velocidade, estado aplicado pela própria ação gatilho, DoT, derived component, summon e callback duplicado não satisfazem. |
| Hook | Antes da resolução do componente ICE direto, consultar UMA vez CHILL_STATE_REGISTRY_V1 para o alvo e capturar {chill_state_id, source_owner/source_action, present_before_outcome}. Se o estado for allowlisted, pré-existente e atribuível ao jogador, aplicar ICE_CHILLED_TARGET_DAMAGE uma única vez à camada ICE direta daquele action_id/outcome_id. A0264 é read-only sobre CHILL: não cria, renova, amplia, converte nem remove o estado. |
| Fallback | FAIL-CLOSED se Specialist Gelo não estiver desbloqueada ou se não houver chill_state_id real/compatível allowlisted com autoria segura. Não aceitar minecraft:slowness genérica, Cold Sweat/temperatura corporal, freeze ticks sem mapping, lentidão ambiental, efeito visual ou debuff aproximado como substituto. Sem registry, A0264 não vira bônus ICE genérico. |
| Regra | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Perk interna da Specialist Gelo. Frio Cortante recompensa PREPARAÇÃO REAL do alvo e não cria/renova CHILL. Dano ICE, CHILL, freeze buildup/Frozen e deslocamento térmico corporal são eixos separados. irons_spellbooks:chilled é provider CHILL explícito; Cold Sweat permanece no eixo corporal. Uma ação/outcome recebe no máximo uma avaliação e um multiplicador A0264; estado aplicado pela própria ação não retroage. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
+5% de dano ICE direto por rank (×1,05 / ×1,10 / ×1,15) contra alvo que, antes do outcome, possua um chill_state_id real e hostil atribuído ao jogador, explicitamente allowlisted pelo adapter da versão. minecraft:slowness só é elegível quando o adapter de uma fonte ICE concreta declarar aquela aplicação específica como representação semântica de CHILL, com autoria preservada; Slowness genérica nunca basta.

### Escalonamento aprovado

Até 3 ranks. Dano ICE direto elegível contra alvo com CHILL pré-existente do jogador: ×1,05 / ×1,10 / ×1,15. Exatamente em ausência do estado antes da ação: contribuição 0. A0264 não aumenta duração, amplificador, buildup ou chance de CHILL.

### Gate de compra/ativação

SPECIALIST_UNLOCK:ICE válido + componente ICE direto atribuído ao jogador + alvo possuindo ANTES daquele outcome_id um chill_state_id allowlisted e com autoria preservada. irons_spellbooks:chilled é elegível quando aplicado pelo jogador/ação atribuível conforme adapter. Slowness genérica, freeze ticks sem origem CHILL mapeada, temperatura corporal Cold Sweat, lentidão ambiental, efeito visual, baixa velocidade, estado aplicado pela própria ação gatilho, DoT, derived component, summon e callback duplicado não satisfazem.

### Hook e ordem de execução

Antes da resolução do componente ICE direto, consultar UMA vez CHILL_STATE_REGISTRY_V1 para o alvo e capturar {chill_state_id, source_owner/source_action, present_before_outcome}. Se o estado for allowlisted, pré-existente e atribuível ao jogador, aplicar ICE_CHILLED_TARGET_DAMAGE uma única vez à camada ICE direta daquele action_id/outcome_id. A0264 é read-only sobre CHILL: não cria, renova, amplia, converte nem remove o estado.
...
| Item | Decisão |
|---|---|
| Região | Especialista — Gelo / Criomancia — Calafrio e Controle |
| Camada e papel | Camada 1; Ramo |
| Pré-requisito visual/estrutural | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) por Gate A/B/C: fundamentos ICE + ≥100 PP válidos em SPECIALIST_REGION:ICE + terminal A0169. |
| Dependência semântica completa | SPECIALIST_UNLOCK:ICE confirmado server-side por Gate A/B/C: Gate A = fundamentos exteriores ICE canônicos; Gate B = ≥100 Passive Points válidos em SPECIALIST_REGION:ICE; Gate C = A0169 Maestria de Gelo. A0169 isolada, conexão visual ou topologia local não permitem comprar A0264. |
...
- **Contratos/capabilities nomeados no registro:** <code>pipeline canônica TreeUnlockResolver + TreeUnlockDefinition + Stage 04.01</code>, <code>CHILL_STATE_REGISTRY_V1</code>.
- **Estado:** nenhum nome de API é tratado como existente apenas por aparecer no design; FUTURE_PROVIDER_CONTRACT permanece bloqueador até prova em código/API da versão exata.

### Matriz dos quatro projetos próprios

| Projeto | Head auditado | Decisão para A0264 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0264.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks. Dano ICE direto elegível contra alvo com CHILL pré-existente do jogador: ×1,05 / ×1,10 / ×1,15. Exatamente em ausência do estado antes da ação: contribuição 0. A0264 não aumenta duração, amplificador, buildup ou chance de CHILL..
...
- **Capabilities/contracts a provar:** <code>pipeline canônica TreeUnlockResolver + TreeUnlockDefinition + Stage 04.01</code>, <code>CHILL_STATE_REGISTRY_V1</code>.
...
- Página: [A0264 — Frio Cortante](https://app.notion.com/3c569db9f0db81c0864ad7f14e76b2ea)
...
**Resultado final do Chat 1 para A0264:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
