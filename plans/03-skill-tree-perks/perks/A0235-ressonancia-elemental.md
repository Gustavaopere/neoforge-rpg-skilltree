...
- **Fonte canônica:** [registro A0235 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81b7af85e9fff57695db).
...
| Dependências Obrigatórias | A0233 Fundamento Elemental + pelo menos 4 pontos somados em dois ou mais corredores elementais ofensivos distintos. Concentrar todo o investimento em uma única escola não satisfaz o requisito. |
| Pré-requisitos | A0233 Fundamento Elemental + investimento ofensivo em pelo menos 2 elementos distintos. |
| Provider/Mods | RPG Skill Tree + FUTURE_PROVIDER_CONTRACTS ELEMENT_SIGNATURE_REGISTRY_V1 e ELEMENTAL_DIVERSITY_LEDGER_V1. OWNER do registry: RPG core + adapters; OWNER do ledger: RPG core. LEDGER STATE: ELEMENT_OFFENSE_PP[element_id] derivado de compras/refunds autoritativos; shared/general/bridge PP excluídos por padrão; cada node conta uma única vez para um único elemento canônico. VERSION-STATUS: designs congelados, contratos não encontrados no runtime/main atual em 2026-08-29. |
| Efeito | Uma ação elemental direta elegível atribui no máximo um element_signature_id canônico ao alvo e abre Ressonância por 120 ticks (6 s) para aquele par jogador/alvo. A próxima ação elemental direta do mesmo jogador, contra o mesmo alvo, com assinatura diferente, recebe dano elemental direto ×1,10 e consome a Ressonância após commit. |
| Escalonamento | 1 rank. Janela por jogador/alvo: 120 ticks. Segunda assinatura elemental direta distinta: dano elemental direto ×1,10. Um ciclo por alvo; nova assinatura não renova indefinidamente a marca antes do consumo/expiração. |
| Gate | A0233 + ELEMENTAL_DIVERSITY_LEDGER_V1 comprovando ≥4 PP ofensivos válidos somados em ≥2 element_id distintos + ações diretas com um único primary_element_signature_id canônico e estável. PP de ELEMENTAL_SHARED_OFFENSE, bridge, híbrido sem ownership único ou a mesma compra reciclada em dois elementos não satisfazem diversidade. Ações multi-elemento ambíguas, DoT, summons sem autoria direta, automação, fake player, derived component e callback duplicado não armam/consomem. |
| Hook | No primeiro outcome direto elegível, registrar por jogador/alvo source_action_id, assinatura e expiry=now+120. No próximo outcome direto do mesmo alvo com assinatura diferente, deduplicar outcome_id, aplicar ×1,10 ao componente elemental direto consumidor e remover a marca após commit. |
| Fallback | FAIL-CLOSED: sem ELEMENT_SIGNATURE_REGISTRY_V1, sem ELEMENTAL_DIVERSITY_LEDGER_V1 ou sem rastreamento seguro por jogador/alvo, A0235 fica inativa. Não reconstruir diversidade por nomes de perks, proximidade topológica, total bruto de PP ou inferência visual; não usar último elemento global como substituto. |
| Regra | PP_REGION: ELEMENTAL_SHARED_OFFENSE. DIVERSITY_LEDGER: somente ELEMENT_OFFENSE_PP[element_id] de nodes element-specific com ownership semântico único conta para os requisitos de 4 PP/2 elementos; PP shared/general e bridges valem zero por padrão para diversidade e não podem ser duplicados. Uma ação/outcome fornece no máximo um primary_element_signature_id; a segunda assinatura deve ser distinta, do mesmo jogador contra o mesmo alvo e dentro da janela. A0235 não concede mastery nem nova resolução/proc. Ambos os ledgers são FUTURE_PROVIDER_CONTRACTS, não APIs runtime existentes. |
...
Uma ação elemental direta elegível atribui no máximo um element_signature_id canônico ao alvo e abre Ressonância por 120 ticks (6 s) para aquele par jogador/alvo. A próxima ação elemental direta do mesmo jogador, contra o mesmo alvo, com assinatura diferente, recebe dano elemental direto ×1,10 e consome a Ressonância após commit.

### Escalonamento aprovado

1 rank. Janela por jogador/alvo: 120 ticks. Segunda assinatura elemental direta distinta: dano elemental direto ×1,10. Um ciclo por alvo; nova assinatura não renova indefinidamente a marca antes do consumo/expiração.

### Gate de compra/ativação

A0233 + ELEMENTAL_DIVERSITY_LEDGER_V1 comprovando ≥4 PP ofensivos válidos somados em ≥2 element_id distintos + ações diretas com um único primary_element_signature_id canônico e estável. PP de ELEMENTAL_SHARED_OFFENSE, bridge, híbrido sem ownership único ou a mesma compra reciclada em dois elementos não satisfazem diversidade. Ações multi-elemento ambíguas, DoT, summons sem autoria direta, automação, fake player, derived component e callback duplicado não armam/consomem.

### Hook e ordem de execução

No primeiro outcome direto elegível, registrar por jogador/alvo source_action_id, assinatura e expiry=now+120. No próximo outcome direto do mesmo alvo com assinatura diferente, deduplicar outcome_id, aplicar ×1,10 ao componente elemental direto consumidor e remover a marca após commit.

### Fallback sem trocar a identidade

FAIL-CLOSED: sem ELEMENT_SIGNATURE_REGISTRY_V1, sem ELEMENTAL_DIVERSITY_LEDGER_V1 ou sem rastreamento seguro por jogador/alvo, A0235 fica inativa. Não reconstruir diversidade por nomes de perks, proximidade topológica, total bruto de PP ou inferência visual; não usar último elemento global como substituto.
...
| Pré-requisito visual/estrutural | A0233 Fundamento Elemental + investimento ofensivo em pelo menos 2 elementos distintos. |
| Dependência semântica completa | A0233 Fundamento Elemental + pelo menos 4 pontos somados em dois ou mais corredores elementais ofensivos distintos. Concentrar todo o investimento em uma única escola não satisfaz o requisito. |
| Custo topológico | 2 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: ELEMENTAL_SHARED_OFFENSE. DIVERSITY_LEDGER: somente ELEMENT_OFFENSE_PP[element_id] de nodes element-specific com ownership semântico único conta para os requisitos de 4 PP/2 elementos; PP shared/general e bridges valem zero por padrão para diversidade e não podem ser duplicados. Uma ação/outcome fornece no máximo um primary_element_signature_id; a segunda assinatura deve ser distinta, do mesmo jogador contra o mesmo alvo e dentro da janela. A0235 não concede mastery nem nova resolução/proc. Ambos os ledgers são FUTURE_PROVIDER_CONTRACTS, não APIs runtime existentes. |
...
RPG Skill Tree + FUTURE_PROVIDER_CONTRACTS ELEMENT_SIGNATURE_REGISTRY_V1 e ELEMENTAL_DIVERSITY_LEDGER_V1. OWNER do registry: RPG core + adapters; OWNER do ledger: RPG core. LEDGER STATE: ELEMENT_OFFENSE_PP[element_id] derivado de compras/refunds autoritativos; shared/general/bridge PP excluídos por padrão; cada node conta uma única vez para um único elemento canônico. VERSION-STATUS: designs congelados, contratos não encontrados no runtime/main atual em 2026-08-29.
...
| Projeto | Head auditado | Decisão para A0235 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0235.
...
- **Fallback normativo:** FAIL-CLOSED: sem ELEMENT_SIGNATURE_REGISTRY_V1, sem ELEMENTAL_DIVERSITY_LEDGER_V1 ou sem rastreamento seguro por jogador/alvo, A0235 fica inativa. Não reconstruir diversidade por nomes de perks, proximidade topológica, total bruto de PP ou inferência visual; não usar último elemento global como substituto.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Janela por jogador/alvo: 120 ticks. Segunda assinatura elemental direta distinta: dano elemental direto ×1,10. Um ciclo por alvo; nova assinatura não renova indefinidamente a marca antes do consumo/expiração..
...
- Página: [A0235 — Ressonância Elemental](https://app.notion.com/3c569db9f0db81b7af85e9fff57695db)
...
**Resultado final do Chat 1 para A0235:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
