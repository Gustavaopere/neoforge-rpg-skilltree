...
- **Fonte canônica:** [registro A0296 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81c08ce7c84387b6c72d).
...
| Hook | No commit do impacto direto em alvo WET, snapshotar a parcela LIGHTNING raiz após modificadores ofensivos/crítico e ANTES da mitigação do alvo inicial; iniciar/reutilizar hit_target_set do root e claimar A0296 uma vez. Para hop 1 e opcional hop 2: COMBAT_TARGET_QUERY_V1 procura, a partir do alvo anterior e no MESMO level/sublevel, um alvo novo em raio 4/LOS que seja atacável e WET_STATE_V1=true; emitir derived_wet_conduction_hop com base previous_base×0,40, criticalHit=false, adicionar ao set e deixar o novo alvo resolver A0283/WET target-specific e sua mitigação. Coeficientes efetivos: 40% da raiz, depois 16% da raiz. Hops não rerodam multiplicadores ofensivos raiz. |
| Fallback | FAIL-CLOSED se Specialist Raio não estiver desbloqueada, WET não puder ser confirmado, a consulta espacial/targetability não for segura ou o pipeline derivado não puder preservar causalidade/dedupe. Em Sable/Aeronautics, incapacidade de resolver o mesmo sublevel também fecha a cadeia. Não substituir por +dano contra água, WATER→LIGHTNING, AoE genérica, alvo repetido ou chain que ignore PvP/alianças. |
| Regra | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Notable interna da Specialist Raio. A0296 cria no máximo dois hops derivados sequenciais, sempre para alvos NOVOS e WET; não critica, não rola A0284/A0296, não aplica CHARGED, não gera Lightning Mastery, mana, sustain ou novo parcel térmico. WET é reavaliado por alvo e nunca herdado. Hops não duplicam buckets WATER/LIGHTNING e jamais atingem novamente um target já registrado no root. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
1 rank. Máximo 2 hops derivados de condução. Coeficientes sequenciais: 0,40 da raiz no primeiro hop e 0,40 do hop anterior no segundo. Cada hop exige alvo novo WET, raio de seleção de 4 blocos a partir do alvo anterior, linha de visão e filtros hostis válidos. criticalHit=false nos hops derivados.

### Gate de compra/ativação

SPECIALIST_UNLOCK:LIGHTNING válido + A0283=3 + A0293≥1 + impacto LIGHTNING DIRETO do jogador em alvo com WET_STATE_V1=true no instante do outcome + claim A0296 livre para o root_action_id + target query segura. Cada hop exige novo alvo hostil realmente atacável, também WET no seu próprio instante, fora do hit_target_set. WET visual/inferido, isInWaterOrRain fora do adapter, WATER damage isolado, alvo repetido, DoT, aura, summon, automação, fake player e callback duplicado não satisfazem.

### Hook e ordem de execução

No commit do impacto direto em alvo WET, snapshotar a parcela LIGHTNING raiz após modificadores ofensivos/crítico e ANTES da mitigação do alvo inicial; iniciar/reutilizar hit_target_set do root e claimar A0296 uma vez. Para hop 1 e opcional hop 2: COMBAT_TARGET_QUERY_V1 procura, a partir do alvo anterior e no MESMO level/sublevel, um alvo novo em raio 4/LOS que seja atacável e WET_STATE_V1=true; emitir derived_wet_conduction_hop com base previous_base×0,40, criticalHit=false, adicionar ao set e deixar o novo alvo resolver A0283/WET target-specific e sua mitigação. Coeficientes efetivos: 40% da raiz, depois 16% da raiz. Hops não rerodam multiplicadores ofensivos raiz.

### Fallback sem trocar a identidade

FAIL-CLOSED se Specialist Raio não estiver desbloqueada, WET não puder ser confirmado, a consulta espacial/targetability não for segura ou o pipeline derivado não puder preservar causalidade/dedupe. Em Sable/Aeronautics, incapacidade de resolver o mesmo sublevel também fecha a cadeia. Não substituir por +dano contra água, WATER→LIGHTNING, AoE genérica, alvo repetido ou chain que ignore PvP/alianças.
...
| Pré-requisito visual/estrutural | Specialist Raio desbloqueada (SPECIALIST_UNLOCK:LIGHTNING) + A0283 Condutividade 3/3 + A0293 Raio Ramificado ≥1. |
| Dependência semântica completa | SPECIALIST_UNLOCK:LIGHTNING confirmado por Gate A/B/C + A0283 Condutividade =3 ranks + A0293 Raio Ramificado ≥1 rank. Requisitos locais não substituem fundamentos LIGHTNING + ≥100 PP válidos em SPECIALIST_REGION:LIGHTNING + terminal A0176. |
| Custo topológico | 3 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:LIGHTNING=YES. Notable interna da Specialist Raio. A0296 cria no máximo dois hops derivados sequenciais, sempre para alvos NOVOS e WET; não critica, não rola A0284/A0296, não aplica CHARGED, não gera Lightning Mastery, mana, sustain ou novo parcel térmico. WET é reavaliado por alvo e nunca herdado. Hops não duplicam buckets WATER/LIGHTNING e jamais atingem novamente um target já registrado no root. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0296 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>action_id</code>, <code>outcome_id</code>, <code>root_action_id</code>, <code>claim</code>.
...
- **Chaves que o adapter precisa preservar:** <code>action_id</code>, <code>outcome_id</code>, <code>root_action_id</code>, <code>claim</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0296.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Raio não estiver desbloqueada, WET não puder ser confirmado, a consulta espacial/targetability não for segura ou o pipeline derivado não puder preservar causalidade/dedupe. Em Sable/Aeronautics, incapacidade de resolver o mesmo sublevel também fecha a cadeia. Não substituir por +dano contra água, WATER→LIGHTNING, AoE genérica, alvo repetido ou chain que ignore PvP/alianças.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Máximo 2 hops derivados de condução. Coeficientes sequenciais: 0,40 da raiz no primeiro hop e 0,40 do hop anterior no segundo. Cada hop exige alvo novo WET, raio de seleção de 4 blocos a partir do alvo anterior, linha de visão e filtros hostis válidos. criticalHit=false nos hops derivados..
...
6. **Causalidade:** correlacionar <code>action_id</code>, <code>outcome_id</code>, <code>root_action_id</code>, <code>claim</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>action_id</code>, <code>outcome_id</code>, <code>root_action_id</code>, <code>claim</code>. |
...
- Página: [A0296 — Condução pela Água](https://app.notion.com/3c569db9f0db81c08ce7c84387b6c72d)
...
**Resultado final do Chat 1 para A0296:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
