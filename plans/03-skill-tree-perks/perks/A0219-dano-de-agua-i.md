...
- **Fonte canônica:** [registro A0219 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db8173aa9bea81c4844d5d).
...
| Provider/Mods | Somake Spells 1.0.8-1.21.1-fix como provider WATER/Aqua principal dentro do framework Iron's Spells 'n Spellbooks 3.16.3 + Ars Elemental 0.7.10.1 e outros providers somente quando a ação concreta estiver explicitamente mapeada WATER + classificador canônico WATER do RPG Skill Tree. Estado térmico só participa se a habilidade declarar parcel térmico próprio. |
| Efeito | +3% de dano WATER por rank (máx. +12%) em componentes mágicos WATER diretos elegíveis e atribuídos ao jogador. Água não possui direção térmica universal; qualquer parcel térmico só existe quando a ação concreta o declarar explicitamente. |
| Escalonamento | Até 4 ranks; multiplicador da camada WATER direta: ×1,03 / ×1,06 / ×1,09 / ×1,12. Teto próprio de A0219: +12%. |
| Gate | Gateway ARCANE + A0144 ≥2 + ≥1 técnica arcana + componente mágico direto classificado WATER por adapter versionado. Chuva, bioma, imersão, alvo molhado, balde, criatura aquática, DoT já derivado e derived_component não são WATER por inferência. |
| Hook | Pipeline canônico: Potência Mágica universal → camada WATER A0219 uma única vez por componente direto → especializações posteriores. Preservar action_id/outcome_id; qualquer parcel térmico da ação é processado separadamente. |
| Fallback | Sem classificação WATER explícita e ownership mágico direto confiável, A0219 fica inativa naquela fonte. Não inferir por chuva, imersão, natação, bioma, umidade, balde, criatura aquática, estado WET ambiental, frio ou cura; não converter em dano mágico genérico. |
| Regra | FUNDAMENTO_EXTERIOR: ARCANE/WATER. PP_REGION: ARCANE/WATER. Water Mastery nasce somente de ações/resultados WATER reais; chuva, permanecer submerso, nadar, molhar entidades ou automação não concedem mastery. WATER não possui direção térmica universal e é distinto de WET ambiental. A0219 pode compor Gate A da futura Specialist Água; Gate B ≥100 PP válidos e terminal correto continuam obrigatórios. |
...
+3% de dano WATER por rank (máx. +12%) em componentes mágicos WATER diretos elegíveis e atribuídos ao jogador. Água não possui direção térmica universal; qualquer parcel térmico só existe quando a ação concreta o declarar explicitamente.

### Escalonamento aprovado

Até 4 ranks; multiplicador da camada WATER direta: ×1,03 / ×1,06 / ×1,09 / ×1,12. Teto próprio de A0219: +12%.

### Gate de compra/ativação

Gateway ARCANE + A0144 ≥2 + ≥1 técnica arcana + componente mágico direto classificado WATER por adapter versionado. Chuva, bioma, imersão, alvo molhado, balde, criatura aquática, DoT já derivado e derived_component não são WATER por inferência.

### Hook e ordem de execução

Pipeline canônico: Potência Mágica universal → camada WATER A0219 uma única vez por componente direto → especializações posteriores. Preservar action_id/outcome_id; qualquer parcel térmico da ação é processado separadamente.

### Fallback sem trocar a identidade

Sem classificação WATER explícita e ownership mágico direto confiável, A0219 fica inativa naquela fonte. Não inferir por chuva, imersão, natação, bioma, umidade, balde, criatura aquática, estado WET ambiental, frio ou cura; não converter em dano mágico genérico.
...
| Item | Decisão |
|---|---|
| Região | Principal — ARCANE ↔ AGILITY / Água — Potência Fluida |
...
| Regra de região/PP | FUNDAMENTO_EXTERIOR: ARCANE/WATER. PP_REGION: ARCANE/WATER. Water Mastery nasce somente de ações/resultados WATER reais; chuva, permanecer submerso, nadar, molhar entidades ou automação não concedem mastery. WATER não possui direção térmica universal e é distinto de WET ambiental. A0219 pode compor Gate A da futura Specialist Água; Gate B ≥100 PP válidos e terminal correto continuam obrigatórios. |
...
Somake Spells 1.0.8-1.21.1-fix como provider WATER/Aqua principal dentro do framework Iron's Spells 'n Spellbooks 3.16.3 + Ars Elemental 0.7.10.1 e outros providers somente quando a ação concreta estiver explicitamente mapeada WATER + classificador canônico WATER do RPG Skill Tree. Estado térmico só participa se a habilidade declarar parcel térmico próprio.
...
| Projeto | Head auditado | Decisão para A0219 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0219.
...
- **Fallback normativo:** Sem classificação WATER explícita e ownership mágico direto confiável, A0219 fica inativa naquela fonte. Não inferir por chuva, imersão, natação, bioma, umidade, balde, criatura aquática, estado WET ambiental, frio ou cura; não converter em dano mágico genérico.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 4 ranks; multiplicador da camada WATER direta: ×1,03 / ×1,06 / ×1,09 / ×1,12. Teto próprio de A0219: +12%..
...
- Página: [A0219 — Dano de Água I](https://app.notion.com/3c569db9f0db8173aa9bea81c4844d5d)
...
**Resultado final do Chat 1 para A0219:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
