...
- **Fonte canônica:** [registro A0233 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db81dc8338c896c06bfa15).
...
- **Dependências externas à faixa:** <code>A0144</code>. Elas permanecem sinalizadas e não são presumidas como concluídas.
...
| Dependências Obrigatórias | A0144 Poder Mágico ≥ 1 rank. O bônus exige classificação elemental reconhecida; dano ARCANE puro ou não elemental não conta. |
| Pré-requisitos | A0144 Poder Mágico ≥ 1 rank. |
| Provider/Mods | RPG Skill Tree + FUTURE_PROVIDER_CONTRACT ELEMENT_SIGNATURE_REGISTRY_V1 + adapters versionados dos providers mágicos instalados. OWNER: RPG core + adapters; CONSUMER: perks elementais/convergência; STATE: classificação canônica de componentes por element_id e no máximo um primary_element_signature_id por action_id/outcome_id para sistemas de convergência; BEHAVIOR: somente registro explícito de FIRE/ICE/LIGHTNING/NATURE/HOLY/BLOOD/ELDRITCH/ENDER/EARTH/WATER/WIND e extensões versionadas; VERSION-STATUS: design congelado, element_signature_id não encontrado no runtime/main atual em 2026-08-29. Iron's 3.16.3, Ars Nouveau/Ars Elemental 0.7.10.1, Somake Aqua, Aeromancy, Wind's, Geomancy e demais escolas só entram via adapter explícito; Ars Hex Unity é interoperabilidade, não autoridade elemental. |
| Efeito | +2% de dano em cada componente elemental elegível já classificado pelo registry canônico. A0233 usa ELEMENTAL_GENERAL e atua uma única vez por componente; não reclassifica dano nem multiplica repetidamente um outcome com múltiplos componentes. |
| Escalonamento | 1 rank. Cada componente elemental elegível recebe multiplicador ×1,02 uma única vez. Não existe escalonamento por quantidade de elementos no mesmo outcome. |
| Gate | A0144 ≥1 + componente com element_id reconhecido pelo classificador canônico. FIRE, ICE, LIGHTNING, NATURE, HOLY, BLOOD, ELDRITCH, ENDER, EARTH, WATER, WIND e extensões futuras entram somente por registro explícito. Fonte ambígua/não elemental não entra. |
| Hook | Na composição de dano, após classificação do componente e antes da camada específica de escola conforme ordem canônica, aplicar ELEMENTAL_GENERAL ×1,02 no máximo uma vez ao próprio componente. Um derived_component só participa se seu contrato explícito autorizar a camada elemental; nunca reprocessar o componente pai. |
| Fallback | Sem ELEMENT_SIGNATURE_REGISTRY_V1 implementado ou sem classificação explícita do componente, A0233 fica inativa naquele componente. Não inferir elemento por escola/mod de origem, cor, partícula, nome, dimensão ou efeito visual; ARCANE puro e ação multi-elemento ambígua não recebem assinatura de convergência por aproximação. |
| Regra | FUNDAMENTO_EXTERIOR: ELEMENTAL_SHARED_OFFENSE. PP_REGION: ELEMENTAL_SHARED_OFFENSE. A0233 é fundamento compartilhado generalista da Árvore Exterior e não pertence automaticamente a FIRE/ICE/LIGHTNING/etc. para Gate B: PP compartilhados não contam simultaneamente para todas as Specialists elementais e só podem ser whitelisted explicitamente por uma regra semântica específica. Cada componente elemental elegível recebe ×1,02 uma vez; para convergência, uma ação/outcome fornece no máximo um primary_element_signature_id. ELEMENT_SIGNATURE_REGISTRY_V1 é FUTURE_PROVIDER_CONTRACT, não API runtime existente. |
...
+2% de dano em cada componente elemental elegível já classificado pelo registry canônico. A0233 usa ELEMENTAL_GENERAL e atua uma única vez por componente; não reclassifica dano nem multiplica repetidamente um outcome com múltiplos componentes.

### Escalonamento aprovado

1 rank. Cada componente elemental elegível recebe multiplicador ×1,02 uma única vez. Não existe escalonamento por quantidade de elementos no mesmo outcome.

### Gate de compra/ativação

A0144 ≥1 + componente com element_id reconhecido pelo classificador canônico. FIRE, ICE, LIGHTNING, NATURE, HOLY, BLOOD, ELDRITCH, ENDER, EARTH, WATER, WIND e extensões futuras entram somente por registro explícito. Fonte ambígua/não elemental não entra.

### Hook e ordem de execução

Na composição de dano, após classificação do componente e antes da camada específica de escola conforme ordem canônica, aplicar ELEMENTAL_GENERAL ×1,02 no máximo uma vez ao próprio componente. Um derived_component só participa se seu contrato explícito autorizar a camada elemental; nunca reprocessar o componente pai.

### Fallback sem trocar a identidade

Sem ELEMENT_SIGNATURE_REGISTRY_V1 implementado ou sem classificação explícita do componente, A0233 fica inativa naquele componente. Não inferir elemento por escola/mod de origem, cor, partícula, nome, dimensão ou efeito visual; ARCANE puro e ação multi-elemento ambígua não recebem assinatura de convergência por aproximação.
...
| Pré-requisito visual/estrutural | A0144 Poder Mágico ≥ 1 rank. |
| Dependência semântica completa | A0144 Poder Mágico ≥ 1 rank. O bônus exige classificação elemental reconhecida; dano ARCANE puro ou não elemental não conta. |
| Custo topológico | 1 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | FUNDAMENTO_EXTERIOR: ELEMENTAL_SHARED_OFFENSE. PP_REGION: ELEMENTAL_SHARED_OFFENSE. A0233 é fundamento compartilhado generalista da Árvore Exterior e não pertence automaticamente a FIRE/ICE/LIGHTNING/etc. para Gate B: PP compartilhados não contam simultaneamente para todas as Specialists elementais e só podem ser whitelisted explicitamente por uma regra semântica específica. Cada componente elemental elegível recebe ×1,02 uma vez; para convergência, uma ação/outcome fornece no máximo um primary_element_signature_id. ELEMENT_SIGNATURE_REGISTRY_V1 é FUTURE_PROVIDER_CONTRACT, não API runtime existente. |
...
- **Contratos/capabilities nomeados no registro:** <code>ELEMENT_SIGNATURE_REGISTRY_V1</code>.
- **Estado:** nenhum nome de API é tratado como existente apenas por aparecer no design; FUTURE_PROVIDER_CONTRACT permanece bloqueador até prova em código/API da versão exata.

### Matriz dos quatro projetos próprios

| Projeto | Head auditado | Decisão para A0233 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>action_id</code>, <code>outcome_id</code>, <code>signature</code>, <code>element_id</code>, <code>primary_element_signature_id</code>.
...
- **Chaves que o adapter precisa preservar:** <code>action_id</code>, <code>outcome_id</code>, <code>signature</code>, <code>element_id</code>, <code>primary_element_signature_id</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0233.
...
- **Fallback normativo:** Sem ELEMENT_SIGNATURE_REGISTRY_V1 implementado ou sem classificação explícita do componente, A0233 fica inativa naquele componente. Não inferir elemento por escola/mod de origem, cor, partícula, nome, dimensão ou efeito visual; ARCANE puro e ação multi-elemento ambígua não recebem assinatura de convergência por aproximação.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Cada componente elemental elegível recebe multiplicador ×1,02 uma única vez. Não existe escalonamento por quantidade de elementos no mesmo outcome..
...
6. **Causalidade:** correlacionar <code>action_id</code>, <code>outcome_id</code>, <code>signature</code>, <code>element_id</code>, <code>primary_element_signature_id</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
- **Capabilities/contracts a provar:** <code>ELEMENT_SIGNATURE_REGISTRY_V1</code>.
- **Dependências fora desta faixa:** <code>A0144</code>.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>action_id</code>, <code>outcome_id</code>, <code>signature</code>, <code>element_id</code>, <code>primary_element_signature_id</code>. |
...
- Página: [A0233 — Fundamento Elemental](https://app.notion.com/3c569db9f0db81dc8338c896c06bfa15)
...
**Resultado final do Chat 1 para A0233:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
