...
- **PR de implementação/revalidação:** #237 — `test(perks): revalidate A0001-A0020 provider boundaries`.
- **PR predecessora:** #234 — fechada sem merge exclusivamente porque permaneceu draft após falha interna da mutação GraphQL usada para marcar a PR como ready; #237 reutiliza a mesma branch e o mesmo conteúdo validado.
...
## Estado para fechamento da PR #237
...
A confirmação definitiva de implementação desta revalidação é dada pelo merge da PR #237 e pela confirmação da `main` pós-merge.
