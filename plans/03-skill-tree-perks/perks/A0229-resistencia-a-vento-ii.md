...
- **Fonte canônica:** [registro A0229 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db819494a2cd37e797192b).
...
| Dependências Obrigatórias | A0228 Resistência a Vento I ≥ 3 ranks + executar ação de mobilidade ativa reconhecida. Corrida e salto normais não armam o benefício. |
| Pré-requisitos | A0228 Resistência a Vento I ≥ 3 ranks. |
| Provider/Mods | ParCool 4.0.0.2 + Epic ParCool 21.0.0 + Epic Fight 21.17.3.1 somente como providers/bridges de eventos de mobilidade ativa reconhecida; SnackPirate's Aeromancy Additions 1.2.8, Wind's Spellbooks 1.0.5/Iron's Spells 3.16.3 e adapters WIND classificam apenas o componente de dano recebido quando explicitamente WIND. Mobilidade nunca classifica dano ou mastery como WIND por si só. |
| Efeito | Após uma ação de mobilidade ativa explicitamente reconhecida pelo provider — dodge, dash, vault ou equivalente semanticamente curado — A0229 arma RPG_WIND_CURRENT_DEFLECTION por 80 ticks (4 s). O próximo componente WIND de um dano hostil direto durante a janela é multiplicado por ×0,75 e, se o mesmo evento possuir knockback modificável, sua magnitude é multiplicada por ×0,50; então o estado é consumido. |
| Escalonamento | 1 rank. Janela após mobilidade ativa: 80 ticks. Próximo componente WIND hostil direto: ×0,75. Knockback do mesmo evento, quando modificável: ×0,50. O estado não acumula; nova mobilidade válida apenas renova a expiração. |
| Gate | A0228 ≥3 + evento de mobilidade ativa reconhecido por adapter + janela ativa + hostile_direct_damage_outcome com componente WIND positivo. Sprint, salto, bunny hop, elytra comum, delta de velocidade, movimento forçado, DoT, custo e callback duplicado não armam nem consomem. |
| Hook | No evento de mobilidade ativa válida, registrar um único estado com expiração now+80. No próximo outcome WIND hostil direto, tratar a mitigação como estado transitório distinto no DamageMitigationResolver, aplicar ×0,75 e, se disponível, ×0,50 ao knockback do mesmo evento; consumir após commit e deduplicar por outcome_id. |
| Fallback | Sem evento autoritativo de mobilidade ativa reconhecida, A0229 fica inativa. Se knockback nativo não puder ser modificado, preservar somente a mitigação WIND ×0,75. Não inferir mobilidade por velocidade/posição e não transformar dodge, dash, vault, sprint ou ParCool em origem WIND sem adapter explícito. |
| Regra | PP_REGION: VITALITY_AGILITY_WIND_BRIDGE/DEFENSE. BRIDGE_PP_POLICY: estes PP não contam simultaneamente para VITALITY, AGILITY e ARCANE/WIND. Dodge/dash/vault reconhecido pode armar a janela como contexto de mobilidade, mas não gera Wind Mastery, element_signature WIND ou dano WIND. Sprint, salto, bunny hop, elytra comum, delta de velocidade e movimento forçado permanecem inelegíveis. RPG_WIND_CURRENT_DEFLECTION é mitigador transitório distinto do bucket base; não empilha e é consumido após commit. Notable/bridge da Árvore Exterior; não é terminal. |
...
Após uma ação de mobilidade ativa explicitamente reconhecida pelo provider — dodge, dash, vault ou equivalente semanticamente curado — A0229 arma RPG_WIND_CURRENT_DEFLECTION por 80 ticks (4 s). O próximo componente WIND de um dano hostil direto durante a janela é multiplicado por ×0,75 e, se o mesmo evento possuir knockback modificável, sua magnitude é multiplicada por ×0,50; então o estado é consumido.

### Escalonamento aprovado

1 rank. Janela após mobilidade ativa: 80 ticks. Próximo componente WIND hostil direto: ×0,75. Knockback do mesmo evento, quando modificável: ×0,50. O estado não acumula; nova mobilidade válida apenas renova a expiração.

### Gate de compra/ativação

A0228 ≥3 + evento de mobilidade ativa reconhecido por adapter + janela ativa + hostile_direct_damage_outcome com componente WIND positivo. Sprint, salto, bunny hop, elytra comum, delta de velocidade, movimento forçado, DoT, custo e callback duplicado não armam nem consomem.

### Hook e ordem de execução

No evento de mobilidade ativa válida, registrar um único estado com expiração now+80. No próximo outcome WIND hostil direto, tratar a mitigação como estado transitório distinto no DamageMitigationResolver, aplicar ×0,75 e, se disponível, ×0,50 ao knockback do mesmo evento; consumir após commit e deduplicar por outcome_id.

### Fallback sem trocar a identidade

Sem evento autoritativo de mobilidade ativa reconhecida, A0229 fica inativa. Se knockback nativo não puder ser modificado, preservar somente a mitigação WIND ×0,75. Não inferir mobilidade por velocidade/posição e não transformar dodge, dash, vault, sprint ou ParCool em origem WIND sem adapter explícito.
...
| Pré-requisito visual/estrutural | A0228 Resistência a Vento I ≥ 3 ranks. |
| Dependência semântica completa | A0228 Resistência a Vento I ≥ 3 ranks + executar ação de mobilidade ativa reconhecida. Corrida e salto normais não armam o benefício. |
| Custo topológico | 1 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | PP_REGION: VITALITY_AGILITY_WIND_BRIDGE/DEFENSE. BRIDGE_PP_POLICY: estes PP não contam simultaneamente para VITALITY, AGILITY e ARCANE/WIND. Dodge/dash/vault reconhecido pode armar a janela como contexto de mobilidade, mas não gera Wind Mastery, element_signature WIND ou dano WIND. Sprint, salto, bunny hop, elytra comum, delta de velocidade e movimento forçado permanecem inelegíveis. RPG_WIND_CURRENT_DEFLECTION é mitigador transitório distinto do bucket base; não empilha e é consumido após commit. Notable/bridge da Árvore Exterior; não é terminal. |
...
ParCool 4.0.0.2 + Epic ParCool 21.0.0 + Epic Fight 21.17.3.1 somente como providers/bridges de eventos de mobilidade ativa reconhecida; SnackPirate's Aeromancy Additions 1.2.8, Wind's Spellbooks 1.0.5/Iron's Spells 3.16.3 e adapters WIND classificam apenas o componente de dano recebido quando explicitamente WIND. Mobilidade nunca classifica dano ou mastery como WIND por si só.
...
| Projeto | Head auditado | Decisão para A0229 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>outcome_id</code>, <code>signature</code>.
...
- **Chaves que o adapter precisa preservar:** <code>outcome_id</code>, <code>signature</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0229.
...
- **Fallback normativo:** Sem evento autoritativo de mobilidade ativa reconhecida, A0229 fica inativa. Se knockback nativo não puder ser modificado, preservar somente a mitigação WIND ×0,75. Não inferir mobilidade por velocidade/posição e não transformar dodge, dash, vault, sprint ou ParCool em origem WIND sem adapter explícito.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Janela após mobilidade ativa: 80 ticks. Próximo componente WIND hostil direto: ×0,75. Knockback do mesmo evento, quando modificável: ×0,50. O estado não acumula; nova mobilidade válida apenas renova a expiração..
...
6. **Causalidade:** correlacionar <code>outcome_id</code>, <code>signature</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>outcome_id</code>, <code>signature</code>. |
...
- Página: [A0229 — Resistência a Vento II](https://app.notion.com/3c569db9f0db819494a2cd37e797192b)
...
**Resultado final do Chat 1 para A0229:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
