...
- **Fonte canônica:** [registro A0282 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db817081aac3e88c047541).
...
| Dependências Obrigatórias | SPECIALIST_UNLOCK:ICE confirmado por Gate A/B/C + A0281 Permafrost + A0276 Estilhaçar + (A0277 Aura de Geada OU A0278 Escudo de Gelo) + ≥18 pontos gastos na árvore interna Especialista — Gelo + Ice Mastery ≥180. Os ≥18 PP internos e requisitos locais não substituem Gate B global (≥100 PP válidos em SPECIALIST_REGION:ICE) nem fundamentos/terminal A0169. |
| Pré-requisitos | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) + A0281 + A0276 + (A0277 OU A0278) + ≥18 PP internos em Especialista — Gelo + Ice Mastery ≥180. |
| Provider/Mods | RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + FULL_FREEZE_STATE_V1 + FULL_FREEZE_TRANSITION_RECEIPT_V1 + FUTURE_PROVIDER_CONTRACT DAMAGE_VULNERABILITY_RESOLVER_V1 + classificadores ICE/PHYSICAL + FUTURE_PROVIDER_CONTRACT BOSS_CLASSIFIER_V1. Iron's Spells 'n Spellbooks runtime 1.21.1-3.16.3 é provider full-freeze quando adapter comprovar a transição nativa; Ars Nouveau 5.13.1/Ars Elemental 0.7.10.1 e outros só entram por mapping explícito. BOSS deve ser classificação positiva/negativa autoritativa por tag/adapter curado; ausência de classificação não autoriza coeficiente normal por heurística. VERSION-STATUS: transition/vulnerability/boss classifier são contratos RPG a implementar/adaptar; não presumir boss por nome, barra de vida ou max health. |
| Efeito | Quando um alvo alcança Congelamento máximo por uma transição legítima atribuída a ações elegíveis do jogador, A0282 aplica Vulnerabilidade Glacial por 120 ticks (6 s). Contra alvo não-chefe: multiplicador de dano recebido ICE ×1,18 e dano físico ×1,10. Contra BOSS explicitamente classificado: os coeficientes são reduzidos pela metade, ICE ×1,09 e físico ×1,05. A recarga por alvo começa no fim da vulnerabilidade e dura 400 ticks (20 s). |
| Escalonamento | 1 rank. Janela 120 ticks. Cooldown por alvo 400 ticks após o término. Coeficientes normais: +18% ICE / +10% físico. Bosses explicitamente classificados: +9% ICE / +5% físico. Cada bucket participa uma única vez do mesmo outcome; outros tipos de dano não recebem bônus. |
| Gate | SPECIALIST_UNLOCK:ICE válido + requisitos locais + alvo fora da própria janela/recarga + FULL_FREEZE_TRANSITION_RECEIPT_V1 comprovando uma transição real false→true para FULLY_FROZEN causada por ação elegível do jogador + classificação BOSS explicitamente resolvida como BOSS ou NON_BOSS. Renovação/manutenção de full-freeze, atingir apenas buildup parcial, aura derivada sem autoria elegível, retaliação, summon, automação, fake player e callback duplicado não ativam. Se a classificação boss for UNKNOWN, A0282 fica fail-closed naquele alvo. |
| Hook | No commit da transição canônica, deduplicar por transition_receipt_id/action_id+outcome_id+target_uuid, resolver BOSS_CLASSIFIER_V1 UMA vez e registrar RPG_GLACIAL_VULNERABILITY {owner_uuid,target_uuid,instance_id,expires_at=now+120,boss_class,ice_multiplier,physical_multiplier}. Coeficientes snapshot: NON_BOSS → ICE ×1,18 e PHYSICAL ×1,10; BOSS → ICE ×1,09 e PHYSICAL ×1,05. Durante a janela, DAMAGE_VULNERABILITY_RESOLVER_V1 aplica cada bucket no máximo uma vez por outcome elegível e nunca cria dano adicional. Ao expirar a janela, definir cooldown_expiry=now+400; tentativas bloqueadas durante janela/recarga não alteram o cooldown. Estilhaçar pode consumir o full-freeze depois sem apagar a vulnerabilidade já aplicada. |
| Fallback | Se Specialist Gelo não estiver desbloqueada, a transição causal não puder ser comprovada ou BOSS/NON_BOSS não puder ser classificado com segurança, FAIL-CLOSED. Se o pipeline PHYSICAL não puder ser modificado com segurança, omitir somente o componente físico e preservar ICE com o coeficiente correspondente à classificação já comprovada. Se nem ICE puder ser aplicado de modo causal/deduplicado, A0282 fica integralmente inativa. Não usar coeficiente normal em boss desconhecido, não inferir boss por nome/vida, não reduzir arbitrariamente ICE pela ausência de físico e não substituir por debuff genérico. |
| Regra | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Capstone INTERNO da Specialist Gelo; não é a terminal exterior A0169. Zero Absoluto aplica uma vulnerabilidade TEMPORÁRIA decorrente de transição real para full-freeze e não cria dano por si. Não acumula consigo mesma, não rearma por renovação do mesmo estado e não desaparece se Estilhaçar consumir o controle depois. Cada bucket ICE/PHYSICAL participa uma única vez; callbacks derivados não geram nova vulnerabilidade, mastery, sustain ou procs. Bosses usam coeficientes definidos e somente quando a classificação BOSS for explícita. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Quando um alvo alcança Congelamento máximo por uma transição legítima atribuída a ações elegíveis do jogador, A0282 aplica Vulnerabilidade Glacial por 120 ticks (6 s). Contra alvo não-chefe: multiplicador de dano recebido ICE ×1,18 e dano físico ×1,10. Contra BOSS explicitamente classificado: os coeficientes são reduzidos pela metade, ICE ×1,09 e físico ×1,05. A recarga por alvo começa no fim da vulnerabilidade e dura 400 ticks (20 s).

### Escalonamento aprovado

1 rank. Janela 120 ticks. Cooldown por alvo 400 ticks após o término. Coeficientes normais: +18% ICE / +10% físico. Bosses explicitamente classificados: +9% ICE / +5% físico. Cada bucket participa uma única vez do mesmo outcome; outros tipos de dano não recebem bônus.

### Gate de compra/ativação

SPECIALIST_UNLOCK:ICE válido + requisitos locais + alvo fora da própria janela/recarga + FULL_FREEZE_TRANSITION_RECEIPT_V1 comprovando uma transição real false→true para FULLY_FROZEN causada por ação elegível do jogador + classificação BOSS explicitamente resolvida como BOSS ou NON_BOSS. Renovação/manutenção de full-freeze, atingir apenas buildup parcial, aura derivada sem autoria elegível, retaliação, summon, automação, fake player e callback duplicado não ativam. Se a classificação boss for UNKNOWN, A0282 fica fail-closed naquele alvo.

### Hook e ordem de execução

No commit da transição canônica, deduplicar por transition_receipt_id/action_id+outcome_id+target_uuid, resolver BOSS_CLASSIFIER_V1 UMA vez e registrar RPG_GLACIAL_VULNERABILITY {owner_uuid,target_uuid,instance_id,expires_at=now+120,boss_class,ice_multiplier,physical_multiplier}. Coeficientes snapshot: NON_BOSS → ICE ×1,18 e PHYSICAL ×1,10; BOSS → ICE ×1,09 e PHYSICAL ×1,05. Durante a janela, DAMAGE_VULNERABILITY_RESOLVER_V1 aplica cada bucket no máximo uma vez por outcome elegível e nunca cria dano adicional. Ao expirar a janela, definir cooldown_expiry=now+400; tentativas bloqueadas durante janela/recarga não alteram o cooldown. Estilhaçar pode consumir o full-freeze depois sem apagar a vulnerabilidade já aplicada.
...
| Pré-requisito visual/estrutural | Specialist Gelo desbloqueada (SPECIALIST_UNLOCK:ICE) + A0281 + A0276 + (A0277 OU A0278) + ≥18 PP internos em Especialista — Gelo + Ice Mastery ≥180. |
| Dependência semântica completa | SPECIALIST_UNLOCK:ICE confirmado por Gate A/B/C + A0281 Permafrost + A0276 Estilhaçar + (A0277 Aura de Geada OU A0278 Escudo de Gelo) + ≥18 pontos gastos na árvore interna Especialista — Gelo + Ice Mastery ≥180. Os ≥18 PP internos e requisitos locais não substituem Gate B global (≥100 PP válidos em SPECIALIST_REGION:ICE) nem fundamentos/terminal A0169. |
| Custo topológico | 5 PP por rank; 1 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:ICE=YES. Capstone INTERNO da Specialist Gelo; não é a terminal exterior A0169. Zero Absoluto aplica uma vulnerabilidade TEMPORÁRIA decorrente de transição real para full-freeze e não cria dano por si. Não acumula consigo mesma, não rearma por renovação do mesmo estado e não desaparece se Estilhaçar consumir o controle depois. Cada bucket ICE/PHYSICAL participa uma única vez; callbacks derivados não geram nova vulnerabilidade, mastery, sustain ou procs. Bosses usam coeficientes definidos e somente quando a classificação BOSS for explícita. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
RPG Skill Tree + SPECIALIST_GATE_RESOLVER_V1 + FULL_FREEZE_STATE_V1 + FULL_FREEZE_TRANSITION_RECEIPT_V1 + FUTURE_PROVIDER_CONTRACT DAMAGE_VULNERABILITY_RESOLVER_V1 + classificadores ICE/PHYSICAL + FUTURE_PROVIDER_CONTRACT BOSS_CLASSIFIER_V1. Iron's Spells 'n Spellbooks runtime 1.21.1-3.16.3 é provider full-freeze quando adapter comprovar a transição nativa; Ars Nouveau 5.13.1/Ars Elemental 0.7.10.1 e outros só entram por mapping explícito. BOSS deve ser classificação positiva/negativa autoritativa por tag/adapter curado; ausência de classificação não autoriza coeficiente normal por heurística. VERSION-STATUS: transition/vulnerability/boss classifier são contratos RPG a implementar/adaptar; não presumir boss por nome, barra de vida ou max health.
...
- **Contratos/capabilities nomeados no registro:** <code>SPECIALIST_GATE_RESOLVER_V1</code>, <code>FULL_FREEZE_STATE_V1</code>, <code>FULL_FREEZE_TRANSITION_RECEIPT_V1</code>, <code>DAMAGE_VULNERABILITY_RESOLVER_V1</code>, <code>BOSS_CLASSIFIER_V1</code>.
- **Estado:** nenhum nome de API é tratado como existente apenas por aparecer no design; FUTURE_PROVIDER_CONTRACT permanece bloqueador até prova em código/API da versão exata.

### Matriz dos quatro projetos próprios

| Projeto | Head auditado | Decisão para A0282 | Authority/boundary |
|---|---|---|---|
...
- **Evidência causal:** <code>action_id</code>, <code>outcome_id</code>, <code>target_uuid</code>, <code>receipt</code>.
...
- **Chaves que o adapter precisa preservar:** <code>action_id</code>, <code>outcome_id</code>, <code>target_uuid</code>, <code>receipt</code>.
- **Produtor:** o provider que confirma a ação/estado/componente nativo descrito em Provider/Mods.
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0282.
...
- **Fallback normativo:** Se Specialist Gelo não estiver desbloqueada, a transição causal não puder ser comprovada ou BOSS/NON_BOSS não puder ser classificado com segurança, FAIL-CLOSED. Se o pipeline PHYSICAL não puder ser modificado com segurança, omitir somente o componente físico e preservar ICE com o coeficiente correspondente à classificação já comprovada. Se nem ICE puder ser aplicado de modo causal/deduplicado, A0282 fica integralmente inativa. Não usar coeficiente normal em boss desconhecido, não inferir boss por nome/vida, não reduzir arbitrariamente ICE pela ausência de físico e não substituir por debuff genérico.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de 1 rank. Janela 120 ticks. Cooldown por alvo 400 ticks após o término. Coeficientes normais: +18% ICE / +10% físico. Bosses explicitamente classificados: +9% ICE / +5% físico. Cada bucket participa uma única vez do mesmo outcome; outros tipos de dano não recebem bônus..
...
6. **Causalidade:** correlacionar <code>action_id</code>, <code>outcome_id</code>, <code>target_uuid</code>, <code>receipt</code>; callback duplicado, reordenado, cancelado ou target trocado não reaplica.
...
- **Capabilities/contracts a provar:** <code>SPECIALIST_GATE_RESOLVER_V1</code>, <code>FULL_FREEZE_STATE_V1</code>, <code>FULL_FREEZE_TRANSITION_RECEIPT_V1</code>, <code>DAMAGE_VULNERABILITY_RESOLVER_V1</code>, <code>BOSS_CLASSIFIER_V1</code>.
...
| 8 | Atribuição causal | **APROVADO** — chaves/receipt exigidos: <code>action_id</code>, <code>outcome_id</code>, <code>target_uuid</code>, <code>receipt</code>. |
...
- Página: [A0282 — Zero Absoluto](https://app.notion.com/3c569db9f0db817081aac3e88c047541)
...
**Resultado final do Chat 1 para A0282:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
