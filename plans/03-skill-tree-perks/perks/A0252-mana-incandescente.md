...
- **Fonte canônica:** [registro A0252 no Catálogo Mestre do Notion](https://app.notion.com/3c569db9f0db818aab65da829ed29d2e).
...
- **Dependências externas à faixa:** <code>A0145</code>. Elas permanecem sinalizadas e não são presumidas como concluídas.
...
|---|---|
| Código | A0252 |
| Nome | Mana Incandescente |
| Domínio | ARCANE/FIRE |
| Árvore | Especialista — Fogo |
| Ramo | Piromancia — Eficiência Ígnea |
| Camada | 3 |
...
| Hook | Após commit da aplicação/renovação de Ignição elegível, deduplicar por direct_fire_outcome_id; se cooldown livre, registrar expiry_tick=now+80 e cooldown_expiry_tick=now+120. Enquanto ativo, aplicar uma única camada multiplicativa à regeneração positiva nativa de mana no hook do provider; se a regeneração base for zero/bloqueada, resultado permanece zero. Não escrever saldo de mana nem interceptar débito/custo. |
| Fallback | FAIL-CLOSED se Specialist Fire não estiver desbloqueada, se a aplicação causal de Ignição não puder ser observada ou se a regen nativa de mana não for modificável com segurança. Não substituir por restauração fixa, redução de custo, polling de barra, ResourceDebitReceipt, Source ou outro recurso. |
| Regra | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Mana Incandescente acelera temporariamente regeneração nativa positiva; não cria mana, não escreve saldo, não reduz custo e não converte recursos. Uma aplicação/renovação causal de Ignição arma no máximo uma vez por direct_fire_outcome_id; ticks/propagação/callbacks não geram crédito. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
Quando uma ação FIRE direta elegível do jogador resultar, pela decisão canônica do IgnitionResolver, em aplicação nova ou renovação legítima de uma Ignição real com autoria preservada, A0252 arma RPG_INCANDESCENT_MANA por 80 ticks (4 s) se a recarga interna estiver livre. Durante a janela, a regeneração nativa positiva de mana do provider daquela build é multiplicada por ×1,04 / ×1,08 / ×1,12 conforme rank. Regeneração igual a zero ou bloqueada permanece zero. A0252 não restaura mana diretamente, não reduz custos, não cria pulsos e não afeta Source, Soul Energy, Spirit ou outro recurso diferente.

### Escalonamento aprovado

Até 3 ranks. Janela: 80 ticks. Multiplicador sobre regeneração nativa positiva de mana: ×1,04 / ×1,08 / ×1,12. Recarga interna por jogador: 120 ticks (6 s), iniciada somente após armamento bem-sucedido. Nova Ignição/renovação durante a recarga não renova a janela nem reduz a recarga.

### Gate de compra/ativação

SPECIALIST_UNLOCK:FIRE válido + requisitos locais + Ignição real aplicada/renovada pela decisão causal de uma ação FIRE direta própria + cooldown livre + MANA_REGEN_MODIFIER_V1 disponível para a mana daquele provider. DoT, tick de Ignição, propagação, derived component, summon, callback duplicado e estado sem autoria não armam.
...
FAIL-CLOSED se Specialist Fire não estiver desbloqueada, se a aplicação causal de Ignição não puder ser observada ou se a regen nativa de mana não for modificável com segurança. Não substituir por restauração fixa, redução de custo, polling de barra, ResourceDebitReceipt, Source ou outro recurso.
...
| Item | Decisão |
|---|---|
| Região | Especialista — Fogo / Piromancia — Eficiência Ígnea |
| Camada e papel | Camada 3; Ramo |
| Pré-requisito visual/estrutural | Specialist Fire desbloqueada (SPECIALIST_UNLOCK:FIRE) + A0243 Ignição ≥2 + A0145 Eficiência Arcana ≥2 + provider de mana com regen modificável. |
| Dependência semântica completa | SPECIALIST_UNLOCK:FIRE confirmado por Gate A/B/C + A0243 Ignição ≥2 + A0145 Eficiência Arcana ≥2. Exige provider com mana nativa e regeneração positiva/modificável; requisitos locais não substituem o unlock global da Specialist Fire. |
| Custo topológico | 1 PP por rank; 3 rank(s); extra 0 |
| Regra de região/PP | COUNTS_AS_SPECIALIST_PERK:FIRE=YES. Perk interna da Specialist Fire. Mana Incandescente acelera temporariamente regeneração nativa positiva; não cria mana, não escreve saldo, não reduz custo e não converte recursos. Uma aplicação/renovação causal de Ignição arma no máximo uma vez por direct_fire_outcome_id; ticks/propagação/callbacks não geram crédito. Gate A/B/C permanece obrigatório durante ownership/respec. |
...
| Projeto | Head auditado | Decisão para A0252 | Authority/boundary |
|---|---|---|---|
...
- **Consumidor:** o serviço RPG indicado no Hook; ele aplica somente a contribuição de A0252.
...
- **Fallback normativo:** FAIL-CLOSED se Specialist Fire não estiver desbloqueada, se a aplicação causal de Ignição não puder ser observada ou se a regen nativa de mana não for modificável com segurança. Não substituir por restauração fixa, redução de custo, polling de barra, ResourceDebitReceipt, Source ou outro recurso.
...
3. **Escalonamento:** validar cada rank/coeficiente/TTL/cap exato de Até 3 ranks. Janela: 80 ticks. Multiplicador sobre regeneração nativa positiva de mana: ×1,04 / ×1,08 / ×1,12. Recarga interna por jogador: 120 ticks (6 s), iniciada somente após armamento bem-sucedido. Nova Ignição/renovação durante a recarga não renova a janela nem reduz a recarga..
...
- Página: [A0252 — Mana Incandescente](https://app.notion.com/3c569db9f0db818aab65da829ed29d2e)
...
**Resultado final do Chat 1 para A0252:** design suficientemente especificado para implementação sem redesign; qualquer capability/dependência ausente mantém a perk ou sua parcela dependente fail-closed.
