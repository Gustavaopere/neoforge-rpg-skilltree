...
- `scripts/verify-relevant-player-runtime.py`, executado por `scripts/test-core.sh`, proíbe regressão para `level.players()` sem limite, exige probe bounded, fail-closed em saturação, query canônica de nível, cache/TTL/LRU, métricas, invalidações e wiring do bootstrap.
- A API NeoForge utilizada para o scan bounded foi confirmada contra a documentação 1.21.x: `ServerLevel.getPlayers(Predicate<? super ServerPlayer>, int)`.

## Verificação de fechamento

- Implementação canônica consolidada no merge `2ca59a9d3f1c704f1e92198102ab51d833607ef0`.
- `RPG Skill Tree CI` run `33245953398` no merge canônico fechou GREEN; o commit também disparou os demais workflows de contrato/Compendium sem regressão observada.
- O contrato executável permanece ligado ao `scripts/test-core.sh`, portanto regressões de bounded scan/cache/localidade são bloqueadas em CI.
