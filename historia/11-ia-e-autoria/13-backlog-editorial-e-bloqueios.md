...
| NPC | `NPC-0004` Liora | `PRONTO` para lore e boundary provider/runtime / `PENDENTE DE DECISÃO EDITORIAL` para repertório pessoal | dossiê + entidade Grimoire + `NPC-0004-liora-provider-audit.md` + adapter/GameTests Ars Nouveau 5.13.1 | usar identidade, voz, limites de knowledge e princípio de pluralidade já registrados; usar as boundaries verificadas do provider/runtime; materializar capability pessoal somente com estado próprio | inventar spell, glyph, ritual, local obrigatório ou facção; atribuir a Liora nodes/Mastery/gates player-facing; converter Source em recurso universal |
